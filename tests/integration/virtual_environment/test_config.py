...
class VirtualTestConfig:
    ...
    device_connection_delay_seconds: float = 1.0  # Stagger device connections
    device_response_delay_ms: int = 50  # Simulate network/processing latency
    
    # Session and recording configuration
    ...
    stress_event_intensity: float = 0.7  # 0.0 to 1.0
    
    # File transfer simulation
    simulate_file_transfers: bool = True
    video_file_size_mb: float = 50.0  # Per device
    thermal_file_size_mb: float = 10.0  # Per device
    ...
    timeout_multiplier: float = 1.0  # Multiply all timeouts for slow CI
    ...
    def validate(self) -> List[str]:
        ...
        # Check data rate feasibility
        total_gsr_samples_per_sec = self.device_count * self.gsr_sampling_rate_hz
        if total_gsr_samples_per_sec > 10000:  # Arbitrary high limit
        ...
        if total_video_frames_per_sec > 300:  # Arbitrary limit
    ...
    def estimate_memory_usage(self) -> float:
        ...
        # Memory per device (connection overhead, buffers)
        memory_per_device_mb = 5.0
        
        # Data buffering memory
        gsr_buffer_mb = self.device_count * 0.1  # Small buffer for GSR data
        video_buffer_mb = self.device_count * 2.0  # Larger buffer for video
        thermal_buffer_mb = self.device_count * 0.5  # Medium buffer for thermal
    ...
    def estimate_data_volume(self) -> Dict[str, float]:
        ...
        # GSR data: samples * devices * bytes_per_sample
        gsr_samples_total = duration_seconds * self.gsr_sampling_rate_hz * self.device_count
        gsr_mb = (gsr_samples_total * 12) / (1024 * 1024)  # 12 bytes per sample (timestamp + value)
        
        # Video frames
        video_frames_total = duration_seconds * self.rgb_fps * self.device_count
        video_mb = video_frames_total * 0.05  # Assume 50KB per compressed frame
        
        # Thermal frames
        thermal_frames_total = duration_seconds * self.thermal_fps * self.device_count
        thermal_mb = thermal_frames_total * 0.006  # 6KB per thermal frame
...
class VirtualTestScenario:
    ...
    def create_synchronization_test(cls) -> "VirtualTestScenario":
        ...
        config = VirtualTestConfig(
            ...
            enable_stress_events=False,  # Cleaner sync validation
            simulate_file_transfers=False,
        )
    ...
    def create_ci_test(cls) -> "VirtualTestScenario":
        ...
        config = VirtualTestConfig(
            ...
            enable_stress_events=False,
            simulate_file_transfers=True,
            video_file_size_mb=10.0,  # Smaller files for CI
...
def validate_system_requirements(config: VirtualTestConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check network connectivity for localhost
        import socket
        try:
            ...
            pass  # Ignore network check errors
