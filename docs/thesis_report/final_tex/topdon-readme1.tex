...
\item \textbf{Camera Permission:} Even though the TC001 is an external camera,
  ...
  \texttt{<uses-permission android:name="android.permission.CAMERA" />}\cite{ElectrodermalActivityWiki}.
...
\item \texttt{WRITE_EXTERNAL_STORAGE} and \texttt{READ_EXTERNAL_STORAGE} (for saving
  images/video)\cite{DeviceServer}.
  ...
  support\cite{GSRPPGMachineLearning2024},
...
\item \texttt{android.permission.WAKE_LOCK} to keep the CPU awake during capture
  ...
  recording)\cite{SimulatorValidityPhysiological2025}
...
\item Begin streaming thermal frames. You may choose to store the raw
  ...
  responses\cite{StressDefinitionHH}.
...
\item \textbf{Data Fusion:} The GSR signal primarily reflects sympathetic nervous
  ...
  signals\cite{DriverStressThermal2020}\cite{StressDefinitionHH}.
  ...
  vasoconstriction)\cite{CortisolStressIndicator2020}.
...
stress/fear)\cite{CortisolStressIndicator2020},
...
fluctuations\cite{MainViewModel}\cite{StressDefinitionHH}.
...
noisy\cite{DeviceServer}\cite{MainViewModel}.
...
\item \textbf{Performance and Threading:} Processing every frame (especially at
  ...
  up\cite{ContactlessStressThermal2022}\cite{ContactlessStressThermal2022}.
...
\item \textbf{Topdon Technology Thermal SDK (v1.3.7) --- Android Development
  ...
  extraction).\cite{ContactlessStressThermal2022}\cite{DeviceServer}
\item Gioia, F. }et al.\textit{ (2022). \textbf{"Towards a Contactless Stress
  ...
  detection)\cite{DriverStressThermal2020}\cite{StressDefinitionHH}.
\item Topdon Community Forum --- \textbf{Developing Android Apps with TC001} (Q&A
  thread)\cite{ShimmerRecorder}.
