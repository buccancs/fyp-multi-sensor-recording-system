...
% This file provides the minimal LaTeX structure needed to compile Chapter\_5.tex as a standalone document
...
values with known accuracy (e.g., thermal data accurate to within 0.1\textdegree C
...
%                   modalities                    \$\leq\$0.1 \textdegree C        reference
%                                                 thermal        standards
%                                                 accuracy
  ------------------------------------------------------------------------------------------------------------------
...
resistance in GSR, or a human temperature of 60\textdegree C). By validating test
...
the sensor.
    ...
    class ShimmerRecorderTest {
        ...
        @Test
        fun \texttt{connection to Shimmer device should configure sensors
        correctly}() = runTest {
            // Arrange
...
cases.
    ...
    class TestCalibrationManager:
        ...
        calibration\_manager, sample\_calibration\_images):
            ...
            assert len(result.quality\_metrics) > 0 # Some quality metrics (e.g.,
            per-view error) should be reported
        ...
        def test\_calibration\_with\_insufficient\_images(self, calibration\_manager):
            ...
            # Arrange
            insufficient\_images = [np.random.randint(0, 255, (480, 640, 3),
        ...
        calibration\_manager, sample\_calibration\_images):
            ...
            # Arrange
            mock\_find\_corners.return\_value = (False, None) # Simulate OpenCV
            failing to find pattern
            pattern\_config = PatternConfig(
                pattern\_type=PatternType.CHESSBOARD,
                pattern\_size=(9, 6),
                square\_size=25.0
...
required precision.
    ...
    class TestSynchronizationEngine:
        ...
        async def test\_device\_synchronization\_success(self, sync\_engine, mock\_devices):
            ...
            for device in mock\_devices:
                ...
                device.send\_sync\_request.return\_value = SyncResponse(
                    device\_timestamp=reference\_time + random.uniform(-0.001,
                    0.001), # within 1ms of reference
        ...
        async def test\_sync\_handles\_device\_timeouts(self, sync\_engine, mock\_devices):
            ...
            assert "device\_0" in sync\_results.warnings # a warning or note about
            the timed-out device
            # The other devices should have been synchronized (e.g., check their
            offsets are set, not shown here)
...
"multi-participant research session" simulation:
    ...
    class TestCompleteSystemWorkflow:
        ...
        async def test\_multi\_participant\_research\_session(self, full\_system\_setup):
            ...
            # Phase 2: Calibration Verification (e.g., check all devices calibrated
            their thermal camera)
            calibration\_status = await system.verify\_calibration\_status()
            assert calibration\_status.all\_devices\_calibrated
            assert calibration\_status.calibration\_quality >= 0.8 # e.g., all
            calibrations at least 80% quality score
            ...
            assert validation\_result.data\_completeness >= 0.99 # at least 99% of
...
across all data sources:
    ...
    class TestDataQualityValidation:
        ...
        async def test\_temporal\_synchronization\_accuracy(self):
            ...
            session = TestSession()
            await session.start\_recording(duration=60) # record for 60 seconds with,
            say, 2 devices
            ...
            for source in temporal\_data.sources:
                ...
                assert all(gap > 0 for gap in gaps), "No non-positive timestamp gaps
                (monotonic increase)"
...
\texttt{ThermalQualityAnalyzer}:
    ...
    assert thermal\_quality.temperature\_accuracy < 0.1 # within 0.1\textdegree C of
    reference
    assert thermal\_quality.spatial\_resolution >= 160 # at least 160x120 effective
    resolution
...
\textbf{Table 5.3: Scalability Testing Results}
  ...
  ---------------- ------------------ --------------- ------------------
  ------------------ --------------- ----------------------
  1 $\sim$1.0 ms 0\% 100\% Excellent 100\% Baseline
  Device                                      (baseline)              performance
                                                                           (single device)

  2 $\sim$1.2 ms 0\% 100\% Excellent 100\% Linear scaling
  Devices                                                             so far
...
Table 5.4: Stress Testing Results Under Network Conditions
  ...
  Packet Loss      20.8 s      50 ms latency, 5\% Error         $\sim$98\%       100\%
  Burst                          loss bursts       recovery                  preserved
                                                   active

  Limited          21.6 s      100 ms latency,   Adaptive data $\sim$98\%       100\%
  Bandwidth                      1\% loss, 1 Mbps   throttling                preserved

  Unstable         20.8 s      200 ms latency,   Connection    $\sim$93\%       100\%
...
hour in \$\sim\$30\textdegree C ambient, pushing devices hard). On the PC side, we
...
\textbf{Table 5.1: complete Testing Results Summary}
  ...
  \textbf{Component Modules and 342 tests 99.1\% 1 critical \checkmark Resolved
  99.8\%
  Testing}       classes

  \textbf{Integration Inter-component 156 tests 97.4\% 2 critical \checkmark Resolved
  99.5\%
  ...
  **Overall complete 618 tests\\textit{ }Pending* Env.  In progress In progress
  Config.
...
reading had a consistent 0.2\textdegree C bias vs a reference --- not critical but
...
device integration tests and accuracy tests (achieved 0.1\textdegree C accuracy at
...
\$\sim\$0.67 s, target was \$<\$1.0 s --- so syncing is quick. - \textbf{Throughput:}
We measured network throughput when all sensors are active. The system
handled \$\sim\$45.2 Mbps on average and peaks \$\sim\$78 Mbps without issue, within
...
observed. - \textbf{Storage Rate:} Data was written to disk at \$\sim\$3.2 GB/hour
...
\item \textbf{FR-003 Thermal Imaging Integration}: \checkmark \textit{Confirmed ±0.1\textdegree C accuracy
  ...
  proper calibration.)
\item \textbf{FR-004 Reference GSR Measurement}: \checkmark \textit{Validated 512 Hz GSR sampling with $<$ 0.1\% data loss.} (The Shimmer sensor integration delivered essentially continuous data; any tiny gaps were negligible. Cross-correlation with a reference GSR instrument gave r $\approx$ 0.89, p $<$ 0.001, indicating strong agreement.)
...
tested; most edge cases (\$\sim\$87%) and error handling paths (\$\sim\$92%) were
