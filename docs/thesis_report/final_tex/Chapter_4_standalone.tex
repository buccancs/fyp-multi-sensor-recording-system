% Standalone wrapper for Chapter_4.tex
% This file provides the minimal LaTeX structure needed to compile Chapter_4.tex as a standalone document
...
\end{document}\label{chap:4}

\chapter{Chapter 4: Design and Implementation}

\section{4.1 System Architecture Overview (PC---Android System Design)}

The \textbf{Multi-Sensor Recording System} is implemented as a distributed
...
coordinator or hub. This \textbf{PC---Android system design} follows a
...
devices\cite{Boucsein2012}\cite{AppleHealthWatch2019}.
...
each modality --- e.g. a \texttt{CameraRecorder} for the phone's RGB camera, a
...
to the desktop
controller\cite{ElectrodermalActivityWiki}\cite{DeviceServer}.
...
connects via Bluetooth. The Android application's \texttt{ShimmerRecorder}
...
the Shimmer's internal clock. The \texttt{ShimmerRecorder} runs a dedicated
...
\textit{Timestamp_ms, DeviceTime_ms, SystemTime_ms, GSR_Conductance_uS,
...
data streaming} capability for monitoring. The \texttt{ShimmerRecorder} can
...
with video and thermal data. The modular design of the \texttt{ShimmerRecorder}
...
robustness; the \texttt{ShimmerManager} on the PC can accept data from either
...
etc.)\cite{DeviceServer}\cite{MainViewModel}.
...
correctly\cite{MainViewModel}\cite{DeviceServer}.
...
JSON)\cite{DeviceServer}\cite{DeviceServer}.
The protocol defines several message types, including: \textit{control
commands} (e.g., \texttt{start_recording}, \texttt{stop_recording}), \textit{status updates}
...
messages} for streaming (such as \texttt{preview_frame} for a JPEG preview
image, or \texttt{gsr_sample} for a live GSR data point), and \textit{acknowledgments}
(the devices reply to key commands with an ACK/NACK to confirm
receipt)\cite{WebcamCapture}.
...
session\cite{ShimmerRecorder}\cite{ShimmerRecorder}.
...
message to each device: \textit{"start_recording at T with session_id X"}. Each
Android device receives this and waits until its local clock
(synchronized to master) hits T to begin capturing
data\cite{DeviceServer}\cite{DeviceServer}.
...
accumulate)\cite{DeviceServer}\cite{DeviceServer}.
...
real-time\cite{DeviceServer}\cite{MainViewModel}.
...
synchronization\cite{DeviceServer}.
...
\texttt{session_001_metadata.json} (with high-level info like session ID, start
time, participant ID), \texttt{session_001_camera.mp4} (phone RGB video),
\texttt{session_001_thermal.raw} (thermal binary file),
\texttt{session_001_shimmer.csv} (physio data), etc. This local organization is
...
verification\cite{DeviceServer}.
...
\item \textbf{Bluetooth Reliability and Sensor Connectivity:} The wireless nature
  ...
  robust \textbf{reconnection logic and buffering}. The \texttt{ShimmerRecorder} was
