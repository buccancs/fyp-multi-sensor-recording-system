...
after initial deployment testing (see security configuration in
`protocol/config.json:111-128`).
...
- **FR1: Multi-Device Sensor Integration** \-- The system supports
  ...
  when real sensors are unavailable (implemented in
  `PythonApp/shimmer_manager.py:234-267`).
...
- **FR3: Time Synchronisation Service** \-- The system synchronises
  ...
  runs a time synchronisation service on port 8889 (implemented in
  `PythonApp/ntp_time_server.py:38-50`) where each Android device
  ...
  100ms (see `results/endurance_test_results/endurance_20250806_065637_detailed_metrics.json`).
...
- **FR4: Session Management** \-- The system organises recordings
  ...
  **terminate the session** when finished. Upon session start, the
  ...
  metadata file using `SessionManager.create_session()` (lines 46-50 in
  `PythonApp/session/session_manager.py`). When the session ends, the
...
- **FR10: Data Transfer and Aggregation** \-- Once a session is stopped,
  ...
  over the network [14]. The PC saves each incoming file in the
  session folder and updates the session metadata with that file's entry
  (including file type and size) [8]. This automation ensures the
...
them simultaneously is handled by the*
`ShimmerPCApplication.start_session` *logic [8], and session metadata
management is implemented in the* `SessionManager` *class [8].)*
...
- **NFR5: Security** \-- The system protects recorded data through multiple
  ...
  uses TLS 1.3 encryption (configured in `protocol/config.json:116-117`
  with `"tls_version": "1.3"` and `"secure_transfer": true`) [8]. Device
  authentication requires 32-character minimum tokens (set via
  `"auth_token_min_length": 32` configuration) to prevent unauthorised
  device connections [8]. The system includes runtime security checks via
  `PythonApp/production/runtime_security_checker.py` that validate
...
set in the config file [8] indicate that performance requirements were
satisfied.)*
...
- The Researcher opens the PC control interface (MainWindow via
  `PythonApp/gui/enhanced_ui_main_window.py`) and **creates a new
  ...
  system validates the name using `SessionManager.validate_session_name()`
...
- The Researcher selects devices through the device manager panel. The
  ...
  `protocol/config.json:7-15` network settings). If the Shimmer is not yet
  connected, the Researcher clicks "Scan for Devices" in the DevicesFragment.
  The system performs a Bluetooth scan using `ShimmerManager.connect_devices()`
...
- After stopping, the system **automatically initiates data transfer**
  ...
  [14]. The PC receives each file (via its network server) and saves
