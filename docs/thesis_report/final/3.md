...
The system supports **contactless Galvanic Skin Response (GSR) prediction
research** using the Shimmer3 GSR+ device [8] alongside contactless sensors.
Traditional GSR measurement requires contact sensors attached to a person's
skin, but this project enables researchers to collect **synchronised
multi-modal data** by combining **wearable GSR readings** with **contactless
signals** such as thermal imagery and video. This approach addresses the
research need for ground-truth physiological data alongside contactless
sensor inputs, with all data streams time-aligned for analysis [1].

The system integrates **thermal cameras, RGB video, and inertial sensors**
with GSR to build datasets for exploring correlations between observable
signals (such as facial thermal patterns or motion) and skin conductance
changes. This **multi-sensor platform** operates in laboratory settings and
emphasises **temporal precision** through NTP-based synchronisation (port
8889) and data integrity protocols. During August 2025 testing sessions,
the system achieved clock synchronisation with sub-100ms response times
across networked devices (documented in
`results/endurance_test_results/endurance_20250806_065637_detailed_metrics.json`).

## 3.2 Requirements Engineering Approach

The system's requirements were derived through **iterative development** 
based on researcher feedback and hardware constraints. Initial objectives
(e.g. *"enable synchronised GSR and video recording"*) were identified from
research goals and refined through prototyping. The project adopted a
**test-driven methodology**: early system versions were built and tested,
with feedback incorporated into subsequent iterations. For example, security
features like **data encryption** and **device fault tolerance** were added
after initial deployment testing (see Appendix F.1.5 for security configuration details).
...
requirement describes what the system implements. These
requirements were derived from the system's capabilities and
verified through the source code and testing:

- **FR1: Multi-Device Sensor Integration** \-- The system supports
  connecting and managing multiple sensor devices simultaneously. This
  includes **discovering and pairing Shimmer3 GSR+ sensors** via direct
  Bluetooth or through an Android device acting as a bridge. The system
  offers a **simulation mode** to generate dummy sensor data for testing
  when real sensors are unavailable (see Appendix F.1.1).

- **FR2: Synchronised Multi-Modal Recording** \-- The system starts
  and stops data recording **synchronously** across all connected
  devices. When a session begins, the PC instructs each Android device
  to begin recording **GSR data**, **video (RGB camera)**, and **thermal
  imaging** in parallel [8]. At the same time, the PC begins logging
  data from any directly connected Shimmer sensors. All streams share a
  common session timestamp to enable later alignment.

- **FR3: Time Synchronisation Service** \-- The system synchronises
  device clocks to ensure temporal alignment of all data streams. The PC
  runs a time synchronisation service on port 8889 (see Appendix F.1.3) where each Android device
  calibrates its clock to the PC's reference time before and during
  recording [9]. In endurance testing from August 2025, the system
  maintained clock synchronisation accuracy with response times under
  100ms (see Appendix F.1.7).

- **FR4: Session Management** \-- The system organises recordings
  into sessions, each with a unique ID or name. The researcher can
  **create a new session** (automatically timestamped) and later
  **terminate the session** when finished. Upon session start, the
  PC creates a directory for data storage and initialises a session
  metadata file using session management functions (see Appendix F.1.2). When the session ends, the
  metadata (start/end time, duration, status) is finalised and saved.
...
- **FR5: Data Recording and Storage** \-- For each session, the system
  ...
  accelerometer, sampled at 128 Hz) [8], and (b) **video and thermal
  data** from each Android device (with at least 1920×1080 video at
  30 FPS) [8]. Sensor readings stream to the PC in real time and are
  written to local CSV files as they arrive to avoid data loss [8].
  Each Android device stores its own raw video/thermal files locally
...
- **FR6: User Interface for Monitoring & Control** \-- The system shall
  ...
  their status (e.g. battery level, streaming/recording state) [8],
  ...
  seconds with the number of samples received) [8]. If a device
  disconnects or encounters an error, the UI should highlight that
  device so the researcher can take appropriate action [8].
...
- **FR7: Device Synchronisation and Signals** \-- The system shall
  ...
  [8]. These signals (e.g. a visual flash on each device's screen)
...
- **FR8: Fault Tolerance and Recovery** \-- If a device (Android or
  ...
  the issue and continue the session with the remaining devices [8].
  The PC will log a warning and mark that device as offline [9]. When
  ...
  while it was offline [9]. This ensures a temporary network drop
...
- **FR9: Calibration Utilities** \-- The system shall include tools for
  **calibrating sensors and cameras**. In particular, it provides a
  ...
  configuration [8]. The resulting calibration data is saved so that
...
- **FR10: Data Transfer and Aggregation** \-- Once a session is stopped,
  ...
  over the network (see Appendix F.1.6). The PC saves each incoming file in the
  session folder and updates the session metadata with that file's entry
  (including file type and size) using session management functions (see Appendix F.1.2). This automation ensures the
  ...
  possible) retries, so that data is not lost [14].
...
them simultaneously is handled by session management logic [8], and session metadata
management is implemented in the session management architecture [8]. See Appendix F.1 for detailed code references.)*
...
- **NFR1: Performance (Real-Time Data Handling)** \-- The system must
  ...
  **30 FPS** video recording concurrently without data loss or buffering
  ...
  which the system writes to storage in real time [8]. Even with
...
- **NFR2: Temporal Accuracy** \-- Clock synchronisation accuracy between
  ...
  after synchronisation) [9]. This is critical for valid sensor
...
- **NFR4: Data Integrity and Validation** \-- All recorded data should
  ...
  mode for sensor data [8] that checks incoming values are within
  expected ranges (for example, verifying GSR readings stay between 0.0
  and 100.0 μS) [8]. Each file transfer from devices is verified for
  completeness (expected file sizes are known and logged in metadata)
  [8]. Session metadata acts as a manifest so that missing or
...
- **NFR5: Security** \-- The system protects recorded data through multiple
  security measures. Network communication between PC and Android devices
  uses TLS 1.3 encryption (see Appendix F.1.5 for configuration details) [8]. Device
  authentication requires 32-character minimum tokens to prevent unauthorised
  device connections [8]. The system includes runtime security checks via
  security validation systems (see Appendix F.1.5) that validate
  encryption and authentication at startup [8]. All recorded data files
  remain local to the PC with no automatic cloud uploads. The system
  verifies file permissions and environment security on startup to avoid
  insecure defaults.

- **NFR6: Usability** \-- The system is designed for ease of use for
  ...
  time [8]. Sensible default settings (e.g. a default dark theme and
  window size) ensure a good user experience out of the box [8]. The
...
- **NFR7: Scalability** \-- The system's architecture should scale to
  ...
  concurrently (the configuration allows up to 10 connections) [8].
  Similarly, the system supports sessions up to at least *120 minutes*
  in duration by default [8]. To manage large video files, recordings
  can be chunked into \~1 GB segments automatically so that individual
  file sizes remain manageable [8]. This ensures that even
...
- **NFR8: Maintainability and Modularity** \-- The system is built in a
  ...
  **Network Server**) and communicate via clear interfaces (see Appendix F.1 for architectural details). This modular
  ...
  affecting others. Configuration is externalised (through configuration files
  and other settings) [8], so that changes in requirements (e.g.,
...
TLS settings and runtime security checks [8] shows that the security
requirements were met, and the multi-threaded design and resource limits
set in the configuration files [8] indicate that performance requirements were
satisfied. See Appendix F.1 for detailed technical implementation references.)*
...
- The Researcher opens the PC control interface (see Appendix F.1.4) and **creates a new
  session** using the SessionManager dialog (providing a session name or
  accepting a default timestamped format "session_YYYYMMDD_HHMMSS"). The
  system validates the name using session validation functions (see Appendix F.1.2)
  and creates a session folder and metadata file on disk[8]. The session
  is now "active" but not recording yet.

- The Researcher selects devices through the device manager panel. The
  interface displays the Shimmer3 GSR+ sensor status and connected Android
  devices with their battery levels (configured via
  network settings as detailed in Appendix F.1.5). If the Shimmer is not yet
  connected, the Researcher clicks "Scan for Devices" in the DevicesFragment.
  The system performs a Bluetooth scan using device management functions (see Appendix F.1.1)
  and finds sensors either directly via PC Bluetooth or through Android's
  paired devices [8]. The Researcher then clicks "Connect" for the Shimmer.
  The system establishes a connection (or uses simulated device mode if
  hardware is unavailable) and updates the UI status to "Connected."
...
- The Researcher checks that video previews from each Android (if
  ...
  receiving data from the Shimmer sensor continuously [8]. The system
...
- The Researcher initiates recording by clicking "Start Recording." The
  ...
  data (if any) back to PC [8]. Simultaneously, the PC instructs the
  Shimmer Manager to start logging data to file [8]. This is done
...
- During the recording, the Researcher can observe real-time status. For
  ...
  Android, 1 Shimmer, 3000 samples") [8]. If the Researcher has a
  ...
  `send_sync_signal` to all Androids to flash their screen LEDs [8]
...
- If any device disconnects mid-session (e.g. an Android phone's WiFi
  ...
  any commands for it [9]. The Researcher can continue the session if
  ...
  commands (all in the background) [9]. This recovery happens without
...
- The Researcher decides to end the recording after, say, 15 minutes.
  ...
  [8]. The Shimmer Manager stops logging GSR data at the same time
  [8]. Each component flushes and closes its output files. The session
  manager marks the session as completed, calculates the duration, and
  updates the session metadata (end time, duration, and status) [8]. A
  log message confirms the session has ended along with its duration and
  sample count stats [8].
...
- After stopping, the system **automatically initiates data transfer**
  ...
  (see Appendix F.1.6). The PC receives each file (via its network server) and saves
  it into the session folder, simultaneously calling
  session management functions (see Appendix F.1.2) to record the file's name and
  size in the metadata [8]. A progress indicator may be shown to the
  Researcher.
...
- **Alternate Flows:**\
  ...
  device status includes battery level) [8]. The Researcher might
...
- The Researcher initiates capture (maybe pressing a "Capture Image"
  ...
  the configuration might specify capturing, say, 10 images [8]. The
...
- The system stores the resulting calibration parameters (e.g. in a
  ...
  For instance, if the reprojection error exceeds the threshold [8]
...
closure) [8]. -- The **Network Server** component (within the
...
9000) [8]. It uses a custom JSON-based protocol for commands and
...
available) and data streaming to the PC [8]. It also multiplexes
...
services time-sync requests from clients [9]. It periodically syncs
...
[8]. These recorders interface with hardware (camera APIs, etc.) and
...
**FileTransferManager** on Android, which, after recording, handles
sending the recorded files to the PC upon request [14]. --
...
connections defined) [8], and each Android client connects to it when
ready. Messages are likely encoded in JSON and could be sent over a
persistent TCP socket (the config specifies `protocol: "TCP"` [8]).
...
samples) over the network to the PC [8]. This is handled by the
...
them similarly. In both cases, each GSR sample is timestamped (using the
...
them out via a background file-writing thread [8]. -- **Video and
...
SessionSynchroniser updates) so the PC is aware of recording progress
[9]. -- **Time Sync and Heartbeats:** Throughout a session, the
...
while, marking it offline after a threshold [9]. Android devices
...
*post-session hand segmentation processing* on the recorded video
[8]. In practice, after all files are in place, the PC could
...
resource usage but not deadlock the system [8].
...
session can still be coherent [9].
...
[14] *bucika_gsr* Repository (GitHub),
**AndroidApp/src/main/java/com/multisensor/recording/managers/FileTransferManager.kt**,
commit 7048f7f6, 2025. ]Online\]. Available:
...
[8] *bucika_gsr* Repository (GitHub),
**PythonApp/production/runtime_security_checker.py**, commit 7048f7f6,
