...
- **NFR5: Security** \-- The system must safeguard the security and
  ...
  configuration) [8]. The system requires authentication tokens
  ...
  [8]. Security checks at startup will warn if encryption or
  authentication is not properly configured [8]. All recorded data
...
- The Researcher opens the PC control interface and **creates a new
  ...
  file on disk[8]. The session is now "active" but not recording yet.
...
- The Researcher selects the devices to use. For example, they ensure
  ...
  Bluetooth or through an Android's paired devices [8]. The Researcher
