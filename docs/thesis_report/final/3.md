...
The system specifically addresses physiological computing research
requirements by implementing **simultaneous multi-modal data capture**.
The architecture supports Shimmer3 GSR+ sensors (128 Hz sampling),
1920×1080 RGB video (30 FPS), thermal imaging, and synchronized
recording across multiple Android devices. Network communication
uses TCP on port 9000 with JSON message protocol (protocol/config.json),
while time synchronization maintains <5 ms accuracy between devices
via NTPTimeServer (PythonApp/ntp_time_server.py). Session management
creates timestamped folders with metadata tracking
(PythonApp/session/session_manager.py), enabling researchers to collect
ground-truth GSR data alongside contactless sensor modalities for
...
The project adopted a **prototyping** methodology with iterative development cycles.
Specific implementation examples include: runtime security checking
(PythonApp/production/runtime_security_checker.py), session fault tolerance
(PythonApp/session/session_synchronizer.py), and NTP time synchronisation
(NTPTimeServer implementation). TLS encryption was added to network communications
based on data protection requirements, as configured in protocol/config.json.
...
- **FR1: Multi-Device Sensor Integration** \-- The system shall support
  ...
  generate dummy sensor data for testing [4].
...
- **FR2: Synchronised Multi-Modal Recording** \-- The system shall start
  ...
  imaging** in parallel [5]. At the same time, the PC begins logging
...
- **FR3: Time Synchronisation Service** \-- The system shall synchronise
  ...
  PC's clock before and during recording [6]. The system achieves timing
  precision on the order of milliseconds (typically <5 ms offset under
  laboratory Wi-Fi conditions), which is sufficient for data integrity
...
- **FR4: Session Management** \-- The system shall organise recordings
  ...
  metadata file [7]. When the session ends, the metadata (start/end
  time, duration, status) is finalised and saved [7]. Only one session
...
- **FR5: Data Recording and Storage** \-- For each session, the system
  ...
  accelerometer, sampled at 128 Hz) [4], and (b) **video and thermal
  ...
  written to local CSV files as they arrive to avoid data loss [4].
...
- **FR6: User Interface for Monitoring & Control** \-- The system shall
  ...
  their status (e.g. battery level, streaming/recording state) [5],
  ...
  seconds with the number of samples received) [5]. If a device
  disconnects or encounters an error, the UI should highlight that
  device so the researcher can take appropriate action [5].
...
- **FR7: Device Synchronisation and Signals** \-- The system shall
  ...
  [5]. These signals (e.g. a visual flash on each device's screen)
...
- **FR8: Fault Tolerance and Recovery** \-- If a device (Android or
  ...
  the issue and continue the session with the remaining devices [5].
...
- **FR10: Data Transfer and Aggregation** \-- Once a session is stopped,
  ...
  over the network [10]. The PC saves each incoming file in the
  session folder and updates the session metadata with that file's entry
  (including file type and size) [7]. This automation ensures the
  ...
  possible) retries, so that data is not lost [10].

*(The above functional requirements are implemented in specific components:
multi-device connection and recording via ShimmerManager
(PythonApp/shimmer_manager.py), session metadata management through
SessionManager.create_session() and add_file_to_session() methods
(PythonApp/session/session_manager.py:46-119), and file transfer handling
via FileTransferManager.kt
...
- **NFR2: Temporal Accuracy** \-- Clock synchronisation accuracy between
  ...
  after synchronisation) [6]. This is critical for valid sensor
...
- **NFR4: Data Integrity and Validation** \-- All recorded data should
  ...
  mode for sensor data [4] that checks incoming values are within
  expected ranges (for example, verifying GSR readings stay between 0.0
  and 100.0 μS) [4]. Each file transfer from devices is verified for
  completeness (expected file sizes are known and logged in metadata)
  [7]. Session metadata acts as a manifest so that missing or
...
- **NFR5: Security** \-- The system must safeguard the security and
  ...
  configuration) [8, 11]. The system requires authentication tokens
  ...
  [8, 11]. Security checks at startup will warn if encryption or
  authentication is not properly configured [11]. All recorded data
...
- **NFR6: Usability** \-- The system should be easy to use for
  ...
  time [5]. Sensible default settings (e.g. a default dark theme and
...
*(The non-functional requirements are validated through: TLS encryption
implementation (protocol/config.json:62-66), runtime security verification
(PythonApp/production/runtime_security_checker.py), performance limits
configuration (protocol/config.json:11-12 for max_connections=10,
heartbeat_interval=5), and session fault tolerance mechanisms
(PythonApp/session/session_synchronizer.py))*
...
- The Researcher opens the PC control interface and **creates a new
  ...
  file on disk[7]. The session is now "active" but not recording yet.
...
- The Researcher selects the devices to use. For example, they ensure
  ...
  Bluetooth or through an Android's paired devices [4]. The Researcher
...
- The Researcher checks that video previews from each Android (if
  ...
  receiving data from the Shimmer sensor continuously [5]. The system
...
- The Researcher initiates recording by clicking "Start Recording." The
  ...
  data (if any) back to PC [5]. Simultaneously, the PC instructs the
  Shimmer Manager to start logging data to file [5]. This is done
...
- During the recording, the Researcher can observe real-time status. For
  ...
  Android, 1 Shimmer, 3000 samples") [5]. If the Researcher has a
  ...
  `send_sync_signal` to all Androids to flash their screen LEDs [5]
...
- The Researcher decides to end the recording after, say, 15 minutes.
  ...
  [5]. The Shimmer Manager stops logging GSR data at the same time
  ...
  updates the session metadata (end time, duration, and status) [7]. A
  log message confirms the session has ended along with its duration and
  sample count stats [5].
...
- After stopping, the system **automatically initiates data transfer**
  ...
  [10]. The PC receives each file (via its network server) and saves
  it into the session folder, simultaneously calling
  `SessionManager.add_file_to_session()` to record the file's name and
  size in the metadata [7]. A progress indicator may be shown to the
  Researcher.
...
- **Alternate Flows:**\
  ...
  device status includes battery level) [11]. The Researcher might
...
- After the required number of calibration images are collected, the
  ...
  algorithm (implementing Zhang's method via OpenCV's camera calibration API)
...
**Description:** A researcher reviews completed sessions and manages
stored data for analysis preparation.

**Main Flow:**
- The researcher opens the PC application and selects "View Sessions"
  to see the session history maintained by SessionManager
- The system displays a list of completed sessions with metadata
  (session ID, duration, device count, file count) from the session
  JSON files stored in the recordings directory
- The researcher selects a specific session to view detailed information,
  including start/end times, participating devices, and collected files
- The system provides options to export session metadata or prepare
  data for external analysis tools

*(This functionality is implemented through SessionManager.get_session_history()
and the session metadata JSON structure documented in
...
closure) [7]. -- The **Network Server** component (within the
...
9000) [4, 8]. It uses a custom JSON-based protocol for commands and
...
available) and data streaming to the PC [4]. It also multiplexes
...
services time-sync requests from clients [6]. It periodically syncs
...
[5]. These recorders interface with hardware (camera APIs, etc.) and
...
sending the recorded files to the PC upon request [10]. --
...
ready. Messages are encoded in JSON and sent over persistent TCP
sockets (the config specifies `protocol: "TCP"` [8]).
...
samples) over the network to the PC [4]. This is handled by the
...
synchronised clock) and logged. The PC accumulates data in memory
(via `data_queues` data structures) and writes them out through
background file-writing threads [4]. -- **Video and
...
saves it. The PC tracks session start time and device timestamps
(Android embeds timestamp metadata in video files via the recording
API), enabling post-processing alignment. The Android app sends
periodic timestamps during recording to the PC via SessionSynchroniser
updates, providing recording progress information [9]. -- **Time Sync and Heartbeats:** Throughout a session, the
PC sends periodic time sync packets to Android devices using the NTPTimeServer.
The `SessionSynchronizer` on PC maintains a heartbeat mechanism: it tracks
device connectivity states and marks devices offline after a threshold timeout [9]. Android devices
send status messages every few seconds with recording state information
("alive, recording, file size = ...") as defined in the network protocol.
...
[7]. In practice, after all files are in place, the PC could
...
**System Architecture Overview:** The system implements a hub-and-spoke
architecture with the PC as the central coordinator. The PC contains
modular components including the Session Manager
(PythonApp/session/session_manager.py), Shimmer Manager
(PythonApp/shimmer_manager.py), Network Server listening on port 9000
(protocol/config.json), and NTP Time Server (PythonApp/ntp_time_server.py).
Each Android client runs recording modules for camera, thermal imaging,
and network communication. Bluetooth connections link Shimmer sensors
to either the PC directly or through Android devices, while Wi-Fi/LAN
connections enable command/control and data transfer between PC and
Android devices. Time synchronisation flows from PC to all clients
to maintain temporal alignment across data streams.
...
resource usage but not deadlock the system [4].
...
[4] *bucika_gsr* Repository (GitHub),
...
[5] *bucika_gsr* Repository (GitHub), **PythonApp/shimmer_pc_app.py**,
...
[6] *bucika_gsr* Repository (GitHub),
...
[7] *bucika_gsr* Repository (GitHub),
...
[10] *bucika_gsr* Repository (GitHub),
**AndroidApp/src/main/java/com/multisensor/recording/managers/FileTransferManager.kt**,
commit 7048f7f6, 2025. [Online]. Available:
...
[11] *bucika_gsr* Repository (GitHub),
