...
(commit 7a2f9b4), Android capture application (tag v0.8.1), and
Shimmer3 GSR+ sensor interface to record synchronised physiological and
visual data. Across 15 test sessions (8-12 minutes each), the system
...
- **Star-topology Network Architecture:** The implementation uses a
  ...
  status every 2 seconds. Testing with 4 Samsung Galaxy devices
  (S10, S21, A52, A54) revealed reliable coordination up to 6 concurrent
...
- **Camera Calibration Implementation:** The calibration system
  ...
  calibration using 9×6 checkerboard patterns. For spatial alignment,
...
- **TCP Socket Protocol Implementation:** The networking layer uses
  ...
  message protocol for device coordination. Message types include:
...
- **Qt5 Desktop Interface Implementation:** The desktop controller
  ...
  management, session control, and live status monitoring. Key
...
controller (commit 7a2f9b4), Android app (tag v0.8.1), and Shimmer3 GSR+
into a functioning system. During the final demonstration on 2024-12-15,
...
15:41, device Samsung_A52 (IP 192.168.1.107) remained "Connecting..." for
...
library. Android devices would register as `_bucika._tcp.local` services
...
The complete system is available in the GitHub repository at commit
`d2be9e0`. Key components:
...
Build the Android app: `./gradlew assembleDebug` (requires Android SDK 34)
Python environment: `conda env create -f environment.yml` (Python 3.10+)
...
Missing: No human participant data due to lack of pilot study.

**Reproducibility Verification**

To replicate the synchronisation results: run `python test_sync_accuracy.py`
using the GPS reference clock setup documented in `docs/test_execution_guide.md`.
Expected output: ~2.7ms median drift across 4+ devices.

The system can be reproduced from these artifacts, though UI stability
issues will require the specific hardware/software versions listed above.
