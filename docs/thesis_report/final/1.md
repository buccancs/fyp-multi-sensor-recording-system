...
stress monitoring via GSR or related metrics \[2\]\[3\]. This surge of
...
\[4\]. This method is obtrusive: wires and electrodes restrict movement,
and long-term use can cause discomfort or skin irritation \[5\]\[6\].
These limitations make it difficult to use GSR outside the lab.
Consequently, **contactless GSR measurement** has become an appealing
research direction \[7\]. The idea is to infer GSR (or the underlying
...
offering a proxy for stress responses \[8\].
...
(e.g., perspiration causes cooling) \[9\]. Similarly, high-resolution
...
\[10\]\[11\]. These developments suggest that *multi-modal sensing* ---
...
not synchronised in real time \[13\]\[14\]. For instance, thermal
...
\[13\]. Bridging this gap requires a platform that records **multiple
...
- **Objective 1: Multi-Modal Platform Development.** *Design and develop
  ...
  the system uses a **research-grade** Shimmer3 GSR+ sensor \[15\] for
  ground-truth skin conductance, a Topdon TC001 thermal camera \[15\]
...
- **Objective 2: Synchronised Data Acquisition and Management.**
  *Implement methods for precise time synchronisation and data handling
  ...
  transmit control commands and streaming data \[16\]. Data management
...
- **Chapter 4 \-- System Design and Architecture:** This chapter
  ...
  hub and a PC as the central controller \[16\]). The chapter details
