...
*distress* (negative, harmful stress)[7]. For example, excitement
...
of an acute stressor[9]. For example, a sudden fright or mental
...
than cortisol changes[9]. GSR is essentially measuring one facet of
...
typically followed by a rise in cortisol 20--30 minutes later[9].
...
experiments[9]. Unlike cortisol, measuring GSR is non-invasive and
...
tens of minutes to hours[9]. GSR is a **neuronal indicator**
...
conductance, though the correlation is far from perfect[9]. This
...
peaks, effectively estimating cortisol from GSR[9]. This reinforces
...
high density on palms, fingers, and soles)[10]. These glands are
...
baseline level (*skin conductance level*, SCL)[10][11].
...
regarded as a pure measure of *autonomic arousal*[12].
Physiologically, the mechanism can be summarized as: **emotional
sweating** causes ionic changes that increase skin conductance[13].
...
linked to psychological stimuli[10]. (Historically, this is why the
...
- **Motion Artifacts and Contact Issues:** Because GSR electrodes are
  ...
  flag affected data segments[14]. Contact quality is another issue:
...
- **Units and Calibration:** GSR can be reported either as conductance
  ...
  convert to conductance[15]. Calibration to absolute units can be
...
convenient measures of emotional arousal[11]. Its drawbacks can be
...
resolution (16-bit data at 128 Hz sampling[15]) to capture
...
(sweating), each of which has thermal consequences[4][9].
...
temperature (hence the term "cold sweat")[9]. These physiological
...
the cheeks) during a startle or mental stress task[4][9]. The nose,
...
with a nose temperature drop[4][9]. Essentially, the face shows a
...
autonomic measures under stress[4][9]. They confirmed the
...
shifts in cardiac autonomic balance[9]. Another line of work involves
...
is why thermal is used in night vision and surveillance[12]. For
...
256×192 pixels[16], compared to a typical RGB video of 1920×1080 or
...
computing[13]. Moreover, unlike purely vision-based methods on RGB
...
align the two camera views[14]. There is also the issue of data
...
- *High-Quality GSR Data:* The Shimmer GSR+ provides a high sampling
  ...
  resolution** on the GSR channel[15], which is well above the minimum
  ...
  conductance values[15]. This ensures that both very small responses
...
- *Multi-Channel Capability:* Although GSR is our primary interest, the
  ...
  package (accelerometer, gyroscope, etc.)[14]. These extra channels
  add value: the PPG can be used to derive heart rate and heart rate
  variability, providing another stress indicator alongside GSR[14].
...
- *Bluetooth Wireless Connectivity:* The Shimmer connects via Bluetooth,
  ...
  handles connecting to the Shimmer and streaming its data[14]. We
...
- *Open SDK and Integration:* Shimmer provides open-source APIs (for
  ...
  library on the PC side[16]. This saved significant development time.
  ...
  the recording session[14]. The PC controller includes a
  `ShimmerManager` that can manage multiple Shimmer devices and
  coordinate their data with incoming camera data[14]. Using the
...
- *Resolution and Frame Rate:* The Topdon TC camera offers a **thermal
  ...
  25 Hz[16]. This is a higher resolution than older consumer thermal
  ...
  resolution and data size[16]. (Many lower-cost thermal devices cap
...
- *Radiometric Data Access:* Importantly, the Topdon SDK provides
  **radiometric data** -- meaning we can retrieve the actual temperature
  ...
  image and a temperature matrix for each frame[17][18]. The
  ...
  representation[18]. This quantitative data is crucial for analysis
  ...
  full access. Having the **image + temperature mode** enabled[16]
...
- *SDK and Support:* The Topdon came with an **InfiSense IRUVC SDK** (as
  seen in our code imports like `com.infisense.iruvc.*`)[19], which
  ...
  permissions and streaming in the Android app[20]. The SDK supports
  pulling frames via a callback -- we use an `IFrameCallback` interface
  to get each frame's byte data in real time[21]. Without such SDK
...
- *Synchronization and System Fit:* By using the Topdon with an Android
  ...
  be aligned post-hoc with sub-millisecond precision[14]. When
  ...
  mind[14]. The PC acts as a master coordinating multiple Android
  ...
  *star-mesh topology* of our system means each Android device is
  relatively self-contained in sensing capability[14]. The Topdon
