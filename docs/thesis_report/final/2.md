...
of an acute stressor[22]. For example, a sudden fright or mental
...
than cortisol changes[22]. GSR is essentially measuring one facet of
...
typically followed by a rise in cortisol 20--30 minutes later[22].
...
experiments[22]. Unlike cortisol, measuring GSR is non-invasive and
...
tens of minutes to hours[22]. GSR is a **neuronal indicator**
...
conductance, though the correlation is far from perfect[22]. This
...
peaks, effectively estimating cortisol from GSR[22]. This reinforces
...
- **Units and Calibration:** GSR can be reported either as conductance
  ...
  convert to conductance[7]. Calibration to absolute units can be
...
resolution (16-bit data at 128 Hz sampling[7]) to capture
...
(sweating), each of which has thermal consequences[4][20].
...
temperature (hence the term "cold sweat")[20]. These physiological
...
the cheeks) during a startle or mental stress task[4][20]. The nose,
...
with a nose temperature drop[4][20]. Essentially, the face shows a
...
autonomic measures under stress[4][20]. They confirmed the
...
shifts in cardiac autonomic balance[20]. Another line of work involves
...
256×192 pixels[8], compared to a typical RGB video of 1920×1080 or
...
- *High-Quality GSR Data:* The Shimmer GSR+ provides a high sampling
  ...
  resolution** on the GSR channel[7], which is well above the minimum
  ...
  conductance values[7]. This ensures that both very small responses
...
- *Open SDK and Integration:* Shimmer provides open-source APIs (for
  ...
  library on the PC side[12]. This saved significant development time.
...
- *Resolution and Frame Rate:* The Topdon TC camera offers a **thermal
  ...
  resolution and data size[8]. (Many lower-cost thermal devices cap
...
- *Radiometric Data Access:* Importantly, the Topdon SDK provides
  **radiometric data** -- meaning we can retrieve the actual temperature
  ...
  full access. Having the **image + temperature mode** enabled[8]
