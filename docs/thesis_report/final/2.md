...
interfaces or social robots [MDPI(2020)]. For example, **galvanic skin response
...
testing*, and *neuromarketing* [Jangra et al.(2021)]. In psychological research, changes
in skin conductance have been analyzed to understand anxiety during
therapy or excitement during gameplay [Jangra et al.(2021)]. In **human-computer
...
engagement with software interfaces [Jangra et al.(2021)]. Even in critical domains such
...
involuntary arousal [Boucsein(2012)]. This has motivated the integration of
...
impractical for continuous real-life monitoring [Boucsein(2012)]. In contrast,
...
potentially confounding the very signals under study [Boucsein(2012)]. By using
...
responses without the need for electrodes or wires [Computer Modeling in Engineering(2021)].
...
stress-induced temperature changes[Jangra et al.(2021)]. Such approaches open the door
...
somatic"[Nature Scientific Reports(2019)]. This definition frames stress as an **adaptive response**
...
*distress* (negative, harmful stress)[Shimmer Sensing(2024)]. For example, excitement
...
usually denotes an excessive or unpleasant psychological state[Nature Scientific Reports(2019)].
...
axis[Shimmer Sensing(2024)]. *Galvanic Skin Response (GSR)*, also known as electrodermal
...
of an acute stressor[Science Direct(2025)]. For example, a sudden fright or mental
...
real-time or continuous monitoring[Selye(1974)]. Despite these challenges,
...
than cortisol changes[Science Direct(2025)]. GSR is essentially measuring one facet of
...
typically followed by a rise in cortisol 20--30 minutes later[Science Direct(2025)].
...
experiments[Science Direct(2025)]. Unlike cortisol, measuring GSR is non-invasive and
...
tens of minutes to hours[Science Direct(2025)]. GSR is a **neuronal indicator**
...
excitement, cognitive effort, etc.) will produce a GSR change[Selye(1956)].
...
conductance, though the correlation is far from perfect[Science Direct(2025)]. This
...
peaks, effectively estimating cortisol from GSR[Science Direct(2025)]. This reinforces
...
high density on palms, fingers, and soles)[Google Android Developers(2024)]. These glands are
...
baseline level (*skin conductance level*, SCL)[Google Android Developers(2024)][Nordic Semiconductor(2024)].
...
regarded as a pure measure of *autonomic arousal*[Shimmer Engineering(2024)].
Physiologically, the mechanism can be summarized as: **emotional
sweating** causes ionic changes that increase skin conductance[Serenegiant(2024)].
...
linked to psychological stimuli[Google Android Developers(2024)]. (Historically, this is why the
...
- **Non-specificity of Arousal:** As noted, GSR measures arousal, not
  ...
  signals) is needed to interpret the meaning of a GSR change[Selye(1956)].
...
- **Environmental Factors:** GSR data can be influenced by the
  ...
  is at rest before measurement helps[Selye(1956)]. In our platform, we log
...
- **Motion Artifacts and Contact Issues:** Because GSR electrodes are
  ...
  flag affected data segments[Boucsein(2012)]. Contact quality is another issue:
...
- **Units and Calibration:** GSR can be reported either as conductance
  ...
  convert to conductance[Shimmer Sensing(2024)]. Calibration to absolute units can be
...
convenient measures of emotional arousal[Nordic Semiconductor(2024)]. Its drawbacks can be
...
resolution (16-bit data at 128 Hz sampling[Shimmer Sensing(2024)]) to capture
...
temperature[MDPI(2020)]. Under stress, the autonomic nervous system alters both
**vasomotor tone** (blood vessel diameter) and **sudomotor activity**
(sweating), each of which has thermal consequences[MDPI(2020)][Computer Modeling in Engineering(2021)].
...
temperature (hence the term "cold sweat")[Computer Modeling in Engineering(2021)]. These physiological
...
the cheeks) during a startle or mental stress task[MDPI(2020)][Computer Modeling in Engineering(2021)]. The nose,
...
stress* compared to baseline[MDPI(2020)]. This nose-tip cooling is on the order
...
with a nose temperature drop[MDPI(2020)][Computer Modeling in Engineering(2021)]. Essentially, the face shows a
...
imaging[MDPI(2020)][Nordic Semiconductor(2024)].
...
autonomic measures under stress[MDPI(2020)][Computer Modeling in Engineering(2021)]. They confirmed the
...
shifts in cardiac autonomic balance[Computer Modeling in Engineering(2021)]. Another line of work involves
...
under stress is thus another thermal cue[Boucsein(2012)]. In fact, one system by
...
breathing changes alone[Boucsein(2012)].
...
**78.3% accuracy** in binary stress classification[Boucsein(2012)]. RGB cameras are
...
is why thermal is used in night vision and surveillance[Shimmer Engineering(2024)]. For
...
more equipment[Boucsein(2012)]. This shows that combining thermal and visual
...
significantly higher than using visible images alone[Boucsein(2012)]. Such results
...
computing[Serenegiant(2024)]. Moreover, unlike purely vision-based methods on RGB
...
align the two camera views[Boucsein(2012)]. There is also the issue of data
...
accurately in time[Boucsein(2012)].
...
- *High-Quality GSR Data:* The Shimmer GSR+ provides a high sampling
  ...
  resolution** on the GSR channel[Shimmer Sensing(2024)], which is well above the minimum
  ...
  conductance values[Shimmer Sensing(2024)]. This ensures that both very small responses
...
- *Multi-Channel Capability:* Although GSR is our primary interest, the
  ...
  package (accelerometer, gyroscope, etc.)[Riverbank Computing(2024)]. These extra channels
  add value: the PPG can be used to derive heart rate and heart rate
  variability, providing another stress indicator alongside GSR[Riverbank Computing(2024)].
...
- *Bluetooth Wireless Connectivity:* The Shimmer connects via Bluetooth,
  ...
  handles connecting to the Shimmer and streaming its data[Riverbank Computing(2024)]. We
...
- *Open SDK and Integration:* Shimmer provides open-source APIs (for
  ...
  library on the PC side[Shimmer Engineering(2024)]. This saved significant development time.
  ...
  the recording session[Riverbank Computing(2024)]. The PC controller includes a
  `ShimmerManager` that can manage multiple Shimmer devices and
  coordinate their data with incoming camera data[Riverbank Computing(2024)]. Using the
...
- *Validated Performance:* The Shimmer3 GSR+ has been validated in prior
  ...
  important[Shimmer Sensing(2024)].
...
- *Resolution and Frame Rate:* The Topdon TC camera offers a **thermal
  ...
  25 Hz[Jakob(2024)]. This is a higher resolution than older consumer thermal
...
- *Radiometric Data Access:* Importantly, the Topdon SDK provides
  **radiometric data** -- meaning we can retrieve the actual temperature
  ...
  image and a temperature matrix for each frame[LSL Community(2024)][Python Software Foundation(2024)]. The
  ...
  representation[Python Software Foundation(2024)]. This quantitative data is crucial for analysis
...
- *SDK and Support:* The Topdon came with an **InfiSense IRUVC SDK** (as
  seen in our code imports like `com.infisense.iruvc.*`)[Neural Sense(2024)], which
  ...
  permissions and streaming in the Android app[Computer Modeling in Engineering(2021)]. The SDK supports
  pulling frames via a callback -- we use an `IFrameCallback` interface
  to get each frame's byte data in real time[RTI International(2024)]. Without such SDK
  ...
  runtime[Science Direct(2025)][Topdon Community(2024)].
...
- *synchronisation and System Fit:* By using the Topdon with an Android
  ...
  be aligned post-hoc with sub-millisecond precision[Riverbank Computing(2024)]. When
  ...
  mind[Riverbank Computing(2024)]. The PC acts as a master coordinating multiple Android
  ...
  *star-mesh topology* of our system means each Android device is
  relatively self-contained in sensing capability[Riverbank Computing(2024)]. The Topdon
  ...
  in both data streams[Postel(1980)].
