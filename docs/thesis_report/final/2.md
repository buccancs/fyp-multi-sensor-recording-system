...
**wearable sensors** and **imaging** for richer emotion analysis. Our
work follows this trend: we employ a wearable GSR sensor alongside
camera-based thermal imaging to capture both external and internal
indicators of stress. By collecting synchronized video, thermal, and
...
impractical for continuous real-life monitoring【3】【4】. In contrast,
...
potentially confounding the very signals under study【4】. By using
...
responses without the need for electrodes or wires【4】.
...
stress-induced temperature changes【5】. Such approaches open the door
...
In our context of GSR prediction, the goal is to **infer stress-related
...
the latter. The motivation is that if we can reliably predict GSR from,
...
user-friendly**, which is why our platform emphasizes integrating a
...
somatic"【6】. This definition frames stress as an **adaptive response**
...
*distress* (negative, harmful stress)【6】. For example, excitement
...
stress," focusing on external stressors, whereas scientifically we
distinguish the *stressor* (e.g. job demands) from the *stress response*
...
work, we interpret elevated GSR or cortisol as indicators of
...
usually denotes an excessive or unpleasant psychological state【6】.
...
axis【7】. *Galvanic Skin Response (GSR)*, also known as electrodermal
...
of an acute stressor【8】. For example, a sudden fright or mental
...
real-time or continuous monitoring【3】. Despite these challenges,
...
than cortisol changes【8】. GSR is essentially measuring one facet of
...
typically followed by a rise in cortisol 20--30 minutes later【8】.
...
experiments【8】. Unlike cortisol, measuring GSR is non-invasive and
...
tens of minutes to hours【8】. GSR is a **neuronal indicator**
...
excitement, cognitive effort, etc.) will produce a GSR change【2】.
...
conductance, though the correlation is far from perfect【8】. This
...
peaks, effectively estimating cortisol from GSR【8】. This reinforces
...
high density on palms, fingers, and soles)【9】. These glands are
...
baseline level (*skin conductance level*, SCL)【9】【10】.
...
regarded as a pure measure of *autonomic arousal*【11】.
Physiologically, the mechanism can be summarized as: **emotional
sweating** causes ionic changes that increase skin conductance【12】.
...
linked to psychological stimuli【9】. (Historically, this is why the
...
- **Non-specificity of Arousal:** As noted, GSR measures arousal, not
  ...
  signals) is needed to interpret the meaning of a GSR change【2】.
...
- **Environmental Factors:** GSR data can be influenced by the
  ...
  is at rest before measurement helps【2】. In our platform, we log
...
- **Motion Artifacts and Contact Issues:** Because GSR electrodes are
  ...
  flag affected data segments【13】. Contact quality is another issue:
...
- **Units and Calibration:** GSR can be reported either as conductance
  ...
  convert to conductance【14】. Calibration to absolute units can be
  ...
  standard scores. Nonetheless, for our predictive modeling it is
  helpful to work in consistent units (we use μS), so we calibrate the
  Shimmer output accordingly.

Despite these limitations, GSR remains one of the most sensitive and
convenient measures of emotional arousal【10】. Its drawbacks can be
managed through careful design and data processing. In the context of
this thesis, GSR provides the ground truth "stress signal" we aim to
predict using other sensors. We leverage the Shimmer GSR sensor's high
resolution (16-bit data at 128 Hz sampling【14】) to capture
fine-grained electrodermal dynamics. At the same time, we implement
strategies like baseline normalization, synchronization with other
channels, and artifact filtering to ensure the GSR data is reliable. By
acknowledging GSR's limitations, our approach -- especially the
...
temperature【4】. Under stress, the autonomic nervous system alters both
**vasomotor tone** (blood vessel diameter) and **sudomotor activity**
(sweating), each of which has thermal consequences【4】【8】.
...
temperature (hence the term "cold sweat")【8】. These physiological
...
the cheeks) during a startle or mental stress task【4】【8】. The nose,
...
stress* compared to baseline【4】. This nose-tip cooling is on the order
...
with a nose temperature drop【4】【8】. Essentially, the face shows a
...
autonomic measures under stress【4】【8】. They confirmed the
...
shifts in cardiac autonomic balance【8】. Another line of work involves
...
This section outlines the rationale and hypothesis guiding our use of
...
temperature terms if muscle movements don't alter blood flow). Instead,
...
is why thermal is used in night vision and surveillance【11】. For
...
256×192 pixels【15】, compared to a typical RGB video of 1920×1080 or
higher). They also lack colour or texture information -- everything is a
temperature reading -- which means they won't capture certain stress
cues like trembling (unless it causes temperature fluctuation) or facial
flushing that doesn't significantly change heat emission. Additionally,
...
identifying individuals or analyzing facial expressions is harder.
...
images to recognize emotional states, using a blood perfusion model on
...
computing【12】. Moreover, unlike purely vision-based methods on RGB
...
registration are needed. Our system tackles this with calibration
procedures (e.g. using an Android calibration routine and OpenCV) to
align the two camera views【13】. There is also the issue of data
...
synchronization make this manageable. Our design includes a
synchronization engine that timestamps frames from both the RGB and
thermal cameras to within 1 ms, ensuring data streams can be fused
accurately in time【13】.
...
hardware components that balance **signal quality**, **integration
...
how their characteristics support our system's goals.
...
- *High-Quality GSR Data:* The Shimmer GSR+ provides a high sampling
  ...
  resolution** on the GSR channel【14】, which is well above the minimum
  ...
  conductance values【14】. This ensures that both very small responses
...
- *Multi-Channel Capability:* Although GSR is our primary interest, the
  ...
  package (accelerometer, gyroscope, etc.)【13】. These extra channels
  add value: the PPG can be used to derive heart rate and heart rate
  variability, providing another stress indicator alongside GSR【13】.
  ...
  signals, the Shimmer offers them in one unit, time-synchronized. In
  our implementation, we enable the accelerometer to log motion, which
  helps in data cleaning (e.g. if a participant moves suddenly and a GSR
  spike occurs, we can attribute it to motion). Having all these streams
...
- *Bluetooth Wireless Connectivity:* The Shimmer connects via Bluetooth,
  ...
  the sensor data to be synchronized easily with other mobile devices
  (like an Android phone running the cameras). The Shimmer's Bluetooth
  interface is supported by an official API. In our system architecture,
  a **Shimmer Manager** module on the PC (and optionally on Android)
  handles connecting to the Shimmer and streaming its data【13】. We
  enabled the Bluetooth interface to integrate Shimmer data seamlessly
  into our multi-device recording sessions. The alternative, a wired GSR
...
- *Open SDK and Integration:* Shimmer provides open-source APIs (for
  ...
  sensor without reverse-engineering proprietary formats. We leveraged
  ...
  For example, our Android app includes a `ShimmerRecorder` component
  that interfaces with the Shimmer over Bluetooth and streams data into
  the recording session【13】. The PC controller includes a
  `ShimmerManager` that can manage multiple Shimmer devices and
  coordinate their data with incoming camera data【13】. Using the
...
component, we selected a **Topdon** USB thermal camera (specifically the
...
- *Smartphone Integration:* The Topdon camera is designed to plug into
  ...
  aligns perfectly with our system architecture: we wanted the thermal
  camera to be part of a mobile setup, leveraged by an Android app.
  Using a smartphone-based thermal camera means we can use the phone's
...
- *Resolution and Frame Rate:* The Topdon TC camera offers a **thermal
  ...
  25 Hz【15】. This is a higher resolution than older consumer thermal
  ...
  capturing dynamic changes and aligning frames reasonably well with our
  ...
  resolution and data size【15】. (Many lower-cost thermal devices cap
...
- *Radiometric Data Access:* Importantly, the Topdon SDK provides
  **radiometric data** -- meaning we can retrieve the actual temperature
  reading for each pixel, not just a false-colour image. In our
  implementation, we configure the camera to output both the thermal
  image and a temperature matrix for each frame【16】【17】. The
  `ThermalRecorder` splits the incoming frame bytes into an image buffer
  and a temperature buffer, so we record a raw thermal matrix (with
  calibrated temperature values per pixel) alongside the visual
  representation【17】. This quantitative data is crucial for analysis
  (we can measure, say, that the nose is at 33.1 °C and dropped to
  ...
  full access. Having the **image + temperature mode** enabled【15】
...
- *Cost and Availability:* The Topdon camera is relatively affordable
  ...
  made it feasible to acquire and deploy for our project. High-end
  ...
  a widely available consumer device aligns with our vision of future
  ...
  performance and cost that matched our requirements (we did evaluate
...
- *SDK and Support:* The Topdon came with an **InfiSense IRUVC SDK** (as
  seen in our code imports like `com.infisense.iruvc.*`)【18】, which
  was crucial for rapid integration. Through this SDK, we control camera
  settings (emissivity, temperature range, etc.) and handle USB
  permissions and streaming in the Android app【19】. The SDK supports
  pulling frames via a callback -- we use an `IFrameCallback` interface
  to get each frame's byte data in real time【20】. Without such SDK
  support, integrating a raw thermal feed into our app would have been
...
- *Synchronization and System Fit:* By using the Topdon with an Android
  ...
  be aligned post-hoc with sub-millisecond precision【13】. When
  ...
  mind【13】. The PC acts as a master coordinating multiple Android
  units (each potentially running a Topdon and phone camera). The
  *star-mesh topology* of our system means each Android device is
  relatively self-contained in sensing capability【13】. The Topdon
  ...
  both are streamed live, and our software can inject synchronization
  ...
  in both data streams【23】.

------------------------------------------------------------------------

**References:**

\[1\] C. Filippini, D. Perpetuini, D. Cardone, A. M. Chiarelli, and A.
Merla, "Thermal infrared imaging-based affective computing and its
application to facilitate human--robot interaction: a review," *Applied
Sciences*, vol. 10, no. 8, Article 2924, 2020. DOI: 10.3390/app10082924.

\[2\] Noldus Information Technology, "What is Galvanic Skin Response
(GSR)? -- The complete pocket guide," *Noldus Innovation Blog*, 2018.
\[Online\]. Available:
https://noldus.com/blog/what-is-galvanic-skin-response (accessed Jul.
2025).

\[3\] J. H. D. M. Westerink *et al.*, "Deriving a cortisol-related
stress indicator from wearable skin conductance measurements:
quantitative model & experimental validation," *Frontiers in Computer
Science*, vol. 2, art. 39, 2020. DOI: 10.3389/fcomp.2020.00039.

\[4\] F. Gioia, M. Nardelli, E. P. Scilingo, and A. Greco, "Autonomic
regulation of facial temperature during stress: a cross-mapping
analysis," *Sensors (Basel)*, vol. 23, no. 14, Article 6403, Jul. 2023.
DOI: 10.3390/s23146403.

\[5\] Y. Cho, S. J. Julier, and N. Bianchi-Berthouze, "Instant Stress:
detection of perceived mental stress through smartphone
photoplethysmography and thermal imaging," *JMIR Mental Health*, vol. 6,
no. 4, p. e10140, Apr. 2019. DOI: 10.2196/10140.

\[6\] Wikipedia, "Psychological stress," *Wikipedia, The Free
Encyclopedia*. \[Online\]. Available:
https://en.wikipedia.org/wiki/Psychological_stress (accessed Jul. 2025).

\[7\] M. Czaban and C. Himmels, "Investigating simulator validity by
using physiological and cognitive stress indicators," *Transportation
Research Part F: Traffic Psychology and Behaviour*, 2025 (in press).

\[8\] iMotions, "Galvanic Skin Response (GSR): The complete pocket
guide," *iMotions Blog*, 2021. \[Online\]. Available:
https://imotions.com/blog/galvanic-skin-response (accessed Jul. 2025).

\[9\] M. T. Willoughby, T. Slade, P. Gaur, A. Wylie, and C. Strigel,
*Using Thermal Imaging to Measure Mental Effort: Does the Nose Know?*
Research Triangle Park, NC: RTI Press, Mar. 2025, Research Brief
RB-0041-2503. DOI: 10.3768/rtipress.2025.rb.0041.2503.

\[10\] S. Sonkusare *et al.*, "Detecting changes in facial temperature
induced by a sudden auditory stimulus based on deep learning-assisted
face tracking," *Scientific Reports*, vol. 9, no. 1, p. 4729, 2019. DOI:
10.1038/s41598-019-41172-7.

\[11\] Lynred USA, "Visible vs. thermal detection: advantages and
disadvantages," *Lynred Blog*, 2021. \[Online\]. Available:
https://www.lynred-usa.com/blog/visible-vs-thermal-detection-advantages-and-disadvantages
(accessed Jun. 2025).

\[12\] B. L. Arasu *et al.*, "Human stress recognition from facial
thermal-based signature: a literature survey," *Computer Modeling in
Engineering & Sciences*, vol. 130, no. 2, pp. 633--652, 2022. DOI:
10.32604/cmes.2022.018702.

\[13\] C. Buccancs, **System Architecture Documentation
(architecture.md)**, *bucika_gsr* GitHub repository, commit 7048f7f,
2024. \[Online\]. Available:
https://github.com/buccancs/bucika_gsr/docs/architecture.md (accessed
Jul. 2025).

\[14\] C. Buccancs, **Hardware Documentation (hardware.md)**,
*bucika_gsr* GitHub repository, commit ea44d02, 2024. \[Online\].
Available: https://github.com/buccancs/bucika_gsr/docs/hardware.md
(accessed Jul. 2025).

\[15\] C. Buccancs, **ThermalRecorder.kt Source Code**, *bucika_gsr*
GitHub repository, commit 7048f7f, 2024. \[Online\]. Available:
https://github.com/buccancs/bucika_gsr/AndroidApp/\.../ThermalRecorder.kt
(accessed Jul. 2025).

\[16\] C. Buccancs, **monitor_vendor_sdks.py Source Code**, *bucika_gsr*
GitHub repository, commit 7048f7f, 2024. \[Online\]. Available:
https://github.com/buccancs/bucika_gsr/scripts/monitor_vendor_sdks.py
(accessed Jul. 2025).

\[17\] C. Buccancs, **shimmer_manager.py Source Code**, *bucika_gsr*
GitHub repository, commit 7048f7f, 2024. \[Online\]. Available:
https://github.com/buccancs/bucika_gsr/PythonApp/shimmer_manager.py
(accessed Jul. 2025).

\[18\] C. Buccancs, **shimmer_pc_app.py Source Code**, *bucika_gsr*
GitHub repository, commit 7048f7f, 2024. \[Online\]. Available:
https://github.com/buccancs/bucika_gsr/PythonApp/shimmer_pc_app.py
(accessed Jul. 2025).
