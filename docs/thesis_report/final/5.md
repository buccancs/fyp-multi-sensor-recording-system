...
Robolectric test runner ]1\]. In the setup, dummy objects replace the
...
UI ]2\]. The tests cover initialisation and all major methods of
...
`initialize()` returns true (indicating a successful setup) and logs an
appropriate message ]3\]. Others validate error handling: for example,
...
emitting an error log ]4\]]5\]. This ensures the app won't crash if,
...
and does not produce unexpected errors ]8\]]9\]. Other test classes
...
internal state changes or interactions. Using relaxed mocks (which don't
...
authentication tokens are accepted ]20\]]21\], and that production
configuration files have the required security settings (certificate
pinning, data encryption flags) enabled ]22\]. These tests confirm that
...
replaced with patched versions returning controlled data ]23\]]24\].
...
that an object is not null ]25\]]26\]. These were likely placeholders
...
webcam devices) to mimic a heterogeneous device ensemble ]27\]]28\].
...
connected, recording, etc.) ]29\]]30\]. The integration test connects
...
all device statuses and seeing `recording=True` for every device ]31\],
and it prints "✓ Synchronised recording start works" on success ]32\].
...
teardown ]33\]]34\]. This simulation demonstrates that the system's
...
message serialization, transmission, and reception ]35\]]36\]. For
...
]37\]]38\], then deserialises it back and asserts that the data
integrity is preserved (confirming that no information was lost in
transit) ]36\]. Next, a loopback client-server socket interaction is
simulated on localhost: the PC opens a listening socket, and a client
connects to send the JSON command string ]39\]]40\]. The server logic
...
and an echo of the original payload ]41\]]42\]. The client receives
this response and the test asserts that the echoed content matches the
original command ]43\]. A "✓ Socket communication works" log confirms a
successful round trip ]44\]. Although this is a local loopback test, it
...
logging ]45\]]46\]. In this test, the mock device generates synthetic
GSR and PPG sensor readings at a specified sampling rate and invokes a
callback for each sample as if streaming live data ]47\]]48\]. The
...
channels, etc.) ]49\]]50\]. It then verifies that stopping the stream
and disconnecting work as intended, returning the device to a clean
state ]51\]]52\]. The suite also tests session management: after
simulating the device, it uses a `SessionManager` to accumulate incoming
sensor samples into a session record ]53\]]54\]. It starts a session,
...
recorded ]55\]]56\]. Finally, it exports the session data to CSV and
reads it back to ensure the file contains the expected number of entries
and columns ]57\]]58\]. This end-to-end test (using all simulated
...
steps (e.g., "Test with real Shimmer3 device") ]59\]]60\]. These
...
periodically ]61\]]62\]. Essentially, the endurance test loop
...
might start and stop recordings throughout a day ]62\]. During the
entire run, system metrics are sampled at regular intervals (every 30
seconds by default) ]63\]]64\]. Each sample captures a snapshot of key
...
descriptors, and more ]65\]]66\]. The Endurance Runner also leverages
Python's `tracemalloc` to track memory allocations, recording the
current and peak memory usage at each sample ]67\]]68\]. All these
...
data points ]69\]]70\]. If the projected growth over that window
exceeds a threshold (100 MB by default), the system flags a potential
memory leak ]71\]]72\]. The unit tests confirmed this mechanism's
effectiveness: when fed an increasing memory pattern, the detector
correctly reported a leak with the expected growth rate ]10\]]11\]. A
complementary test feeding a fluctuating memory pattern (with no net
growth) confirmed that no false-positive leak was reported ]12\].
...
objects or subsystems accumulating memory. In our evaluation, these
...
levels ]73\]]74\]. Throughout the 8-hour test, CPU usage on the PC
...
environment for simplicity ]75\]. Therefore, these results reflect the
...
and live threads at each sample ]76\]. These counts remained stable
...
detected leaks, and recommendations for improvements ]77\]]78\]. In
our evaluation, the final report indicated that 100% of the planned test
...
]4\]]79\]. Likewise, the PC-side logic correctly implements protocols
...
designed ]18\]]20\]. The fact that all these tests passed indicates a
...
]31\]]33\]. This validates the design choice of a centralised session
...
were transmitted without loss or corruption ]36\]]41\]. In other
...
across devices ]54\]]58\]. When multiple subsystems were combined
