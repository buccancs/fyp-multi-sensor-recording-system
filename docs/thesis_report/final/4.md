...
timestamps\citep{AndroidCameraX, NordicBLE}.
...
stream\citep{AndroidCameraX, NordicBLE}.
...
readings\citep{AndroidCameraX, NordicBLE}.
...
frame\citep{AndroidCameraX, NordicBLE}.
...
marker\citep{AndroidCameraX, NordicBLE}.

See Listing 4.1 (Appendix H.4)
See Appendix H.4 for implementation details.
*Source: see [4], [53]*

See Figure 4.2 (Appendix H.2) for \2.
...
devices\citep{AndroidCameraX, NordicBLE},
...
instance\citep{AndroidCameraX, NordicBLE}.
...
live\citep{AndroidCameraX, NordicBLE}.
...
constants\citep{AndroidCameraX, NordicBLE}.
...
it\citep{AndroidCameraX, NordicBLE}.
...
startup and keep it
ready\citep{AndroidCameraX, NordicBLE}.
...
writer for
output\citep{AndroidCameraX, NordicBLE}.
...
sensor's TX
characteristic\citep{AndroidCameraX, NordicBLE}.
...
from the byte
sequence\citep{AndroidCameraX, NordicBLE}.
...
resistor\*\citep{AndroidCameraX, NordicBLE}.
Similar piecewise formulas are used for the other ranges (287kΩ, 1MΩ,
3.3MΩ)\citep{AndroidCameraX, NordicBLE}.
...
line\citep{AndroidCameraX, NordicBLE}.
...
timed\citep{AndroidCameraX, NordicBLE}.
...
`"Android_GSR"`\citep{AndroidCameraX, NordicBLE}.
...
interface\citep{AndroidCameraX, NordicBLE}.
...
port\citep{AndroidCameraX, NordicBLE}.
...
identifier\citep{AndroidCameraX, NordicBLE}.
...
options\citep{AndroidCameraX, NordicBLE}.
...
\[Model\]"\citep{AndroidCameraX, NordicBLE}.
...
latency\citep{AndroidCameraX, NordicBLE}.
...
See Figure 4.4 (Appendix H.2) for \2.
...
`{"id": <command_id>, "command": "<action>", "params": { ... }}`\citep{AndroidCameraX, NordicBLE}.
...
hardware
capabilities\citep{AndroidCameraX, NordicBLE}.
...
rates)\citep{AndroidCameraX, NordicBLE}.
...
base64 JPEG
string\citep{AndroidCameraX, NordicBLE}.
...
See Figure 4.5 (Appendix H.2) illustrates the communication sequence and
...
etc.)\citep{AndroidCameraX, NordicBLE}.
...
status\citep{AndroidCameraX, NordicBLE}.
...
directly\citep{AndroidCameraX, NordicBLE}.
...
See Figure 4.5 (Appendix H.2) for \2.
...
session\citep{AndroidCameraX, NordicBLE}.
...
upfront\citep{AndroidCameraX, NordicBLE}.
...
the session
folder\citep{AndroidCameraX, NordicBLE}.
Video is encoded with H.264 at 1080p/30fps (Quality.HD) as
configured\citep{AndroidCameraX, NordicBLE}.
...
FPS) on a background
executor\citep{AndroidCameraX, NordicBLE}.
...
(e.g.,
`raw_rgb_frame_<timestamp>.jpg`)\citep{AndroidCameraX, NordicBLE}.
...
when streaming
starts\citep{AndroidCameraX, NordicBLE}.
...
values\citep{AndroidCameraX, NordicBLE}.
...
described
earlier\citep{AndroidCameraX, NordicBLE}.
...
capture\citep{AndroidCameraX, NordicBLE},
...
method\citep{AndroidCameraX, NordicBLE}.
...
PC\citep{AndroidCameraX, NordicBLE}.
...
See Figure 4.6 (Appendix H.2) for \2.
...
- **High Data Throughput and Storage Management:** Recording
  ...
  thread\citep{AndroidCameraX, NordicBLE},
  ...
  callback\citep{AndroidCameraX, NordicBLE},
...
- **Thermal Camera USB Integration:** Using the Topdon TC001 thermal
  ...
  camera\citep{AndroidCameraX, NordicBLE}
...
- **Reliable BLE Sensor Streaming:** The Shimmer GSR+ streaming over BLE
  ...
  flow\citep{AndroidCameraX, NordicBLE}.
  ...
  hardware\citep{AndroidCameraX, NordicBLE};
...
- **Cross-Platform Performance in the PC App:** Python is an interpreted
  ...
  **solution** was to implement those parts in C++ and integrate via
  ...
  queue\citep{AndroidCameraX, NordicBLE}.
  ...
  NumPy\citep{AndroidCameraX, NordicBLE}
  ...
  C++)\citep{AndroidCameraX, NordicBLE}
...
- **User Interface and Multi-Device Coordination:** Another challenge
  ...
  QLabel\citep{AndroidCameraX, NordicBLE}.
...
\citep{AndroidCameraX, NordicBLE}
\citep{AndroidCameraX, NordicBLE}
\citep{AndroidCameraX, NordicBLE}
\citep{AndroidCameraX, NordicBLE}
\citep{AndroidCameraX, NordicBLE}
