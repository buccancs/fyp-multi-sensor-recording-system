...
recordings and aggregate data. See Figure 4.1 (Appendix H.2) for system
...
enabled sensor/camera capture with nanosecond-precision timestamps
\[1\], \[2\]. Each modality's data is written to device storage in real
...
simplifies synchronization logic, since the controller can treat each
data source uniformly (start all, stop all) and trust each to timestamp
its output. *(Figure 4.2 in Appendix H.2 provides an overview of the
...
desired frame size and mode before starting the video stream \[1\],
\[2\]. By default, the camera is set to its **native thermal
...
readings \[1\], \[2\]. The `TopdonThermalCamera` writes each frame's
...
values of all 49,152 pixels (256×192) in that frame \[1\], \[2\]. This
...
which is used to mark events (for synchronization or debugging) via a
Lab Streaming Layer (LSL) marker \[1\], \[2\].
...
devices \[1\], \[2\], and unregisters on app pause to release resources.
...
\[1\], \[2\]. The camera is then configured and flagged as *connected*.
...
`startPreview(surface)` to render the thermal feed live \[1\], \[2\].
...
closing the file writer, and destroying the UVC camera instance \[1\],
\[2\]. This orderly shutdown ensures the USB device is released for
...
interface are defined as constants \[1\], \[2\]. The Shimmer uses a
...
and keeps it ready \[1\], \[2\]. Upon beginning a recording session, if
GSR recording is enabled, the controller invokes
`physiologicalSensor.startStreaming(...)` with a file writer for output
\[1\], \[2\]. Internally, this triggers the BLE manager to connect (if
not already connected) and then write the **Start Streaming** command
(0x07) to the sensor's TX characteristic \[1\], \[2\]. The Shimmer
...
\[1\], \[2\]. As data arrives, the `onShimmerDataReceived()` function
parses the byte payload according to Shimmer's data protocol \[1\],
\[2\]. The first byte acts as an identifier (0x00 indicates a standard
...
from the byte sequence \[1\], \[2\]. The GSR reading includes a range
...
each new sample as a new line \[1\], \[2\]. The timestamp is obtained
via the app's `TimeManager.getCurrentTimestampNanos()` to ensure
consistency with how other modalities are timed \[1\], \[2\]. In
addition to logging to file, the app feeds the live data into an
in-memory stream for synchronization with video: the
...
short delay \[1\], \[2\]. The app also provides graceful shutdown: when
recording stops, it sends the stop command (`0x20`) to halt streaming
\[1\], \[2\], and closes the file writer. This ensures the CSV is
properly finalized.
...
streaming and logging.
    ...
    if (bytes.isNotEmpty() && bytes[0].toInt() == 0x00) {
        ...
        val ppgValue = ... // combine two bytes for PPG
        val gsrRaw = ...   // combine two bytes for GSR (14-bit value + range bits)
...
*(Figure 4.3 in Appendix H.2 illustrates the desktop controller
...
Windows, Linux, and macOS) built with Qt for the GUI (PyQt6/PySide6) and
...
`newPreviewFrame` signal carrying the image and device identifier \[1\],
\[2\]. The main GUI thread connects this signal to a slot that displays
...
The application uses **Zeroconf (mDNS)** to simplify device discovery:
...
latency \[1\], \[2\]. The controller instantiates these at startup: for
example, `NativeWebcam(0)` opens the local webcam (device 0) and begins
capturing frames in a loop \[1\], \[2\], and `NativeShimmer("COM3")`
connects to a Shimmer GSR device via a serial port (COM3 on Windows)
\[1\], \[2\]. These native objects start immediately and run
...
latest data from the native threads \[1\], \[2\]. On each tick, it pulls
...
class for new GSR samples and updates the live plot \[1\], \[2\]. Using
a C++ backend drastically improved performance: the webcam thread
captures at a steady \~60 FPS by sleeping \~16 ms per iteration \[1\],
\[2\], and yields frames without significant buffering. Meanwhile, the
...
consumption of data \[1\], \[2\]. The C++ code directly converts camera
...
into Python tuples \[1\], \[2\]. This design minimizes overhead,
latency, and jitter -- imperative for synchronizing local PC data with
remote device data.
...
which helps calibrate cameras after recordings. Using OpenCV, it can
...
*(Figure 4.4 in Appendix H.2 illustrates the playback and annotation
...
`{"id": <command_id>, "command": "<action>", "params": { ... }}` \[1\],
\[2\]. The device, upon receiving a command, executes the requested
...
report its hardware capabilities \[1\], \[2\]. The Android app responds
with a message like
`{"ack_id": 1, "status": "capabilities_data", "capabilities": { ... }}`
including details such as available cameras (with their identifiers,
resolutions, and frame rates) \[1\], \[2\]. This exchange allows the PC
...
base64 JPEG string \[1\], \[2\]. The PC's worker thread listens for
...
Figure 4.5 (Appendix H.2) illustrates the communication sequence and
time synchronization strategy. When the PC and a phone connect, they
perform a simple handshake (exchange of hello messages and
capabilities). Part of this handshake is a **time synchronization
...
globally synced via LSL's internal clock synchronization. However, in
...
relative to a GSR sample). The main synchronization still relies on the
...
on a specified port and streams the file bytes directly \[1\], \[2\].
...
*(Figure 4.5 in Appendix H.2 illustrates the messaging sequence and
clock synchronization described above.)*
...
dedicated directory is created in the device's storage for that session
\[1\], \[2\]. All data files for that session are saved under this
directory, organized by modality. For example, within a session
directory the app creates sub-folders for raw images and thermal frames
upfront \[1\], \[2\]. This ensures that as data starts streaming in, the
...
named `RGB_<sessionId>.mp4` and saved in the session folder \[1\],
\[2\]. Video is encoded with H.264 at 1080p 30 FPS (Quality.HD) as
configured \[1\], \[2\]. Recording continues until stopped, at which
...
(\~30 FPS) on a background executor \[1\], \[2\]. Each image is saved as
a JPEG file in a `raw_rgb_<sessionId>` directory, with a filename
containing its exact nanosecond timestamp (e.g.,
`raw_rgb_frame_<timestamp>.jpg`) \[1\], \[2\]. These images are
...
`thermal_<sessionId>` directory when streaming starts \[1\], \[2\]. The
...
separate audio file with a known start time simplifies synchronization
during analysis.\
...
at the end of capture \[1\], \[2\], which would include details like
...
`FileTransferManager.zipSession()` method \[1\], \[2\]. This zips up all
files (video, images, CSVs, etc.) from that recording session. The app
places this ZIP in a cache directory and then uses
`FileTransferManager.sendFile()` to initiate a transfer to the PC \[1\],
\[2\]. The transfer is done via a simple socket stream --- the phone
...
*(Figure 4.6 in Appendix H.2 provides an overview of the data processing
...
- **Ensuring Precise Synchronization:** Achieving tight time
  ...
  two-tier synchronization mechanism. Each device timestamps data with a
...
- **High Data Throughput and Storage Management:** Recording
  ...
  separate from the main thread \[1\], \[2\], ensuring the CameraX
  ...
  callback \[1\], \[2\], and GSR samples are appended to a file (and
...
- **Thermal Camera USB Integration:** Using the Topdon TC001 thermal
  ...
  correct mode (frame size and format) to avoid negotiation issues
  \[1\], \[2\]. Handling the permission prompt promptly was also
...
- **Reliable BLE Sensor Streaming:** The Shimmer GSR+ streaming over BLE
  ...
  times with a short delay \[1\], \[2\] to overcome transient failures
  during the handshake. Moreover, once connected, the app immediately
  sets up notifications and starts streaming to lock in the data flow
  \[1\], \[2\]. The design of writing data to file versus sending it
  ...
  \[1\], \[2\]; otherwise, if a user restarted recording immediately,
...
- **Cross-Platform Performance in the PC App:** Python is interpreted
  ...
  Initially, we used OpenCV in Python for webcam capture and PySerial
  ...
  integrate via PyBind11. The `NativeWebcam` class uses OpenCV's
  `VideoCapture` in a separate thread to grab frames and push them into
  a queue \[1\], \[2\]. As C++ code, it's compiled and optimized,
  ...
  memory pointer of the `cv::Mat` with NumPy \[1\], \[2\] -- essentially
  ...
  the Android (a mirror implementation in C++) \[1\], \[2\] and pushes
  timestamped samples into a queue \[1\], \[2\]. Measurements showed a
...
- **User Interface and Multi-Device Coordination:** Another challenge
  ...
  the main thread's slot sets that image on a `QLabel` \[1\], \[2\].
  ...
  which our synchronization logic then corrects to under 1 ms alignment.
