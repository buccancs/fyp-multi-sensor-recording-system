...
stream \citep{TopdonTC001Specs, TopdonThermalSDK}.
...
readings. The `TopdonThermalCamera` writes each frame's data to a CSV file: each
...
debugging) via a Lab Streaming Layer marker.

See Listing 4.1 (Appendix H.4) for thermal frame capture and CSV logging implementation.

See Figure 4.2 (Appendix H.2) for thermal camera integration flow showing USB-C connection, UVCCamera library processing, and multi-threaded data handling.
...
constants \citep{ShimmerGSRSpecs, ShimmerAndroidAPI}.
...
payload according to Shimmer's data protocol.
...
computed from the 14-bit ADC value using that resistor.
...
mechanism attempts reconnection up to 3 times with a short delay.
The app also provides graceful shutdown: when recording stops, it sends
the stop command (0x20) to halt streaming,
and closes the file writer. This ensures the CSV is properly finalized.

See Listing 4.2 (Appendix H.4) for Shimmer GSR+ BLE streaming and data parsing implementation.

See Figure 4.3 (Appendix H.2) for Shimmer GSR integration showing BLE communication protocol, data parsing pipeline, and CSV logging with optional LSL streaming.
...
![Figure 4.4: Desktop controller GUI layout showing Dashboard tab with live previews, Logs tab for monitoring, and Playback tab for analysis with synchronised video and sensor data](../diagrams/fig_4_04_desktop_gui_layout.png)
