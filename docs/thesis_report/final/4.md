...
[13] to interface with the device. A dedicated class
...
desired frame size and mode before starting the video stream [13]. By default, the camera is set to its **native thermal
...
readings [13]. The `TopdonThermalCamera` writes each frame's
...
values of all 49,152 pixels (256×192) in that frame [13]. This
...
devices [13], and unregisters on app pause to release resources.
...
[13]. The camera is then configured and flagged as *connected*.
...
`startPreview(surface)` to render the thermal feed live [13].
...
closing the file writer, and destroying the UVC camera instance [13]. This orderly shutdown ensures the USB device is released for
...
interface are defined as constants [12]. The Shimmer uses a
...
(0x07) to the sensor's TX characteristic [12]. The Shimmer
...
[12]. As data arrives, the `onShimmerDataReceived()` function
parses the byte payload according to Shimmer's data protocol [7]. The first byte acts as an identifier (0x00 indicates a standard
...
from the byte sequence [7]. The GSR reading includes a range
...
resistor [7]. Similar piecewise formulas are used for the other ranges
(287 kΩ, 1 MΩ, 3.3 MΩ) [7]. After conversion, each data point consists
...
short delay [12]. The app also provides graceful shutdown: when
recording stops, it sends the stop command (`0x20`) to halt streaming
[12], and closes the file writer. This ensures the CSV is
properly finalized.
...
The application uses **Zeroconf (mDNS)** [18] to simplify device discovery:
...
Android device, built on TCP/IP sockets [24] with JSON message
...
certain data streams (GSR, events, etc.) [17]. If the PC were also
...
`thermal_<sessionId>` directory when streaming starts [13]. The
...
timestamp and the rest are temperature values [13]. (If needed,
...
named `GSR_<sessionId>.csv` in the session folder [12]. The file
begins with a header (`timestamp_ns, GSR_uS, PPG_raw`), and each
subsequent line represents one sample, as recorded by the
`ShimmerGsrSensor` described earlier [12]. Sampling at 128 Hz
...
- **Cross-Platform Performance in the PC App:** Python is interpreted
  ...
  Initially, we used OpenCV [15] in Python for webcam capture and PySerial
  ...
  integrate via PyBind11 [16]. The `NativeWebcam` class uses OpenCV's [15]
