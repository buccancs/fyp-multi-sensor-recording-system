...
[Serenegiant(2024)] to interface with the device. A dedicated class
...
desired frame size and mode before starting the video stream [Serenegiant(2024)]. By default, the camera is set to its **native thermal
...
readings [Serenegiant(2024)]. The `TopdonThermalCamera` writes each frame's
...
values of all 49,152 pixels (256×192) in that frame [Serenegiant(2024)]. This
...
Lab Streaming Layer (LSL) marker [Jangra et al.(2021)].
...
devices [Serenegiant(2024)], and unregisters on app pause to release resources.
...
[Serenegiant(2024)]. The camera is then configured and flagged as *connected*.
...
`startPreview(surface)` to render the thermal feed live [Serenegiant(2024)].
...
closing the file writer, and destroying the UVC camera instance [Serenegiant(2024)]. This orderly shutdown ensures the USB device is released for
...
interface are defined as constants [Shimmer Engineering(2024)]. The Shimmer uses a
...
`0x20` stops it [Shimmer Engineering(2024)].
...
(0x07) to the sensor's TX characteristic [Shimmer Engineering(2024)]. The Shimmer
...
[Shimmer Engineering(2024)]. As data arrives, the `onShimmerDataReceived()` function
parses the byte payload according to Shimmer's data protocol [Shimmer Sensing(2024)]. The first byte acts as an identifier (0x00 indicates a standard
...
from the byte sequence [Shimmer Sensing(2024)]. The GSR reading includes a range
...
resistor [Shimmer Sensing(2024)]. Similar piecewise formulas are used for the other ranges
(287 kΩ, 1 MΩ, 3.3 MΩ) [Shimmer Sensing(2024)]. After conversion, each data point consists
...
named `"Android_GSR"` [Jangra et al.(2021)]. This allows GSR data to be monitored in
...
short delay [Shimmer Engineering(2024)]. The app also provides graceful shutdown: when
recording stops, it sends the stop command (`0x20`) to halt streaming
[Shimmer Engineering(2024)], and closes the file writer. This ensures the CSV is
properly finalized.
...
streaming and logging.
    ...
    if (bytes.isNotEmpty() && bytes[0].toInt() == 0x00) {
        ...
        val ppgValue = (bytes[MDPI(2020)].toInt() and 0xFF) or ((bytes[Jangra et al.(2021)].toInt() and 0xFF) shl 8)
        val gsrRaw = (bytes[Nature Scientific Reports(2019)].toInt() and 0xFF) or ((bytes[Shimmer Sensing(2024)].toInt() and 0xFF) shl 8)
...
Windows, Linux, and macOS) built with Qt for the GUI (PyQt6/PySide6) [Riverbank Computing(2024)] and
...
The application uses **Zeroconf (mDNS)** [Python Software Foundation(2024)] to simplify device discovery:
...
extension module (built via PyBind11) [Jakob(2024)] named `native_backend`. It
...
which helps calibrate cameras after recordings. Using OpenCV [OpenCV Foundation(2024)], it can
...
Android device, built on TCP/IP sockets [Postel(1980)] with JSON message
...
certain data streams (GSR, events, etc.) [LSL Community(2024)]. If the PC were also
...
`thermal_<sessionId>` directory when streaming starts [Serenegiant(2024)]. The
...
timestamp and the rest are temperature values [Serenegiant(2024)]. (If needed,
...
named `GSR_<sessionId>.csv` in the session folder [Shimmer Engineering(2024)]. The file
begins with a header (`timestamp_ns, GSR_uS, PPG_raw`), and each
subsequent line represents one sample, as recorded by the
`ShimmerGsrSensor` described earlier [Shimmer Engineering(2024)]. Sampling at 128 Hz
...
the implementation uses libraries like **pandas** [pandas Development Team(2024)] and **h5py** [HDF5 Group(2024)] to
...
- **Cross-Platform Performance in the PC App:** Python is interpreted
  ...
  Initially, we used OpenCV [OpenCV Foundation(2024)] in Python for webcam capture and PySerial
  ...
  integrate via PyBind11 [Jakob(2024)]. The `NativeWebcam` class uses OpenCV's [OpenCV Foundation(2024)]
