...
*Figure 4.2: Thermal Integration Flow showing Android app architecture for USB thermal camera integration.*
...
[16] to interface with the device. A dedicated class
...
readings [16]. The `TopdonThermalCamera` writes each frame's
...
Lab Streaming Layer (LSL) marker [3].
...
`startPreview(surface)` to render the thermal feed live [16].
...
interface are defined as constants [15]. The Shimmer uses a
...
named `"Android_GSR"` [3]. This allows GSR data to be monitored in
...
short delay [15]. The app also provides graceful shutdown: when
...
application architecture.* The Shimmer GSR+ sensor streams data at 128 Hz via BLE, with each sample timestamped using the Android device's monotonic clock for subsequent temporal alignment with other modalities.
...
follows strict type safety principles as documented in [ADR-002: Strict Type Safety](../adr/adr-002-strict-type-safety.md)
to ensure data integrity and research-grade reliability.
...
updates, errors) for debugging. A **Playback & Annotation** tab is
planned for reviewing recorded sessions (see Backlog item BL-UI-003).
Each tab's UI elements are created and managed
...
class for new GSR samples and updates the live plot. The C++ backend
provides improved performance over pure Python implementation: the webcam
thread targets ~60 FPS through 16 ms sleep intervals, while the Shimmer
thread processes incoming data at the sensor's 128 Hz rate. Both use
lock-free queues to decouple data production and consumption. The C++
implementation converts camera frames to NumPy arrays via shared memory
and packages GSR readings into Python tuples \[1\], \[2\]. Performance
benchmarking of this architecture remains to be conducted.

### 4.3.1 Implemented Features

The current implementation includes live monitoring with device discovery
and command execution. The main window displays connected device status
and provides basic session control via `MainWindow` class in
`pc_app/ui/main_window.py`.

### 4.3.2 Planned Features

**Post-session analysis tools** are planned for future implementation
(see Backlog items BL-UI-003 through BL-UI-006):

- **Playback & Annotation** (Planned): A tab designed to load recorded
video files (RGB and thermal) along with sensor data for synchronised
session replay. Implementation would use PyAV for video handling and
PyQtGraph for time-series plotting.

- **Calibration Utility** (Planned): Camera calibration using OpenCV to
detect calibration patterns and compute intrinsic/extrinsic parameters.

- **Data Export** (Planned): Conversion tools for session datasets into
MATLAB `.mat` files or HDF5 format for research distribution.

![Figure 4.4: Desktop GUI Layout](../../diagrams/fig_4_04_desktop_gui_layout.png)

*Figure 4.4: Desktop GUI Layout showing the main dashboard interface with device management tabs.*
...
master clock domain. Initial testing suggests cross-device synchronisation
within single-digit milliseconds over Wi-Fi, though comprehensive
benchmarking across different network conditions remains to be conducted.
...
Communication security is planned for future implementation: the design
allows for TLS encryption of messages and data transmission (see 
Backlog item BL-SEC-001). Currently, the prototype operates over local
network connections without encryption. The messages themselves
...
*Figure 4.5: Protocol Sequence showing TCP/JSON command-response flow and network time synchronization.*
...
- **Annotations/Events:** Event logging is implemented with basic
timestamp recording. The `SessionManager` creates a `session_metadata.json`
file containing start/stop times and device information. User annotation
features are planned for future implementation (see Backlog item BL-ANNO-001).
...
Data collection involves creating timestamped session directories on each
device, transferring compressed session archives to the PC via TCP, and
converting to research formats (HDF5/MATLAB) using pandas and h5py libraries.
Session archives typically contain MP4 video files, CSV sensor data,
and metadata JSON files.

![Figure 4.6: Data Processing Pipeline](../../diagrams/fig_4_06_data_processing_pipeline.png)

*Figure 4.6: Data Processing Pipeline showing session data flow from device capture to research export formats.*
...
- **Ensuring Precise synchronisation:** Achieving tight time
  ...
  at connection time and occasionally during recording. Initial testing suggests timing coordination within single-digit milliseconds, though comprehensive validation across different network environments remains to be conducted. **Solution highlights:** use monotonic clock
...
- **Cross-Platform Performance in the PC App:** Python is interpreted
  ...
  timestamped samples into a queue. Performance testing indicated
  improved latency and reduced timing jitter compared to the Python-only
  implementation, though comprehensive benchmarking across different
  hardware configurations remains to be conducted. The trade-off was the
  added complexity of compiling C++ code for multiple platforms, which
  was addressed using CMake and continuous integration testing on each OS.
...
- **User Interface and Multi-Device Coordination:** Another challenge
  ...
  offset if drift is minimal over a short period. Testing is needed to
  characterize the actual synchronization performance under different
  network conditions and recording durations.
...
The development and potential deployment of this multi-sensor recording system requires careful consideration of ethical principles and data protection standards, particularly given its capacity to collect biometric data including GSR measurements and video recordings.

### Ethics Approval Requirements

This study has been approved by the UCLIC Ethics Committee with Project ID: 1428 under the research title "Investigating AI and physiological computing" with subtitle "App for Camera-based Contactless Sensing of Physiological Signals". The Principal Investigator is Prof. Youngjun Cho (youngjun.cho@ucl.ac.uk), with researchers Duy An Tran, Zikun Quan, and Jitesh Joshi. The ethics approval covers the research methodology involving 30-minute participant sessions using thermal cameras, RGB video recording, and GSR sensors for contactless physiological monitoring research. The approved research protocol includes comprehensive participant information sheets with informed consent procedures, supervisor-approved risk assessment protocols, and data protection measures ensuring participant safety throughout the research process.

### Data Protection and GDPR Compliance

The system architecture incorporates several data protection principles aligned with GDPR requirements:

- **Data minimisation**: The system only collects data types explicitly configured by researchers, with no extraneous personal information gathered
- **Anonymisation**: Participant identification is handled through configurable ID codes rather than personal identifiers, with no personally identifiable information stored alongside sensor data
- **Secure storage**: All collected data is stored locally on encrypted devices during recording sessions, with secure transfer protocols (encrypted TCP connections) used for data synchronisation
- **Access controls**: The system requires explicit researcher authentication and maintains audit logs of data access and modifications
- **Data retention**: The system supports configurable data retention policies, allowing researchers to implement appropriate disposal schedules as required by their specific ethics approvals

### Device Safety and Participant Wellbeing

The hardware components have been selected and configured to ensure participant safety:

- **GSR sensors**: Operate at safe, low-voltage levels (typically <1V) with current-limited outputs to prevent any electrical risk to participants
- **Video recording**: Uses standard consumer cameras with no enhanced illumination or invasive positioning requirements
- **Thermal imaging**: Employs passive infrared sensing with no energy emission towards participants
- **Wireless communications**: All wireless protocols (Bluetooth LE, Wi-Fi) operate within standard regulatory limits for electromagnetic emissions

### Research Deployment Framework

This research has obtained appropriate ethics approval from the UCLIC Ethics Committee (Project ID: 1428) for research titled "Investigating AI and physiological computing: App for Camera-based Contactless Sensing of Physiological Signals", which includes:

1. **Approved research methodology**: The ethics approval covers 30-minute experimental sessions using thermal cameras, RGB video recording, and GSR sensors with healthy adult participants aged 18+ who provide informed consent
2. **Participant information and consent procedures**: Standardised information sheets detail voluntary participation, data collection procedures involving physiological signals and thermal imaging, participant rights including withdrawal procedures, and data anonymisation protocols that have been approved for participant recruitment
3. **Risk assessment protocols**: Comprehensive risk assessment covering technical safety, data protection, participant welfare, and laboratory environment safety has been reviewed and approved by supervisor Prof. Youngjun Cho and the ethics committee
4. **Data management framework**: Approved procedures for data collection, storage, anonymisation, and retention that comply with UCL data protection policies and GDPR requirements, including specific protocols for physiological sensor data and thermal imaging
5. **Participant protection measures**: Clear protocols for participant withdrawal, data deletion, ongoing safety monitoring throughout research sessions, and provisions for participants with exclusion criteria (cardiovascular or neurological conditions)

The approved ethics framework ensures that this multi-sensor recording system can be deployed for research while maintaining the highest standards of participant protection, data security, and research integrity.

This ethical framework ensures that the technical capabilities developed in this project can be deployed responsibly in future research contexts while maintaining the highest standards of participant protection and data security.
