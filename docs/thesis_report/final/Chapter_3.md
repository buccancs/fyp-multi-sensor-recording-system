...
**Table 3.1 -- Functional Requirements**
  ...
  FR-03                   **High-Precision Synchronization:** The system shall synchronize all data streams (video frames, thermal frames, GSR samples) with a unified timeline. Recording on all devices must start nearly simultaneously, achieving time alignment with an accuracy on the order of 1 millisecond or better. Each data sample/frame will be timestamped to enable precise cross-modal      H
                          correlation [\[1\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L144-L151).                                                                                                                                                                                                                                  
  ...
  FR-07                   **Session Management and Metadata:** The system shall allow the user to create a new *recording session* and automatically assign it a unique Session ID. During a session, the controller will maintain metadata including session start time, configured duration (if applicable), and the list of active devices/sensors. Upon session start, each device and sensor is         H
                          ...
                          recorded[\[3\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L74-L81)[\[4\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L334-L342). A session metadata file (e.g. JSON or      
  ...
  FR-10                   **Real-Time Status Monitoring:** The PC interface shall display real-time status updates from each connected device, including indicators such as recording state (recording/idle), battery level, storage space, and connectivity health[\[6\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L26-L34). M
                          ...
                          update)[\[7\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L244-L252). These status and preview updates help the operator ensure data quality throughout the session.                                                                                                                                  
  ...
  FR-11                   **Event Annotation:** The system shall allow the researcher to **annotate events** or markers during a recording session. For example, if a stimulus is presented to the participant at a specific time, the researcher can log an event (through the PC app UI or a hardware trigger). The event is recorded with a timestamp (relative to session start) and a short description M
                          or type[\[8\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L124-L133). All such events are saved (e.g. in a `stimulus_events.csv` in the session folder) to facilitate aligning external events with physiological responses during data analysis.           
  ...
  FR-12                   **Sensor Calibration Mode:** The system shall provide a mode or tools for calibration and configuration of sensors before a session. This includes the ability to capture calibration data for the cameras (e.g. a one-time procedure to spatially align the thermal and RGB cameras using a reference pattern) and to configure sensor settings (focus, exposure, thermal range,  M
                          ...
                          device[\[9\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L300-L308)[\[10\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L311-L319).   
  ...
  FR-13                   **Post-Session Data Processing:** The system shall support optional **post-processing steps** on the recorded data to enrich the dataset. For example, after a session, a *hand segmentation* algorithm can be run on the recorded video frames to identify and crop the participant's hand region (since GSR is often measured on the hand). If enabled, the PC controller will   M
                          automatically invoke the hand segmentation module on the session's video files and save the results (segmented images or masks) in the session folder[\[11\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L216-L224). This automates part of the data analysis preparation (e.g. extracting relevant 
...
sensors[\[2\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L138-L145)[\[1\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L144-L151),
and the session management logic creates metadata files and directory
structures as
specified[\[3\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L74-L81)[\[4\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L334-L342).
...
**Table 3.2 -- Non-Functional Requirements**
  ...
  NFR-02                  **Synchronization Accuracy:** The system's clock synchronization and triggering mechanisms shall be precise, as reflected in FR-03. The design should ensure that any timestamp discrepancies between devices are below acceptable thresholds (on the order of milliseconds). In practice, this may   High
                          ...
                          analysis[\[12\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L91-L99). This requirement guarantees the **temporal integrity** of the multi-modal dataset.                                                                       
  ...
  NFR-03                  **Reliability and Fault Tolerance:** The system must be reliable during long recording sessions. It shall handle errors gracefully -- for instance, if a device temporarily disconnects (due to network drop or power issues), the system will attempt to reconnect automatically and continue the    High
                          session without crashing[\[13\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L146-L150). Data already recorded should remain safe in such events (thanks to local storage on devices). The system should not lose data or       
  ...
  NFR-04                  **Data Integrity and Accuracy:** The system shall ensure the integrity and accuracy of recorded data. All data files (videos, sensor CSV) are verified for correctness after recording -- for example, the file transfer process includes confirming file sizes and sending                           High
                          acknowledgments[\[5\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L355-L364). Time stamps must be consistent and accurate (no clock drift over typical session durations). The GSR sensor data should be sampled with    
  ...
  NFR-05                  **Scalability (Multi-Device Support):** The architecture should scale to **multiple recording devices** operating concurrently. Adding more Android devices (or additional Shimmer sensors) to a session should have a linear or manageable impact on performance. The network and PC controller must Medium
                          ...
                          modification[\[14\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L424-L432)[\[15\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L484-L492).   
  ...
  NFR-07                  **Maintainability and Extensibility:** The software shall be designed following clean code and modular architecture principles to facilitate maintenance and future extension. For example, the Android app follows an MVVM (Model-View-ViewModel) architecture with dependency injection (Hilt) to   Medium
                          ...
                          testable[\[16\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/changelog.md#L34-L41)[\[17\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/changelog.md#L60-L64). Additionally, a high level of automated test coverage 
...
check[\[16\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/changelog.md#L34-L41)).
...
1.  **Setup:** The researcher ensures all Android devices (e.g., two
    ...
    registration to the
    PC[\[18\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L216-L224),
...
2.  **Initiate Recording:** The researcher configures session parameters
    ...
    devices[\[14\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L424-L432)[\[19\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L486-L494).
...
5.  **Stop Session:** Once the desired recording duration or
    ...
    chunks[\[20\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L313-L322)[\[5\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L355-L364),
    ...
    duration)[\[21\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L82-L91),
...
1.  **Camera Calibration:** The researcher wants to calibrate the
    ...
    **calibration capture** -- the system might capture a set of images
    ...
    directory for that
    session)[\[9\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L300-L308)[\[10\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L311-L319).
...
2.  **Sensor Configuration:** The system allows adjusting certain sensor
    ...
    accelerometer,
    etc.)[\[22\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L72-L80)[\[23\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L96-L104).
    ...
    and sampling
    rate[\[24\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L184-L191).
...
4.  **Result:** After this use case, the system is calibrated and
    ...
    etc.)[\[25\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L354-L362)[\[26\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L370-L378),
...
1.  **Live Monitoring:** As the session proceeds, the researcher watches
    ...
    **preview frames** from the devices (for example, a downsampled
    image every few
    seconds)[\[7\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L244-L252).
...
2.  **Stimulus/Event Annotation:** During the recording, certain events
    ...
    timestamp[\[8\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L124-L133).
...
5.  **Session End and Event Log:** After the session, the
    ...
    and
    labels)[\[27\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L126-L134)[\[28\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L140-L148).
...
9000)[\[29\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L129-L137)[\[30\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L120-L128).
...
GSR, etc.) and
status[\[31\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L18-L26)[\[32\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L40-L48).
...
background
thread[\[33\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L19-L28)[\[34\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L51-L59).
...
"sensor_data")[\[35\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L256-L265).
...
testing[\[36\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/ui/MainViewModel.kt#L32-L40)[\[37\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/ui/MainViewModel.kt#L34-L42).
...
1.  **Session Initiation (Command Flow):** The user action of starting a
    ...
    device[\[38\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L410-L419)[\[14\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L424-L432).
    These messages are small JSON payloads, prefixed with a length
    header (to ensure the receiver knows how many bytes to
    read)[\[39\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L156-L165).
    When an Android device receives the start command, it immediately
    responds (if needed) with an acknowledgment
    (`{"type": "ack", "cmd": "start_recording", "status": "ok"}`)[\[40\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L276-L284)
    ...
    all[\[14\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L424-L432)).
...
3.  The **camera data** (visual and thermal) flows from the camera
    ...
    system[\[41\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/webcam/webcam_capture.py#L98-L106)[\[42\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/webcam/webcam_capture.py#L159-L168).
...
4.  The **GSR sensor data** flow varies by mode: in direct PC mode, the
    ...
    queues[\[43\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L9-L17)[\[12\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L91-L99).
    ...
    data[\[44\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt#L74-L82)[\[45\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt#L76-L84)).
    ...
    PC[\[12\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L91-L99),
...
5.  **Real-Time Data Communication (Status/Preview Flow):** Throughout
    ...
    few
    seconds)[\[46\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L228-L237)[\[47\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L240-L248).
    ...
    small image
    string[\[7\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L244-L252).
    ...
    messages[\[35\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L256-L265);
...
8.  Device sends a `file_info` message indicating it is about to send a
    ...
    size[\[48\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L284-L293)[\[49\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L298-L306).
...
10. Device then sends a series of `file_chunk` messages, each containing
    ...
    Base64)[\[50\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L320-L328).
    The PC decodes each chunk and writes to the file
    handle[\[51\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L323-L331).
    ...
    progress[\[52\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L330-L338).

11. When the device finishes sending, it sends `file_end` message with
    the file
    name[\[53\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L347-L355).
    PC then closes the file and compares the received byte count to the
    expected
    size[\[54\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L356-L364).
    If they match, PC logs success and sends back a confirmation
    (`file_received` message with status
    ok)[\[55\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L364-L372).
...
12. This process repeats for each file (each device might have multiple
    ...
    device[\[56\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L509-L518)
    to avoid network congestion (with a short delay between as
    indicated[\[57\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L516-L524)).
...
13. Throughout this, the Session Manager on PC is aware of incoming
    ...
    the session
    metadata[\[58\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L120-L129)[\[59\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L130-L139).
...
14. **Post-Processing Flow:** If post-processing (FR-13) is enabled,
    ...
    timestamp[\[60\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L218-L226)[\[61\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L232-L240)).
...
- recordings/session_20250806_101530/
  ...
  side[\[62\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L374-L382),
...
16. **Scalability and Data Flow Considerations:** The architecture's
    ...
    data[\[50\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L320-L328).
...
code[\[63\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/CameraRecorder.kt#L74-L82)[\[64\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/CameraRecorder.kt#L75-L83)),
...
configuration
file[\[65\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L356-L364)[\[26\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L370-L378)
...
BatteryLevel*[\[66\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt#L113-L119).
...
produced[\[67\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L104-L112)[\[68\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L130-L138).
Additionally, a human-readable **session_info.txt** is generated on the
Android side listing the folder contents and
status[\[69\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L322-L330)[\[4\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L334-L342).
...
event is
added[\[27\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L126-L134).
...
recording[\[70\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L424-L428)
...
compared[\[54\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L356-L364).
...
size[\[71\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L410-L418).
...
[\[1\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L144-L151)
[\[2\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L138-L145)
[\[12\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L91-L99)
[\[13\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L146-L150)
[\[22\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L72-L80)
[\[23\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L96-L104)
[\[24\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L184-L191)
[\[33\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L19-L28)
[\[34\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L51-L59)
[\[43\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py#L9-L17)
shimmer_manager.py

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/shimmer_manager.py>

[\[3\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L74-L81)
[\[11\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L216-L224)
[\[21\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L82-L91)
[\[58\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L120-L129)
[\[59\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L130-L139)
[\[60\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L218-L226)
[\[61\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L232-L240)
[\[67\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L104-L112)
[\[68\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py#L130-L138)
session_manager.py

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/session/session_manager.py>

[\[4\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L334-L342)
[\[8\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L124-L133)
[\[9\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L300-L308)
[\[10\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L311-L319)
[\[25\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L354-L362)
[\[26\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L370-L378)
[\[27\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L126-L134)
[\[28\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L140-L148)
[\[62\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L374-L382)
[\[65\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L356-L364)
[\[69\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L322-L330)
[\[70\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L424-L428)
[\[71\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt#L410-L418)
SessionManager.kt

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/service/SessionManager.kt>

[\[5\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L355-L364)
[\[6\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L26-L34)
[\[7\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L244-L252)
[\[14\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L424-L432)
[\[15\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L484-L492)
[\[18\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L216-L224)
[\[19\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L486-L494)
[\[20\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L313-L322)
[\[29\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L129-L137)
[\[30\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L120-L128)
[\[31\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L18-L26)
[\[32\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L40-L48)
[\[35\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L256-L265)
[\[38\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L410-L419)
[\[39\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L156-L165)
[\[40\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L276-L284)
[\[46\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L228-L237)
[\[47\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L240-L248)
[\[48\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L284-L293)
[\[49\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L298-L306)
[\[50\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L320-L328)
[\[51\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L323-L331)
[\[52\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L330-L338)
[\[53\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L347-L355)
[\[54\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L356-L364)
[\[55\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L364-L372)
[\[56\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L509-L518)
[\[57\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py#L516-L524)
device_server.py

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/network/device_server.py>

[\[16\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/changelog.md#L34-L41)
[\[17\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/changelog.md#L60-L64)
changelog.md

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/changelog.md>

[\[36\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/ui/MainViewModel.kt#L32-L40)
[\[37\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/ui/MainViewModel.kt#L34-L42)
MainViewModel.kt

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/ui/MainViewModel.kt>

[\[41\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/webcam/webcam_capture.py#L98-L106)
[\[42\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/webcam/webcam_capture.py#L159-L168)
webcam_capture.py

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/PythonApp/webcam/webcam_capture.py>

[\[44\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt#L74-L82)
[\[45\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt#L76-L84)
[\[66\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt#L113-L119)
ShimmerRecorder.kt

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/ShimmerRecorder.kt>

[\[63\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/CameraRecorder.kt#L74-L82)
[\[64\]](https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/CameraRecorder.kt#L75-L83)
CameraRecorder.kt

<https://github.com/buccancs/bucika_gsr/blob/d466bcfa17062fa7f2e081dacd3ea6542ee7fd37/AndroidApp/src/main/java/com/multisensor/recording/recording/CameraRecorder.kt>
