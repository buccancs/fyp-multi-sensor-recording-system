...
| FR2 | synchronised Multi-Modal Recording | Must | SessionManager, PC-Android coordination |
| FR3 | Time synchronisation Service | Must | NTPTimeServer, clock alignment protocols |
...
| FR7 | Device synchronisation and Signals | Should | Sync signal broadcasting, JSON protocol |
...
- **FR2: synchronised Multi-Modal Recording (Priority: Must)** -- The system shall start and stop data recording **synchronously** across all connected devices. When a recording session is initiated, the PC controller instructs each Android device to begin recording **GSR data**, **video (RGB camera)**, and **thermal imaging** in parallel. At the same time, the PC begins logging local sensor streams (e.g. from any directly connected Shimmer devices). All streams share a common session timestamp to enable later alignment.
...
| FR2 | SessionManager, PC-Android coordination | shimmer_pc_app.py#L120-L128, session coordination | config.json recording settings | synchronised recording initiation |
| FR3 | NTPTimeServer, clock synchronisation | ntp_time_server.py#L66-L74, L26-L34 | Time sync configuration | Clock alignment protocols |
...
This section presents the system architecture and data flow design, supported by architectural diagrams that illustrate key design decisions and data synchronisation mechanisms.
...
- The **Time synchronisation Service** (Master Clock) runs on the PC to keep device clocks aligned. An `NTPTimeServer` thread on the PC listens on a port (e.g. 8889) and services time-sync requests from clients
...
(the PC's file system) and organised.
...
- **Quality Requirements:** For research validity, the data must be high quality:
  ...
  - synchronisation quality ensures all data streams carry timestamps from a common reference, allowing GSR samples to be aligned to exact video frames within a few milliseconds tolerance.
...
- **Volume and Storage Management:** The system generates **large volumes of data** per session. A 10-minute session with one 1080p camera (~5 Mbps) produces around 375 MB of video data, plus sensor data. To manage this:
  ...
  - Session directory structure organises all data in timestamped folders with systematic naming conventions
...
See Figures 3.7 and 3.8 (Appendix H.2) for clock synchronisation performance and synchronisation accuracy distribution.
...
*Figure 3.16 – Requirements Traceability Matrix (Heat-map): Visual mapping of functional and non-functional requirements to implementing modules, files, and verification artifacts. colour coding indicates implementation completeness and testing coverage.*
...
*Figure 3.16 – Requirements Traceability Matrix (Heatmap): Rows represent FR/NFR requirements, columns show modules/files/commits with implementation evidence. colour coding indicates coverage density and verification artifact locations.*
...
*Figure 3.17 – Session Directory Structure (Tree Diagram): Example session folder hierarchy showing systematic file organisation with metadata entries, naming conventions, and file type classifications.*

![Protocol Schema](../../diagrams/fig_3_18_protocol_schema.png)

*Figure 3.18 – Protocol Message Schema (Annotated JSON): Command protocol examples showing session management, device control, and synchronisation messages with detailed field annotations and validation requirements.*
...
- **CPU utilisation**: 25-40% average (video encoding + network)
...
```
*Links FR2 - synchronised Multi-Modal Recording*
...
```
*Links FR3, NFR2 - Time synchronisation Service*
...
```
*Links FR7 - Device synchronisation Signals*
...
- synchronised multi-modal recording across all connected devices (FR2)
- NTP-based time synchronisation with sub-5ms accuracy (FR3)
...
- Device synchronisation signals and JSON-based command protocol (FR7)
...
- **Advanced Event Annotation**: Real-time manual event marking during recording sessions with timestamp synchronisation
...
- **Advanced Analytics Dashboard**: Real-time physiological data visualisation and basic stress indicator computation
