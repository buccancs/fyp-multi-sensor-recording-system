...
module remains inactive). It also simplifies synchronisation logic,
...
file is saved, which is used to mark events (for synchronisation or
...
See Figure 4.2 (Appendix H.2) for thermal camera integration architecture and data flow.
...
See Figure 4.3 (Appendix H.2) for Shimmer GSR sensor data flow and BLE communication protocol.
...
tuples[1, 2].
This design minimises overhead and latency/jitter -- an imperative for
synchronising local PC data with remote device data.
...
See Figure 4.4 (Appendix H.2) for desktop controller GUI layout and playback interface.
...
`{"id": <command_id>, "command": "<action>", "params": { ... }}`[1, 2].
...
synchronisation strategy. When a PC and phone connect, they perform a
simple handshake (exchange of hello and capabilities). Part of this
handshake is a **time synchronisation routine**. The system employs an
...
synchronisation. However, in this system, LSL is primarily used locally
...
synchronisation still relies on the custom network time alignment, which
...
See Figure 4.5 (Appendix H.2) for communication protocol sequence diagram and message flow.
...
(e.g.,
`raw_rgb_frame_<timestamp>.jpg`)[1, 2].
...
synchronisation, having a separate audio file with a known start time
...
See Figure 4.6 (Appendix H.2) for data processing pipeline architecture and session workflow.
...
- **Ensuring Precise synchronisation:** Achieving tight time
  ...
  the two-tier synchronisation mechanism. Each device timestamps data
  ...
  a significant improvement over naive synchronisation. **Solution
...
IEEE Computer Society. (2019). *IEEE 1588-2019: IEEE Standard for a Precision Clock synchronisation Protocol for Networked Measurement and Control Systems*. IEEE Standards Association.
