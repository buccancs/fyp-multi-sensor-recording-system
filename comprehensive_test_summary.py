...
logging.basicConfig(level=logging.INFO, format=
    ...
    'comprehensive_test_summary.log'), logging.StreamHandler(sys.stdout)])
...
class ComprehensiveTestSummary:
    ...
    def __init__(self):
        ...
        self.summary = {'timestamp': datetime.now().isoformat(),
            'system_info': self._get_system_info(), 'test_categories': {
            'python_orchestrator_tests': {}, 'gradle_tests': {},
            'pytest_tests': {}, 'android_tests': {},
            'individual_test_outputs': {}}, 'consolidated_results': {},
            'overall_statistics': {}}

    def _get_system_info(self) ->Dict[str, Any]:
        try:
            python_version = subprocess.check_output([sys.executable,
            ...
            return {'python_version': python_version, 'java_version':
                java_version, 'platform': sys.platform, 'working_directory':
                str(Path.cwd()), 'test_directory': str(self.
                test_results_dir.absolute())}
    ...
    def collect_python_orchestrator_results(self):
        ...
        for result_file in self.test_results_dir.glob(
            'all_full_suites_results_*.json'):
            try:
                with open(result_file, 'r') as f:
                    data = json.load(f)
                    orchestrator_results.append({'file': str(result_file.
                        name), 'data': data, 'timestamp': data.get(
                        'start_time', 'unknown')})
                logger.info(
                    f'✅ Loaded orchestrator results from {result_file.name}')
            except Exception as e:
                logger.error(f'❌ Failed to load {result_file}: {e}')
        self.summary['test_categories']['python_orchestrator_tests'] = {
            'total_files': len(orchestrator_results), 'results':
            orchestrator_results}
    ...
    def collect_individual_test_outputs(self):
        ...
        for output_file in self.test_results_dir.glob('test_*_output.txt'):
            try:
                with open(output_file, 'r') as f:
                    content = f.read()
                    output_files.append({'file': str(output_file.name),
                        'size': len(content), 'lines': len(content.split(
                        '\n')), 'preview': content[:500] + '...' if len(
        ...
        self.summary['test_categories']['individual_test_outputs'] = {
            'total_files': len(output_files), 'files': output_files}
        return output_files
    ...
    def run_pytest_summary(self):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True,
                timeout=60)
            pytest_summary = {'command': ' '.join(cmd), 'exit_code': result
                .returncode, 'stdout': result.stdout, 'stderr': result.
                stderr, 'success': result.returncode == 0}
    ...
    def run_gradle_test_summary(self):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True,
                timeout=60)
            gradle_summary = {'command': ' '.join(cmd), 'exit_code': result
                .returncode, 'available_tasks': result.stdout if result.
                returncode == 0 else None, 'stderr': result.stderr,
    ...
    def calculate_overall_statistics(self):
        ...
        stats = {'total_test_suites_run': 0, 'successful_test_suites': 0,
            'failed_test_suites': 0, 'total_test_files_found': 0,
            'test_result_files': 0, 'test_output_files': 0,
            'overall_success_rate': 0.0, 'test_coverage_areas': []}
        orchestrator_results = self.summary['test_categories'][
            'python_orchestrator_tests']['results']
        for result in orchestrator_results:
            data = result.get('data', {})
            summary_data = data.get('summary', {})
            stats['total_test_suites_run'] += summary_data.get('total_suites',
                0)
            stats['successful_test_suites'] += summary_data.get(
                'successful_suites', 0)
            stats['failed_test_suites'] += summary_data.get('failed_suites', 0)
        stats['test_result_files'] = self.summary['test_categories'][
            'python_orchestrator_tests']['total_files']
        stats['test_output_files'] = self.summary['test_categories'][
            'individual_test_outputs']['total_files']
        if stats['total_test_suites_run'] > 0:
            stats['overall_success_rate'] = stats['successful_test_suites'
                ] / stats['total_test_suites_run'] * 100
        coverage_areas = ['Recording Functionality', 'Device Management',
            'Calibration', 'File Management', 'Network Connectivity']
    ...
    def generate_comprehensive_report(self):
        ...
        json_file = (self.test_results_dir /
            f'comprehensive_test_summary_{timestamp}.json')
        with open(json_file, 'w') as f:
            json.dump(self.summary, f, indent=2, default=str)
        md_file = (self.test_results_dir /
            f'comprehensive_test_report_{timestamp}.md')
...
"""
        for result in self.summary['test_categories'][
            'python_orchestrator_tests']['results']:
            data = result.get('data', {})
            overall_success = data.get('overall_success', False)
            status = '✅ PASSED' if overall_success else '❌ FAILED'
            content += (
                f"- **{result['file']}**: {status} ({result['timestamp']})\n")
        ...
        if pytest_info.get('success'):
            ...
            content += (
                '⚠️ Pytest has collection issues (dependency problems resolved)\n'
                )
        content += f'\n### Gradle Test Status\n'
...
"""
        if stats['overall_success_rate'] >= 80:
            content += (
                '✅ **Overall Status: HEALTHY** - Test infrastructure is working well.\n\n'
                )
        else:
...
"""
    ...
    def run_comprehensive_analysis(self):
        logger.info('=' * 80)
        logger.info(
            'COMPREHENSIVE TEST SUMMARY - Multi-Sensor Recording System')
        logger.info('=' * 80)
        ...
        logger.info(
            f"Successful Test Suites: {stats['successful_test_suites']}")
        logger.info(f"Failed Test Suites: {stats['failed_test_suites']}")
        logger.info(
            f"Overall Success Rate: {stats['overall_success_rate']:.1f}%")
