...
from bucika_gsr.network.pc_server import PCServer, JsonMessage, StartRecordCommand, StopRecordCommand
...
class MasterClockSynchronizer:
    ...
    def __init__(self, ntp_port: int=8889, pc_server_port: int=9000,
        sync_interval: float=5.0, logger_instance: Optional[logging.Logger]
        =None):
        ...
        self.sync_status_callbacks: List[Callable[[Dict[str, SyncStatus]],
            None]] = []
    ...
    def start(self) ->bool:
        try:
            ...
            self.sync_thread = threading.Thread(target=self.
                _sync_monitoring_loop, name='SyncMonitor')
            self.sync_thread.daemon = True
            self.sync_thread.start()
            self.logger.info(
                'Master clock synchronization system started successfully')
            return True
    ...
    def start_synchronized_recording(self, session_id: str, target_devices:
        Optional[List[str]]=None, record_video: bool=True, record_thermal:
        bool=True, record_shimmer: bool=False) ->bool:
        try:
            ...
            session = RecordingSession(session_id=session_id,
                start_timestamp=master_timestamp, devices=set(
                target_devices), webcam_files={}, android_files={},
                is_active=True, sync_quality=1.0)
            self.active_sessions[session_id] = session
            android_devices = [d for d in target_devices if d in self.
                connected_devices and self.connected_devices[d].device_type ==
                'android']
            for device_id in android_devices:
                start_cmd = StartRecordCommand(session_id=session_id,
                    record_video=record_video, record_thermal=
                    record_thermal, record_shimmer=record_shimmer)
                start_cmd.timestamp = master_timestamp
                success = self.pc_server.send_message(device_id, start_cmd)
                if not success:
                    self.logger.error(
                        f'Failed to send start command to {device_id}')
                else:
                    self.logger.info(
                        f'Start recording command sent to {device_id}')
    ...
    def stop_synchronized_recording(self, session_id: str) ->bool:
        try:
            ...
            if not session.is_active:
                ...
                return True
            master_timestamp = self.get_master_timestamp()
            android_devices = [d for d in session.devices if d in self.
                connected_devices and self.connected_devices[d].device_type ==
                'android']
            for device_id in android_devices:
                ...
                if not success:
                    self.logger.error(
                        f'Failed to send stop command to {device_id}')
                else:
                    self.logger.info(
                        f'Stop recording command sent to {device_id}')
    ...
    def add_sync_status_callback(self, callback: Callable[[Dict[str,
        SyncStatus]], None]):
    ...
    def _on_device_connected(self, device_id: str, device_info):
        try:
            sync_status = SyncStatus(device_id=device_id, device_type=
                'android', is_synchronized=False, time_offset_ms=0.0,
                last_sync_time=time.time(), sync_quality=0.0,
                recording_active=False, frame_count=0)
        ...
        try:
            if device_id in self.connected_devices:
                ...
                if abs(time_offset_ms) <= self.sync_tolerance_ms:
                    device_status.sync_quality = 1.0 - abs(time_offset_ms
    ...
    def _initiate_device_sync(self, device_id: str):
        try:
            ...
            if success:
                ...
                self.logger.error(
                    f'Failed to initiate sync for device {device_id}')
        except Exception as e:
        ...
        while self.is_running:
            try:
                for device_id, status in self.connected_devices.items():
                    current_time = time.time()
                    if (current_time - status.last_sync_time > self.
                        sync_interval * 2):
                        self.logger.warning(
                ...
                for session in self.active_sessions.values():
                    if session.is_active:
                        ...
                        for device_id in session.devices:
                            if device_id in self.connected_devices:
                                session_sync_qualities.append(self.
                                    connected_devices[device_id].sync_quality)
                        if session_sync_qualities:
                            session.sync_quality = sum(session_sync_qualities
                                ) / len(session_sync_qualities)
                        else:
...
def initialize_master_synchronizer(ntp_port: int=8889, pc_server_port: int=9000
    ) ->bool:
    global _master_synchronizer
    try:
        _master_synchronizer = MasterClockSynchronizer(ntp_port=ntp_port,
