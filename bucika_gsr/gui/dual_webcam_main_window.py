...
from bucika_gsr.utils.logging_config import get_logger, performance_timer, log_method_entry
...
class DualWebcamSettingsPanel(QGroupBox):
    ...
    def setup_ui(self):
        ...
        self.resolution_combo.addItems(['3840x2160 (4K)', '1920x1080 (HD)',
            '1280x720 (HD Ready)', '640x480 (VGA)'])
        self.resolution_combo.currentTextChanged.connect(self.
            on_settings_changed)
    ...
    def get_settings(self) ->dict:
        ...
        return {'camera1_index': self.camera1_spinbox.value(),
            'camera2_index': self.camera2_spinbox.value(), 'width': width,
            'height': height, 'fps': self.fps_spinbox.value()}
...
class DualWebcamMainWindow(QMainWindow):
    ...
    def test_cameras(self):
        ...
        try:
            ...
            camera_indices = [settings['camera1_index'], settings[
                'camera2_index']]
    ...
    def start_preview(self):
        try:
            ...
            self.dual_webcam_capture = DualWebcamCapture(camera1_index=
                settings['camera1_index'], camera2_index=settings[
                'camera2_index'], width=settings['width'], height=settings[
                'height'], fps=settings['fps'])
            self.dual_webcam_capture.dual_frame_ready.connect(self.
                on_dual_frame_ready)
    ...
    def on_settings_changed(self, settings: dict):
        ...
        self.camera1_preview.title_label.setText(
            f"Camera {settings['camera1_index']}")
        self.camera2_preview.title_label.setText(
            f"Camera {settings['camera2_index']}")
        ...
        try:
            if self.dual_webcam_capture and self.is_previewing:
                ...
                if frame_data:
                    ...
                    self.camera1_preview.update_status(frame_data.
                        camera1_frame is not None, camera1_fps)
                    self.camera2_preview.update_status(frame_data.
                        camera2_frame is not None, camera2_fps)
        except Exception as e:
