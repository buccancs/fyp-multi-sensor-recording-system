...
from bucika_gsr.gui.stimulus_controller import StimulusController
...
class MainController(QObject):
    ...
    def inject_dependencies(self, session_manager: SessionManager,
        json_server: JsonSocketServer, webcam_capture: WebcamCapture,
    ...
    def _connect_service_signals(self):
        if not all([self._json_server, self._webcam_capture, self.
            _stimulus_controller]):
            raise RuntimeError(
        ...
        self._json_server.device_disconnected.connect(self.
        ...
        self._json_server.preview_frame_received.connect(self.
            _on_preview_frame_received)
        self._json_server.sensor_data_received.connect(self.
            _on_sensor_data_received)
        self._json_server.notification_received.connect(self.
        ...
        self._webcam_capture.recording_started.connect(self.
            _on_webcam_recording_started)
        self._webcam_capture.recording_stopped.connect(self.
            _on_webcam_recording_stopped)
        self._webcam_capture.error_occurred.connect(self._on_webcam_error)
        self._webcam_capture.status_changed.connect(self.
            _on_webcam_status_changed)
        self._stimulus_controller.seek_requested.connect(self.
            _on_stimulus_seek_requested)
        self._stimulus_controller.screen_changed.connect(self.
            _on_stimulus_screen_changed)
        self._stimulus_controller.start_recording_play_requested.connect(self
            ._on_start_recording_play_requested)
        self._stimulus_controller.mark_event_requested.connect(self.
            _on_mark_event_requested)
        self._stimulus_controller.status_changed.connect(self.
            _on_stimulus_status_changed)
        self._stimulus_controller.experiment_started.connect(self.
            _on_stimulus_experiment_started)
        self._stimulus_controller.experiment_ended.connect(self.
            _on_stimulus_experiment_ended)
        self._stimulus_controller.error_occurred.connect(self.
            _on_stimulus_error)
        if self._session_logger:
            self._session_logger.session_started.connect(self.
                _on_session_logger_session_started)
            self._session_logger.session_ended.connect(self.
                _on_session_logger_session_ended)
            self._session_logger.error_occurred.connect(self.
                _on_session_logger_error)
    ...
    def stop_session(self) ->bool:
        try:
            if not self._session_manager or not self._current_session_id:
                return False
            duration = self._session_manager.end_session(self.
                _current_session_id)
    ...
    def start_webcam_recording(self, session_id: str) ->bool:
        try:
            ...
            if not self._webcam_recording:
                ...
                self.logger.info(
                    f'Webcam recording started for session: {session_id}')
            return True
        except Exception as e:
    ...
    def send_command_to_device(self, device_id: str, command: str, params:
    ...
    def broadcast_command(self, command: str, params: Dict[str, Any]=None
        ) ->bool:
        try:
            if not self._json_server:
                raise RuntimeError('JSON server not injected')
    ...
    def _on_ack_received(self, device_id: str, cmd: str, success: bool,
        message: str):
        if not success:
            self.error_occurred.emit(device_id,
                f'Command {cmd} failed: {message}')

    def _on_preview_frame_received(self, device_id: str, frame_type: str,
        base64_data: str):
    ...
    def _on_notification_received(self, device_id: str, event_type: str,
        event_data: dict):
    ...
    def _on_stimulus_seek_requested(self, position: float):
        try:
            if hasattr(self, 'stimulus_controller'
                ) and self.stimulus_controller:
                ...
                if duration > 0:
                    ...
                    self.stimulus_status_changed.emit(
                        'Cannot seek: no media loaded')
            else:
                self.stimulus_status_changed.emit(
                    'Stimulus controller not available')
        except Exception as e:
    ...
    def _on_mark_event_requested(self):
        if self._session_logger and self._current_session_id:
            self._session_logger.log_event(self._current_session_id,
                'user_marked_event', {})
    ...
    def _on_session_logger_error(self, error_type: str, error_message: str):
        self.error_occurred.emit('session_logger',
