...
class HandSegmentationEngine:
    ...
    def initialize(self) ->bool:
        try:
            if self.config.method == SegmentationMethod.MEDIAPIPE:
                ...
                self.segmentation_model = ColorBasedHandSegmentation(self.
                    config)
            elif self.config.method == SegmentationMethod.CONTOUR_BASED:
                self.segmentation_model = ContourBasedHandSegmentation(self
                    .config)
            else:
                print(
                    f'[ERROR] Unknown segmentation method: {self.config.method}'
                    )
                return False
            if not self.segmentation_model.initialize():
                print(
                    f'[ERROR] Failed to initialize {self.config.method.value} model'
                    )
                return False
    ...
    def process_video(self, input_video_path: str, output_directory: str
        ) ->ProcessingResult:
        start_time = time.time()
        result = ProcessingResult(input_video_path=input_video_path,
        ...
        try:
            ...
            if self.config.output_cropped:
                cropped_video_path = os.path.join(output_directory,
                    'hands_cropped.mp4')
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                crop_width, crop_height = 640, 480
                video_writers['cropped'] = cv2.VideoWriter(cropped_video_path,
                    fourcc, fps, (crop_width, crop_height))
                output_files['cropped_video'] = cropped_video_path
            if self.config.output_masks:
                mask_video_path = os.path.join(output_directory,
                    'hand_masks.mp4')
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                video_writers['mask'] = cv2.VideoWriter(mask_video_path,
            ...
            while True:
                ...
                frame_info = {'frame': frame_count, 'hands_detected': len(
                    hand_regions), 'regions': []}
                for i, hand_region in enumerate(hand_regions):
                    frame_info['regions'].append({'bbox': hand_region.bbox,
                        'confidence': hand_region.confidence, 'hand_label':
                        hand_region.hand_label})
                    if self.config.output_cropped and video_writers.get(
                        'cropped'):
                    ...
                    for hand_region in hand_regions:
                        if hand_region.mask is not None:
                            combined_mask = cv2.bitwise_or(combined_mask,
                                hand_region.mask)
                    mask_colored = cv2.cvtColor(combined_mask, cv2.
                        COLOR_GRAY2BGR)
            ...
            for writer in video_writers.values():
                writer.release()
            detection_log_path = os.path.join(output_directory,
                'detection_log.json')
            import json
    ...
    def process_frame_batch(self, frames: List[np.ndarray]) ->List[List[
        HandRegion]]:
        if not self.is_initialized:
            return []
        results = []
...
def create_segmentation_engine(method: str='mediapipe', **kwargs
