    ...
    def initialize(self) ->bool:
        try:
            ...
            self.hands = self.mp_hands.Hands(static_image_mode=False,
                ...
                min_detection_confidence=self.config.
                min_detection_confidence, min_tracking_confidence=self.
    ...
    def process_frame(self, frame: np.ndarray) ->List[HandRegion]:
        ...
        if results.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                bbox = create_bounding_box_from_landmarks(landmarks, width,
                    height, self.config.crop_padding)
                mask = None
                if self.config.output_masks:
                    mask = create_hand_mask_from_landmarks(landmarks, frame
                        .shape)
                hand_label = 'Unknown'
                if results.multi_handedness:
                    if idx < len(results.multi_handedness):
                        hand_label = results.multi_handedness[idx
                            ].classification[0].label
                hand_region = HandRegion(bbox=bbox, mask=mask, landmarks=
                    landmarks, confidence=results.multi_handedness[idx].
                    classification[0].score if results.multi_handedness and
                    idx < len(results.multi_handedness) else 1.0,
...
class ColorBasedHandSegmentation(BaseHandSegmentation):
    ...
    def process_frame(self, frame: np.ndarray) ->List[HandRegion]:
        ...
        contours, _ = cv2.findContours(skin_mask, cv2.RETR_EXTERNAL, cv2.
            CHAIN_APPROX_SIMPLE)
        for contour in contours:
            area = cv2.contourArea(contour)
            if (self.config.contour_min_area <= area <= self.config.
                contour_max_area):
                ...
                hand_region = HandRegion(bbox=bbox, mask=mask, landmarks=
                    None, confidence=confidence, hand_label='Unknown')
                hand_regions.append(hand_region)
        ...
        for contour in contours:
            ...
            if (self.config.contour_min_area <= area <= self.config.
                ...
                if 0.1 <= compactness <= 0.8:
                    ...
                    hand_region = HandRegion(bbox=bbox, mask=mask,
                        landmarks=None, confidence=confidence, hand_label=
                        'Unknown')
                    hand_regions.append(hand_region)
        hand_regions.sort(key=lambda x: x.confidence, reverse=True)
        return hand_regions[:self.config.max_num_hands]
