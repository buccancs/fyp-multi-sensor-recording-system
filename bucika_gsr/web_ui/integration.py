...
class WebDashboardIntegration:
    ...
    def __init__(self, enable_web_ui: bool=True, web_port: int=5000,
        ...
        if main_controller is not None:
            ...
            self.using_web_controller = False
        logger.info(
            f'Web Dashboard Integration initialized (enabled: {enable_web_ui})'
            )
        logger.info(
    ...
    def start_web_dashboard(self) ->bool:
        ...
        if self.is_running:
            ...
            return True
        try:
            self.web_server = WebDashboardServer(host='0.0.0.0', port=self.
                web_port, debug=False, controller=self.controller)
            self.web_server.start_server()
            self.is_running = True
            logger.info(
                f'Web dashboard started on http://localhost:{self.web_port}')
            if self.controller is not None:
                ...
                logger.warning(
                    'No controller available for real data integration')
            return True
    ...
    def _connect_to_controller(self):
        if self.controller is None:
            return
        try:
            self.controller.device_status_received.connect(self.
                _on_device_status_received)
            self.controller.sensor_data_received.connect(self.
                _on_sensor_data_received)
            self.controller.session_status_changed.connect(self.
                _on_session_status_changed)
            controller_type = ('WebController' if self.using_web_controller
                 else 'MainController')
    ...
    def _on_device_status_received(self, device_id: str, status_data: dict):
        if self.is_running and self.web_server:
            ...
            self.web_server.update_device_status(device_type, device_id,
                status_data)
        ...
        if self.is_running and self.web_server:
            for sensor_type, value in sensor_data.items():
                if isinstance(value, (int, float)):
                    self.web_server.update_sensor_data(device_id,
                        sensor_type, value)

    def _on_session_status_changed(self, session_id: str, is_active: bool):
        if self.is_running and self.web_server:
            session_info = {'active': is_active, 'session_id': session_id if
                is_active else None, 'start_time': time.time() if is_active
                 else None}
...
def get_web_integration(enable_web_ui: bool=True, web_port: int=5000,
    ...
    if _web_integration_instance is None:
        _web_integration_instance = WebDashboardIntegration(enable_web_ui,
            web_port, main_controller, session_manager, shimmer_manager,
...
def start_web_dashboard(enable_web_ui: bool=True, web_port: int=5000,
    ...
    integration = get_web_integration(enable_web_ui, web_port,
        main_controller, session_manager, shimmer_manager,
        android_device_manager)
...
if __name__ == '__main__':
    ...
    if integration.start_web_dashboard():
        print(
            f'Web dashboard available at: {integration.get_web_dashboard_url()}'
            )
