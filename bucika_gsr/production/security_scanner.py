...
class SecurityScanner:
    ...
    async def _scan_python_files(self):
        self.logger.info(
            'Scanning Python files for security vulnerabilities...')
    ...
    async def _analyze_python_file(self, file_path: Path):
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                await self._check_dangerous_python_patterns(file_path,
                    line_num, line)
    ...
    async def _check_dangerous_python_patterns(self, file_path: Path,
        line_num: int, line: str):
        dangerous_patterns = [('eval\\s*\\(', 'Use of eval() function'), (
            'exec\\s*\\(', 'Use of exec() function'), ('__import__\\s*\\(',
            'Dynamic import usage'), ('pickle\\.loads?\\s*\\(',
            'Insecure pickle usage'), (
            'subprocess\\.call\\s*\\(.*shell\\s*=\\s*True',
            'Shell injection risk'), ('os\\.system\\s*\\(',
            'Command injection risk'), ('input\\s*\\(.*\\)',
            'Potential input validation issue')]
        for pattern, description in dangerous_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                self.issues.append(SecurityIssue(severity='high', category=
                    'code_security', title=description, description=
                    f'Potentially dangerous pattern found: {description}',
                    file_path=str(file_path), line_number=line_num,
                    recommendation=
                    'Review usage and implement proper input validation/sanitization'
                    ))

    async def _check_hardcoded_secrets(self, file_path: Path, line_num: int,
        line: str):
        secret_patterns = [('password\\s*=\\s*["\\\'][^"\\\']{3,}["\\\']',
            'Hardcoded password'), (
            'api_key\\s*=\\s*["\\\'][^"\\\']{10,}["\\\']',
            'Hardcoded API key'), (
            'secret\\s*=\\s*["\\\'][^"\\\']{10,}["\\\']',
        ...
        for pattern, description in secret_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                if any(word in line.lower() for word in ['example', 'test',
                    'dummy', 'placeholder']):
                    continue
                self.issues.append(SecurityIssue(severity='critical',
                    category='credentials', title=description, description=
                    f'Potential hardcoded credential found: {description}',
                    file_path=str(file_path), line_number=line_num,
                    recommendation=
                    'Move secrets to environment variables or secure configuration'
                    ))

    async def _check_sql_injection(self, file_path: Path, line_num: int,
        line: str):
        sql_patterns = [('execute\\s*\\([^)]*%[^)]*\\)',
        ...
        for pattern, description in sql_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                self.issues.append(SecurityIssue(severity='high', category=
                    'injection', title='Potential SQL injection',
                    description=
                    f'SQL injection vulnerability: {description}',
                    file_path=str(file_path), line_number=line_num,
                    recommendation=
                    'Use parameterized queries or prepared statements'))

    async def _check_insecure_random(self, file_path: Path, line_num: int,
        line: str):
        if re.search('random\\.random\\(\\)|random\\.randint\\(', line):
            if any(word in line.lower() for word in ['password', 'token',
                'key', 'secret', 'nonce']):
                self.issues.append(SecurityIssue(severity='medium',
                    category='cryptography', title=
                    'Insecure random generation', description=
                    'Using non-cryptographic random for security-sensitive operation'
                    , file_path=str(file_path), line_number=line_num,
                    recommendation=
    ...
    async def _scan_kotlin_java_files(self):
        self.logger.info(
            'Scanning Kotlin/Java files for security vulnerabilities...')
        android_files = list(self.project_root.rglob('*.kt')) + list(self.
            project_root.rglob('*.java'))
    ...
    async def _analyze_android_file(self, file_path: Path):
        try:
            ...
            lines = content.split('\n')
            for line_num, line in enumerate(lines, 1):
                await self._check_dangerous_android_patterns(file_path,
                    line_num, line)
                await self._check_insecure_storage(file_path, line_num, line)
                await self._check_network_security_issues(file_path,
                    line_num, line)
        except Exception as e:
            self.logger.warning(
                f'Error analyzing Android file {file_path}: {e}')

    async def _check_dangerous_android_patterns(self, file_path: Path,
        line_num: int, line: str):
        dangerous_patterns = [('setJavaScriptEnabled\\s*\\(\\s*true\\s*\\)',
            'JavaScript enabled in WebView'), (
            'addJavascriptInterface\\s*\\(',
            'JavaScript interface in WebView'), (
            'MODE_WORLD_READABLE|MODE_WORLD_WRITABLE',
            'World-accessible file modes'), (
        ...
        for pattern, description in dangerous_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                self.issues.append(SecurityIssue(severity='high', category=
                    'android_security', title=description, description=
                    f'Dangerous Android pattern: {description}', file_path=
                    str(file_path), line_number=line_num, recommendation=
                    'Review and implement secure alternatives'))

    async def _check_insecure_storage(self, file_path: Path, line_num: int,
        line: str):
        storage_patterns = [('SharedPreferences.*MODE_WORLD_READABLE',
            'World-readable SharedPreferences'), (
            'openFileOutput.*MODE_WORLD_READABLE',
            'World-readable file output'), (
            'getExternalStorageDirectory\\(\\)', 'External storage usage')]
        for pattern, description in storage_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                self.issues.append(SecurityIssue(severity='medium',
                    category='data_storage', title='Insecure data storage',
                    description=f'Insecure storage pattern: {description}',
                    file_path=str(file_path), line_number=line_num,
                    recommendation=
                    'Use private storage modes and encrypt sensitive data'))

    async def _check_network_security_issues(self, file_path: Path,
        line_num: int, line: str):
        network_patterns = [('http://', 'Unencrypted HTTP usage'), (
        ...
        for pattern, description in network_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                severity = 'critical' if 'trust' in description.lower(
                    ) else 'medium'
                self.issues.append(SecurityIssue(severity=severity,
                    category='network_security', title=
                    'Network security issue', description=
                    f'Network security vulnerability: {description}',
                    file_path=str(file_path), line_number=line_num,
                    recommendation=
    ...
    async def _scan_configuration_files(self):
        ...
        config_files = list(self.project_root.rglob('*.json')) + list(self.
            project_root.rglob('*.xml')) + list(self.project_root.rglob(
            '*.yml')) + list(self.project_root.rglob('*.yaml')) + list(self
            .project_root.rglob('*.properties'))
    ...
    async def _analyze_config_file(self, file_path: Path):
        try:
            content = file_path.read_text(encoding='utf-8', errors='ignore')
            if re.search('password|secret|key.*[=:].*[^{]', content, re.
                IGNORECASE):
                self.issues.append(SecurityIssue(severity='high', category=
                    'configuration', title=
                    'Potential credentials in config', description=
                    'Configuration file may contain hardcoded credentials',
                    file_path=str(file_path), recommendation=
                    'Use environment variables or secure configuration management'
                    ))
            if file_path.name.endswith('.xml'):
                if re.search('android:debuggable\\s*=\\s*["\\\']true["\\\']',
                    content):
                    self.issues.append(SecurityIssue(severity='medium',
                        category='configuration', title=
                        'Debug mode enabled', description=
                        'Application has debug mode enabled', file_path=str
        ...
        if config_file.exists():
            try:
                with open(config_file) as f:
                    config = json.load(f)
                if config.get('security', {}).get('encryption_enabled', False
                    ) is False:
                    self.issues.append(SecurityIssue(severity='high',
                        category='network_security', title=
                        'Encryption disabled', description=
                        'Network encryption is disabled in configuration',
                        file_path=str(config_file), recommendation=
                        'Enable encryption for network communications'))
                if config.get('security', {}).get('authentication_required',
                    False) is False:
                    self.issues.append(SecurityIssue(severity='medium',
                        category='network_security', title=
                        'Authentication disabled', description=
                        'Network authentication is disabled', file_path=str
                        (config_file), recommendation=
                        'Enable authentication for network connections'))
            except Exception as e:
        ...
        try:
            ...
            for vuln_pkg in vulnerable_packages:
                if vuln_pkg in content:
                    self.issues.append(SecurityIssue(severity='high',
                        category='dependencies', title=
                        'Vulnerable dependency', description=
                        f'Potentially vulnerable package: {vuln_pkg}',
                        file_path=str(requirements_file), recommendation=
                        'Update to secure version or replace with alternative')
                        )
        except Exception as e:
        ...
        try:
            ...
            for pattern, description in vulnerable_patterns:
                if re.search(pattern, content):
                    self.issues.append(SecurityIssue(severity='medium',
                        category='dependencies', title=
                        'Outdated dependency', description=
                        f'Potentially outdated dependency: {description}',
                        file_path=str(gradle_file), recommendation=
            ...
            if file_path.is_file():
                try:
                    ...
                    if mode.endswith('6') or mode.endswith('7'):
                        self.issues.append(SecurityIssue(severity='medium',
                            category='file_permissions', title=
                            'Overly permissive file permissions',
                            description=
                            f'File has world-writable permissions: {mode}',
                            file_path=str(file_path), recommendation=
                            'Restrict file permissions to necessary access only'
                            ))
                except Exception:
                    pass
    ...
    async def _scan_for_secrets(self):
        ...
        secret_patterns = [('sk_live_[0-9a-zA-Z]{24}',
            'Stripe live secret key'), ('sk_test_[0-9a-zA-Z]{24}',
            'Stripe test secret key'), ('AKIA[0-9A-Z]{16}',
            'AWS access key'), ('AIza[0-9A-Za-z\\-_]{35}', 'Google API key'
            ), ('[0-9a-fA-F]{32}', 'Potential MD5 hash'), (
            'github_pat_[0-9a-zA-Z_]{82}', 'GitHub personal access token')]
        for file_path in self.project_root.rglob('*'):
            if file_path.is_file() and file_path.suffix in ['.py', '.kt',
                '.java', '.js', '.json', '.yml', '.yaml']:
                try:
                    content = file_path.read_text(encoding='utf-8', errors=
                        'ignore')
                    for pattern, description in secret_patterns:
                        if re.search(pattern, content):
                            self.issues.append(SecurityIssue(severity=
                                'critical', category='secrets', title=
                                'Exposed secret', description=
                                f'Potential exposed secret: {description}',
                                file_path=str(file_path), recommendation=
                                'Remove secret from code and use secure storage'
                                ))
                except Exception:
                    pass
    ...
    async def _scan_crypto_usage(self):
        ...
        crypto_patterns = [('MD5|md5', 'Weak hash algorithm MD5'), (
            'SHA1|sha1', 'Weak hash algorithm SHA1'), ('DES|des',
            'Weak encryption algorithm DES'), ('RC4|rc4',
            ...
            try:
                ...
                for pattern, description in crypto_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        self.issues.append(SecurityIssue(severity='medium',
                            category='cryptography', title=
                            'Weak cryptographic algorithm', description=
                            f'Usage of weak algorithm: {description}',
                            file_path=str(file_path), recommendation=
                            'Use strong cryptographic algorithms (AES, SHA-256, etc.)'
                            ))
            except Exception:
                pass
    ...
    async def _scan_android_security(self):
        ...
        manifest_path = (self.project_root / 'AndroidApp' / 'src' / 'main' /
            'AndroidManifest.xml')
    ...
    async def _analyze_android_manifest(self, manifest_path: Path):
        try:
            ...
            for pattern, description in security_checks:
                if re.search(pattern, content):
                    severity = 'high' if 'debug' in description.lower(
                        ) else 'medium'
                    self.issues.append(SecurityIssue(severity=severity,
                        category='android_manifest', title=
                        f'Android manifest issue: {description}',
                        description=
                        f'Potentially insecure manifest setting: {description}'
                        , file_path=str(manifest_path), recommendation=
                        'Review and secure manifest configuration'))
        except Exception as e:
    ...
    async def _analyze_proguard_config(self, proguard_path: Path):
        try:
            ...
            if re.search('-dontobfuscate', content):
                self.issues.append(SecurityIssue(severity='medium',
                    category='code_protection', title=
                    'Obfuscation disabled', description=
                    'Code obfuscation is disabled in ProGuard', file_path=
                    str(proguard_path), recommendation=
    ...
    def _generate_report(self, duration: float) ->SecurityReport:
        critical_count = len([i for i in self.issues if i.severity ==
            'critical'])
        ...
        return SecurityReport(timestamp=datetime.now().isoformat(),
            scan_duration_seconds=duration, total_issues=len(self.issues),
            critical_issues=critical_count, high_issues=high_count,
            medium_issues=medium_count, low_issues=low_count, issues=self.
            issues, scanned_files=self.scanned_files, recommendations=
            recommendations)
        ...
        with open(txt_file, 'w') as f:
            ...
            if report.issues:
                ...
                for issue in sorted(report.issues, key=lambda x: {
                    'critical': 0, 'high': 1, 'medium': 2, 'low': 3}[x.
                    severity]):
