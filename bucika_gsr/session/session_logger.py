...
class SessionLogger(QObject):
    ...
    def start_session(self, session_name: Optional[str]=None, devices:
        Optional[List[Dict]]=None) ->Dict:
        with self.lock:
            ...
            if session_name:
                safe_name = ''.join(c for c in session_name if c.isalnum() or
                    c in (' ', '-', '_')).rstrip()
            ...
            self.current_session = {'session': session_id, 'session_name':
                ...
                'calibration_files': [], 'status': 'active'}
            ...
            self._flush_to_disk()
            ui_message = (
                f"Session {session_id} started. Devices: {', '.join([d.get('id', 'unknown') for d in devices or []])}"
                )
            self.log_entry_added.emit(ui_message)
            self.session_started.emit(session_id)
            self.logger.info(
                f'Session started: {session_id} at {session_folder}')
            return {'session_id': session_id, 'start_time': self.
                session_start_time.isoformat(), 'log_file_path': str(self.
                log_file_path), 'session_folder': str(session_folder)}
    ...
    def log_event(self, event_type: str, details: Optional[Dict]=None) ->None:
        ...
        with self.lock:
            ...
            event_entry = {'event': event_type, 'time': event_time.strftime
                ('%H:%M:%S.%f')[:-3], 'timestamp': event_time.isoformat()}
            if details:
            ...
            if event_type == 'error':
                error_type = details.get('error_type', 'unknown'
                    ) if details else 'unknown'
                error_message = details.get('message', 'No details'
    ...
    def log_device_connected(self, device_id: str, device_type: str=
        'unknown', capabilities: Optional[List[str]]=None) ->None:
        self.log_event('device_connected', {'device': device_id,
            'device_type': device_type, 'capabilities': capabilities or []})

    def log_device_disconnected(self, device_id: str, reason: str='unknown'
        ) ->None:
        self.log_event('device_disconnected', {'device': device_id,
            'reason': reason})
    ...
    def log_file_received(self, device_id: str, filename: str, file_size:
        Optional[int]=None, file_type: str='unknown') ->None:
        details = {'device': device_id, 'filename': filename, 'file_type':
            file_type}
    ...
    def log_calibration_capture(self, device_id: str, filename: str) ->None:
        self.log_event('calibration_capture', {'device': device_id, 'file':
            filename})
        if self.current_session and filename not in self.current_session[
            'calibration_files']:
    ...
    def log_error(self, error_type: str, message: str, device_id: Optional[
        str]=None) ->None:
    ...
    def end_session(self) ->Optional[Dict]:
        ...
        with self.lock:
            ...
            ui_message = (
                f'Session {session_id} completed. Duration: {duration:.1f}s. Log saved to {self.log_file_path}'
                )
            self.log_entry_added.emit(ui_message)
            ...
            self.logger.info(
                f'Session ended: {session_id} (duration: {duration:.1f}s)')
    ...
    def _format_event_for_ui(self, event_entry: Dict) ->str:
        ...
        if event_type == 'session_start':
            ...
            return (
                f"{time_str} - Session started. Devices: {', '.join(devices) if devices else 'None'}"
                )
            ...
            return (
                f'{time_str} - Device disconnected: {device} (reason: {reason})'
                )
        elif event_type == 'start_record':
            devices = event_entry.get('devices', [])
            return (
                f"{time_str} - Recording started on devices: {', '.join(devices) if devices else 'None'}"
                )
            ...
            if stim_time:
                return (
                    f"{time_str} - Marker '{label}' inserted (stimulus time: {stim_time})"
                    )
            else:
            ...
            if size:
                ...
                return (
                    f'{time_str} - File received from {device}: {filename} ({size_mb:.1f} MB)'
                    )
            else:
            ...
            return (
                f'{time_str} - Calibration image captured from {device}: {filename}'
                )
        elif event_type == 'calibration_done':
            result_file = event_entry.get('result_file')
            if result_file:
                return (
                    f'{time_str} - Calibration completed. Results saved to: {result_file}'
                    )
            else:
