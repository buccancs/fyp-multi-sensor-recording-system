...
from bucika_gsr.shimmer_manager import ShimmerManager, ShimmerSample, ShimmerStatus, ConnectionType, DeviceState
...
class ShimmerPCApplication:
    ...
    def initialize(self) ->bool:
        try:
            ...
            self.shimmer_manager = ShimmerManager(enable_android_integration
            ...
            self.shimmer_manager.add_android_device_callback(self.
                _on_android_device_event)
            self.shimmer_manager.add_connection_state_callback(self.
        ...
        try:
            ...
            if android_devices:
                android_success = self.shimmer_manager.start_android_session(
                    session_id, record_shimmer=record_shimmer, record_video
                    =True, record_thermal=True)
                if android_success:
                    self.logger.info(
    ...
    def stop_session(self) ->bool:
        try:
            ...
            if self.session_start_time:
                ...
                self.logger.info(
                    f'  Data samples: {self.data_samples_received}')
                self.logger.info(
    ...
    def get_status_summary(self) ->Dict[str, Any]:
        ...
        return {'android_devices': len(android_devices), 'shimmer_devices':
            len(shimmer_devices), 'active_session': self.current_session_id,
            'data_samples_received': self.data_samples_received,
            'devices_streaming': sum(1 for status in shimmer_devices.values
            () if status.is_streaming), 'devices_recording': sum(1 for
            status in shimmer_devices.values() if status.is_recording)}
    ...
    def _on_shimmer_data(self, sample: ShimmerSample) ->None:
        ...
        if sample.device_id not in self.device_stats:
            self.device_stats[sample.device_id] = {'samples_received': 0,
                'last_timestamp': None, 'connection_type': sample.
        ...
        if self.data_samples_received % 100 == 0:
            self.logger.debug(
                f'Received {self.data_samples_received} samples total')

    def _on_status_update(self, device_id: str, status: ShimmerStatus) ->None:
        self.connected_devices[device_id] = {'connection_type': status.
            connection_type.value, 'device_state': status.device_state.
            value, 'is_streaming': status.is_streaming, 'is_recording':
            status.is_recording, 'battery_level': status.battery_level,
            'samples_recorded': status.samples_recorded}

    def _on_android_device_event(self, device_id: str, status: Dict[str, Any]
        ) ->None:
        self.logger.debug(f'Android device {device_id} status: {status}')

    def _on_connection_state_change(self, device_id: str, state:
        DeviceState, connection_type: ConnectionType) ->None:
        self.logger.info(
            f'Device {device_id} ({connection_type.value}): {state.value}')

    def _start_monitoring_threads(self) ->None:
        self.monitor_thread = threading.Thread(target=self.
            _device_monitor_loop, daemon=True)
        self.monitor_thread.start()
        self.stats_thread = threading.Thread(target=self._statistics_loop,
            daemon=True)
...
def main():
    ...
    parser.add_argument('--port', type=int, default=9000, help=
        'Android device server port (default: 9000)')
    parser.add_argument('--log-level', default='INFO', choices=['DEBUG',
        'INFO', 'WARNING', 'ERROR'], help='Logging level (default: INFO)')
    parser.add_argument('--session-id', type=str, help=
        'Automatically start a recording session with this ID')
    parser.add_argument('--duration', type=int, default=0, help=
    ...
    logging.basicConfig(level=getattr(logging, args.log_level), format=
        ...
        logging.StreamHandler(sys.stdout), logging.FileHandler(
        'shimmer_pc_app.log')])
