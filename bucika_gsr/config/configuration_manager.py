...
class ConfigurationManager:
    ...
    def save_device_configuration(self, config: DeviceConfig) ->bool:
        try:
            ...
            self.logger.info(
                f'Saved device configuration for {config.device_id}')
            return True
    ...
    def get_active_device_configurations(self) ->List[DeviceConfig]:
        return [config for config in self.device_configs.values() if config
            .active]
    ...
    def remove_device_configuration(self, device_id: str) ->bool:
        try:
            if device_id in self.device_configs:
                ...
                self._save_device_configs()
                self.logger.info(
                    f'Removed device configuration for {device_id}')
                return True
            return False
        except Exception as e:
    ...
    def save_session_configuration(self, config: SessionConfig) ->bool:
        try:
            ...
            self.logger.info(
                f'Saved session configuration for {config.session_id}')
            return True
        except Exception as e:
    ...
    def restore_last_session(self) ->Optional[SessionConfig]:
        try:
            if not self.session_configs:
                ...
                return None
            latest_config = max(self.session_configs.values(), key=lambda x:
                x.modified_timestamp)
            self.logger.info(
                f'Restored last session: {latest_config.session_id}')
            return latest_config
        except Exception as e:
            self.logger.error(f'Error restoring last session: {e}')
            return None

    def get_session_configuration(self, session_id: str) ->Optional[
        SessionConfig]:
        return self.session_configs.get(session_id)

    def export_session_settings(self, session_id: str, export_path:
        Optional[str]=None) ->Optional[str]:
        try:
            config = self.session_configs.get(session_id)
            if not config:
                self.logger.error(
                    f'Session configuration not found: {session_id}')
                return None
            if not export_path:
                export_path = (self.config_dir /
                    f"export_{session_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                    )
            else:
                export_path = Path(export_path)
            export_data = {'export_type': 'session_configuration',
    ...
    def import_session_settings(self, import_path: str) ->Optional[
        SessionConfig]:
        try:
            ...
            session_config = SessionConfig(session_id=config_data[
                'session_id'], device_configs=device_configs,
    ...
    def create_session_config_from_devices(self, session_id: str,
        device_ids: List[str]) ->Optional[SessionConfig]:
        try:
            device_configs = []
            ...
            session_config = SessionConfig(session_id=session_id,
                device_configs=device_configs, recording_settings=self.
                get_app_setting('default_recording_settings', {}),
                calibration_settings=self.get_app_setting(
                'default_calibration_settings', {}), created_timestamp=
                datetime.now().isoformat(), modified_timestamp=datetime.now
                ().isoformat())
            self.save_session_configuration(session_config)
            ...
            if self.session_config_file.exists():
                ...
                for session_id, config_data in data.items():
                    ...
                    for device_data in config_data['device_configs']:
                        device_config = DeviceConfig(**device_data)
                        device_configs.append(device_config)
                    config = SessionConfig(session_id=config_data[
                        'session_id'], device_configs=device_configs,
                        recording_settings=config_data['recording_settings'
                        ], calibration_settings=config_data[
                        'calibration_settings'], created_timestamp=
                        config_data['created_timestamp'],
                        modified_timestamp=config_data['modified_timestamp'])
