...
class WebcamConfiguration:
    ...
    def __post_init__(self):
        ...
        if self.fallback_codecs is None:
            self.fallback_codecs = [VideoCodec.MP4V, VideoCodec.XVID,
                VideoCodec.MJPG]
...
class CameraDetector:
    ...
    def detect_cameras(self, max_cameras: int=10) ->List[CameraInfo]:
        print(
            f'[DEBUG_LOG] Detecting cameras (testing up to {max_cameras} indices)...'
            )
        ...
        working_cameras = [cam for cam in self.detected_cameras if cam.
            is_working]
        print(
    ...
    def _analyze_camera(self, camera_index: int) ->Optional[CameraInfo]:
        ...
        try:
            ...
            if not cap.isOpened():
                return CameraInfo(index=camera_index, name=
                    f'Camera {camera_index}', resolution=(0, 0), max_fps=0,
                    supported_resolutions=[], is_working=False,
            ...
            camera_name = self._generate_camera_name(camera_index, width,
                height)
            return CameraInfo(index=camera_index, name=camera_name,
                resolution=(width, height), max_fps=fps if fps > 0 else 30,
                supported_resolutions=supported_resolutions, is_working=True)
        except Exception as e:
            return CameraInfo(index=camera_index, name=
                f'Camera {camera_index}', resolution=(0, 0), max_fps=0,
                supported_resolutions=[], is_working=False, error_message=
                str(e))
        finally:
            if cap:
                cap.release()
    ...
    def _test_resolutions(self, cap: cv2.VideoCapture) ->List[Tuple[int, int]]:
        test_resolutions = [(320, 240), (640, 480), (854, 480), (1280, 720),
            (1920, 1080), (3840, 2160)]
        supported = []
...
class CodecValidator:
    ...
    @staticmethod
    def test_codec(codec: VideoCodec, resolution: Tuple[int, int]=(640, 480)
        ) ->bool:
        try:
            ...
            if not writer.isOpened():
                return False
            import numpy as np
            test_frame = np.zeros((resolution[1], resolution[0], 3), dtype=
                np.uint8)
            for _ in range(5):
    ...
    @staticmethod
    def get_fallback_codec(preferred_codecs: List[VideoCodec]) ->Optional[
        VideoCodec]:
...
class WebcamConfigManager:
    ...
    def detect_and_configure_cameras(self) ->List[CameraInfo]:
        ...
        if working_cameras:
            ...
            if working_cameras[0].supported_resolutions:
                best_resolution = max(working_cameras[0].
                    supported_resolutions, key=lambda r: r[0] * r[1])
    ...
    def auto_configure(self) ->Dict[str, Any]:
        ...
        config_summary = {'cameras_detected': len(cameras),
            'working_cameras': len(working_cameras), 'available_codecs': [
            codec.value for codec in codecs], 'selected_camera': {'index':
            self.config.camera_index, 'name': self.config.camera_name,
            'resolution': self.config.recording.resolution},
            'selected_codec': self.config.recording.codec.value,
            'fallback_codecs': [codec.value for codec in self.config.
            fallback_codecs]}
        self.save_config()
        print(
    ...
    def set_camera(self, camera_index: int) ->bool:
        ...
        if camera_info and camera_info.is_working:
            ...
            if camera_info.supported_resolutions:
                best_resolution = max(camera_info.supported_resolutions,
                    key=lambda r: r[0] * r[1])
                self.config.recording.resolution = best_resolution
            self.save_config()
            print(
                f'[DEBUG_LOG] Camera set to index {camera_index}: {camera_info.name}'
                )
            return True
        print(
        ...
        try:
            if 'codec' in kwargs:
                ...
                if codec in self.available_codecs:
                    ...
                    fallback = CodecValidator.get_fallback_codec(self.
                        config.fallback_codecs)
                    if fallback:
    ...
    def set_preview_config(self, **kwargs) ->bool:
        try:
            ...
            if 'enable_scaling' in kwargs:
                self.config.preview.enable_scaling = bool(kwargs[
                    'enable_scaling'])
            if 'maintain_aspect_ratio' in kwargs:
                self.config.preview.maintain_aspect_ratio = bool(kwargs[
    ...
    def get_config_dict(self) ->Dict[str, Any]:
        return {'camera_index': self.config.camera_index, 'camera_name':
            self.config.camera_name, 'recording': asdict(self.config.
            recording), 'preview': asdict(self.config.preview),
            'auto_detect_cameras': self.config.auto_detect_cameras,
            'fallback_codecs': [codec.value for codec in self.config.
            fallback_codecs], 'available_cameras': [asdict(cam) for cam in
            self.available_cameras], 'available_codecs': [codec.value for
            codec in self.available_codecs]}

    def save_config(self):
        try:
            config_dict = {'camera_index': self.config.camera_index,
                'camera_name': self.config.camera_name, 'recording': {
                'codec': self.config.recording.codec.value, 'resolution':
                self.config.recording.resolution, 'fps': self.config.
                recording.fps, 'quality': self.config.recording.quality,
    ...
    def load_config(self):
        try:
            if self.config_file.exists():
                ...
                self.config.camera_name = config_dict.get('camera_name',
                    'Default Camera')
                self.config.auto_detect_cameras = config_dict.get(
                    'auto_detect_cameras', True)
                recording_dict = config_dict.get('recording', {})
                self.config.recording.codec = VideoCodec(recording_dict.get
                    ('codec', 'mp4v'))
                self.config.recording.resolution = tuple(recording_dict.get
                    ('resolution', [1280, 720]))
                self.config.recording.fps = recording_dict.get('fps', 30)
                self.config.recording.quality = recording_dict.get('quality',
                    80)
                self.config.recording.file_format = recording_dict.get(
                    'file_format', 'mp4')
                preview_dict = config_dict.get('preview', {})
                self.config.preview.max_width = preview_dict.get('max_width',
                    640)
                self.config.preview.max_height = preview_dict.get('max_height',
                    480)
                ...
                fallback_codec_names = config_dict.get('fallback_codecs', [
                    'mp4v', 'XVID', 'MJPG'])
                self.config.fallback_codecs = [VideoCodec(name) for name in
                    fallback_codec_names]
                print(
                    f'[DEBUG_LOG] Configuration loaded from {self.config_file}'
                    )
            else:
                print(
                    '[DEBUG_LOG] No existing configuration file, using defaults'
                    )
        except Exception as e:
            print(
                f'[DEBUG_LOG] Failed to load configuration: {e}, using defaults'
                )
            self.config = WebcamConfiguration()
...
if __name__ == '__main__':
    ...
    config_manager.set_recording_config(resolution='1280x720', fps=30,
        quality=85)
