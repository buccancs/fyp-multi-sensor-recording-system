...
from bucika_gsr.webcam.advanced_sync_algorithms import AdaptiveSynchronizer, SynchronizationStrategy
from bucika_gsr.webcam.cv_preprocessing_pipeline import AdvancedROIDetector, PhysiologicalSignalExtractor, ROIDetectionMethod, SignalExtractionMethod
...
class DualWebcamCapture(QThread):
    ...
    def __init__(self, camera1_index: int=0, camera2_index: int=1,
        preview_fps: int=30, recording_fps: int=30, resolution: Tuple[int,
        int]=(3840, 2160), sync_callback: Optional[Callable[[float], None]]
        =None):
        super().__init__()
        ...
        self.synchronizer = AdaptiveSynchronizer(target_fps=preview_fps,
            sync_threshold_ms=self.sync_threshold_ms, strategy=
        ...
        self.roi_detector = AdvancedROIDetector(method=ROIDetectionMethod.
            DNN_FACE, tracking_enabled=True, stability_threshold=0.8)
        self.signal_extractor = PhysiologicalSignalExtractor(method=
            SignalExtractionMethod.CHROM_METHOD, sampling_rate=preview_fps,
            signal_length_seconds=10.0)
        self.enable_physio_monitoring = False
        self.current_roi = None
        self.latest_physio_signal = None
        self.camera1_status = CameraStatus(camera1_index, False, 0, (0, 0),
            0, None, None)
        self.camera2_status = CameraStatus(camera2_index, False, 0, (0, 0),
            0, None, None)
        self.performance_stats = {'frames_processed': 0, 'sync_violations':
            0, 'dropped_frames': 0, 'average_processing_time_ms': 0.0}
        logger.info(
        ...
        try:
            ...
            if not self.cap1.isOpened():
                self.error_occurred.emit(
                    f'Could not open camera {self.camera1_index}')
                return False
            self.cap2 = cv2.VideoCapture(self.camera2_index)
            if not self.cap2.isOpened():
                self.error_occurred.emit(
                    f'Could not open camera {self.camera2_index}')
                return False
    ...
    def _configure_brio_camera(self, cap: cv2.VideoCapture, camera_num: int
        ) ->bool:
        try:
            ...
            cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('M', 'J',
                'P', 'G'))
            ...
            if not ret or frame is None:
                logger.error(
                    f'Camera {camera_num} failed initial frame capture test')
                return False
            return True
        except Exception as e:
    ...
    def _update_camera_status(self):
        try:
            ...
            status_dict = {'camera1': {'index': self.camera1_status.
                camera_index, 'active': self.camera1_status.is_active,
                'fps': self.camera1_status.fps, 'resolution': self.
                camera1_status.resolution, 'frames': self.camera1_status.
                frames_captured}, 'camera2': {'index': self.camera2_status.
                camera_index, 'active': self.camera2_status.is_active,
                'fps': self.camera2_status.fps, 'resolution': self.
                camera2_status.resolution, 'frames': self.camera2_status.
                frames_captured}}
    ...
    def start_recording(self, session_id: str) ->bool:
        ...
        try:
            ...
            self.recording_filepath1 = os.path.join(self.output_directory,
                filename1)
            self.recording_filepath2 = os.path.join(self.output_directory,
                filename2)
            resolution = self.camera1_status.resolution
            self.writer1 = cv2.VideoWriter(self.recording_filepath1, self.
                recording_codec, self.recording_fps, resolution)
            self.writer2 = cv2.VideoWriter(self.recording_filepath2, self.
                recording_codec, self.recording_fps, resolution)
            ...
            if not self.is_previewing:
                self.start_preview()
            self.recording_started.emit(self.recording_filepath1, self.
                recording_filepath2)
            ...
            logger.info(
                f'Dual camera recording started: {filename1}, {filename2}')
    ...
    def stop_recording(self) ->Tuple[Optional[str], Optional[str]]:
        ...
        try:
            self.is_recording = False
            duration = time.time(
                ) - self.recording_start_time if self.recording_start_time else 0
            if self.writer1:
            ...
            logger.info(
                f'Dual camera recording stopped (duration: {duration:.1f}s)')
        ...
        while self.running:
            try:
                ...
                if not (ret1 and ret2):
                    ...
                    break
                frame_data = self._process_advanced_synchronization(frame1,
                    frame2, capture_timestamp1, capture_timestamp2)
                ...
                with self.frame_lock:
                    self.last_frames['camera1'
                        ] = frame_data.camera1_frame.copy()
                    self.last_frames['camera2'
                        ] = frame_data.camera2_frame.copy()
                if (self.is_recording and self.writer1 and self.writer2 and
                    current_time - last_recording_time >= self.
                    recording_interval):
                ...
                if (self.is_previewing and current_time - last_preview_time >=
                    self.frame_interval):
                ...
                self.performance_stats['average_processing_time_ms'] = (
                    self.performance_stats['average_processing_time_ms'] *
                    (self.performance_stats['frames_processed'] - 1) +
                    processing_time_ms) / self.performance_stats[
                    'frames_processed']
                time.sleep(0.001)
            except Exception as e:
    ...
    def _frame_to_pixmap(self, frame: np.ndarray, max_width: int=640,
        max_height: int=360) ->Optional[QPixmap]:
        try:
    ...
    def get_roi_metrics(self) ->Dict:
        if hasattr(self, 'roi_detector'):
            ...
            return {'area_pixels': metrics.area_pixels,
                ...
                'is_valid': metrics.is_valid, 'confidence_score': metrics.
                confidence_score, 'frame_count': metrics.frame_count}
        else:
    ...
    def get_latest_physiological_signal(self) ->Optional[Dict]:
        if self.latest_physio_signal is not None:
            ...
            return {'timestamp': signal.timestamp, 'signal_type': signal.
                signal_type, 'extraction_method': signal.extraction_method,
                'signal_length': len(signal.signal_data), 'sampling_rate':
                signal.sampling_rate, 'snr_db': signal.snr_db,
    ...
    def export_synchronization_data(self, filepath: str) ->bool:
        try:
            ...
            export_data = {'export_timestamp': datetime.now().isoformat(),
                'camera_configuration': {'camera1_index': self.
                camera1_index, 'camera2_index': self.camera2_index,
                'target_resolution': self.target_resolution,
                'recording_fps': self.recording_fps, 'preview_fps': self.
                preview_fps}, 'synchronization_diagnostics': diagnostics,
                'roi_metrics': roi_metrics, 'physiological_signal':
                physio_signal, 'performance_stats': self.performance_stats,
                'camera_status': {'camera1': {'index': self.camera1_status.
                camera_index, 'active': self.camera1_status.is_active,
                'fps': self.camera1_status.fps, 'resolution': self.
                camera1_status.resolution, 'frames_captured': self.
                camera1_status.frames_captured}, 'camera2': {'index': self.
                camera2_status.camera_index, 'active': self.camera2_status.
                is_active, 'fps': self.camera2_status.fps, 'resolution':
                self.camera2_status.resolution, 'frames_captured': self.
                camera2_status.frames_captured}}}
    ...
    def _process_advanced_synchronization(self, frame1: np.ndarray, frame2:
        np.ndarray, timestamp1: float, timestamp2: float) ->DualFrameData:
        try:
            sync_frame = self.synchronizer.synchronize_frames(frame1,
                frame2, timestamp1, timestamp2)
            frame_data = DualFrameData(timestamp=sync_frame.timestamp,
                frame_id=sync_frame.frame_id, camera1_frame=sync_frame.
                camera1_frame, camera2_frame=sync_frame.camera2_frame,
                camera1_timestamp=timestamp1, camera2_timestamp=timestamp2,
            ...
            return DualFrameData(timestamp=min(timestamp1, timestamp2),
                frame_id=self.frame_counter, camera1_frame=frame1.copy(),
                camera2_frame=frame2.copy(), camera1_timestamp=timestamp1,
                camera2_timestamp=timestamp2, sync_quality=sync_quality)
        ...
        try:
            ...
            if roi is not None:
                ...
                if roi_region.size > 0:
                    physio_signal = self.signal_extractor.extract_signal(
                        roi_region)
                    if physio_signal is not None:
                        self.latest_physio_signal = physio_signal
                        physio_signal.roi_metrics = (self.roi_detector.
                            get_metrics())
        except Exception as e:
...
def test_dual_webcam_access(camera_indices: List[int]=None):
    ...
    if len(camera_indices) != 2:
        logger.error(
            f'ERROR: Expected 2 camera indices, got {len(camera_indices)}')
        return False
    ...
    if ret1:
        ...
        logger.error(
            f'ERROR: Could not capture frame from camera {camera1_index}')
        cap1.release()
        return False
    ...
    if ret2:
        ...
        logger.error(
            f'ERROR: Could not capture frame from camera {camera2_index}')
        cap1.release()
        cap2.release()
        return False
    cap1.release()
