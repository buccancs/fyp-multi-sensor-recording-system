...
class SyncFrame:
    ...
    def get_sync_offset_ms(self) ->float:
        if self.camera1_hardware_ts and self.camera2_hardware_ts:
            return abs(self.camera1_hardware_ts - self.camera2_hardware_ts
                ) * 1000
        return 0.0
...
class AdaptiveSynchronizer:
    ...
    def __init__(self, target_fps: float=30.0, sync_threshold_ms: float=
        16.67, buffer_size: int=100, strategy: SynchronizationStrategy=
    ...
    def synchronize_frames(self, frame1: np.ndarray, frame2: np.ndarray,
        timestamp1: float, timestamp2: float, hardware_ts1: Optional[float]
        =None, hardware_ts2: Optional[float]=None) ->SyncFrame:
        process_start = time.time()
        sync_frame = SyncFrame(timestamp=min(timestamp1, timestamp2),
            frame_id=self.metrics.frames_processed, camera1_frame=frame1,
            camera2_frame=frame2, camera1_hardware_ts=hardware_ts1,
            camera2_hardware_ts=hardware_ts2, software_capture_ts=time.time())
    ...
    def _master_slave_sync(self, sync_frame: SyncFrame) ->SyncFrame:
        if sync_frame.camera1_hardware_ts and sync_frame.camera2_hardware_ts:
            offset = (sync_frame.camera1_hardware_ts - sync_frame.
                camera2_hardware_ts)
            self.master_clock_offset = offset
        sync_frame.sync_quality = min(1.0, 1.0 - abs(self.
            master_clock_offset) / self.sync_threshold_ms)
        return sync_frame
    ...
    def _hardware_sync(self, sync_frame: SyncFrame) ->SyncFrame:
        if sync_frame.camera1_hardware_ts and sync_frame.camera2_hardware_ts:
            ...
            sync_frame.sync_quality = max(0.0, 1.0 - offset_ms / self.
                sync_threshold_ms)
        else:
            software_offset = abs(sync_frame.timestamp - sync_frame.
                software_capture_ts) * 1000
            sync_frame.sync_quality = max(0.0, 1.0 - software_offset / self
                .sync_threshold_ms)
        return sync_frame
    ...
    def _calculate_sync_quality(self, offset_ms: float) ->float:
        if offset_ms <= self.adaptive_threshold:
            ...
            return 1.0 - (offset_ms - self.adaptive_threshold
    ...
    def _update_metrics(self, sync_frame: SyncFrame, offset_ms: float):
        ...
        if len(self.offset_history) > 1:
            ...
            self.metrics.std_dev_offset_ms = statistics.stdev(self.
                offset_history)
    ...
    def get_diagnostics(self) ->Dict:
        with self._lock:
            return {'strategy': self.current_strategy.value,
                ...
                'buffer_sizes': {'timing': len(self.timing_buffer),
                'offset': len(self.offset_history), 'quality': len(self.
                quality_history)}, 'metrics': {'frames_processed': self.
                metrics.frames_processed, 'sync_violations': self.metrics.
                sync_violations, 'violation_rate': self.metrics.
                sync_violations / max(1, self.metrics.frames_processed),
                'mean_offset_ms': self.metrics.mean_offset_ms,
                'std_dev_offset_ms': self.metrics.std_dev_offset_ms,
                'max_offset_ms': self.metrics.max_offset_ms, 'jitter_ms':
                self.metrics.jitter_ms, 'drift_rate_ppm': self.metrics.
                drift_rate_ppm, 'current_quality': self.metrics.quality_score}}
...
def test_dual_camera_sync(camera1_index: int=0, camera2_index: int=1,
    duration_seconds: int=10) ->Dict:
    logger.info(
        f'Starting dual camera synchronization test (cameras {camera1_index}, {camera2_index})'
        )
    synchronizer = AdaptiveSynchronizer(target_fps=30.0, strategy=
        SynchronizationStrategy.ADAPTIVE_HYBRID)
    try:
        ...
        while time.time() - start_time < duration_seconds:
            ...
            ts2 = time.time()
            if ret1 and ret2:
                sync_frame = synchronizer.synchronize_frames(frame1, frame2,
                    ts1, ts2)
        ...
        test_results = {'success': True, 'duration_seconds': time.time() -
            ...
            sync_frames]) if sync_frames else 0, 'min_quality': min([f.
            sync_quality for f in sync_frames]) if sync_frames else 0,
            'max_quality': max([f.sync_quality for f in sync_frames]) if
            sync_frames else 0, 'frames_above_threshold': sum(1 for f in
            sync_frames if f.sync_quality > 0.8)}}
        logger.info(
