...
class SchemaManager:
    ...
    def _load_schema(self) ->None:
        try:
            ...
            logger.info(
                f'Successfully loaded message schema from {self.schema_path}')
    ...
    def _basic_validate(self, message: Dict[str, Any]) ->bool:
        ...
        if message_type == 'start_record':
            ...
            required_fields = ['file_id', 'chunk_index', 'total_chunks',
                'chunk_data', 'chunk_size', 'file_type']
            return all(field in message for field in required_fields)
    ...
    def get_message_template(self, message_type: str) ->Dict[str, Any]:
        templates = {'start_record': {'type': 'start_record', 'timestamp':
            ...
            '', 'width': 0, 'height': 0}, 'file_chunk': {'type':
            'file_chunk', 'timestamp': 0, 'file_id': '', 'chunk_index': 0,
            'total_chunks': 0, 'chunk_data': '', 'chunk_size': 0,
            'file_type': 'video'}, 'device_status': {'type':
            'device_status', 'timestamp': 0, 'device_id': '', 'status':
            'idle'}, 'ack': {'type': 'ack', 'timestamp': 0, 'message_id':
            '', 'success': True}, 'calibration_start': {'type':
            'calibration_start', 'timestamp': 0, 'pattern_type':
            'chessboard', 'pattern_size': {'rows': 7, 'cols': 6}},
            'calibration_result': {'type': 'calibration_result',
            'timestamp': 0, 'success': False}}
        return templates.get(message_type, {'type': message_type,
            'timestamp': 0})
