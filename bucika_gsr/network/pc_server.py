    ...
    def from_json(cls, json_str: str) ->Optional['JsonMessage']:
        try:
            ...
            if message_type == 'hello':
                ...
                return JsonMessage(type=message_type, **{k: v for k, v in
                    data.items() if k != 'type'})
...
class HelloMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'HelloMessage':
        return cls(type=data.get('type', 'hello'), device_id=data.get(
            'device_id', ''), capabilities=data.get('capabilities', []),
...
class StatusMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'StatusMessage':
        return cls(type=data.get('type', 'status'), battery=data.get(
            'battery'), storage=data.get('storage'), temperature=data.get(
            'temperature'), recording=data.get('recording', False),
            connected=data.get('connected', True), timestamp=data.get(
            'timestamp'))
...
class SensorDataMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'SensorDataMessage':
        return cls(type=data.get('type', 'sensor_data'), values=data.get(
...
class AckMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'AckMessage':
        return cls(type=data.get('type', 'ack'), cmd=data.get('cmd', ''),
            status=data.get('status', 'ok'), message=data.get('message'),
            timestamp=data.get('timestamp'))
...
class FileInfoMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'FileInfoMessage':
        return cls(type=data.get('type', 'file_info'), name=data.get('name',
            ''), size=data.get('size', 0), timestamp=data.get('timestamp'))
...
class FileChunkMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'FileChunkMessage':
        return cls(type=data.get('type', 'file_chunk'), seq=data.get('seq',
            0), data=data.get('data', ''), timestamp=data.get('timestamp'))
...
class FileEndMessage(JsonMessage):
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'FileEndMessage':
        return cls(type=data.get('type', 'file_end'), name=data.get('name',
            ''), timestamp=data.get('timestamp'))
...
class PCServer:
    ...
    def start(self) ->bool:
        try:
            ...
            self.logger.info(
                f'PC server started successfully on port {self.port}')
            return True
    ...
    def add_message_callback(self, callback: Callable[[str, JsonMessage], None]
        ) ->None:
        self.message_callbacks.append(callback)

    def add_device_callback(self, callback: Callable[[str, ConnectedDevice],
        None]) ->None:
            ...
            while self.is_running:
                try:
                    ...
                    client_thread = threading.Thread(target=self.
                        _handle_client, args=(client_socket, address), name
    ...
    def _handle_client(self, client_socket: socket.socket, address: tuple
        ...
        try:
            ...
            while self.is_running:
                ...
                if (message_length <= 0 or message_length > self.
                    max_message_size):
                    self.logger.error(
                ...
                if not message:
                    self.logger.warning(
                        f'Failed to parse message from {address}')
                    continue
                if isinstance(message, HelloMessage) and not device_id:
                    device_id = message.device_id
                    device = ConnectedDevice(device_id=device_id,
                        capabilities=message.capabilities, connection_time=
                        time.time(), last_heartbeat=time.time(), status={},
                ...
                if device_id:
                    self.logger.debug(
                        f'Received {message.type} from {device_id}')
        ...
        while self.is_running:
            try:
                ...
                for device_id, device in self.connected_devices.items():
                    if (current_time - device.last_heartbeat > self.
                        heartbeat_timeout):
    ...
    try:
        if server.start():
            ...
            while True:
                ...
                if devices:
                    sync_cmd = FlashSyncCommand(duration_ms=200, sync_id=
                        'test_sync')
