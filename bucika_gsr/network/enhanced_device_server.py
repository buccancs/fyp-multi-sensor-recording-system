...
class EnhancedRemoteDevice:
    ...
    def update_latency(self, latency: float):
        with QMutexLocker(self.mutex):
            ...
            if self.stats.latency_samples:
                self.stats.average_latency = sum(self.stats.latency_samples
                    ) / len(self.stats.latency_samples)
                if len(self.stats.latency_samples) >= 2:
                    variance = sum((x - self.stats.average_latency) ** 2 for
                        x in self.stats.latency_samples) / len(self.stats.
                        latency_samples)
                    self.stats.jitter = variance ** 0.5
                if self.stats.ping_count > 0:
                    self.stats.packet_loss_rate = max(0, (self.stats.
                        ping_count - self.stats.pong_count) / self.stats.
                        ping_count * 100)
    ...
    def get_status_summary(self) ->Dict[str, Any]:
        with QMutexLocker(self.mutex):
            return {'device_id': self.device_id, 'state': self.state.name,
                'capabilities': self.capabilities, 'address':
                f'{self.address[0]}:{self.address[1]}', 'is_alive': self.
                is_alive(), 'streaming_quality': self.streaming_quality,
                'stats': {'messages_sent': self.stats.messages_sent,
                'messages_received': self.stats.messages_received,
                'bytes_sent': self.stats.bytes_sent, 'bytes_received': self
                .stats.bytes_received, 'error_count': self.stats.
                error_count, 'average_latency': round(self.stats.
                average_latency, 2), 'min_latency': round(self.stats.
                min_latency, 2) if self.stats.min_latency != float('inf') else
                0, 'max_latency': round(self.stats.max_latency, 2),
                'jitter': round(self.stats.jitter, 2), 'packet_loss_rate':
                round(self.stats.packet_loss_rate, 2), 'ping_count': self.
                stats.ping_count, 'pong_count': self.stats.pong_count,
                'connection_duration': round(time.time() - self.stats.
                connected_at, 1), 'latency_samples': len(self.stats.
                latency_samples)}}
...
class EnhancedDeviceServer(QThread):
    ...
    def __init__(self, host: str='0.0.0.0', port: int=9000, max_connections:
        int=10, heartbeat_interval: float=5.0):
        super().__init__()
        self.host = host
        self.port = port
        ...
        self.network_stats = {'total_connections': 0, 'active_connections':
            0, 'total_messages': 0, 'total_bytes': 0, 'error_rate': 0.0,
            'average_latency': 0.0}
    ...
    def start_server(self):
        ...
        try:
            ...
            logger.info(
                f'Enhanced Device Server started on {self.host}:{self.port}')
            return True
        ...
        while self.running and self.server_socket:
            try:
                ...
                if len(self.devices) >= self.max_connections:
                    logger.warning(
                        f'Maximum connections reached, rejecting {address}')
                    client_socket.close()
                    continue
                client_socket.setsockopt(socket.IPPROTO_TCP, socket.
                    TCP_NODELAY, 1)
                client_socket.settimeout(30.0)
                handler_thread = threading.Thread(target=self.
                    handle_client_connection, args=(client_socket, address),
                    daemon=True)
                handler_thread.start()
                logger.info(
                    f'New client connection from {address[0]}:{address[1]}')
    ...
    def handle_client_connection(self, client_socket: socket.socket,
        ...
        try:
            ...
            if not device_id:
                logger.warning(
                    f'Missing device_id in handshake from {client_addr}')
                return
            device = EnhancedRemoteDevice(device_id, capabilities,
                client_socket, address)
            with QMutexLocker(self.devices_mutex):
                self.devices[device_id] = device
                self.client_handlers[device_id] = threading.current_thread()
            ack_msg = {'type': 'handshake_ack', 'protocol_version': 1,
                'server_name': 'Enhanced Device Server', 'server_version':
                '1.0.0', 'compatible': True, 'timestamp': time.time()}
            self.send_message(device, ack_msg, MessagePriority.CRITICAL)
            self.device_connected.emit(device_id, device.get_status_summary())
            sender_thread = threading.Thread(target=self.
                message_sender_loop, args=(device,), daemon=True)
            sender_thread.start()
            self.message_receiver_loop(device)
        except Exception as e:
            logger.error(f'Client handler error for {client_addr}: {e}')
            self.error_occurred.emit(device_id or client_addr, 'handler',
                str(e))
        finally:
    ...
    def message_receiver_loop(self, device: EnhancedRemoteDevice):
        while self.running and device.state == ConnectionState.CONNECTED:
            try:
                message = self.receive_message(device.client_socket,
                    timeout=1.0)
                if not message:
                    continue
                ...
                if not device.is_alive():
                    logger.warning(
                        f'Device {device.device_id} heartbeat timeout')
                    break
            except Exception as e:
        ...
        while self.running and device.state == ConnectionState.CONNECTED:
            try:
                ...
                if success:
                    ...
                    device.increment_error_count()
                    self.message_failed.emit(device.device_id, message.
                        payload, 'Send failed')
                    if device.should_reconnect():
                        break
            except Exception as e:
    ...
    def send_message_immediate(self, device: EnhancedRemoteDevice, message:
        NetworkMessage) ->bool:
        try:
            json_data = json.dumps(message.payload).encode('utf-8')
            if self.enable_compression and len(json_data
    ...
    def receive_message(self, sock: socket.socket, timeout: float=1.0
    ...
    def process_message(self, device: EnhancedRemoteDevice, message: Dict[
        str, Any]):
    ...
    def handle_preview_frame(self, device: EnhancedRemoteDevice, message:
        Dict[str, Any]):
        ...
        try:
            ...
            metadata = {'width': width, 'height': height, 'frame_type':
                frame_type, 'quality': device.streaming_quality,
                'timestamp': message.get('timestamp', time.time())}
            self.preview_frame_received.emit(device.device_id, frame_type,
                image_bytes, metadata)
            error_rate = device.stats.error_count / max(1, device.stats.
                messages_received)
            device.adapt_streaming_quality(device.stats.average_latency,
                error_rate)
        except Exception as e:
            logger.error(f'Preview frame processing error: {e}')

    def handle_heartbeat(self, device: EnhancedRemoteDevice, message: Dict[
        str, Any]):
        response = NetworkMessage(type='heartbeat_response', payload={
            'type': 'heartbeat_response', 'timestamp': time.time(),
            'server_time': time.time()}, priority=MessagePriority.HIGH)
    ...
    def send_heartbeats(self):
        with QMutexLocker(self.devices_mutex):
            ...
            for device in list(self.devices.values()):
                if device.is_alive():
                    heartbeat = NetworkMessage(type='heartbeat', payload={
    ...
    def send_command_to_device(self, device_id: str, command: str, **kwargs
        ...
        with QMutexLocker(self.devices_mutex):
            ...
            if not device:
                ...
                return False
        message = NetworkMessage(type='command', payload={'type': 'command',
            'command': command, 'timestamp': time.time(), **kwargs},
            priority=MessagePriority.CRITICAL, requires_ack=True)
    ...
    def get_network_statistics(self) ->Dict[str, Any]:
        with QMutexLocker(self.devices_mutex):
            stats = {'active_devices': len(self.devices),
                'total_connections': self.network_stats['total_connections'
                ], 'overall_stats': {'total_messages': sum(device.stats.
                messages_sent + device.stats.messages_received for device in
                self.devices.values()), 'total_bytes': sum(device.stats.
                bytes_sent + device.stats.bytes_received for device in self
                .devices.values()), 'average_latency': self.
                _calculate_overall_latency(), 'network_quality': self.
                _assess_overall_network_quality()}, 'devices': {}}
    ...
    def get_device_latency_statistics(self, device_id: str) ->Dict[str, Any]:
        with QMutexLocker(self.devices_mutex):
            ...
            with QMutexLocker(device.mutex):
                return {'device_id': device_id, 'average_latency': device.
                    stats.average_latency, 'min_latency': device.stats.
                    min_latency if device.stats.min_latency != float('inf')
                     else 0, 'max_latency': device.stats.max_latency,
                    'jitter': device.stats.jitter, 'packet_loss_rate':
                    device.stats.packet_loss_rate, 'ping_count': device.
                    stats.ping_count, 'pong_count': device.stats.pong_count,
                    'sample_count': len(device.stats.latency_samples),
                    'recent_samples': list(device.stats.latency_samples)[-
                    10:] if device.stats.latency_samples else []}

    def handle_status_update(self, device: EnhancedRemoteDevice, message:
        Dict[str, Any]):
        storage = message.get('storage', '')
        if isinstance(storage, str) and storage.startswith('ping:'):
            self.handle_ping_message(device, storage, message.get(
                'timestamp', time.time()))
        else:
            status_data = {k: v for k, v in message.items() if k != 'type'}
            self.message_received.emit(device.device_id, message)

    def handle_ping_message(self, device: EnhancedRemoteDevice, ping_data:
        str, original_timestamp: float):
        try:
            ...
            if len(parts) >= 4:
                ...
                current_time = time.time()
                pong_data = (
                    f'pong:{ping_id}:{ping_timestamp}:{current_time}:{sequence}'
                    )
                response = NetworkMessage(type='status', payload={'type':
                    'status', 'storage': pong_data, 'timestamp':
                    current_time, 'battery': None, 'temperature': None,
                    'recording': False, 'connected': True}, priority=
    ...
    def handle_acknowledgment(self, device: EnhancedRemoteDevice, message:
        Dict[str, Any]):
    ...
    def handle_sensor_data(self, device: EnhancedRemoteDevice, message:
        Dict[str, Any]):
        self.message_received.emit(device.device_id, message)

    def send_message(self, device: EnhancedRemoteDevice, message_data: Dict
        [str, Any], priority: MessagePriority=MessagePriority.NORMAL) ->bool:
        message = NetworkMessage(type=message_data.get('type', 'unknown'),
            payload=message_data, priority=priority)
        device.queue_message(message)
        return True


def create_command_message(command: str, **kwargs) ->Dict[str, Any]:
    return {'type': 'command', 'command': command, 'timestamp': time.time(),
        **kwargs}
