...
from bucika_gsr.network.android_device_manager import AndroidDeviceManager, ShimmerDataSample
from bucika_gsr.network.pc_server import PCServer
...
class ShimmerManager:
    ...
    def __init__(self, session_manager=None, logger=None,
        ...
        self.android_device_callbacks: List[Callable[[str, Dict[str, Any]],
            None]] = []
        self.connection_state_callbacks: List[Callable[[str, DeviceState,
            ConnectionType], None]] = []
        self.default_sampling_rate = 128
        self.data_buffer_size = 1000
        self.connection_timeout = 30.0
        self.android_server_port = 9000
        self.sensor_ranges = {'gsr_conductance': (0.0, 100.0), 'ppg_a13': (
            0.0, 4095.0), 'accel_x': (-16.0, 16.0), 'accel_y': (-16.0, 16.0
            ), 'accel_z': (-16.0, 16.0), 'gyro_x': (-2000.0, 2000.0),
            'gyro_y': (-2000.0, 2000.0), 'gyro_z': (-2000.0, 2000.0),
    ...
    def initialize(self) ->bool:
        try:
            ...
            if self.enable_android_integration:
                ...
                self.android_device_manager = AndroidDeviceManager(server_port
                    =self.android_server_port, logger=self.logger)
                self.android_device_manager.add_data_callback(self.
                    _on_android_shimmer_data)
                self.android_device_manager.add_status_callback(self.
                    _on_android_device_status)
                if not self.android_device_manager.initialize():
                    self.logger.error(
                        'Failed to initialize Android device manager')
                    if not PYSHIMMER_AVAILABLE:
                        return False
                    else:
                        self.logger.warning(
                            'Continuing with direct connections only')
            ...
            self.logger.error(
                f'Failed to initialize Enhanced ShimmerManager: {e}')
            return False
    ...
    def scan_and_pair_devices(self) ->Dict[str, List[str]]:
        ...
        try:
            ...
            if self.enable_android_integration and self.android_device_manager:
                android_devices = (self.android_device_manager.
            ...
            if not PYSHIMMER_AVAILABLE or len(discovered_devices['direct']
                ) == 0:
            ...
            total_devices = sum(len(devices) for devices in
    ...
    def connect_devices(self, device_info: Union[List[str], Dict[str, List[
        str]]]) ->bool:
        try:
                ...
                if 'direct' in device_info:
                    for mac_address in device_info['direct']:
                        ...
                        if self._connect_single_device(mac_address,
                            ...
                            success_count += 1
                if ('android' in device_info and self.
    ...
    def _connect_single_device(self, mac_address: str, connection_type:
        ConnectionType) ->bool:
        try:
            device_id = f"shimmer_{mac_address.replace(':', '_')}"
            if (connection_type == ConnectionType.SIMULATION or not
                PYSHIMMER_AVAILABLE):
                self.device_status[device_id] = ShimmerStatus(is_available=
                    True, is_connected=True, device_state=DeviceState.
                    CONNECTED, connection_type=connection_type, device_name
                    =f'Shimmer3_{device_id[-4:]}', mac_address=mac_address,
                    firmware_version='1.0.0', sampling_rate=self.
                    default_sampling_rate)
                self.device_configurations[device_id] = DeviceConfiguration(
                    device_id=device_id, mac_address=mac_address,
                    enabled_channels={'GSR', 'PPG_A13', 'Accel_X',
                    'Accel_Y', 'Accel_Z'}, connection_type=connection_type)
                self.data_queues[device_id] = queue.Queue(maxsize=self.
                    data_buffer_size)
                ...
                try:
                    ...
                    if not connect_success:
                        ...
                        return False
                    shimmer_device.set_data_callback(lambda data: self.
                        _on_shimmer_data_received(device_id, data))
                    default_config = self.device_configurations.get(device_id)
                    if default_config:
                        self._configure_shimmer_device(shimmer_device,
                            default_config)
                    self.shimmer_devices[device_id] = shimmer_device
                    self.device_status[device_id] = DeviceStatus(device_id=
                        device_id, mac_address=mac_address,
                        connection_status=ConnectionStatus.CONNECTED,
                        connection_type=connection_type, last_seen=datetime
                        .now())
                    if device_id not in self.device_configurations:
                        self.device_configurations[device_id
                            ] = DeviceConfiguration(device_id=device_id,
                            mac_address=mac_address, enabled_channels={
                            'GSR', 'PPG_A13', 'Accel_X', 'Accel_Y',
                            'Accel_Z'}, connection_type=connection_type)
                    self.data_queues[device_id] = queue.Queue(maxsize=self.
                        data_buffer_size)
                    self.logger.info(
    ...
    def _connect_android_device(self, android_device_id: str) ->bool:
        try:
            if (not self.enable_android_integration or not self.
            ...
            if android_device_id not in android_devices:
                self.logger.error(
                    f'Android device not connected: {android_device_id}')
                return False
            ...
            self.device_status[shimmer_device_id] = ShimmerStatus(is_available
                =True, is_connected=True, device_state=DeviceState.
                CONNECTED, connection_type=ConnectionType.ANDROID_MEDIATED,
                device_name=f'AndroidShimmer_{android_device_id}',
                mac_address='android_mediated', android_device_id=
                android_device_id, sampling_rate=self.default_sampling_rate)
            self.device_configurations[shimmer_device_id
                ] = DeviceConfiguration(device_id=shimmer_device_id,
                mac_address='android_mediated', enabled_channels={'GSR',
                'PPG_A13', 'Accel_X', 'Accel_Y', 'Accel_Z'},
                connection_type=ConnectionType.ANDROID_MEDIATED,
                android_device_id=android_device_id)
            self.android_shimmer_mapping[android_device_id] = shimmer_device_id
            self.data_queues[shimmer_device_id] = queue.Queue(maxsize=self.
                data_buffer_size)
            self.logger.info(
    ...
    def _scan_direct_bluetooth_devices(self) ->List[str]:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    'ShimmerBluetooth.scan_devices not available')
        ...
        try:
            try:
                ...
                for addr, name in nearby_devices:
                    if name and ('shimmer' in name.lower() or 'rn42' in
                        name.lower()):
            ...
            if len(discovered_devices) == 0:
                try:
                    ...
                    nearby_devices = pybluez.discover_devices(duration=8,
                        lookup_names=True)
                    for addr, name in nearby_devices:
                        if name and ('shimmer' in name.lower() or 'rn42' in
                            name.lower()):
            ...
            if len(discovered_devices) == 0:
                ...
                self.logger.info(
                    'Or ensure pyshimmer library is properly installed.')
        except Exception as e:
    ...
    def set_enabled_channels(self, device_id: str, channels: Set[str]) ->bool:
        try:
            ...
            if device_id not in self.device_configurations:
                self.device_configurations[device_id] = DeviceConfiguration(
                    device_id=device_id, mac_address=self.device_status[
                    device_id].mac_address or '', enabled_channels=channels)
            else:
                self.device_configurations[device_id
            ...
            self.logger.error(
                f'Error configuring channels for {device_id}: {e}')
            return False
        ...
        try:
            ...
            for device_id in self.device_status:
                if not self._initialize_csv_file(device_id, session_dir):
                    self.logger.error(
                        f'Failed to initialize CSV file for {device_id}')
                    return False
    ...
    def add_data_callback(self, callback: Callable[[ShimmerSample], None]
        ) ->None:
        self.data_callbacks.append(callback)

    def add_status_callback(self, callback: Callable[[str, ShimmerStatus],
        None]) ->None:
        self.status_callbacks.append(callback)

    def add_android_device_callback(self, callback: Callable[[str, Dict[str,
        Any]], None]) ->None:
        self.android_device_callbacks.append(callback)

    def add_connection_state_callback(self, callback: Callable[[str,
        DeviceState, ConnectionType], None]) ->None:
    ...
    def _on_android_shimmer_data(self, sample: ShimmerDataSample) ->None:
        try:
            if sample.android_device_id not in self.android_shimmer_mapping:
                ...
                return
            shimmer_device_id = self.android_shimmer_mapping[sample.
                android_device_id]
            shimmer_sample = ShimmerSample(timestamp=sample.timestamp,
                system_time=datetime.fromtimestamp(sample.timestamp).
                isoformat(), device_id=shimmer_device_id, connection_type=
                ConnectionType.ANDROID_MEDIATED, android_device_id=sample.
                android_device_id, session_id=sample.session_id)
            for sensor_name, value in sample.sensor_values.items():
                if sensor_name == 'gsr_conductance':
                    ...
                    shimmer_sample.battery_percentage = int(value
            ...
            if self._validate_sample_data(shimmer_sample):
                ...
                self.logger.warning(
                    f'Invalid data sample from {shimmer_device_id}')
        except Exception as e:
            self.logger.error(f'Error processing Android Shimmer data: {e}')

    def _on_android_device_status(self, android_device_id: str, android_device
        ) ->None:
        try:
            if android_device_id in self.android_shimmer_mapping:
                shimmer_device_id = self.android_shimmer_mapping[
                    android_device_id]
                if shimmer_device_id in self.device_status:
                    ...
                    status.last_data_timestamp = (android_device.
                        last_data_timestamp)
    ...
    def _validate_sample_data(self, sample: ShimmerSample) ->bool:
        try:
            if sample.timestamp <= 0:
                return False
            sensor_checks = [('gsr_conductance', sample.gsr_conductance), (
                'ppg_a13', sample.ppg_a13), ('accel_x', sample.accel_x), (
                'accel_y', sample.accel_y), ('accel_z', sample.accel_z), (
                'gyro_x', sample.gyro_x), ('gyro_y', sample.gyro_y), (
                'gyro_z', sample.gyro_z), ('battery_percentage', sample.
                battery_percentage)]
    ...
    def _find_serial_port_for_device(self, mac_address: str) ->Optional[str]:
        try:
            ...
            for port in ports:
                if hasattr(port, 'device') and hasattr(port, 'description'):
                    ...
                    if ('bluetooth' in description or 'bt' in description or
                        ...
                        if hasattr(port, 'hwid') and mac_address.replace(':',
                            '').lower() in port.hwid.lower():
                            self.logger.info(
            ...
            if system == 'windows':
                for i in range(1, 20):
                    ...
                    try:
                        ...
                        test_port.close()
                        self.logger.info(
                            f'Found available COM port: {port_name}')
                        return port_name
                    except:
                        continue
            elif system == 'linux':
                ...
                if bt_ports:
                    self.logger.info(
                        f'Found potential Bluetooth ports: {bt_ports}')
                    return bt_ports[0]
            elif system == 'darwin':
                import glob
            ...
            self.logger.error(
                f'Error finding serial port for {mac_address}: {e}')
            return None

    def _configure_shimmer_device(self, shimmer_device, device_config:
        DeviceConfiguration) ->bool:
        try:
            self.logger.info(
                f'Configuring Shimmer device {device_config.device_id}')
            ...
            if hasattr(shimmer_device, 'set_enabled_sensors'):
                sensor_ids = self._channels_to_sensor_ids(device_config.
                    enabled_channels)
                shimmer_device.set_enabled_sensors(sensor_ids)
                self.logger.info(
                    f'Enabled sensors: {device_config.enabled_channels}')
            if device_config.sensor_range and hasattr(shimmer_device,
                'set_sensor_range'):
    ...
    def _channels_to_sensor_ids(self, channels: Set[str]) ->List[int]:
        channel_mapping = {'GSR': 4, 'PPG_A13': 16, 'Accel_X': 128,
            'Accel_Y': 128, 'Accel_Z': 128, 'Gyro_X': 64, 'Gyro_Y': 64,
            'Gyro_Z': 64, 'Mag_X': 32, 'Mag_Y': 32, 'Mag_Z': 32}
        sensor_ids = []
            ...
            if sample:
                if device_id in self.data_queues:
                    try:
                        ...
                        self.logger.warning(
                            f'Data queue full for device {device_id}')
                if self.is_recording and device_id in self.recording_files:
                    self._write_sample_to_file(device_id, sample)
        except Exception as e:
            self.logger.error(
                f'Error processing Shimmer data for {device_id}: {e}')

    def _convert_pyshimmer_data(self, device_id: str, data) ->Optional[Dict
        [str, Any]]:
        try:
            sample = {'device_id': device_id, 'timestamp': datetime.now().
                isoformat(), 'sample_number': getattr(data, 'packet_id', 0),
                'channels': {}}
    ...
    def cleanup(self) ->None:
        try:
            ...
            if self.is_streaming:
                self.stop_streaming()
            self.stop_event.set()
            if (self.data_processing_thread and self.data_processing_thread
                .is_alive()):
            ...
            for device_id, device in self.connected_devices.items():
                try:
                    if isinstance(device, str):
                        ...
                        device.shutdown()
                        self.logger.info(
                            f'Disconnected direct device: {device_id}')
                except Exception as e:
            ...
            for device_id, shimmer_device in self.shimmer_devices.items():
                try:
                    ...
                    self.logger.info(
                        f'Disconnected pyshimmer device: {device_id}')
                except Exception as e:
                    self.logger.error(
    ...
    def get_android_devices(self) ->Dict[str, Any]:
        ...
        for device_id, device in android_devices.items():
            if 'shimmer' in device.capabilities:
                shimmer_capable[device_id] = {'capabilities': device.
                    capabilities, 'status': device.status, 'is_recording':
                    device.is_recording, 'shimmer_devices': device.
                    shimmer_devices}
    ...
    def _create_session_directory(self, session_id: str) ->Optional[Path]:
        try:
            if self.session_manager:
                session_dir = Path(self.session_manager.
    ...
    def _initialize_csv_file(self, device_id: str, session_dir: Path) ->bool:
        try:
            ...
            fieldnames = ['timestamp', 'system_time', 'device_id',
                'connection_type', 'android_device_id', 'session_id',
                'gsr_conductance', 'ppg_a13', 'accel_x', 'accel_y',
                'accel_z', 'gyro_x', 'gyro_y', 'gyro_z', 'mag_x', 'mag_y',
                'mag_z', 'ecg', 'emg', 'battery_percentage', 'signal_strength']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            writer.writeheader()
            self.csv_files[device_id] = csv_file
            self.csv_writers[device_id] = writer
            self.logger.info(
                f'Initialized CSV file for {device_id}: {csv_file_path}')
            return True
        except Exception as e:
            self.logger.error(
                f'Error initializing CSV file for {device_id}: {e}')
            return False

    def _start_background_threads(self) ->None:
        self.stop_event.clear()
        self.data_processing_thread = threading.Thread(target=self.
            _data_processing_loop, name='ShimmerDataProcessing')
        self.data_processing_thread.daemon = True
        self.data_processing_thread.start()
        self.file_writing_thread = threading.Thread(target=self.
            _file_writing_loop, name='ShimmerFileWriting')
            ...
            try:
                for device_id, data_queue in self.data_queues.items():
                    try:
                        ...
                        self.logger.error(
                            f'Error processing data for {device_id}: {e}')
                time.sleep(0.01)
            except Exception as e:
    ...
    def _start_simulated_streaming(self, device_id: str) ->None:
        ...
        def simulate_data():
            while not self.stop_event.is_set() and self.is_streaming:
                try:
                    if device_id in self.device_status and self.device_status[
                        device_id].is_streaming:
                        ...
                        if device_id in self.data_queues:
                            try:
                                ...
                                try:
                                    ...
                                    self.data_queues[device_id].put_nowait(
                                        sample)
                                except queue.Empty:
                                    pass
    ...
    def _generate_simulated_sample(self, device_id: str) ->ShimmerSample:
        ...
        return ShimmerSample(timestamp=timestamp, system_time=system_time,
            device_id=device_id, gsr_conductance=gsr_conductance, ppg_a13=
            ppg_a13, accel_x=accel_x, accel_y=accel_y, accel_z=accel_z,
                ...
                if manager.start_streaming():
                    ...
                    for i in range(30):
                        ...
                        if i % 10 == 0:
                            ...
                            if android_devices:
                                sync_count = manager.send_sync_signal('flash',
                                    duration_ms=200)
                                print(
                                    f'  Sent sync flash to {sync_count} devices'
                                    )
                    android_devices = manager.get_android_devices()
                    if android_devices:
                        ...
                        if manager.start_android_session(session_id,
                            record_shimmer=True):
                    ...
                    if manager.start_recording(session_id):
                        print(
                            f'Local recording started for session: {session_id}'
                            )
                        time.sleep(10)
