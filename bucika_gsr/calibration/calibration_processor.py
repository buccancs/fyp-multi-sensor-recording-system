...
class CalibrationProcessor:
    ...
    def __init__(self):
        ...
        self.corner_criteria = (cv2.TERM_CRITERIA_EPS + cv2.
            TERM_CRITERIA_MAX_ITER, 30, 0.001)
        self.calibration_flags = (cv2.CALIB_RATIONAL_MODEL | cv2.
            CALIB_THIN_PRISM_MODEL | cv2.CALIB_TILTED_MODEL)
        print('[DEBUG_LOG] CalibrationProcessor initialized')

    def detect_chessboard_corners(self, image: np.ndarray, pattern_size:
        Tuple[int, int]) ->Tuple[bool, Optional[np.ndarray]]:
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
            gray = image.copy()
        ret, corners = cv2.findChessboardCorners(gray, pattern_size, cv2.
    ...
    def detect_circles_grid(self, image: np.ndarray, pattern_size: Tuple[
        int, int]) ->Tuple[bool, Optional[np.ndarray]]:
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
            gray = image.copy()
        ret, centers = cv2.findCirclesGrid(gray, pattern_size, cv2.
            CALIB_CB_SYMMETRIC_GRID)
        return ret, centers if ret else None

    def find_calibration_corners(self, image: np.ndarray) ->dict:
        try:
            success, corners = self.detect_chessboard_corners(image, self.
                pattern_size)
            if success:
                return {'success': True, 'corners': corners}
            else:
                return {'success': False, 'error':
                    'No chessboard pattern detected'}
        except Exception as e:
            return {'success': False, 'error':
                f'Corner detection failed: {str(e)}'}

    def detect_aruco_markers(self, image: np.ndarray, dictionary_id: int=
        cv2.aruco.DICT_6X6_250) ->Tuple[bool, List[np.ndarray]]:
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
            gray = image.copy()
    ...
    def calibrate_camera_intrinsics(self, object_points: List[np.ndarray],
        image_points: List[np.ndarray], image_size: Tuple[int, int]) ->Tuple[
        float, np.ndarray, np.ndarray, List[np.ndarray], List[np.ndarray]]:
        ret, camera_matrix, dist_coeffs, rvecs, tvecs = cv2.calibrateCamera(
            object_points, image_points, image_size, None, None, flags=self
            .calibration_flags)
        return ret, camera_matrix, dist_coeffs, rvecs, tvecs

    def calibrate_stereo_cameras(self, object_points: List[np.ndarray],
        image_points1: List[np.ndarray], image_points2: List[np.ndarray],
        camera_matrix1: np.ndarray, dist_coeffs1: np.ndarray,
        camera_matrix2: np.ndarray, dist_coeffs2: np.ndarray, image_size:
        Tuple[int, int]) ->Tuple[float, np.ndarray, np.ndarray, np.ndarray,
        np.ndarray]:
        ret, _, _, _, _, R, T, E, F = cv2.stereoCalibrate(object_points,
            image_points1, image_points2, camera_matrix1, dist_coeffs1,
            camera_matrix2, dist_coeffs2, image_size, flags=cv2.
            CALIB_FIX_INTRINSIC)
        return ret, R, T, E, F
    ...
    def compute_homography(self, points1: np.ndarray, points2: np.ndarray
        ...
        try:
            ...
            if H is not None:
                print(
                    f'[DEBUG_LOG] Homography computed with {np.sum(mask)} inliers'
                    )
                return H
            else:
    ...
    def compute_reprojection_error(self, object_points: List[np.ndarray],
        image_points: List[np.ndarray], camera_matrix: np.ndarray,
        dist_coeffs: np.ndarray, rvecs: List[np.ndarray], tvecs: List[np.
        ndarray]) ->Tuple[float, List[float]]:
        total_error = 0
        per_image_errors = []
        for i in range(len(object_points)):
            projected_points, _ = cv2.projectPoints(object_points[i], rvecs
                [i], tvecs[i], camera_matrix, dist_coeffs)
    ...
    def undistort_image(self, image: np.ndarray, camera_matrix: np.ndarray,
        ...
        undistorted = cv2.undistort(image, camera_matrix, dist_coeffs, None,
            new_camera_matrix)
        x, y, w, h = roi
        if w > 0 and h > 0:
    ...
    def create_rectification_maps(self, camera_matrix1: np.ndarray,
        dist_coeffs1: np.ndarray, camera_matrix2: np.ndarray, dist_coeffs2:
        np.ndarray, image_size: Tuple[int, int], R: np.ndarray, T: np.ndarray
        ) ->Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        R1, R2, P1, P2, Q, validPixROI1, validPixROI2 = cv2.stereoRectify(
            camera_matrix1, dist_coeffs1, camera_matrix2, dist_coeffs2,
            image_size, R, T)
        map1x, map1y = cv2.initUndistortRectifyMap(camera_matrix1,
            dist_coeffs1, R1, P1, image_size, cv2.CV_16SC2)
        map2x, map2y = cv2.initUndistortRectifyMap(camera_matrix2,
            dist_coeffs2, R2, P2, image_size, cv2.CV_16SC2)
        return map1x, map1y, map2x, map2y

    def apply_rectification(self, image1: np.ndarray, image2: np.ndarray,
        map1x: np.ndarray, map1y: np.ndarray, map2x: np.ndarray, map2y: np.
    ...
    def validate_calibration_quality(self, reprojection_error: float,
        num_images: int, pattern_coverage: float=None) ->dict:
        quality = {'reprojection_error': reprojection_error, 'num_images':
            num_images, 'quality_score': 'UNKNOWN', 'recommendations': []}
    ...
    def draw_chessboard_corners(self, image: np.ndarray, pattern_size:
        Tuple[int, int], corners: np.ndarray, pattern_found: bool
        ) ->np.ndarray:
...
if __name__ == '__main__':
    ...
    success, corners = processor.detect_chessboard_corners(test_image,
        pattern_size)
