...
class CalibrationManager:
    ...
    def capture_calibration_images(self, device_client=None, num_images: int=20
        ) ->bool:
        ...
        try:
            for i in range(num_images):
                print(
                    f'[INFO] Capturing calibration image pair {i + 1}/{num_images}'
                    )
                command_data = {'image_id': i, 'pattern_type': 'chessboard',
                    'pattern_size': self.chessboard_size}
                print(
                    f'[INFO] Would send CAPTURE_CALIBRATION command: {command_data}'
                    )
                progress = (i + 1) / num_images * 100
                print(f'[INFO] Calibration capture progress: {progress:.1f}%')
            print(
                f'[INFO] Calibration capture framework ready for device integration'
                )
            return True
    ...
    def detect_calibration_pattern(self, image: np.ndarray, pattern_type:
        str='chessboard') ->Tuple[bool, Optional[np.ndarray]]:
    ...
    def calibrate_single_camera(self, images: List[np.ndarray],
        image_points: List[np.ndarray], object_points: List[np.ndarray]
        ...
        try:
            ...
            ret, camera_matrix, dist_coeffs, rvecs, tvecs = (cv2.
                calibrateCamera(object_points, image_points, image_size,
                None, None, flags=self.calibration_flags))
            if ret:
                total_error = 0
                for i in range(len(object_points)):
                    projected_points, _ = cv2.projectPoints(object_points[i
                        ], rvecs[i], tvecs[i], camera_matrix, dist_coeffs)
                    error = cv2.norm(image_points[i], projected_points, cv2
                        .NORM_L2) / len(projected_points)
                    total_error += error
    ...
    def calibrate_stereo_cameras(self, rgb_images: List[np.ndarray],
        thermal_images: List[np.ndarray], rgb_points: List[np.ndarray],
        thermal_points: List[np.ndarray], object_points: List[np.ndarray]
        ) ->Tuple[Optional[np.ndarray], Optional[np.ndarray], float]:
        print('[DEBUG_LOG] Performing stereo calibration')
        if (self.rgb_camera_matrix is None or self.thermal_camera_matrix is
            None):
            print(
                '[ERROR] Individual camera calibrations must be completed first'
                )
            return None, None, float('inf')
        ...
        try:
            ...
            ret, _, _, _, _, R, T, E, F = cv2.stereoCalibrate(object_points,
                rgb_points, thermal_points, self.rgb_camera_matrix, self.
                rgb_distortion_coeffs, self.thermal_camera_matrix, self.
                thermal_distortion_coeffs, image_size, flags=cv2.
                CALIB_FIX_INTRINSIC)
            if ret:
    ...
    def assess_calibration_quality(self, images: List[np.ndarray],
        image_points: List[np.ndarray], object_points: List[np.ndarray],
        camera_matrix: np.ndarray, dist_coeffs: np.ndarray, rvecs: List[np.
        ndarray], tvecs: List[np.ndarray]) ->Dict:
        print('[DEBUG_LOG] Assessing calibration quality')
        quality_metrics = {'mean_reprojection_error': 0.0,
            'max_reprojection_error': 0.0, 'std_reprojection_error': 0.0,
            'pattern_coverage': 0.0, 'quality_score': 'UNKNOWN',
            'recommendations': []}
        try:
            errors = []
            for i in range(len(object_points)):
            ...
            if quality_metrics['std_reprojection_error'] > quality_metrics[
    ...
    def save_calibration_data(self, filename: str) ->bool:
        ...
        calibration_data = {'rgb_camera_matrix': self.rgb_camera_matrix.
            tolist() if self.rgb_camera_matrix is not None else None,
            'rgb_distortion_coeffs': self.rgb_distortion_coeffs.tolist() if
            self.rgb_distortion_coeffs is not None else None,
            'thermal_camera_matrix': self.thermal_camera_matrix.tolist() if
            self.thermal_camera_matrix is not None else None,
            ...
            'chessboard_size': self.chessboard_size, 'square_size': self.
            square_size, 'calibration_flags': int(self.calibration_flags)}}
        try:
    ...
    def load_calibration_data(self, filename: str) ->bool:
        ...
        try:
            ...
            if calibration_data.get('rgb_camera_matrix'):
                self.rgb_camera_matrix = np.array(calibration_data[
                    'rgb_camera_matrix'])
            if calibration_data.get('rgb_distortion_coeffs'):
                self.rgb_distortion_coeffs = np.array(calibration_data[
                    'rgb_distortion_coeffs'])
            if calibration_data.get('thermal_camera_matrix'):
                self.thermal_camera_matrix = np.array(calibration_data[
                    'thermal_camera_matrix'])
            if calibration_data.get('thermal_distortion_coeffs'):
                self.thermal_distortion_coeffs = np.array(calibration_data[
                    'thermal_distortion_coeffs'])
            if calibration_data.get('rotation_matrix'):
                self.rotation_matrix = np.array(calibration_data[
                    'rotation_matrix'])
            if calibration_data.get('translation_vector'):
                self.translation_vector = np.array(calibration_data[
                    'translation_vector'])
            self.calibration_quality = calibration_data.get(
                'calibration_quality')
            if 'calibration_parameters' in calibration_data:
                params = calibration_data['calibration_parameters']
                self.chessboard_size = tuple(params.get('chessboard_size',
                    self.chessboard_size))
                self.square_size = params.get('square_size', self.square_size)
                self.calibration_flags = params.get('calibration_flags',
    ...
    def load_calibration_images_from_directory(self, directory_path: str,
        rgb_pattern: str='*rgb*.jpg', thermal_pattern: str='*thermal*.jpg'
        ) ->Tuple[List[np.ndarray], List[np.ndarray]]:
        import glob
        rgb_images = []
        thermal_images = []
        try:
            rgb_files = sorted(glob.glob(os.path.join(directory_path,
                rgb_pattern)))
            thermal_files = sorted(glob.glob(os.path.join(directory_path,
                thermal_pattern)))
            print(
            ...
            for thermal_file in thermal_files:
                ...
                if img is not None:
                    ...
                    print(
                        f'[WARNING] Could not load thermal image: {thermal_file}'
                        )
    ...
    def perform_complete_calibration(self, rgb_images: List[np.ndarray],
        thermal_images: Optional[List[np.ndarray]]=None, pattern_type: str=
        'chessboard') ->Dict:
        print('[DEBUG_LOG] Starting complete calibration workflow')
        results = {'success': False, 'rgb_calibration': None,
            'thermal_calibration': None, 'stereo_calibration': None,
            'quality_metrics': {}}
        try:
            if not validate_calibration_images(rgb_images):
                return results
            object_points_3d = create_calibration_pattern_points(self.
                chessboard_size, self.square_size)
            rgb_image_points = []
            rgb_object_points = []
            valid_rgb_images = []
            for i, img in enumerate(rgb_images):
                success, corners = self.detect_calibration_pattern(img,
                    pattern_type)
                if success:
            ...
            if len(rgb_image_points) < 10:
                print(
                    f'[ERROR] Insufficient valid RGB images: {len(rgb_image_points)}'
                    )
                return results
            ...
            if rgb_matrix is not None:
                ...
                results['rgb_calibration'] = {'camera_matrix': rgb_matrix,
                    'distortion_coeffs': rgb_dist, 'rms_error': rgb_error}
                print(
                ...
                if thermal_images:
                    if not validate_calibration_images(thermal_images):
                        ...
                        if len(thermal_image_points) >= 10:
                            thermal_matrix, thermal_dist, thermal_error = (self
                                .calibrate_single_camera(
                                valid_thermal_images, thermal_image_points,
                                thermal_object_points))
                            if thermal_matrix is not None:
                                ...
                                min_pairs = min(len(rgb_image_points), len(
                                    thermal_image_points))
                                if min_pairs >= 10:
                                    R, T, stereo_error = (self.
                ...
                print(
                    '[DEBUG_LOG] Complete calibration workflow finished successfully'
                    )
            return results
        except Exception as e:
...
def create_calibration_pattern_points(pattern_size: Tuple[int, int],
    square_size: float) ->np.ndarray:
    print(
        f'[DEBUG_LOG] Creating calibration pattern points {pattern_size} with square size {square_size}mm'
        )
    pattern_points = np.zeros((pattern_size[0] * pattern_size[1], 3), np.
        float32)
    pattern_points[:, :2] = np.mgrid[0:pattern_size[0], 0:pattern_size[1]
        ].T.reshape(-1, 2)
    pattern_points *= square_size
    return pattern_points


def validate_calibration_images(images: List[np.ndarray], min_images: int=10
    ) ->bool:
    if len(images) < min_images:
        print(
            f'[ERROR] Insufficient calibration images: {len(images)} < {min_images}'
            )
        return False
    if not images:
        return False
    ...
    for i, img in enumerate(images[1:], 1):
        if img.shape != first_shape:
            ...
            return False
    print(
        f'[DEBUG_LOG] Calibration image validation passed: {len(images)} images'
        )
    return True


def draw_calibration_pattern(image: np.ndarray, corners: np.ndarray,
    pattern_size: Tuple[int, int], pattern_found: bool=True) ->np.ndarray:
    result_image = image.copy()
    if pattern_found and corners is not None:
        cv2.drawChessboardCorners(result_image, pattern_size, corners,
            pattern_found)
    return result_image
