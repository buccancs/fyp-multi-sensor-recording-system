...
class CalibrationManager:
    ...
    def start_calibration_session(self, device_ids: List[str], session_name:
        ...
        self.current_session = {'session_name': session_name,
            'session_folder': str(session_folder), 'device_ids': device_ids,
            'start_time': timestamp.isoformat(), 'pattern_type': self.
            pattern_type, 'pattern_size': self.chessboard_size,
            'square_size': self.square_size, 'status': 'active'}
    ...
    def capture_calibration_frame(self, device_server) ->Dict[str, Any]:
        ...
        capture_results = {'success': True, 'message':
            'Calibration frames captured successfully', 'device_results': {
            }, 'total_frames': {}}
        try:
            capture_command = {'cmd': 'capture_calibration', 'session_id':
                self.current_session['session_name'], 'pattern_type': self.
                pattern_type, 'pattern_size': self.chessboard_size}
            ...
            for device_id in self.current_session['device_ids']:
                ...
                if success:
                    ...
                    capture_results['device_results'][device_id] = {'success':
                        True, 'message': 'Frame captured successfully'}
                else:
                    capture_results['device_results'][device_id] = {'success':
                        False, 'message': 'Pattern not detected in frame'}
                capture_results['total_frames'][device_id
                    ] = self.capture_count[device_id]
    ...
    def _simulate_image_capture(self, device_id: str) ->bool:
        ...
        print(
            f'[DEBUG_LOG] Simulated capture for {device_id}: frame {frame_num}'
            )
        return True
    ...
    def can_compute_calibration(self, device_id: str=None) ->Dict[str, bool]:
        if not self.current_session:
            return {}
        devices_to_check = [device_id] if device_id else self.current_session[
            'device_ids']
        readiness = {}
    ...
    def compute_calibration(self, device_id: str=None) ->Dict[str, Any]:
        if not self.current_session:
            raise RuntimeError('No active calibration session')
        devices_to_calibrate = [device_id
            ] if device_id else self.current_session['device_ids']
        computation_results = {'success': True, 'message':
            'Calibration computed successfully', 'device_results': {}}
        for dev_id in devices_to_calibrate:
            try:
                ...
                if not self.can_compute_calibration(dev_id)[dev_id]:
                    computation_results['device_results'][dev_id] = {'success':
                        False, 'message':
                        f'Insufficient images: {self.capture_count[dev_id]}/{self.min_images}'
                        }
                    continue
                rgb_images = self.captured_images[dev_id]['rgb']
                thermal_images = self.captured_images[dev_id]['thermal']
                result = self._compute_device_calibration(dev_id,
                ...
                if result.is_valid():
                    ...
                    computation_results['device_results'][dev_id] = {'success':
                        True, 'message':
                        'Calibration computed successfully', 'rgb_error':
                        result.rgb_rms_error, 'thermal_error': result.
                        thermal_rms_error, 'stereo_error': result.
                        stereo_rms_error, 'quality_score': result.
                        quality_assessment.get('quality_score', 'UNKNOWN')}
                else:
                    computation_results['device_results'][dev_id] = {'success':
                        False, 'message': 'Calibration computation failed'}
            except Exception as e:
                computation_results['device_results'][dev_id] = {'success':
                    False, 'message': f'Calibration error: {str(e)}'}
                print(f'[DEBUG_LOG] Calibration error for {dev_id}: {e}')
        all_successful = all(result.get('success', False) for result in
            computation_results['device_results'].values())
    ...
    def _compute_device_calibration(self, device_id: str, rgb_images: List[
        np.ndarray], thermal_images: List[np.ndarray]) ->CalibrationResult:
        print(f'[DEBUG_LOG] Computing calibration for device {device_id}')
        result = CalibrationResult(device_id)
        object_points = self.processor.create_object_points(self.
            chessboard_size, self.square_size)
        rgb_image_points = []
        thermal_image_points = []
        valid_object_points = []
        for i, (rgb_img, thermal_img) in enumerate(zip(rgb_images,
            thermal_images)):
            rgb_success, rgb_corners = (self.processor.
                detect_chessboard_corners(rgb_img, self.chessboard_size))
            thermal_success, thermal_corners = (self.processor.
                detect_chessboard_corners(thermal_img, self.chessboard_size))
            if rgb_success and thermal_success:
                ...
                print(
                    f'[DEBUG_LOG] Pattern detected in frame {i} for both cameras'
                    )
            else:
        ...
        rgb_ret, rgb_camera_matrix, rgb_dist_coeffs, _, _ = (cv2.
            calibrateCamera(valid_object_points, rgb_image_points,
            rgb_image_size, None, None))
        if rgb_ret:
            result.rgb_camera_matrix = rgb_camera_matrix
            result.rgb_distortion_coeffs = rgb_dist_coeffs
            result.rgb_rms_error = rgb_ret
            print(
                f'[DEBUG_LOG] RGB camera calibrated with RMS error: {rgb_ret:.3f}'
                )
        thermal_image_size = thermal_images[0].shape[1], thermal_images[0
            ].shape[0]
        thermal_ret, thermal_camera_matrix, thermal_dist_coeffs, _, _ = (cv2
            .calibrateCamera(valid_object_points, thermal_image_points,
            thermal_image_size, None, None))
        if thermal_ret:
        ...
        if rgb_ret and thermal_ret:
            stereo_ret, _, _, _, _, R, T, E, F = cv2.stereoCalibrate(
                ...
                rgb_camera_matrix, rgb_dist_coeffs, thermal_camera_matrix,
                thermal_dist_coeffs, rgb_image_size, flags=cv2.
                CALIB_FIX_INTRINSIC)
            if stereo_ret:
    ...
    def _assess_calibration_quality(self, result: CalibrationResult) ->Dict[
        str, Any]:
    ...
    def _save_calibration_result(self, device_id: str, result:
        CalibrationResult):
        session_folder = Path(self.current_session['session_folder'])
    ...
    def get_calibration_result(self, device_id: str) ->Optional[
        CalibrationResult]:
        return self.calibration_results.get(device_id)

    def load_calibration_result(self, device_id: str, calibration_file: str
        ) ->bool:
        try:
            ...
            print(
                f'[DEBUG_LOG] Failed to load calibration for {device_id}: {e}')
        return False

    def apply_thermal_overlay(self, device_id: str, rgb_image: np.ndarray,
        thermal_image: np.ndarray, alpha: float=0.3) ->Optional[np.ndarray]:
        result = self.calibration_results.get(device_id)
        if not result or result.homography_matrix is None:
            return None
        try:
            thermal_warped = cv2.warpPerspective(thermal_image, result.
                homography_matrix, (rgb_image.shape[1], rgb_image.shape[0]))
            thermal_colored = cv2.applyColorMap(thermal_warped, cv2.
                COLORMAP_JET)
            overlay = cv2.addWeighted(rgb_image, 1.0 - alpha,
                thermal_colored, alpha, 0)
            return overlay
        except Exception as e:
    ...
    def end_calibration_session(self) ->Dict[str, Any]:
        ...
        self.current_session['calibrated_devices'] = list(self.
            calibration_results.keys())
        session_folder = Path(self.current_session['session_folder'])
        session_file = session_folder / 'session_info.json'
        with open(session_file, 'w') as f:
            json.dump(self.current_session, f, indent=2)
        session_summary = {'success': True, 'session_name': self.
            current_session['session_name'], 'total_captures': dict(self.
            capture_count), 'calibrated_devices': list(self.
            calibration_results.keys()), 'session_folder': str(session_folder)}
        self.current_session = None
    ...
    def get_session_status(self) ->Dict[str, Any]:
        if not self.current_session:
            return {'active': False}
        return {'active': True, 'session_name': self.current_session[
            'session_name'], 'device_ids': self.current_session[
            'device_ids'], 'capture_counts': dict(self.capture_count),
    ...
    def detect_pattern(self, image, pattern_type='chessboard'):
        if pattern_type == 'chessboard':
            success, corners = self.processor.detect_chessboard_corners(image,
                self.chessboard_size)
            return success, corners
        elif pattern_type == 'circles':
            success, corners = self.processor.detect_circles_grid(image,
                self.chessboard_size)
            return success, corners
        else:
            return False, None
    ...
    def save_calibration(self, device_id, filename):
        ...
        if result:
            import json
            data = {'camera_matrix': result.camera_matrix.tolist(),
                'distortion_coefficients': result.distortion_coefficients.
                tolist(), 'rms_error': result.rms_error, 'image_size':
                result.image_size}
...
if __name__ == '__main__':
    ...
    session = manager.start_calibration_session(['device_1', 'device_2'],
        'test_session')
