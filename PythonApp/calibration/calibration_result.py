...
class CalibrationResult:
    ...
    def __init__(self, device_id: str):
        ...
        print(
            f'[DEBUG_LOG] CalibrationResult initialized for device: {device_id}'
            )

    def is_valid(self) ->bool:
        rgb_valid = (self.rgb_camera_matrix is not None and self.
            rgb_distortion_coeffs is not None and self.rgb_rms_error is not
            None)
        thermal_valid = (self.thermal_camera_matrix is not None and self.
            thermal_distortion_coeffs is not None and self.
            thermal_rms_error is not None)
        stereo_valid = (self.rotation_matrix is not None and self.
            translation_vector is not None and self.stereo_rms_error is not
            None)
        return rgb_valid and thermal_valid and stereo_valid

    def get_calibration_summary(self) ->Dict[str, Any]:
        summary = {'device_id': self.device_id, 'timestamp': self.
            calibration_timestamp, 'is_valid': self.is_valid(),
            'pattern_info': {'type': self.pattern_type, 'size': self.
            pattern_size, 'square_size': self.square_size, 'num_images':
            self.num_images_used}, 'rgb_camera': {'calibrated': self.
            rgb_camera_matrix is not None, 'rms_error': self.rgb_rms_error},
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        data = {'device_id': self.device_id, 'calibration_timestamp': self.
            calibration_timestamp, 'pattern_type': self.pattern_type,
            'pattern_size': self.pattern_size, 'square_size': self.
            square_size, 'num_images_used': self.num_images_used,
            'rgb_camera_matrix': numpy_to_list(self.rgb_camera_matrix),
            'rgb_distortion_coeffs': numpy_to_list(self.
            rgb_distortion_coeffs), 'rgb_rms_error': self.rgb_rms_error,
            'thermal_camera_matrix': numpy_to_list(self.
            thermal_camera_matrix), 'thermal_distortion_coeffs':
            numpy_to_list(self.thermal_distortion_coeffs),
            'thermal_rms_error': self.thermal_rms_error, 'rotation_matrix':
            numpy_to_list(self.rotation_matrix), 'translation_vector':
            numpy_to_list(self.translation_vector), 'essential_matrix':
            numpy_to_list(self.essential_matrix), 'fundamental_matrix':
            numpy_to_list(self.fundamental_matrix), 'stereo_rms_error':
            self.stereo_rms_error, 'homography_matrix': numpy_to_list(self.
            homography_matrix), 'quality_assessment': self.
            quality_assessment, 'format_version': '1.0', 'created_by':
            'Multi-Sensor Recording System v3.4'}
        return data
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'CalibrationResult':
        ...
        result.rgb_distortion_coeffs = list_to_numpy(data.get(
            'rgb_distortion_coeffs'))
        result.rgb_rms_error = data.get('rgb_rms_error')
        result.thermal_camera_matrix = list_to_numpy(data.get(
            'thermal_camera_matrix'))
        result.thermal_distortion_coeffs = list_to_numpy(data.get(
        ...
        result.translation_vector = list_to_numpy(data.get(
            'translation_vector'))
        result.essential_matrix = list_to_numpy(data.get('essential_matrix'))
        result.fundamental_matrix = list_to_numpy(data.get(
            'fundamental_matrix'))
    ...
    def get_rgb_camera_info(self) ->Dict[str, Any]:
        ...
        return {'calibrated': True, 'focal_length': {'fx': fx, 'fy': fy},
            'principal_point': {'cx': cx, 'cy': cy}, 'rms_error': self.
            rgb_rms_error, 'distortion_coeffs': self.rgb_distortion_coeffs.
            tolist() if self.rgb_distortion_coeffs is not None else None}
    ...
    def get_thermal_camera_info(self) ->Dict[str, Any]:
        ...
        return {'calibrated': True, 'focal_length': {'fx': fx, 'fy': fy},
            'principal_point': {'cx': cx, 'cy': cy}, 'rms_error': self.
            thermal_rms_error, 'distortion_coeffs': self.
            thermal_distortion_coeffs.tolist() if self.
            thermal_distortion_coeffs is not None else None}
    ...
    def get_stereo_info(self) ->Dict[str, Any]:
        ...
        try:
            ...
            euler_angles = [0, 0, 0]
        return {'calibrated': True, 'baseline_mm': baseline,
            'rotation_angles_deg': {'rx': euler_angles[0], 'ry':
            euler_angles[1], 'rz': euler_angles[2]}, 'translation_mm': {
            'tx': self.translation_vector[0], 'ty': self.translation_vector
            [1], 'tz': self.translation_vector[2]}, 'rms_error': self.
            stereo_rms_error}
    ...
    def validate_integrity(self) ->Dict[str, Any]:
        ...
        if self.rgb_camera_matrix is not None:
            if self.rgb_camera_matrix.shape != (3, 3):
                ...
                validation['issues'].append(
                    'RGB camera matrix has incorrect shape')
            fx, fy = self.rgb_camera_matrix[0, 0], self.rgb_camera_matrix[1, 1]
            if fx <= 0 or fy <= 0:
                validation['valid'] = False
                validation['issues'].append(
                    'RGB camera has invalid focal lengths')
        ...
        if self.thermal_camera_matrix is not None:
            if self.thermal_camera_matrix.shape != (3, 3):
                validation['valid'] = False
                validation['issues'].append(
                    'Thermal camera matrix has incorrect shape')
            fx, fy = self.thermal_camera_matrix[0, 0
                ], self.thermal_camera_matrix[1, 1]
            if fx <= 0 or fy <= 0:
                validation['valid'] = False
                validation['issues'].append(
                    'Thermal camera has invalid focal lengths')
        if self.rgb_rms_error is not None and self.rgb_rms_error > 5.0:
            validation['warnings'].append(
                'RGB camera RMS error is high (>5.0 pixels)')
        ...
        if self.rotation_matrix is not None:
            if self.rotation_matrix.shape != (3, 3):
                validation['valid'] = False
                validation['issues'].append(
                    'Rotation matrix has incorrect shape')
            else:
        ...
        if self.translation_vector is not None:
            if self.translation_vector.shape != (3, 1
                ...
                validation['valid'] = False
                validation['issues'].append(
                    'Translation vector has incorrect shape')
        return validation

    def __str__(self) ->str:
        summary = self.get_calibration_summary()
        return (
            f"CalibrationResult(device={self.device_id}, valid={summary['is_valid']}, rgb_error={self.rgb_rms_error:.3f if self.rgb_rms_error else 'N/A'}, thermal_error={self.thermal_rms_error:.3f if self.thermal_rms_error else 'N/A'}, stereo_error={self.stereo_rms_error:.3f if self.stereo_rms_error else 'N/A'})"
            )
