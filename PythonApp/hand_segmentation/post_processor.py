...
class SessionPostProcessor:
    ...
    def process_session(self, session_id: str, method: str='mediapipe', **
        config_kwargs) ->Dict[str, ProcessingResult]:
        print(
            f'[INFO] Starting hand segmentation processing for session: {session_id}'
            )
        session_dir = self.base_recordings_dir / session_id
    ...
    def process_video_file(self, video_path: str, output_directory:
        Optional[str]=None, method: str='mediapipe', **config_kwargs
        ) ->ProcessingResult:
        video_path = Path(video_path)
        if not video_path.exists():
            result = ProcessingResult(input_video_path=str(video_path),
                output_directory='', error_message=
                f'Video file not found: {video_path}')
            return result
        if output_directory is None:
            output_directory = (video_path.parent /
                f'hand_segmentation_{video_path.stem}')
        engine = create_segmentation_engine(method=method, **config_kwargs)
        if not engine.initialize():
            result = ProcessingResult(input_video_path=str(video_path),
                output_directory=str(output_directory), error_message=
                'Failed to initialize segmentation engine')
            return result
        try:
            result = engine.process_video(str(video_path), str(
                output_directory))
            self.processing_history.append({'timestamp': datetime.now().
                isoformat(), 'video_path': str(video_path),
                'output_directory': str(output_directory), 'method': method,
                'success': result.success, 'processing_time': result.
                processing_time, 'detected_hands': result.detected_hands_count}
                )
            return result
        finally:
            engine.cleanup()

    def _save_session_summary(self, session_id: str, method: str, results:
        Dict[str, ProcessingResult]):
        session_dir = self.base_recordings_dir / session_id
        summary_path = session_dir / f'hand_segmentation_summary_{method}.json'
        summary = {'session_id': session_id, 'processing_method': method,
            'processed_at': datetime.now().isoformat(), 'total_videos': len
            (results), 'successful_videos': sum(1 for r in results.values() if
            r.success), 'failed_videos': sum(1 for r in results.values() if
            not r.success), 'total_processing_time': sum(r.processing_time for
            r in results.values()), 'total_detections': sum(r.
            detected_hands_count for r in results.values()), 'videos': {}}
        for video_path, result in results.items():
            summary['videos'][video_path] = {'success': result.success,
...
def create_session_post_processor(base_recordings_dir: str='recordings'
