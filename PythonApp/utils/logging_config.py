...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record):
        log_entry = {'timestamp': datetime.fromtimestamp(record.created).
            isoformat(), 'level': record.levelname, 'logger': record.name,
            'module': record.module, 'function': record.funcName, 'line':
            record.lineno, 'thread': record.thread, 'thread_name': record.
            threadName, 'message': record.getMessage()}
        if record.exc_info:
        ...
        extra_fields = {}
        reserved_fields = {'name', 'msg', 'args', 'levelname', 'levelno',
            'pathname', 'filename', 'module', 'lineno', 'funcName',
            'created', 'msecs', 'relativeCreated', 'thread', 'threadName',
            'processName', 'process', 'getMessage', 'exc_info', 'exc_text',
            'stack_info', 'asctime', 'message'}
...
class PerformanceMonitor:
    ...
    def start_timer(cls, operation: str, context: str=None) ->str:
        ...
        with cls._lock:
            cls._timing_data[timer_id] = {'operation': operation, 'context':
                context, 'start_time': time.perf_counter(), 'thread':
    ...
    def end_timer(cls, timer_id: str, logger: logging.Logger=None) ->float:
        ...
        if logger:
            extra_info = {'duration_ms': duration * 1000, 'operation':
                timing_info['operation'], 'context': timing_info['context'],
...
class ColoredFormatter(logging.Formatter):
    COLORS = {'DEBUG': '\x1b[36m', 'INFO': '\x1b[32m', 'WARNING':
        '\x1b[33m', 'ERROR': '\x1b[31m', 'CRITICAL': '\x1b[35m', 'RESET':
        '\x1b[0m'}
...
class AppLogger:
    ...
    @classmethod
    def initialize(cls, log_level: str='INFO', log_dir: Optional[str]=None,
        ...
        if file_output:
            ...
            file_handler = logging.handlers.RotatingFileHandler(app_log_file,
    ...
    @classmethod
    def end_performance_timer(cls, timer_id: str, logger_name: str=None
        ) ->float:
    ...
    @classmethod
    def export_logs(cls, start_date: datetime, end_date: datetime,
        ...
        try:
            ...
            for log_file in cls._log_dir.glob('**/*.log'):
                try:
                    file_mtime = datetime.fromtimestamp(log_file.stat().
                        st_mtime)
            ...
            if export_path.stat().st_size > 1024 * 1024:
                compressed_path = export_path.with_suffix(export_path.
                    suffix + '.gz')
            ...
            for log_file in cls._log_dir.glob('**/*.log*'):
                ...
                try:
                    ...
                    if file_mtime < cutoff_date:
                        ...
                        log_file.unlink()
                        cleanup_report['compressed_files'].append(str(
                            compressed_path))
                        space_saved = file_size - compressed_path.stat(
                            ).st_size
...
def log_memory_usage(context: str, logger_name: str=None):
    ...
    def decorator(func):
        ...
        @wraps(func)
        def wrapper(*args, **kwargs):
            AppLogger.log_memory_usage(f'{context} - before {func.__name__}',
                logger_name)
            try:
                result = func(*args, **kwargs)
                AppLogger.log_memory_usage(f'{context} - after {func.__name__}'
                    , logger_name)
                return result
            except Exception:
                AppLogger.log_memory_usage(
                    f'{context} - after {func.__name__} (exception)',
                    logger_name)
                raise
        return wrapper
    return decorator
