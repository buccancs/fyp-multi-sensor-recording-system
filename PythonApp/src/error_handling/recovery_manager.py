...
class CameraResourceManager:
    ...
    def request_camera_access(self, camera_index: int, process_name: str = "webcam_capture") -> Tuple[bool, Optional[str]]:
        """
        ...
        with self.resource_lock:
            ...
            if camera_index in self.active_cameras:
                ...
                if current_user != process_name:
                    return False, f"Camera {camera_index} is already in use by {current_user}"
                else:
    ...
    def release_camera_access(self, camera_index: int, process_name: str = "webcam_capture"):
        """
        ...
        with self.resource_lock:
            if camera_index in self.active_cameras:
                if self.active_cameras[camera_index] == process_name:
                    ...
                    print(f"[DEBUG_LOG] Camera {camera_index} released by {process_name}")
                else:
                    print(f"[DEBUG_LOG] Warning: {process_name} tried to release camera {camera_index} owned by {self.active_cameras[camera_index]}")
    
    def _test_camera_availability(self, camera_index: int) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if not cap.isOpened():
                return False, f"Camera {camera_index} cannot be opened (may be in use by another application)"
    ...
    def recover_camera_access(self, camera_index: int, process_name: str = "webcam_capture") -> Tuple[bool, Optional[str]]:
        """
        ...
        if self.recovery_attempts[camera_index] > self.max_recovery_attempts:
            return False, f"Maximum recovery attempts ({self.max_recovery_attempts}) exceeded for camera {camera_index}"
        
        print(f"[DEBUG_LOG] Attempting camera {camera_index} recovery (attempt {self.recovery_attempts[camera_index]})")
        ...
        with self.resource_lock:
            if camera_index in self.active_cameras:
                ...
                del self.active_cameras[camera_index]
                print(f"[DEBUG_LOG] Forced release of camera {camera_index} from {old_user}")
...
class NetworkRecoveryManager:
    ...
    def record_sync_attempt(self, device_id: str, success: bool, response_time: float = None):
        """
        ...
        with self.recovery_lock:
            ...
            if success:
                ...
                self.sync_failures[device_id] = 0
                print(f"[DEBUG_LOG] Sync successful with {device_id} (response: {response_time:.2f}s)")
            else:
                self.sync_failures[device_id] += 1
                print(f"[DEBUG_LOG] Sync failed with {device_id} (failure count: {self.sync_failures[device_id]})")
    ...
    def attempt_sync_recovery(self, device_id: str, sync_function: Callable, *args, **kwargs) -> Tuple[bool, Optional[str]]:
        """
        ...
        if failure_count > self.max_sync_failures:
            return False, f"Maximum sync failures ({self.max_sync_failures}) exceeded for {device_id}"
        
        print(f"[DEBUG_LOG] Attempting sync recovery with {device_id} (failure count: {failure_count})")
...
class ErrorRecoveryManager:
    ...
    def _register_default_strategies(self):
        ...
        self.recovery_strategies[ErrorCategory.CAMERA_RESOURCE] = self._recover_camera_resource
        self.recovery_strategies[ErrorCategory.NETWORK_SYNCHRONIZATION] = self._recover_network_sync
        self.recovery_strategies[ErrorCategory.CODEC_ENCODING] = self._recover_codec_encoding
        self.recovery_strategies[ErrorCategory.CAMERA_HARDWARE] = self._recover_camera_hardware
    
    def classify_error(self, error_message: str, context: Dict[str, Any] = None) -> Tuple[ErrorCategory, ErrorSeverity]:
        """
        ...
        if any(keyword in error_lower for keyword in ["in use", "already open", "resource busy", "access denied"]):
            return ErrorCategory.CAMERA_RESOURCE, ErrorSeverity.MEDIUM
        
        # Camera hardware issues
        if any(keyword in error_lower for keyword in ["camera not found", "no camera", "device not available", "hardware error"]):
        ...
        # Codec issues
        if any(keyword in error_lower for keyword in ["codec", "encoding", "fourcc", "video writer"]):
            return ErrorCategory.CODEC_ENCODING, ErrorSeverity.MEDIUM
        
        # File system issues
        if any(keyword in error_lower for keyword in ["file", "directory", "permission", "disk", "space"]):
            return ErrorCategory.FILE_SYSTEM, ErrorSeverity.MEDIUM
        
        # Configuration issues
        if any(keyword in error_lower for keyword in ["config", "setting", "parameter", "invalid"]):
            return ErrorCategory.CONFIGURATION, ErrorSeverity.LOW
        
        return ErrorCategory.UNKNOWN, ErrorSeverity.MEDIUM
    
    def handle_error(self, error_message: str, context: Dict[str, Any] = None, auto_recover: bool = True) -> Tuple[bool, Optional[str]]:
        """
        ...
        print(f"[DEBUG_LOG] Error classified: {category.value} ({severity.value}) - {error_message}")
    ...
    def _recover_camera_resource(self, error_event: ErrorEvent) -> Tuple[bool, str]:
        ...
        success, message = self.camera_manager.recover_camera_access(camera_index, process_name)
    ...
    def _recover_codec_encoding(self, error_event: ErrorEvent) -> Tuple[bool, str]:
        ...
        fallback_codecs = context.get("fallback_codecs", [VideoCodec.MP4V, VideoCodec.XVID, VideoCodec.MJPG])
...
def handle_error_with_recovery(error_message: str, **context) -> Tuple[bool, Optional[str]]:
    """
