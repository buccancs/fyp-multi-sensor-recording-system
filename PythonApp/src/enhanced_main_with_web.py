...
try:
    ...
    logger.error(
        'Enhanced UI main window not available, falling back to standard')
    try:
...
class EnhancedApplicationWithWebUI:
    ...
    def setup_backend_services(self):
        ...
        try:
            if SESSION_MANAGER_AVAILABLE:
                self.session_manager = SessionManager(base_recordings_dir=
                    'recordings')
            ...
            if MAIN_CONTROLLER_AVAILABLE:
                ...
                if all([self.session_manager, self.json_server, self.
                    webcam_capture, self.stimulus_controller]):
                    self.main_controller.inject_dependencies(self.
                        session_manager, self.json_server, self.
                        webcam_capture, self.stimulus_controller)
                    logger.info('MainController initialized with dependencies')
                else:
                    logger.warning(
                        'Some MainController dependencies not available')
    ...
    def setup_web_dashboard(self):
        if not WEB_UI_AVAILABLE:
            ...
            return False
        try:
            self.web_integration = WebDashboardIntegration(enable_web_ui=
                True, web_port=5000, main_controller=self.main_controller,
                session_manager=self.session_manager, shimmer_manager=self.
                shimmer_manager, android_device_manager=self.
    ...
    def _connect_web_integration(self):
        ...
        logger.info(
            'Web dashboard integration connected to desktop application')
    ...
    def _update_web_dashboard_status(self):
        ...
        try:
            try:
                import psutil
                pc_status = {'status': 'running', 'cpu_usage': psutil.
                    cpu_percent(interval=0.1), 'memory_usage': psutil.
                    virtual_memory().percent, 'disk_usage': psutil.
                    disk_usage('/').percent if os.name != 'nt' else psutil.
                    disk_usage('C:').percent, 'ui_active': True,
                    'last_update': 'Now'}
            except ImportError:
                pc_status = {'status': 'running', 'cpu_usage': 0,
                    'memory_usage': 0, 'ui_active': True, 'last_update': 'Now'}
            if self.web_integration.controller and hasattr(self.
                web_integration.controller, 'device_status_received'):
...
The web interface is connected to the same data sources as the desktop application."""
    ...
    def cleanup(self):
        ...
        if self.android_device_manager and hasattr(self.
            android_device_manager, 'shutdown'):
            try:
