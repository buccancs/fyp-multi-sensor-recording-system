...
class RemoteDevice:
    ...
    def __init__(self, device_id: str, capabilities: List[str], client_socket: socket.socket):
        """
        ...
        logger.info(f"RemoteDevice created: {device_id} with capabilities: {capabilities}")
    ...
    def disconnect(self):
        ...
        logger.info(f"RemoteDevice {self.device_id} disconnected after {self.get_connection_duration():.1f} seconds")
...
class JsonSocketServer(QThread):
    ...
    def __init__(self, host: str = '0.0.0.0', port: int = 9000, use_newline_protocol: bool = False):
        ...
        protocol_type = "newline-delimited" if use_newline_protocol else "length-prefixed"
        logger.info(f"JsonSocketServer initialized for {host}:{port} using {protocol_type} JSON protocol")
        ...
        try:
            while self.running:
                ...
                if message_length <= 0 or message_length > 10 * 1024 * 1024:  # Max 10MB
                    ...
                    self.error_occurred.emit(device_id or client_addr, f"Invalid message length: {message_length}")
                    break
                ...
                try:
                    ...
                    self.error_occurred.emit(device_id or client_addr, f"JSON decode error: {str(e)}")
                    
        except Exception as e:
            logger.error(f"JSON client handling error for {client_addr}: {e}")
            self.error_occurred.emit(device_id or client_addr, f"Client handling error: {str(e)}")
        finally:
    ...
    def process_json_message(self, client_socket: socket.socket, client_addr: str, message: Dict[str, Any]) -> Optional[str]:
        """
        ...
        if message_type == 'hello':
            ...
            if device_id:
                ...
                if frame_data:
                    ...
                    logger.debug(f"Preview frame received from {device_id}: {frame_type}")
                else:
