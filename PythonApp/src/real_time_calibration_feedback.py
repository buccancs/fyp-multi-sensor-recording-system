...
class CalibrationFeedbackProcessor(QThread):
    ...
    def start_processing(self):
        ...
        try:
            self.logger.info(f"Starting calibration feedback for {self.config.camera_name}")
    ...
    def _process_frame(self, frame: np.ndarray) -> CalibrationFeedback:
        ...
        try:
            ...
            feedback = CalibrationFeedback(
                ...
                alignment_score=quality_result.alignment_metrics.alignment_score if quality_result.alignment_metrics else None
            )
    ...
    def _identify_primary_issue(self, quality_result: CalibrationQualityResult) -> str:
        ...
        if not quality_result.pattern_detection.pattern_found:
            ...
            return "Low Contrast"
        elif (quality_result.alignment_metrics and 
              quality_result.alignment_metrics.alignment_score < 0.5):
    ...
    def get_performance_stats(self) -> Dict[str, float]:
        ...
        stats = {
            ...
            'quality_trend': np.mean(list(self.quality_history)[-10:]) if len(self.quality_history) >= 10 else 0
        }
...
class RealTimeCalibrationWidget(QWidget):
    ...
    def update_feedback(self, camera_name: str, feedback: CalibrationFeedback):
        ...
        if feedback.is_acceptable:
            self.quality_progress.setStyleSheet("QProgressBar::chunk { background-color: green; }")
        elif feedback.quality_score > 0.5:
            self.quality_progress.setStyleSheet("QProgressBar::chunk { background-color: orange; }")
        else:
            self.quality_progress.setStyleSheet("QProgressBar::chunk { background-color: red; }")
...
class MultiCameraCalibrationManager:
    ...
    def add_camera(self, config: CameraFeedConfig) -> RealTimeCalibrationWidget:
        ...
        widget.processor.add_feedback_callback(
            lambda feedback, name=config.camera_name: self._track_feedback(name, feedback)
        )
    ...
    def _track_feedback(self, camera_name: str, feedback: CalibrationFeedback):
        ...
        if camera_name in self.feedback_history:
            ...
            if len(self.feedback_history[camera_name]) > 1000:
                self.feedback_history[camera_name] = self.feedback_history[camera_name][-500:]
