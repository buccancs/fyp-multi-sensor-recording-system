...
class CalibrationQualityAssessment:
    ...
    def assess_calibration_quality(self, image: np.ndarray, pattern_type:
        PatternType=PatternType.CHESSBOARD, reference_image: Optional[np.
        ndarray]=None) ->CalibrationQualityResult:
        start_time = time.time()
        try:
            self.logger.info(
                f'Starting calibration quality assessment for {pattern_type.value} pattern'
                )
            gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if len(image
                .shape) == 3 else image
            pattern_result = self._detect_calibration_pattern(gray_image,
                pattern_type)
            ...
            if reference_image is not None:
                alignment_metrics = self._analyze_alignment(image,
                    reference_image)
            overall_score = self._calculate_overall_quality(pattern_result,
                sharpness_metrics, contrast_metrics, alignment_metrics)
            is_acceptable = self._is_quality_acceptable(overall_score,
                pattern_result, sharpness_metrics, contrast_metrics,
                alignment_metrics)
            recommendations = self._generate_recommendations(pattern_result,
                sharpness_metrics, contrast_metrics, alignment_metrics)
            processing_time = (time.time() - start_time) * 1000
            result = CalibrationQualityResult(overall_quality_score=
                overall_score, is_acceptable=is_acceptable,
                pattern_detection=pattern_result, sharpness_metrics=
                sharpness_metrics, contrast_metrics=contrast_metrics,
                alignment_metrics=alignment_metrics, recommendations=
            ...
            self.logger.error(
                f'Error during calibration quality assessment: {e}')
            return CalibrationQualityResult(overall_quality_score=0.0,
                is_acceptable=False, pattern_detection=
                PatternDetectionResult(pattern_found=False, pattern_type=
                pattern_type, corner_count=0, pattern_score=0.0,
                geometric_distortion=1.0, completeness=0.0),
    ...
    def _detect_calibration_pattern(self, gray_image: np.ndarray,
        pattern_type: PatternType) ->PatternDetectionResult:
        try:
            ...
            return PatternDetectionResult(pattern_found=False, pattern_type
                =pattern_type, corner_count=0, pattern_score=0.0,
                geometric_distortion=1.0, completeness=0.0)

    def _detect_chessboard_pattern(self, gray_image: np.ndarray
        ...
        if found and corners is not None:
            criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER,
                30, 0.001)
            corners_refined = cv2.cornerSubPix(gray_image, corners, (11, 11
                ), (-1, -1), criteria)
            pattern_score = self._calculate_chessboard_quality(corners_refined,
                pattern_size)
            geometric_distortion = self._calculate_geometric_distortion(
                corners_refined, pattern_size)
            completeness = len(corners_refined) / (pattern_size[0] *
                pattern_size[1])
            return PatternDetectionResult(pattern_found=True, pattern_type=
                PatternType.CHESSBOARD, corner_count=len(corners_refined),
                pattern_score=pattern_score, geometric_distortion=
                geometric_distortion, completeness=completeness, corners=
                corners_refined)
        else:
            return PatternDetectionResult(pattern_found=False, pattern_type
                =PatternType.CHESSBOARD, corner_count=0, pattern_score=0.0,
                geometric_distortion=1.0, completeness=0.0)

    def _detect_circle_grid_pattern(self, gray_image: np.ndarray
        ) ->PatternDetectionResult:
        pattern_size = self.CIRCLE_GRID_COLS, self.CIRCLE_GRID_ROWS
        found, centers = cv2.findCirclesGrid(gray_image, pattern_size, cv2.
            CALIB_CB_SYMMETRIC_GRID)
        if found and centers is not None:
            pattern_score = self._calculate_circle_grid_quality(centers,
                pattern_size)
            geometric_distortion = self._calculate_geometric_distortion(centers
                , pattern_size)
            completeness = len(centers) / (pattern_size[0] * pattern_size[1])
            return PatternDetectionResult(pattern_found=True, pattern_type=
                PatternType.CIRCLE_GRID, corner_count=len(centers),
                pattern_score=pattern_score, geometric_distortion=
                geometric_distortion, completeness=completeness, corners=
                centers)
        else:
            return PatternDetectionResult(pattern_found=False, pattern_type
                =PatternType.CIRCLE_GRID, corner_count=0, pattern_score=0.0,
                geometric_distortion=1.0, completeness=0.0)

    def _detect_asymmetric_circle_grid_pattern(self, gray_image: np.ndarray
        ) ->PatternDetectionResult:
        pattern_size = self.CIRCLE_GRID_COLS, self.CIRCLE_GRID_ROWS
        found, centers = cv2.findCirclesGrid(gray_image, pattern_size, cv2.
        ...
        if found and centers is not None:
            ...
            completeness = len(centers) / (pattern_size[0] * pattern_size[1])
            return PatternDetectionResult(pattern_found=True, pattern_type=
                PatternType.ASYMMETRIC_CIRCLE_GRID, corner_count=len(
                centers), pattern_score=pattern_score, geometric_distortion
                =geometric_distortion, completeness=completeness, corners=
                centers)
        else:
            return PatternDetectionResult(pattern_found=False, pattern_type
                =PatternType.ASYMMETRIC_CIRCLE_GRID, corner_count=0,
                pattern_score=0.0, geometric_distortion=1.0, completeness=0.0)
    ...
    def _analyze_sharpness(self, gray_image: np.ndarray) ->SharpnessMetrics:
        try:
            ...
            sharpness_score = min(1.0, laplacian_variance / self.
                LAPLACIAN_THRESHOLD)
            return SharpnessMetrics(laplacian_variance=laplacian_variance,
                gradient_magnitude=gradient_magnitude, edge_density=
                edge_density, sharpness_score=sharpness_score)
        except Exception as e:
    ...
    def _analyze_contrast(self, gray_image: np.ndarray) ->ContrastMetrics:
        try:
            ...
            return ContrastMetrics(dynamic_range=dynamic_range,
                histogram_spread=hist_spread, local_contrast=local_contrast,
                contrast_score=contrast_score)
        except Exception as e:
            self.logger.error(f'Error analyzing contrast: {e}')
            return ContrastMetrics(0, 0.0, 0.0, 0.0)

    def _analyze_alignment(self, image1: np.ndarray, image2: np.ndarray
        ) ->AlignmentMetrics:
        try:
            gray1 = cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY) if len(image1.
                shape) == 3 else image1
            gray2 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY) if len(image2.
                shape) == 3 else image2
            ...
            if des1 is not None and des2 is not None:
                ...
                if len(matches) >= 4:
                    ...
                    transformation_matrix, mask = cv2.findHomography(src_pts,
                        dst_pts, cv2.RANSAC, 5.0)
                    if transformation_matrix is not None:
                        transformed_pts = cv2.perspectiveTransform(src_pts,
                            transformation_matrix)
                        alignment_error = np.mean(np.linalg.norm(
                            transformed_pts - dst_pts, axis=2))
                        alignment_score = max(0.0, 1.0 - alignment_error /
                            self.ALIGNMENT_MAX_ERROR)
                        return AlignmentMetrics(feature_match_count=len(
                            matches), alignment_error_pixels=
                            alignment_error, transformation_matrix=
                            transformation_matrix, alignment_score=
                            alignment_score)
            return AlignmentMetrics(feature_match_count=0,
                alignment_error_pixels=float('inf'), transformation_matrix=
                None, alignment_score=0.0)
        except Exception as e:
            self.logger.error(f'Error analyzing alignment: {e}')
            return AlignmentMetrics(0, float('inf'), None, 0.0)

    def _calculate_chessboard_quality(self, corners: np.ndarray,
        ...
        try:
            ...
            for i in range(pattern_size[1] - 1):
                for j in range(pattern_size[0] - 1):
                    horizontal_spacing = np.linalg.norm(corners_grid[i, j +
                        1] - corners_grid[i, j])
                    vertical_spacing = np.linalg.norm(corners_grid[i + 1, j
                        ] - corners_grid[i, j])
    ...
    def _calculate_circle_grid_quality(self, centers: np.ndarray,
        pattern_size: Tuple[int, int]) ->float:
        try:
    ...
    def _calculate_geometric_distortion(self, points: np.ndarray,
        pattern_size: Tuple[int, int]) ->float:
        try:
            if len(points) < 4:
                return 1.0
            ...
            actual_ratio = (max_x - min_x) / (max_y - min_y
                ) if max_y - min_y > 0 else 1.0
    ...
    def _calculate_overall_quality(self, pattern_result:
        PatternDetectionResult, sharpness_metrics: SharpnessMetrics,
        contrast_metrics: ContrastMetrics, alignment_metrics: Optional[
        AlignmentMetrics]) ->float:
        try:
            weights = {'pattern': 0.4, 'sharpness': 0.3, 'contrast': 0.2,
                'alignment': 0.1}
            overall_score = weights['pattern'
                ] * pattern_result.pattern_score + weights['sharpness'
                ] * sharpness_metrics.sharpness_score + weights['contrast'
                ] * contrast_metrics.contrast_score
            if alignment_metrics is not None:
                overall_score += weights['alignment'
                    ] * alignment_metrics.alignment_score
            else:
    ...
    def _is_quality_acceptable(self, overall_score: float, pattern_result:
        PatternDetectionResult, sharpness_metrics: SharpnessMetrics,
        contrast_metrics: ContrastMetrics, alignment_metrics: Optional[
        AlignmentMetrics]) ->bool:
        try:
            ...
            if contrast_metrics.contrast_score < self.MIN_CONTRAST_SCORE:
                return False
            if (alignment_metrics is not None and alignment_metrics.
    ...
    def _generate_recommendations(self, pattern_result:
        PatternDetectionResult, sharpness_metrics: SharpnessMetrics,
        contrast_metrics: ContrastMetrics, alignment_metrics: Optional[
        AlignmentMetrics]) ->List[str]:
        recommendations = []
        try:
            if not pattern_result.pattern_found:
...
if __name__ == '__main__':
    ...
    result = assessment.assess_calibration_quality(test_image, PatternType.
        CHESSBOARD)
