...
class CPUOptimizer:
    ...
    def create_optimized_thread_pool(self, name: str, max_workers: Optional[int] = None) -> ThreadPoolExecutor:
        ...
        self.logger.info(f"Created optimized thread pool '{name}' with {max_workers} workers")
        return pool
    ...
    def optimize_cpu_usage(self) -> Dict[str, Any]:
        ...
        for name, pool in self.thread_pools.items():
            if hasattr(pool, '_max_workers'):
                ...
                optimal_workers = self._calculate_optimal_workers(initial_metrics['overall_percent'])
...
class NetworkOptimizer:
    ...
    def get_network_metrics(self) -> Dict[str, Any]:
        ...
        if self.bandwidth_history:
            ...
            if time_diff > 0:
                bytes_diff = (net_io.bytes_sent + net_io.bytes_recv) - last_measurement['total_bytes']
    ...
    def _calculate_average_bandwidth(self) -> float:
        ...
        bandwidths = [m['bandwidth_mbps'] for m in self.bandwidth_history if m['bandwidth_mbps'] > 0]
        return sum(bandwidths) / len(bandwidths) if bandwidths else 0.0
    
    def optimize_network_usage(self, max_bandwidth_mbps: float = 50.0) -> Dict[str, Any]:
        ...
        if current_bandwidth > max_bandwidth_mbps * 0.8:  # 80% threshold
            ...
            if self.current_quality == 'low':
                self.current_quality = 'medium'
                optimization_result['actions_taken'].append('increased_quality_to_medium')
            elif self.current_quality == 'medium':
                self.current_quality = 'high'
                optimization_result['actions_taken'].append('increased_quality_to_high')
        
        optimization_result['new_quality'] = self.current_quality
        optimization_result['quality_settings'] = self.quality_levels[self.current_quality]
...
class PerformanceMonitor:
    ...
    def __init__(self, config: OptimizationConfig, logger=None):
        self.config = config
        self.logger = logger or logging.getLogger(__name__)
        self.metrics_history = deque(maxlen=int(config.history_retention_minutes * 60 / config.monitoring_interval_seconds))
    ...
    def _check_violations(self, metrics: PerformanceMetrics):
        ...
        # Check memory usage
        if metrics.memory_mb > self.config.max_memory_mb * self.config.memory_warning_threshold:
        ...
        # Check CPU usage
        if metrics.cpu_percent > self.config.max_cpu_percent * self.config.cpu_warning_threshold:
        ...
        for violation_type in violations:
            if self.violation_counts[violation_type] >= self.config.alert_threshold_violations:
    ...
    def _perform_automatic_cleanup(self):
        ...
        if current_time - self._last_cleanup_time >= self.config.cleanup_interval_seconds:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            'monitoring_duration_minutes': len(self.metrics_history) * self.config.monitoring_interval_seconds / 60,
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if avg_memory > self.config.max_memory_mb * 0.7:
            recommendations.append("Consider reducing buffer sizes or implementing more aggressive cleanup")
...
class PerformanceManager:
    ...
    def _on_performance_alert(self, violation_type: str, metrics: PerformanceMetrics):
        ...
        self.logger.warning(f"Current usage - CPU: {metrics.cpu_percent:.1f}%, Memory: {metrics.memory_mb:.1f}MB")
