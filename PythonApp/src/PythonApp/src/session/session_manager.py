...
class SessionManager:
    ...
    def __init__(self, base_recordings_dir: str = "recordings"):
        ...
        print(f"[DEBUG_LOG] SessionManager initialized with base directory: {self.base_recordings_dir}")
    ...
    def end_session(self) -> Optional[Dict]:
        ...
        metadata_file = Path(self.current_session["folder_path"]) / "session_metadata.json"
        with open(metadata_file, 'w') as f:
    ...
    def add_file_to_session(self, device_id: str, file_type: str, file_path: str, file_size: Optional[int] = None):
        ...
        print(f"[DEBUG_LOG] File added to session: {device_id} - {file_type} ({file_path})")
    ...
    def cleanup_old_sessions(self, days_old: int = 30):
        """
        Clean up session folders older than specified days.
        
        Args:
            days_old (int): Number of days after which to clean up sessions
        """
        cutoff_time = datetime.now().timestamp() - (days_old * 24 * 60 * 60)
        
        cleaned_count = 0
        for session_folder in self.base_recordings_dir.iterdir():
            if session_folder.is_dir():
                folder_time = session_folder.stat().st_mtime
                if folder_time < cutoff_time:
                    try:
                        ...
                        cleaned_count += 1
                        print(f"[DEBUG_LOG] Cleaned up old session folder: {session_folder.name}")
                    except Exception as e:
                        print(f"[DEBUG_LOG] Failed to clean up session folder {session_folder.name}: {e}")
        
        print(f"[DEBUG_LOG] Cleaned up {cleaned_count} old session folders")
...
def create_session_log_file(session_folder: Path, session_id: str) -> Path:
    """
    Create a session log file for detailed logging.
    
    Args:
        session_folder (Path): Session folder path
        session_id (str): Session identifier
        
    Returns:
        Path: Path to the created log file
    """
    log_file = session_folder / f"{session_id}_log.txt"
...
if __name__ == "__main__":
    ...
    manager.add_device_to_session("phone_1", "android_phone", ["rgb_video", "thermal_video"])
    ...
    print(f"Completed session: {completed['session_id']} (duration: {completed['duration']:.1f}s)")
