...
class CalibrationManager:
    ...
    def __init__(self, output_dir: str = "calibration_data"):
        ...
        print(f"[DEBUG_LOG] CalibrationManager initialized with output directory: {self.output_dir}")
    
    def start_calibration_session(self, device_ids: List[str], session_name: str = None) -> Dict[str, Any]:
        """
    ...
    def capture_calibration_frame(self, device_server) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[DEBUG_LOG] Sent calibration capture command to {device_count} devices")
            ...
            for device_id in self.current_session['device_ids']:
                ...
                capture_results['total_frames'][device_id] = self.capture_count[device_id]
    ...
    def can_compute_calibration(self, device_id: str = None) -> Dict[str, bool]:
        ...
        devices_to_check = [device_id] if device_id else self.current_session['device_ids']
        readiness = {}
    ...
    def compute_calibration(self, device_id: str = None) -> Dict[str, Any]:
        ...
        devices_to_calibrate = [device_id] if device_id else self.current_session['device_ids']
        computation_results = {
            'success': True,
        ...
        for dev_id in devices_to_calibrate:
            try:
                ...
                result = self._compute_device_calibration(dev_id, rgb_images, thermal_images)
                ...
                if result.is_valid():
                    ...
                    computation_results['device_results'][dev_id] = {
                        ...
                        'quality_score': result.quality_assessment.get('quality_score', 'UNKNOWN')
                    }
                else:
                    computation_results['device_results'][dev_id] = {
                        'success': False,
    ...
    def _compute_device_calibration(self, device_id: str, rgb_images: List[np.ndarray], 
        ...
        thermal_ret, thermal_camera_matrix, thermal_dist_coeffs, _, _ = cv2.calibrateCamera(
        ...
        if rgb_ret and thermal_ret:
            stereo_ret, _, _, _, _, R, T, E, F = cv2.stereoCalibrate(
                ...
                thermal_camera_matrix, thermal_dist_coeffs,
    ...
    def _assess_calibration_quality(self, result: CalibrationResult) -> Dict[str, Any]:
        ...
        if max_error < 0.5:
            ...
            assessment['recommendations'].append('Consider recapturing some images for better accuracy')
        else:
    ...
    def apply_thermal_overlay(self, device_id: str, rgb_image: np.ndarray, 
        ...
        try:
            ...
            thermal_warped = cv2.warpPerspective(
                thermal_image, result.homography_matrix, 
    ...
    def end_calibration_session(self) -> Dict[str, Any]:
        ...
        self.current_session['calibrated_devices'] = list(self.calibration_results.keys())
...
if __name__ == "__main__":
    ...
    # Start session
    session = manager.start_calibration_session(['device_1', 'device_2'], 'test_session')
