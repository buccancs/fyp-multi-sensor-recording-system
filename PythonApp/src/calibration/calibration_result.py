...
class CalibrationResult:
    ...
    def is_valid(self) -> bool:
        ...
        rgb_valid = (
            self.rgb_camera_matrix is not None and 
            self.rgb_distortion_coeffs is not None and
            self.rgb_rms_error is not None
        )
        
        # Check if thermal camera is calibrated
        thermal_valid = (
            self.thermal_camera_matrix is not None and 
            self.thermal_distortion_coeffs is not None and
            self.thermal_rms_error is not None
        )
        ...
        stereo_valid = (
            ...
            self.stereo_rms_error is not None
        )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CalibrationResult':
        ...
        result.thermal_distortion_coeffs = list_to_numpy(data.get('thermal_distortion_coeffs'))
    ...
    def get_rgb_camera_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'rms_error': self.rgb_rms_error,
            'distortion_coeffs': self.rgb_distortion_coeffs.tolist() if self.rgb_distortion_coeffs is not None else None
        }
    ...
    def get_thermal_camera_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'principal_point': {'cx': cx, 'cy': cy},
            'rms_error': self.thermal_rms_error,
            'distortion_coeffs': self.thermal_distortion_coeffs.tolist() if self.thermal_distortion_coeffs is not None else None
        }
    ...
    def validate_integrity(self) -> Dict[str, Any]:
        ...
        if self.rgb_camera_matrix is not None:
            ...
            if fx <= 0 or fy <= 0:
                ...
                validation['warnings'].append('RGB camera focal lengths seem unusually low')
        ...
        if self.thermal_rms_error is not None and self.thermal_rms_error > 5.0:
            validation['warnings'].append('Thermal camera RMS error is high (>5.0 pixels)')
        
        if self.stereo_rms_error is not None and self.stereo_rms_error > 5.0:
            validation['warnings'].append('Stereo calibration RMS error is high (>5.0 pixels)')
        ...
        if self.rotation_matrix is not None:
            if self.rotation_matrix.shape != (3, 3):
                ...
                if abs(det - 1.0) > 0.1:
                    validation['warnings'].append('Rotation matrix determinant is not close to 1')
        
        if self.translation_vector is not None:
            if self.translation_vector.shape != (3, 1) and self.translation_vector.shape != (3,):
                validation['valid'] = False
    ...
    def __str__(self) -> str:
        ...
        return f"CalibrationResult(device={self.device_id}, valid={summary['is_valid']}, " \
...
if __name__ == "__main__":
    ...
    # Add some test data
    result.rgb_camera_matrix = np.array([[800, 0, 320], [0, 800, 240], [0, 0, 1]], dtype=np.float32)
    result.rgb_distortion_coeffs = np.array([0.1, -0.2, 0.001, 0.002, 0.1], dtype=np.float32)
    result.rgb_rms_error = 0.5
    
    result.thermal_camera_matrix = np.array([[400, 0, 160], [0, 400, 120], [0, 0, 1]], dtype=np.float32)
    result.thermal_distortion_coeffs = np.array([0.2, -0.3, 0.002, 0.003, 0.2], dtype=np.float32)
