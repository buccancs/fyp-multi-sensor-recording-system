...
class SessionRecoveryManager(QObject):
    ...
    def __init__(self, base_sessions_dir: str = "recordings", backup_dir: Optional[str] = None):
        """
    ...
    def _monitoring_loop(self):
        ...
        while not self.stop_monitoring.is_set():
            try:
                ...
                self.log_recovery_event("monitoring_error", f"Monitoring error: {str(e)}")
    ...
    def check_disk_space(self):
        ...
        try:
            ...
            self.log_recovery_event("disk_check_error", f"Disk space check failed: {str(e)}")
            ...
            for session_folder in self.base_sessions_dir.iterdir():
                ...
                for json_file in session_folder.glob("*.json"):
                    if self.is_file_corrupted(json_file):
                        self.file_corruption_detected.emit(str(json_file), "JSON corruption detected")
                        self.attempt_file_repair(json_file)
                        
        except Exception as e:
            self.log_recovery_event("corruption_scan_error", f"Corruption scan failed: {str(e)}")
        ...
        try:
            ...
            if repaired_content:
                ...
                if not self.is_file_corrupted(file_path):
                    self.log_recovery_event("file_repaired", f"Successfully repaired: {file_path}")
                    return True
            
            self.log_recovery_event("file_repair_failed", f"Failed to repair: {file_path}")
            return False
            
        except Exception as e:
            self.log_recovery_event("file_repair_error", f"Repair error for {file_path}: {str(e)}")
            return False
    ...
    def backup_session(self, session_folder: Path) -> bool:
        ...
        try:
            ...
            self.log_recovery_event("backup_created", f"Backed up session: {session_folder.name}")
            return True
            
        except Exception as e:
            self.log_recovery_event("backup_error", f"Backup failed for {session_folder.name}: {str(e)}")
            return False
    ...
    def recover_incomplete_sessions(self) -> List[Dict]:
        ...
        try:
                ...
                for log_file in log_files:
                    if self.is_session_incomplete(log_file):
                        ...
                        if recovery_info:
                            ...
                            self.session_recovered.emit(recovery_info['session_id'], 
            ...
            self.log_recovery_event("recovery_error", f"Session recovery failed: {str(e)}")
    ...
    def is_session_incomplete(self, log_file: Path) -> bool:
        ...
        try:
            ...
            has_session_end = any(event.get('event') == 'session_end' for event in events)
...
def get_recovery_manager(base_sessions_dir: str = "recordings", 
                        backup_dir: Optional[str] = None) -> SessionRecoveryManager:
    """
    ...
    if _recovery_manager_instance is None:
        _recovery_manager_instance = SessionRecoveryManager(base_sessions_dir, backup_dir)
