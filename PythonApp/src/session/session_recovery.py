...
class SessionRecoveryManager(QObject):
    ...
    def __init__(self, base_sessions_dir: str='recordings', backup_dir:
        Optional[str]=None):
        super().__init__()
    ...
    def init_recovery_system(self):
        try:
            ...
            if not self.recovery_log_file.exists():
                self.log_recovery_event('system_init',
    ...
    def start_monitoring(self):
        ...
        self.log_recovery_event('monitoring_start',
    ...
    def stop_monitoring(self):
        ...
        self.log_recovery_event('monitoring_stop',
    ...
    def _monitoring_loop(self):
        while not self.stop_monitoring.is_set():
            try:
                ...
                self.log_recovery_event('monitoring_error',
    ...
    def check_disk_space(self):
        try:
            ...
            if available_gb < self.disk_critical_threshold_gb:
                self.disk_space_critical.emit(str(self.base_sessions_dir),
                    available_gb)
                self.log_recovery_event('disk_critical',
                    f'Critical disk space: {available_gb:.1f}GB available')
            elif available_gb < self.disk_warning_threshold_gb:
                self.disk_space_warning.emit(str(self.base_sessions_dir),
                    available_gb)
                self.log_recovery_event('disk_warning',
            ...
            if self.backup_dir and self.backup_dir.exists():
                ...
                if backup_available_gb < self.disk_critical_threshold_gb:
                    self.system_health_alert.emit('backup_disk_critical',
                        f'Backup disk critical: {backup_available_gb:.1f}GB')
        except Exception as e:
            self.log_recovery_event('disk_check_error',
            ...
            for session_folder in self.base_sessions_dir.iterdir():
                ...
                for json_file in session_folder.glob('*.json'):
                    if self.is_file_corrupted(json_file):
                        self.file_corruption_detected.emit(str(json_file),
                            'JSON corruption detected')
                        self.attempt_file_repair(json_file)
        except Exception as e:
            self.log_recovery_event('corruption_scan_error',
                f'Corruption scan failed: {str(e)}')
        ...
        try:
            ...
            if repaired_content:
                ...
                if not self.is_file_corrupted(file_path):
                    self.log_recovery_event('file_repaired',
                        f'Successfully repaired: {file_path}')
                    return True
            self.log_recovery_event('file_repair_failed',
                f'Failed to repair: {file_path}')
            return False
        except Exception as e:
            self.log_recovery_event('file_repair_error',
                f'Repair error for {file_path}: {str(e)}')
            return False
    ...
    def auto_cleanup_old_sessions(self):
        try:
            cutoff_date = datetime.now() - timedelta(days=self.
            ...
            for session_folder in self.base_sessions_dir.iterdir():
                if not session_folder.is_dir():
                    continue
                folder_mtime = datetime.fromtimestamp(session_folder.stat()
                    .st_mtime)
                if folder_mtime < cutoff_date:
                    ...
                    self.log_recovery_event('session_cleanup',
                        f'Cleaned up old session: {session_folder.name}')
            if cleaned_count > 0:
                freed_mb = freed_space / 1024 ** 2
                self.log_recovery_event('cleanup_complete',
                    f'Cleaned {cleaned_count} sessions, freed {freed_mb:.1f}MB'
                    )
        except Exception as e:
            self.log_recovery_event('cleanup_error',
                f'Auto cleanup failed: {str(e)}')
    ...
    def backup_session(self, session_folder: Path) ->bool:
        ...
        try:
            ...
            self.log_recovery_event('backup_created',
                f'Backed up session: {session_folder.name}')
            return True
        except Exception as e:
            self.log_recovery_event('backup_error',
                f'Backup failed for {session_folder.name}: {str(e)}')
            return False
                ...
                for log_file in log_files:
                    if self.is_session_incomplete(log_file):
                        ...
                        if recovery_info:
                            ...
                            self.session_recovered.emit(recovery_info[
                                'session_id'], recovery_info[
                                'recovery_details'])
            if recovered_sessions:
                self.log_recovery_event('sessions_recovered',
                    f'Recovered {len(recovered_sessions)} incomplete sessions')
        except Exception as e:
            self.log_recovery_event('recovery_error',
                f'Session recovery failed: {str(e)}')
    ...
    def is_session_incomplete(self, log_file: Path) ->bool:
        try:
            ...
            has_session_end = any(event.get('event') == 'session_end' for
                event in events)
    ...
    def recover_session(self, log_file: Path) ->Optional[Dict]:
        try:
            ...
            if not has_session_end:
                end_event = {'event': 'session_end', 'time': datetime.now()
                    .strftime('%H:%M:%S.%f')[:-3], 'timestamp': datetime.
                    now().isoformat(), 'recovered': True}
            ...
            recovery_info = {'session_id': session_id, 'log_file': str(
                log_file), 'recovery_time': session_data['recovery_time'],
                'recovery_details':
                'Added missing end_time and session_end event'}
            self.log_recovery_event('session_recovered',
                f"Recovered session {session_id}: {recovery_info['recovery_details']}"
                )
            return recovery_info
        except Exception as e:
            self.log_recovery_event('session_recovery_error',
    ...
    def get_recovery_statistics(self) ->Dict:
        try:
            stats = {'total_sessions': len(list(self.base_sessions_dir.glob
                ('*/'))), 'disk_usage': {}, 'backup_enabled': self.
                backup_enabled, 'monitoring_active': self.monitoring_active,
                'recovery_log_size': 0}
            ...
            if self.backup_dir and self.backup_dir.exists():
                backup_usage = psutil.disk_usage(str(self.backup_dir))
                stats['disk_usage']['backup'] = {'total_gb': backup_usage.
                    total / 1024 ** 3, 'used_gb': backup_usage.used / 1024 **
                    3, 'free_gb': backup_usage.free / 1024 ** 3}
            if self.recovery_log_file.exists():
                stats['recovery_log_size'] = self.recovery_log_file.stat(
                    ).st_size
            return stats
        except Exception as e:
...
def get_recovery_manager(base_sessions_dir: str='recordings', backup_dir:
    Optional[str]=None) ->SessionRecoveryManager:
    global _recovery_manager_instance
    if _recovery_manager_instance is None:
        _recovery_manager_instance = SessionRecoveryManager(base_sessions_dir,
            backup_dir)
...
def reset_recovery_manager() ->None:
    global _recovery_manager_instance
    if (_recovery_manager_instance and _recovery_manager_instance.
        monitoring_active):
