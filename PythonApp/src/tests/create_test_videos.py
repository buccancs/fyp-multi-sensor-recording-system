...
class TestVideoCreator:
    ...
    def create_test_video(self, name: str, spec: Dict, format_name: str) -> bool:
        ...
        try:
            ...
            out = cv2.VideoWriter(str(filepath), fourcc, spec['fps'], spec['resolution'])
            ...
            if filepath.exists() and filepath.stat().st_size > 0:
                ...
                print(f"[DEBUG_LOG] Successfully created {filename} ({filepath.stat().st_size} bytes)")
                return True
            else:
    ...
    def create_test_frame(self, frame_num: int, total_frames: int, spec: Dict) -> np.ndarray:
        ...
        cv2.putText(frame, f'Frame: {frame_num}', 
                   (50, 50), font, font_scale, (255, 255, 255), thickness)
        
        # Time stamp
        cv2.putText(frame, f'Time: {current_time:.3f}s', 
                   (50, 100), font, font_scale, (255, 255, 255), thickness)
        ...
        if frame_num % spec['fps'] == 0:
            ...
            cv2.putText(frame, f'{second}s', 
                       (width//2 - 30, height//2), font, font_scale * 2, (255, 255, 0), thickness * 2)
        ...
        if spec.get('description', '').startswith('Precision timing'):
            ...
            cv2.putText(frame, f'{millisecond:03d}ms', 
                       (width - 200, 50), font, font_scale, (0, 255, 255), thickness)
    ...
    def download_sample_videos(self) -> bool:
        ...
        if not REQUESTS_AVAILABLE:
            print("[DEBUG_LOG] Cannot download samples - requests library not available")
            return False
                    ...
                    for chunk in response.iter_content(chunk_size=8192):
                        if chunk:
                            ...
                            if total_size > 0:
                                ...
                                print(f"\r[DEBUG_LOG] Progress: {progress:.1f}%", end='')
    ...
    def create_problematic_videos(self) -> bool:
        ...
        try:
            ...
            cv2.putText(frame, 'TOO SHORT', (200, 240), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
            out.write(frame)
            out.release()
    ...
    def create_all_test_videos(self) -> bool:
        ...
        print(f"[DEBUG_LOG] Video creation complete: {total_created}/{total_attempted} created successfully")
...
def main():
    ...
    # Check dependencies
    if not CV2_AVAILABLE:
        print("[DEBUG_LOG] Warning: OpenCV not available - install with: pip install opencv-python")
    
    if not REQUESTS_AVAILABLE:
        print("[DEBUG_LOG] Warning: Requests not available - install with: pip install requests")
    ...
    if success:
        ...
        print("[DEBUG_LOG] Use these videos for comprehensive stimulus presentation testing")
