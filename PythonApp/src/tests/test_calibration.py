...
class CalibrationTestData:
    ...
    def load_real_calibration_data(self, data_dir: str) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        """
        ...
        for filename in os.listdir(data_dir):
            if filename.endswith('_corners.json'):
                try:
                    ...
                    logger.warning(f"Failed to load calibration data from {filename}: {e}")
...
class CalibrationTester:
    ...
    def run_calibration(self, object_points: List[np.ndarray], 
                       image_points: List[np.ndarray]) -> Tuple[float, np.ndarray, np.ndarray]:
        """
    ...
    def validate_calibration_result(self, rms_error: float, 
        ...
        if rms_error > self.test_data.error_threshold:
            logger.error(f"RMS error {rms_error:.3f} exceeds threshold {self.test_data.error_threshold}")
            return False
        ...
        if len(dist_coeffs) >= 2:
            ...
            if abs(k1) > 1.0 or abs(k2) > 1.0:
                logger.warning(f"Large distortion coefficients: k1={k1:.3f}, k2={k2:.3f}")
...
def test_calibration_with_synthetic_data(calibration_tester, synthetic_calibration_data):
    ...
    # Validate results
    assert calibration_tester.validate_calibration_result(rms_error, camera_matrix, dist_coeffs)
...
def test_calibration_mismatched_points(calibration_tester):
    ...
    # Should raise ValueError
    with pytest.raises(ValueError, match="Number of object and image point sets must match"):
        calibration_tester.run_calibration(object_points, image_points)
...
def test_calibration_with_real_data():
    ...
    test_data = CalibrationTestData()
    data_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'test_data', 'calibration')
...
def test_calibration_performance():
    ...
    logger.info(f"Calibration with {num_images} images took {calibration_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    # Run calibration
    try:
        rms_error, camera_matrix, dist_coeffs = tester.run_calibration(object_points, image_points)
