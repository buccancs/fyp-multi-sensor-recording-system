...
class ComprehensiveVideoTestSuite:
    ...
    def discover_test_videos(self):
        ...
        if not self.test_videos_dir.exists():
            print(f"[DEBUG_LOG] Test videos directory not found: {self.test_videos_dir}")
            return videos
        ...
        for video_file in self.test_videos_dir.iterdir():
            if video_file.suffix.lower() in video_extensions and video_file.stat().st_size > 0:
                videos.append({
                    'path': str(video_file),
    ...
    def test_basic_video_loading(self):
        ...
        for video in self.sample_videos:
            ...
            try:
                ...
                if success:
                    passed += 1
                    self.add_result(f"✓ Basic loading: {video['name']} ({video['format']})")
                else:
                    self.add_result(f"✗ Basic loading failed: {video['name']} ({video['format']})")
    ...
    def test_enhanced_video_loading(self):
        ...
        if not ENHANCED_AVAILABLE:
            self.add_result("! Enhanced controller not available - skipping enhanced tests")
            return 0, 0
        ...
        for video in self.sample_videos:
            ...
            try:
                ...
                if success:
                    passed += 1
                    backend = self.enhanced_controller.current_backend.value if self.enhanced_controller.current_backend else "unknown"
                    self.add_result(f"✓ Enhanced loading: {video['name']} ({video['format']}) - {backend} backend")
                else:
                    self.add_result(f"✗ Enhanced loading failed: {video['name']} ({video['format']})")
            except Exception as e:
    ...
    def test_format_compatibility(self):
        ...
        for format_name, results in format_results.items():
            ...
            self.add_result(f"Format {format_name}: {results['passed']}/{results['total']} ({success_rate:.1f}%)")
        ...
        for video in self.sample_videos:
            ...
            try:
                ...
                if hasattr(self.enhanced_controller, '_load_with_backend'):
                    ...
                    success = self.enhanced_controller._load_with_backend(video['path'], VideoBackend.QT_MULTIMEDIA)
                    if success:
                        qt_results['passed'] += 1
            except Exception:
                pass
            ...
            try:
                ...
                if hasattr(self.enhanced_controller, '_load_with_backend'):
                    from gui.enhanced_stimulus_controller import VideoBackend
                    success = self.enhanced_controller._load_with_backend(video['path'], VideoBackend.VLC)
                    if success:
                        vlc_results['passed'] += 1
            except Exception:
                pass
        
        qt_rate = (qt_results['passed'] / qt_results['total']) * 100 if qt_results['total'] > 0 else 0
        vlc_rate = (vlc_results['passed'] / vlc_results['total']) * 100 if vlc_results['total'] > 0 else 0
        
        self.add_result(f"Qt backend: {qt_results['passed']}/{qt_results['total']} ({qt_rate:.1f}%)")
        self.add_result(f"VLC backend: {vlc_results['passed']}/{vlc_results['total']} ({vlc_rate:.1f}%)")
    ...
    def test_error_handling(self):
        ...
        self.add_result(f"Error handling: {handled_errors}/{error_tests} handled correctly")
        ...
        try:
            ...
            if Path(log_file).exists():
                ...
                if "events" in log_data and len(log_data["events"]) >= 3:
                    self.add_result(f"✓ Timing accuracy test passed with {timing_video['name']}")
    ...
    def test_performance_metrics(self):
        ...
        if performance_results:
            # Calculate averages
            avg_load_time = sum(r['load_time_ms'] for r in performance_results) / len(performance_results)
            self.add_result(f"Performance: Average load time {avg_load_time:.1f}ms across {len(performance_results)} videos")
    ...
    def run_all_tests(self):
        ...
        self.add_result(f"Performance tests: {len(performance_results)} videos analyzed")
        self.add_result(f"Total test time: {total_time:.2f} seconds")
        self.add_result(f"Enhanced controller: {'Available' if ENHANCED_AVAILABLE else 'Not Available'}")
        self.add_result(f"VLC backend: {'Available' if VLC_AVAILABLE else 'Not Available'}")
