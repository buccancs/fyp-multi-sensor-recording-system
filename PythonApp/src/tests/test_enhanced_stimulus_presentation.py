...
from PyQt5.QtWidgets import (
    ...
    QLabel, QFileDialog, QTextEdit, QHBoxLayout, QProgressBar, QComboBox
)
...
class PerformanceTestThread(QThread):
    ...
    def run(self):
        ...
        self.results = {
            ...
            'performance_score': max(0, 100 - (len([t for t in frame_times if t > 0.020]) * 5))
        }
...
class EnhancedStimulusTestWindow(QMainWindow):
    ...
    def connect_test_signals(self):
        ...
        self.stimulus_panel.start_recording_play_requested.connect(self.test_synchronized_start)
        self.stimulus_panel.mark_event_requested.connect(self.enhanced_controller.mark_event)
    ...
    def load_test_video(self):
        ...
        if file_path:
            ...
            if success:
                ...
                self.add_result(f"✓ Video loaded successfully: {os.path.basename(file_path)}")
            else:
                self.add_result(f"✗ Failed to load video: {os.path.basename(file_path)}")
        ...
        try:
            ...
            for i in range(300):  # 10 seconds at 30 fps
                ...
                if i % 30 == 0:  # Every second
                    ...
                    cv2.putText(frame, f'{i//30}s', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                
                # Add frame number
                cv2.putText(frame, f'Frame {i}', (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                
                # Add precision timing marker
                timestamp = i / 30.0
                cv2.putText(frame, f'{timestamp:.3f}s', (400, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    ...
    def test_qt_backend(self):
        ...
        try:
            ...
            if self.enhanced_controller._load_with_backend(
                self.enhanced_controller.current_video_file, 
    ...
    def test_synchronized_start(self):
        ...
        try:
            ...
            if self.enhanced_controller.start_stimulus_playback(screen_index):
                ...
                QTimer.singleShot(3000, lambda: self.enhanced_controller.stop_stimulus_playback("test_complete"))
            else:
    ...
    def test_timing_precision(self):
        ...
        try:
            ...
            self.add_result(f"✓ Timing calibration offset: {test_logger.clock_offset*1000:.3f}ms")
            
            # Test multiple clock sources
            timestamps = test_logger.get_precise_timestamp()
            
            # Verify all clocks are working
            required_clocks = ['system_time', 'monotonic_time', 'performance_time', 'corrected_time']
            ...
            self.add_result(f"✓ Timing precision test: {measured_duration:.3f}ms (target: ~1ms)")
    ...
    def test_codec_detection(self):
        ...
        try:
            ...
            test_formats = ['.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv', '.webm', '.unknown']
            
            for fmt in test_formats:
                test_file = f"test{fmt}"
                
                # Test Qt support
                qt_supported = self.codec_info.is_supported(test_file, VideoBackend.QT_MULTIMEDIA)
                
                # Test VLC support
                vlc_supported = self.codec_info.is_supported(test_file, VideoBackend.VLC) if VLC_AVAILABLE else False
    ...
    def test_format_support(self):
        ...
        try:
            ...
            self.add_result(f"✓ Qt supported formats ({len(qt_formats)}): {', '.join(qt_formats)}")
            
            if VLC_AVAILABLE:
                self.add_result(f"✓ VLC supported formats ({len(vlc_formats)}): {', '.join(vlc_formats)}")
                unique_vlc = set(vlc_formats) - set(qt_formats)
                if unique_vlc:
                    self.add_result(f"✓ VLC additional formats: {', '.join(unique_vlc)}")
            else:
    ...
    def test_enhanced_signal_connections(self):
        ...
        try:
            ...
            # Test timing logger
            assert hasattr(self.enhanced_controller.timing_logger, 'get_precise_timestamp')
    ...
    def test_backend_availability(self):
        ...
        try:
            ...
            self.add_result(f"✓ Qt codec support: {len(self.enhanced_controller.codec_info.qt_supported)} formats")
            
            if VLC_AVAILABLE:
                assert len(self.enhanced_controller.codec_info.vlc_supported) > 0
                self.add_result(f"✓ VLC codec support: {len(self.enhanced_controller.codec_info.vlc_supported)} formats")
    ...
    def test_performance_monitoring(self):
        ...
        try:
            ...
            self.add_result(f"✓ Performance monitoring update time: {update_time:.3f}ms")
