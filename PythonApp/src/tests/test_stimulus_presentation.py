...
class StimulusTestWindow(QMainWindow):
    ...
    def connect_test_signals(self):
        ...
        self.stimulus_panel.mark_event_requested.connect(self.stimulus_controller.mark_event)
    ...
    def test_screen_detection(self):
        ...
        try:
            ...
            self.test_results.append(f"✓ Screen detection test passed ({screen_count} screens)")
            print(f"[DEBUG_LOG] Screen detection test passed: {screen_count} screens detected")
...
def main():
    ...
    if test_video:
        ...
        QTimer.singleShot(1000, lambda: test_window.stimulus_panel.load_file(test_video))
    ...
    print("[DEBUG_LOG] 4. Keyboard Shortcuts - Space (play/pause), Esc (exit fullscreen)")
