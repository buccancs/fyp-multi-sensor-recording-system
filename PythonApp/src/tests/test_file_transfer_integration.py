...
class TestFileTransferIntegration(unittest.TestCase):
    ...
    def test_file_info_message_processing(self):
        ...
        with patch.object(self.server, 'get_session_directory', return_value=self.test_dir):
            # Act
            ...
            self.assertEqual(
                self.test_device.file_transfer_state['filename'], 
                self.test_file_name
            )
            self.assertEqual(
            ...
            self.assertEqual(
                self.test_device.file_transfer_state['received_bytes'], 
                0
            )
    ...
    def test_file_chunk_message_processing(self):
        ...
        test_file_path = os.path.join(self.test_dir, f"{self.device_id}_{self.test_file_name}")
        ...
        try:
            ...
            self.assertEqual(
                self.test_device.file_transfer_state['chunks_received'], 
                1
            )
    ...
    def test_file_end_message_processing(self):
        ...
        self.test_device.file_transfer_state['file_handle'].write(self.test_file_content)
    ...
    def test_request_all_session_files(self):
        ...
        with patch.object(self.server, 'request_file_from_device', return_value=True) as mock_request:
            # Act
    ...
    def test_multiple_concurrent_transfers(self):
        ...
        with patch.object(self.server, 'get_session_directory', return_value=self.test_dir):
            ...
            for device_id, device in devices:
                ...
                self.assertEqual(
                    device.file_transfer_state['filename'], 
