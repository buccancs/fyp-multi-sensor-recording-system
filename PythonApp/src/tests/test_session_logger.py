...
from session.session_logger import (
    SessionLogger,
    get_session_logger,
...
class TestSessionLogger(unittest.TestCase):
    ...
    def test_event_logging(self):
        ...
        self.assertGreater(
            len(events), 8
        )  # Should have at least 9 events plus session_start and session_end
    ...
    def test_large_session_simulation(self):
        ...
        events = completed_session.get("events", [])
        expected_events = (
            cycles * len(event_types) + 2
        )  # +2 for session_start and session_end
    ...
    def test_crash_recovery_simulation(self):
        ...
        self.assertEqual(
            log_data.get("status"), "active"
        )  # Should still be active after crash
    ...
    def test_concurrent_logging_thread_safety(self):
        ...
        events = completed_session.get("events", [])
        expected_events = (
            thread_count * events_per_thread + 2
        )  # +2 for session_start and session_end
        self.assertEqual(
