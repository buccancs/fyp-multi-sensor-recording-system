...
class CameraDetector:
    ...
    def detect_cameras(self, max_cameras: int = 10) -> List[CameraInfo]:
        ...
        for camera_index in range(max_cameras):
            try:
                ...
                if camera_info:
                    ...
                    print(f"[DEBUG_LOG] Camera {camera_index}: {camera_info.name} - {camera_info.resolution[0]}x{camera_info.resolution[1]}")
        ...
        print(f"[DEBUG_LOG] Camera detection complete: {len(working_cameras)} working cameras found")
...
class WebcamConfigManager:
    ...
    def detect_and_configure_cameras(self) -> List[CameraInfo]:
        ...
        if working_cameras:
            ...
            if working_cameras[0].supported_resolutions:
                ...
                best_resolution = max(working_cameras[0].supported_resolutions, 
    ...
    def auto_configure(self) -> Dict[str, Any]:
        ...
        print(f"[DEBUG_LOG] Auto-configuration complete: {len(working_cameras)} cameras, {len(codecs)} codecs")
    ...
    def set_camera(self, camera_index: int) -> bool:
        ...
        print(f"[DEBUG_LOG] Failed to set camera index {camera_index}: camera not available")
        return False
        ...
        try:
            if 'codec' in kwargs:
                ...
                if codec in self.available_codecs:
                    ...
                    print(f"[DEBUG_LOG] Codec {codec.value} not available, using fallback")
                    fallback = CodecValidator.get_fallback_codec(self.config.fallback_codecs)
                    if fallback:
    ...
    def set_preview_config(self, **kwargs) -> bool:
        ...
        try:
            ...
            if 'maintain_aspect_ratio' in kwargs:
                self.config.preview.maintain_aspect_ratio = bool(kwargs['maintain_aspect_ratio'])
    ...
    def load_config(self):
        ...
        try:
            if self.config_file.exists():
                ...
                self.config.camera_name = config_dict.get("camera_name", "Default Camera")
                self.config.auto_detect_cameras = config_dict.get("auto_detect_cameras", True)
                
                # Load recording config
                recording_dict = config_dict.get("recording", {})
                self.config.recording.codec = VideoCodec(recording_dict.get("codec", "mp4v"))
                self.config.recording.resolution = tuple(recording_dict.get("resolution", [1280, 720]))
                self.config.recording.fps = recording_dict.get("fps", 30)
                self.config.recording.quality = recording_dict.get("quality", 80)
                self.config.recording.file_format = recording_dict.get("file_format", "mp4")
                ...
                self.config.preview.enable_scaling = preview_dict.get("enable_scaling", True)
                self.config.preview.maintain_aspect_ratio = preview_dict.get("maintain_aspect_ratio", True)
                
                # Load fallback codecs
                fallback_codec_names = config_dict.get("fallback_codecs", ["mp4v", "XVID", "MJPG"])
                self.config.fallback_codecs = [VideoCodec(name) for name in fallback_codec_names]
...
if __name__ == "__main__":
    ...
    if working_cameras:
        print(f"[DEBUG_LOG] Testing camera selection with camera {working_cameras[0].index}")
        config_manager.set_camera(working_cameras[0].index)
    
    # Test recording configuration
    config_manager.set_recording_config(
        resolution="1280x720",
        fps=30,
        quality=85
    )
    ...
    config_manager.set_preview_config(
        max_width=800,
        max_height=600,
        fps=25
    )
