...
from stimulus_manager import StimulusManager, StimulusEvent
        ...
        try:
            ...
            while self.running:
                try:
                    client_socket, address = self.server_socket.accept()
                    client_thread = threading.Thread(
                        target=self.handle_json_client, args=(client_socket, address)
                    )
                    client_thread.daemon = True
                    client_thread.start()
...
class MultiSensorController(QMainWindow):
    ...
    def __init__(self):
        ...
        if self.stimulus_manager.initialize():
            ...
            self.log_message(
                f"StimulusManager initialized successfully with {monitor_count} monitors"
            )
        else:
    ...
    def start_recording(self):
        ...
        try:
            ...
            if devices:
                ...
                self.log_message(
                    "No Shimmer devices found - continuing without sensor data"
                )
        except Exception as e:
    ...
    def update_status(self):
        ...
        try:
            ...
            connected_count = sum(
                1 for status in shimmer_status.values() if status.is_connected
            )
            streaming_count = sum(
                1 for status in shimmer_status.values() if status.is_streaming
            )
            recording_count = sum(
                1 for status in shimmer_status.values() if status.is_recording
            )
        ...
        try:
            ...
            if time_server_status:
                if time_server_status.is_running:
                    sync_status = (
                        "synchronized"
                        if time_server_status.is_synchronized
                    ...
                    if current_time - self._last_ntp_status_log > 30.0:
                        self.log_message(
                            f"NTP Server: {sync_status}, accuracy: {accuracy}, "
            ...
            if performance_status:
                ...
                if current_metrics:
                    ...
                    if current_time - self._last_performance_status_log > 60.0:
                        ...
                        recommendations = performance_status.get(
                            "optimization_recommendations", []
                        )
                        if recommendations:
                            self.log_message(
                                f"Performance recommendations: {len(recommendations)} items"
                            )
    ...
    def _on_shimmer_data(self, sample: ShimmerSample) -> None:
        ...
        try:
            ...
            if hasattr(self, "log_text"):  # Check if UI is initialized
                ...
                current_time = time.time()
                if (
                    current_time - self._last_shimmer_log_time > 5.0
                ):  # Log every 5 seconds
                    self.log_message(
                        f"Shimmer data: {sample.device_id} - GSR: {sample.gsr_conductance:.2f}μS"
                    )
        ...
        try:
            ...
            if hasattr(self, "log_text"):  # Check if UI is initialized
                if status.is_connected:
                    self.log_message(
                        f"Shimmer {device_id}: Connected (Battery: {status.battery_level}%)"
                    )
                else:
        ...
        try:
            ...
            if hasattr(self, "log_text"):  # Check if UI is initialized
                if event.event_type == "stimulus_start":
                    self.log_message(
                        f"Stimulus started on monitor {event.monitor_id}: {event.stimulus_config.stimulus_type}"
                    )
                elif event.event_type == "stimulus_stop":
                    duration = event.duration_actual_ms or 0
                    self.log_message(
                        f"Stimulus stopped on monitor {event.monitor_id} (duration: {duration:.1f}ms)"
                    )
                elif event.event_type == "synchronized_stimulus_start":
                    self.log_message(
                        f"Synchronized stimulus started on monitor {event.monitor_id}"
                    )
