...
from PyQt5.QtWidgets import (
    ...
    QShortcut, QApplication, QMessageBox
)
...
class TimingLogger:
    ...
    def start_experiment_log(self, video_file: str) -> str:
        ...
        self.current_log_file = os.path.join(
            self.log_directory, 
            f"experiment_log_{timestamp}.json"
        )
        ...
        log_data = {
            "experiment_info": {
                ...
                "start_time_formatted": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(self.experiment_start_time)),
...
class StimulusController(QWidget):
    ...
    def mark_event(self, label: str = ""):
        ...
        try:
            ...
            self.status_changed.emit(f"Event Marker {marker_num} - {current_position/1000:.1f}s")
    ...
    def position_video_on_screen(self, screen_index: int):
        ...
        if 0 <= screen_index < len(screens):
            ...
            print(f"[DEBUG_LOG] Invalid screen index {screen_index}, using primary screen")
    ...
    def update_position(self):
        ...
        if self.is_experiment_active and self.media_player.state() == QMediaPlayer.PlayingState:
            position = self.media_player.position()
            duration = self.media_player.duration()
            
            if duration > 0:
                progress = (position / duration) * 100
                self.status_label.setText(f"Playing: {position/1000:.1f}s / {duration/1000:.1f}s ({progress:.1f}%)")
