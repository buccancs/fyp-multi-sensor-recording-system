...
from PyQt5.QtWidgets import (
    ...
    QDockWidget, QTextEdit, QLabel, QPushButton, QComboBox, QProgressBar
)
...
class EnhancedMainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        switch_backend_action.triggered.connect(self.enhanced_stimulus_controller.switch_backend)
    ...
    def create_enhanced_toolbar(self):
        ...
        self.backend_status_label.setStyleSheet("QLabel { color: #0066cc; font-weight: bold; }")
    ...
    def create_enhanced_central_widget(self):
        ...
        self.vlc_status_label = QLabel(f"VLC Backend: {'Available' if VLC_AVAILABLE else 'Not Available'}")
        ...
        self.timing_precision_label.setStyleSheet("QLabel { color: #666; font-size: 10px; }")
    ...
    def create_enhanced_log_dock(self):
        ...
        self.log_filter_combo.addItems(["All", "Stimulus", "Backend", "Performance", "Errors"])
    ...
    def connect_enhanced_stimulus_signals(self):
        ...
        self.stimulus_panel.file_loaded.connect(self.enhanced_stimulus_controller.load_video)
        self.stimulus_panel.play_requested.connect(self.enhanced_stimulus_controller.test_play)
        self.stimulus_panel.pause_requested.connect(self.enhanced_stimulus_controller.test_pause)
        self.stimulus_panel.seek_requested.connect(self.on_enhanced_stimulus_seek_requested)
        self.stimulus_panel.screen_changed.connect(self.on_enhanced_stimulus_screen_changed)
        
        # Connect enhanced milestone 3.5 signals
        self.stimulus_panel.start_recording_play_requested.connect(self.on_enhanced_start_recording_play_requested)
        self.stimulus_panel.mark_event_requested.connect(self.on_enhanced_mark_event_requested)
        
        # Connect enhanced stimulus controller signals to main window
        self.enhanced_stimulus_controller.status_changed.connect(self.on_enhanced_stimulus_status_changed)
        self.enhanced_stimulus_controller.experiment_started.connect(self.on_enhanced_stimulus_experiment_started)
        self.enhanced_stimulus_controller.experiment_ended.connect(self.on_enhanced_stimulus_experiment_ended)
        self.enhanced_stimulus_controller.error_occurred.connect(self.on_enhanced_stimulus_error)
        self.enhanced_stimulus_controller.backend_changed.connect(self.on_backend_changed)
        
        self.log_message("Enhanced stimulus signals connected to GUI handlers", "Stimulus")
    ...
    def show_vlc_status(self):
        """Show VLC availability status."""
        if VLC_AVAILABLE:
            self.log_message("VLC backend available - Enhanced codec support enabled", "Backend")
        else:
            self.log_message("VLC backend not available - Limited to Qt multimedia codecs", "Backend")
        ...
        if duration > 0:
            ...
            if self.enhanced_stimulus_controller.current_backend:
                if hasattr(self.enhanced_stimulus_controller, 'qt_media_player'):
                    self.enhanced_stimulus_controller.qt_media_player.setPosition(seek_position)
                elif hasattr(self.enhanced_stimulus_controller, 'vlc_video_widget'):
                    self.enhanced_stimulus_controller.vlc_video_widget.set_position(seek_position)
            self.log_message(f"Enhanced stimulus seek to {position}% ({seek_position}ms)", "Stimulus")
    
    def on_enhanced_stimulus_screen_changed(self, screen_index):
        """Handle enhanced stimulus screen selection change."""
        self.log_message(f"Enhanced stimulus output screen changed to index {screen_index}", "Stimulus")
    ...
    def on_enhanced_start_recording_play_requested(self):
        ...
        try:
            ...
            self.log_message("Starting enhanced synchronized recording and stimulus playback...", "Stimulus")
            ...
            self.log_message(f"Error starting enhanced synchronized experiment: {str(e)}", "Errors")
            QMessageBox.critical(self, "Enhanced Experiment Start Error", f"Failed to start experiment: {str(e)}")
    ...
    def on_enhanced_stimulus_experiment_ended(self):
        ...
        try:
            ...
            self.log_message("Enhanced stimulus experiment ended - all recordings stopped", "Stimulus")
    ...
    def test_timing_precision(self):
        ...
        try:
            ...
            precision_info += f"Performance Time: {timestamps['performance_time']:.6f}\n"
        ...
        try:
            ...
            if hasattr(self.enhanced_stimulus_controller, 'frame_drop_count'):
                ...
                if performance_score < 80:
                    self.log_message(f"Performance warning: {performance_score}% (frame drops: {frame_drops})", "Performance")
