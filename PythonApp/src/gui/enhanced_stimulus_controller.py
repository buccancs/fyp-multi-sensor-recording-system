...
from PyQt5.QtWidgets import (
    ...
    QShortcut, QApplication, QMessageBox, QProgressBar
)
from PyQt5.QtCore import Qt, QTimer, pyqtSignal, QUrl, QThread, pyqtSlot
...
class CodecInfo:
    ...
    def __init__(self):
        ...
        self.vlc_supported = [
            '.mp4', '.avi', '.mov', '.mkv', '.wmv', '.m4v', '.3gp', '.flv', 
            '.webm', '.ogv', '.mpg', '.mpeg', '.ts', '.mts', '.m2ts'
        ]
...
class EnhancedTimingLogger:
    ...
    def start_experiment_log(self, video_file: str, backend: str) -> str:
        ...
        log_data = {
            "experiment_info": {
                ...
                "start_time_formatted": datetime.fromtimestamp(self.experiment_start_time).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
    ...
    def log_stimulus_start(self, video_duration_ms: int, frame_rate: float = None):
        ...
        event = {
            ...
            "timestamp_formatted": datetime.fromtimestamp(timing_info['corrected_time']).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
            "experiment_time": timing_info['corrected_time'] - self.experiment_start_time,
    ...
    def log_event_marker(self, video_position_ms: int, marker_label: str = "", frame_number: int = None):
...
class EnhancedStimulusController(QWidget):
    ...
    def load_video(self, file_path: str) -> bool:
        ...
        if not best_backend:
            self.error_occurred.emit(f"Unsupported video format: {os.path.splitext(file_path)[1]}")
            return False
        ...
        if self._load_with_backend(file_path, best_backend):
            ...
            self.status_changed.emit(f"Loaded: {os.path.basename(file_path)} ({best_backend.value})")
            print(f"[DEBUG_LOG] Video loaded with {best_backend.value} backend: {file_path}")
            return True
        
        # Try fallback backend
        fallback_backend = VideoBackend.VLC if best_backend == VideoBackend.QT_MULTIMEDIA else VideoBackend.QT_MULTIMEDIA
        ...
        if self._load_with_backend(file_path, fallback_backend):
            ...
            self.status_changed.emit(f"Loaded: {os.path.basename(file_path)} ({fallback_backend.value} fallback)")
            print(f"[DEBUG_LOG] Video loaded with fallback {fallback_backend.value} backend: {file_path}")
            return True
    ...
    def _enable_controls(self):
        ...
        self.switch_backend_btn.setEnabled(VLC_AVAILABLE and self.current_video_file is not None)
    ...
    def start_stimulus_playback(self, screen_index: int = 0) -> bool:
        ...
        try:
            ...
            self.error_occurred.emit(f"Error starting enhanced stimulus playback: {str(e)}")
            return False
    ...
    def on_qt_media_error(self, error):
        ...
        if error in [QMediaPlayer.FormatError, QMediaPlayer.ServiceMissingError] and VLC_AVAILABLE:
    ...
    def stop_stimulus_playback(self, reason: str = "stopped"):
        ...
        try:
            ...
            self.error_occurred.emit(f"Error stopping enhanced stimulus playback: {str(e)}")
    ...
    def mark_event(self, label: str = ""):
        ...
        try:
            ...
            self.status_changed.emit(f"Enhanced Event Marker {marker_num} - {current_position/1000:.1f}s")
