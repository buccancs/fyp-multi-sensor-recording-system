...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QTextEdit, QTabWidget, QWidget, QGroupBox, QGridLayout,
...
class SessionReviewDialog(QDialog):
    ...
    def __init__(self, session_data: Dict, session_folder: str, parent=None):
        ...
        self.setWindowTitle(f"Session Review - {session_data.get('session', 'Unknown')}")
        self.setModal(True)
        self.resize(900, 700)
        ...
        print(f"[DEBUG_LOG] SessionReviewDialog initialized for session: {session_data.get('session', 'Unknown')}")
    ...
    def create_header(self, parent_layout):
        ...
        summary_label = QLabel(f"Started: {start_time} | Duration: {duration_str} | Status: {status}")
    ...
    def create_statistics_tab(self):
        ...
        stats_layout.addWidget(QLabel(self.session_data.get('session', 'Unknown')), row, 1)
        row += 1
        
        stats_layout.addWidget(QLabel("Session Name:"), row, 0)
        stats_layout.addWidget(QLabel(self.session_data.get('session_name', 'Unknown')), row, 1)
        row += 1
        ...
        device_names = [d.get('id', 'Unknown') for d in devices] if isinstance(devices, list) else []
    ...
    def on_file_selection_changed(self, current, previous):
        ...
        if current:
            ...
            if file_info:
                ...
                details_text += f"Modified: {file_info['modified'].strftime('%Y-%m-%d %H:%M:%S')}\n"
    ...
    def open_session_folder(self):
        ...
        try:
            ...
            print(f"[DEBUG_LOG] Failed to open session folder {self.session_folder}: {e}")
    ...
    def export_session_data(self):
        ...
        try:
            ...
            export_path = self.session_folder / f"{self.session_data.get('session', 'session')}_summary.json"
...
def show_session_review_dialog(session_data: Dict, session_folder: str, parent=None) -> Optional[SessionReviewDialog]:
    """
