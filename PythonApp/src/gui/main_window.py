...
class MainWindow(QMainWindow):
    ...
    def connect_stimulus_signals(self):
        ...
        self.stimulus_panel.start_recording_play_requested.connect(self.on_start_recording_play_requested)
        ...
        self.stimulus_controller.experiment_started.connect(self.on_stimulus_experiment_started)
        self.stimulus_controller.experiment_ended.connect(self.on_stimulus_experiment_ended)
    ...
    def connect_session_logger_signals(self):
        ...
        self.session_logger.session_started.connect(self.on_session_logger_session_started)
    ...
    def on_ack_received(self, device_id, cmd, success, message):
        ...
        if success:
            ...
            self.session_logger.log_error("command_failed", f"Command '{cmd}' failed: {message}", device_id)
    ...
    def on_start_recording_play_requested(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Experiment Start Error", f"Failed to start experiment: {str(e)}")
    ...
    def on_mark_event_requested(self):
        ...
        try:
            ...
            self.session_logger.log_error("marker_failed", f"Failed to add event marker: {str(e)}")
    ...
    def on_stimulus_experiment_started(self):
        ...
        media_name = "unknown"
        if hasattr(self.stimulus_controller, 'current_media_file') and self.stimulus_controller.current_media_file:
    ...
    def on_stimulus_experiment_ended(self):
        ...
        try:
            ...
            self.session_logger.log_error("stimulus_stop_failed", f"Error stopping recordings: {str(e)}")
    ...
    def on_session_logger_session_ended(self, session_id, duration):
        ...
        self.statusBar().showMessage(f"Session logging completed: {session_id} ({duration:.1f}s)")
        self.log_message(f"Session logging finalized for: {session_id} (duration: {duration:.1f}s)")
    ...
    def handle_start(self):
        ...
        if self.server_running:
            ...
            if hasattr(self.json_server, 'connected_devices'):
                connected_devices = [{"id": device_id, "type": "android_phone"} 
            ...
            self.session_logger.start_session(session_name=session_id, devices=connected_devices)
            
            # Log PC webcam device connection
            self.session_logger.log_device_connected("pc_webcam", "pc_webcam", ["video_recording"])
            ...
            if webcam_started:
                ...
                # Log webcam error
                self.session_logger.log_error("webcam_start_failed", "PC webcam failed to start recording")
        else:
        ...
        if self.server_running:
            ...
            if webcam_filepath and self.session_manager.get_current_session():
                try:
                    ...
                    filename = os.path.basename(webcam_filepath) if webcam_filepath else "unknown"
                    self.session_logger.log_file_received("pc_webcam", filename, file_size, "webcam_video")
                    
                except Exception as e:
                    self.log_message(f"Failed to add webcam file to session: {e}")
                    self.session_logger.log_error("file_save_failed", f"Failed to add webcam file to session: {e}", "pc_webcam")
