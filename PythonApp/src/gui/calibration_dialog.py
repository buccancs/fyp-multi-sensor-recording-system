...
class CalibrationDialog(QDialog):
    ...
    def start_calibration_session(self):
        ...
        try:
            ...
            result = self.calibration_manager.start_calibration_session(device_ids, session_name)
            ...
            if result.get('success', False):
                ...
                self.computation_status.setText("Session started. Ready to capture frames.")
            else:
                QMessageBox.critical(self, "Session Error", f"Failed to start session: {result.get('error', 'Unknown error')}")
                
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to start calibration session: {str(e)}")
    ...
    def capture_calibration_frame(self):
        ...
        try:
            ...
            result = self.calibration_manager.capture_calibration_frame(self.device_server)
            ...
            if result.get('success', False):
                ...
                # Add frame to list
                frame_item = QListWidgetItem(f"Frame {total_frames}: {result.get('timestamp', 'Unknown time')}")
                ...
                if total_frames >= 5:
                    ...
                    self.computation_status.setText(f"Ready to compute calibration ({total_frames} frames captured)")
                else:
                    self.computation_status.setText(f"Capture more frames (need at least 5, have {total_frames})")
                    
            else:
                QMessageBox.warning(self, "Capture Failed", f"Frame capture failed: {result.get('error', 'Unknown error')}")
    ...
    def compute_calibration(self):
        ...
        try:
            ...
            if result.get('success', False):
                ...
                QMessageBox.critical(self, "Calibration Failed", f"Calibration computation failed: {result.get('error', 'Unknown error')}")
                
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to compute calibration: {str(e)}")
        finally:
            self.compute_btn.setEnabled(True)
            self.computation_progress.setVisible(False)
    ...
    def save_calibration(self):
        ...
        try:
            if not self.device_results:
                QMessageBox.warning(self, "No Results", "No calibration results to save.")
                return
            ...
            if filename:
                ...
                QMessageBox.information(self, "Saved", f"Calibration results saved successfully.")
    ...
    def load_calibration(self):
        ...
        try:
            ...
            if filename:
                ...
                QMessageBox.information(self, "Loaded", "Calibration results loaded successfully.")
