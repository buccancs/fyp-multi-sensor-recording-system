    ...
    def test_capture_calibration_frame_success(self, mock_time):
        ...
        with patch.object(self.manager, '_receive_calibration_frames') as mock_receive:
            mock_receive.return_value = {
                'success': True,
                'frames': {
                    'device_1': {'rgb': np.zeros((480, 640, 3)), 'thermal': np.zeros((240, 320))},
                    'device_2': {'rgb': np.zeros((480, 640, 3)), 'thermal': np.zeros((240, 320))}
                }
            }
...
class TestCalibrationComputation:
            ...
            frame_data = {
                ...
                'frames': {
                    'device_1': {
                        ...
                        'thermal': np.random.randint(0, 255, (240, 320), dtype=np.uint8)
                    }
                }
            }
            self.manager.captured_frames.append(frame_data)
    
    @patch('src.calibration.calibration_processor.CalibrationProcessor.process_calibration')
    ...
    def test_calibration_quality_assessment(self, mock_process):
        ...
        assert 'quality_warnings' in result or result['results']['device_1'].rgb_reprojection_error > 1.0
...
class TestThermalOverlayFunctionality:
    ...
    def test_overlay_color_mapping(self):
        ...
        with patch('cv2.applyColorMap') as mock_colormap:
            mock_colormap.return_value = np.random.randint(0, 255, (240, 320, 3), dtype=np.uint8)
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_concurrent_operations(self):
        ...
        with patch.object(self.manager, '_receive_calibration_frames') as mock_receive:
            mock_receive.return_value = {
                'success': True,
                'frames': {'device_1': {'rgb': np.zeros((480, 640, 3)), 'thermal': np.zeros((240, 320))}}
            }
    ...
    def test_resource_cleanup_on_error(self):
        ...
        with patch.object(self.manager.calibration_processor, 'process_calibration') as mock_process:
...
class TestIntegrationScenarios:
    ...
    def test_complete_calibration_workflow(self):
        ...
        # 1. Start session
        session_result = self.manager.start_calibration_session(device_ids, "integration_test")
        ...
        with patch.object(self.manager, '_receive_calibration_frames') as mock_receive:
            ...
            # Capture 10 frames
            for i in range(10):
                capture_result = self.manager.capture_calibration_frame(self.mock_server)
    ...
    def test_multi_device_calibration_workflow(self):
        ...
        session_result = self.manager.start_calibration_session(device_ids, "multi_device_test")
        assert session_result['success'] is True
