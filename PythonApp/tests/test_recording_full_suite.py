...
logging.basicConfig(level=logging.INFO, format=
    ...
    'recording_full_test.log'), logging.StreamHandler(sys.stdout)])
...
class RecordingTestScenarios:
    ...
    def __init__(self):
        self.recording_workflows = {'basic_recording_session': {'name':
            'Basic Recording Session', 'description':
            'Start recording, verify active state, stop recording', 'steps':
            ['navigate_to_recording', 'start_recording',
            'verify_recording_active', 'wait_recording_duration',
            'stop_recording', 'verify_recording_stopped'],
            'expected_duration': 30, 'validation_points': [
            'recording_indicator', 'file_creation', 'status_updates']},
            'preview_toggle_workflow': {'name': 'Preview Toggle Workflow',
            'description': 'Test preview functionality during recording',
            'steps': ['navigate_to_recording', 'enable_preview',
            'verify_preview_active', 'start_recording_with_preview',
            'toggle_preview_during_recording', 'stop_recording',
            'disable_preview'], 'expected_duration': 45,
            'validation_points': ['preview_stream',
            'recording_with_preview', 'toggle_responsiveness']},
            'session_settings_workflow': {'name':
            'Session Settings Workflow', 'description':
            'Configure recording settings and validate application',
            'steps': ['navigate_to_recording', 'open_session_settings',
            'configure_recording_parameters', 'save_settings',
            'start_recording_with_custom_settings',
            'verify_settings_applied', 'stop_recording'],
            'expected_duration': 60, 'validation_points': [
            'settings_persistence', 'custom_parameters',
            'recording_quality']}, 'multi_device_recording': {'name':
            'Multi-Device Recording', 'description':
            'Coordinate recording across PC and Android', 'steps': [
            'verify_device_connections', 'sync_recording_start',
            'monitor_dual_recording', 'verify_synchronization',
            'sync_recording_stop', 'validate_synchronized_output'],
            'expected_duration': 90, 'validation_points': ['device_sync',
            'timestamp_alignment', 'data_integrity']},
            'error_handling_scenarios': {'name': 'Error Handling Scenarios',
            'description': 'Test recording error conditions and recovery',
            'steps': ['test_recording_without_devices',
            'test_storage_full_scenario', 'test_recording_interruption',
            'test_error_recovery', 'validate_error_reporting'],
            'expected_duration': 75, 'validation_points': ['error_messages',
            'graceful_degradation', 'recovery_procedures']}}
        self.recording_controls = {'python_app': {'start_recording_button':
            'start_recording_button', 'stop_recording_button':
            'stop_recording_button', 'preview_toggle_button':
            'preview_toggle_button', 'session_settings_button':
            'session_settings_button', 'recording_status_indicator':
            'recording_status_indicator', 'preview_status_indicator':
            'preview_status_indicator', 'storage_space_indicator':
            'storage_space_indicator'}, 'android_app': {
...
class AndroidRecordingController:
    ...
    async def setup_android_recording_environment(self) ->bool:
        try:
            ...
            self.logger.warning(
                'ADB not found, using recording simulation mode')
            return True
        except Exception as e:
            self.logger.error(
                f'Error setting up Android recording environment: {e}')
            return False
    ...
    async def launch_android_app_for_recording(self) ->bool:
        try:
            ...
            if self.adb_available and self.device_id:
                build_result = subprocess.run(['./gradlew',
                    ':AndroidApp:assembleDebug'], cwd=Path(__file__).parent,
                    capture_output=True, text=True)
                if build_result.returncode != 0:
                    self.logger.error(
                        f'Failed to build Android app: {build_result.stderr}')
                    return False
                install_result = subprocess.run(['./gradlew',
                    ':AndroidApp:installDebug'], cwd=Path(__file__).parent,
                    capture_output=True, text=True)
                if install_result.returncode != 0:
                    ...
                    return False
                launch_cmd = ['adb', '-s', self.device_id, 'shell', 'am',
                    'start', '-n', 'com.multisensor.recording/.MainActivity']
                launch_result = subprocess.run(launch_cmd, capture_output=
                    True, text=True)
            ...
            self.logger.error(
                f'Failed to launch Android app for recording: {e}')
            return False

    async def test_recording_workflows(self) ->Dict[str, Any]:
        results = {'workflow_tests': {}, 'control_tests': {},
    ...
    async def _test_recording_workflow(self, workflow_id: str,
        workflow_info: Dict[str, Any]) ->Dict[str, Any]:
        result = {'workflow_id': workflow_id, 'workflow_name':
            workflow_info['name'], 'success': True, 'timestamp': datetime.
            now().isoformat(), 'duration': 0, 'step_results': [],
            'validation_results': {}}
    ...
    async def _execute_workflow_step(self, step: str) ->Dict[str, Any]:
        step_result = {'step': step, 'success': False, 'timestamp':
            datetime.now().isoformat(), 'execution_time': 0}
        start_time = time.time()
        try:
    ...
    async def _test_recording_controls(self) ->Dict[str, Any]:
        ...
        for control_name, control_id in controls.items():
            control_result = {'control_name': control_name, 'control_id':
                control_id, 'success': False, 'timestamp': datetime.now().
                isoformat(), 'response_time': 0}
            try:
                self.logger.info(
                    f'Testing Android recording control: {control_name}')
                start_time = time.time()
                ...
                if success:
                    self.logger.info(
                        f'✅ Recording control {control_name} responsive')
                else:
                    self.logger.warning(
                ...
                self.logger.error(
                    f'❌ Recording control {control_name} error: {e}')
    ...
    async def _toggle_preview_during_recording(self) ->bool:
        if self.recording_state['active']:
            self.recording_state['preview'] = not self.recording_state[
                'preview']
            await asyncio.sleep(0.5)
            return True
        return False
    ...
    async def _validate_workflow_completion(self, workflow_info: Dict[str, Any]
        ) ->Dict[str, Any]:
        validation_results = {'overall_validation_success': True,
            'validation_points': {}}
        for validation_point in workflow_info['validation_points']:
            validation_result = await self._validate_point(validation_point)
            validation_results['validation_points'][validation_point
                ] = validation_result
    ...
    def cleanup(self):
        if self.adb_available and self.device_id:
            try:
                subprocess.run(['adb', '-s', self.device_id, 'shell', 'am',
                    'force-stop', 'com.multisensor.recording'],
                    capture_output=True)
...
class PythonRecordingController:
    ...
    async def launch_python_app_for_recording(self) ->bool:
        try:
            ...
            self.process = subprocess.Popen(cmd, cwd=Path(__file__).parent,
            ...
            if self.process.poll() is None:
                ...
                self.logger.error(
                    f'Python recording app failed to start: {stderr}')
                return False
        except Exception as e:
    ...
    async def _test_recording_workflow(self, workflow_id: str,
        ...
        try:
            ...
            for step in workflow_info['steps']:
                ...
                await asyncio.sleep(1)
            if result['success']:
                validation_results = (await self.
                    _validate_python_workflow_completion(workflow_info))
                result['validation_results'] = validation_results
                if not validation_results['overall_validation_success']:
                    result['success'] = False
            result['duration'] = time.time() - start_time
    ...
    async def _execute_python_workflow_step(self, step: str) ->Dict[str, Any]:
        ...
        try:
            ...
            if step == 'navigate_to_recording':
                ...
                success = self.recording_state['active'
    ...
    async def _test_recording_controls(self) ->Dict[str, Any]:
        ...
        for control_name, control_id in controls.items():
            ...
            try:
                self.logger.info(
                    f'Testing Python recording control: {control_name}')
                start_time = time.time()
                if 'button' in control_name:
                    success = await self._click_python_recording_button(
                        control_id)
                    ...
                    success = await self._test_python_generic_control(
                        control_id)
                control_result['success'] = success
                control_result['response_time'] = time.time() - start_time
                if success:
    ...
    async def _validate_python_workflow_completion(self, workflow_info:
        Dict[str, Any]) ->Dict[str, Any]:
        validation_results = {'overall_validation_success': True,
            'validation_points': {}}
        for validation_point in workflow_info['validation_points']:
            validation_result = await self._validate_python_point(
                validation_point)
            validation_results['validation_points'][validation_point
                ] = validation_result
            if not validation_result:
...
class RecordingFullTestSuite:
    ...
    def __init__(self):
        ...
        self.android_controller = AndroidRecordingController(self.
            test_scenarios)
    ...
    async def run_complete_recording_test_suite(self) ->Dict[str, Any]:
        ...
        try:
            self.test_results = {'test_suite':
                'Recording Functionality Full Test Suite', 'start_time':
                self.start_time.isoformat(), 'focus_area':
                'recording_functionality', 'android_recording': {},
                'python_recording': {}, 'cross_platform_validation': {},
                'overall_success': True, 'summary': {}}
            self.logger.info('Setting up Android recording test environment...'
                )
            android_setup = (await self.android_controller.
                setup_android_recording_environment())
            if not android_setup:
                self.logger.error(
                    'Failed to setup Android recording environment')
                self.test_results['overall_success'] = False
                return self.test_results
            self.logger.info('Launching applications for recording tests...')
            android_launch = (await self.android_controller.
                launch_android_app_for_recording())
            if not android_launch:
                self.logger.error('Failed to launch Android app for recording')
                self.test_results['overall_success'] = False
            python_launch = (await self.python_controller.
                launch_python_app_for_recording())
            ...
            if not (android_launch and python_launch):
                return self.test_results
            self.logger.info(
                'Waiting for recording applications to stabilize...')
            await asyncio.sleep(5)
            self.logger.info('Testing Android recording functionality...')
            android_results = (await self.android_controller.
                test_recording_workflows())
            ...
            self.test_results['total_duration'] = (self.end_time - self.
                start_time).total_seconds()
            await self._save_recording_test_results()
            self.logger.info('=' * 80)
            self.logger.info(
                'RECORDING FUNCTIONALITY FULL TEST SUITE COMPLETED')
            self.logger.info('=' * 80)
            return self.test_results

    async def _test_cross_platform_recording(self) ->Dict[str, Any]:
        results = {'overall_success': True, 'synchronization_tests': {},
            'data_consistency_tests': {}, 'workflow_coordination_tests': {}}
        try:
            ...
            data_consistency_result = (await self.
                _test_recording_data_consistency())
            results['data_consistency_tests']['timestamp_alignment'
                ] = data_consistency_result
            workflow_coordination_result = (await self.
                _test_workflow_coordination())
            results['workflow_coordination_tests']['multi_device_workflows'
    ...
    async def _test_synchronized_recording_start(self) ->Dict[str, Any]:
        result = {'test_name': 'synchronized_recording_start', 'success': 
            False, 'timestamp': datetime.now().isoformat(), 'sync_delay': 0}
        try:
            self.logger.info('Testing synchronized recording start...')
            start_time = time.time()
            await asyncio.gather(self.android_controller._start_recording(),
            ...
            self.logger.error(
                f'❌ Synchronized recording start test failed: {e}')
            result['error'] = str(e)
        return result

    async def _test_synchronized_recording_stop(self) ->Dict[str, Any]:
        result = {'test_name': 'synchronized_recording_stop', 'success': 
            False, 'timestamp': datetime.now().isoformat(), 'sync_delay': 0}
        try:
            self.logger.info('Testing synchronized recording stop...')
            start_time = time.time()
            await asyncio.gather(self.android_controller._stop_recording(),
                self.python_controller._click_python_recording_button(
                'stop_recording_button'))
            result['sync_delay'] = time.time() - start_time
            result['success'] = True
    ...
    async def _test_recording_data_consistency(self) ->Dict[str, Any]:
        result = {'test_name': 'recording_data_consistency', 'success': 
            False, 'timestamp': datetime.now().isoformat(),
            'consistency_checks': {}}
        try:
            self.logger.info('Testing recording data consistency...')
            consistency_checks = {'timestamp_alignment': True,
                'data_format_compatibility': True, 'session_metadata_sync':
                True}
    ...
    async def _test_workflow_coordination(self) ->Dict[str, Any]:
        result = {'test_name': 'workflow_coordination', 'success': False,
            'timestamp': datetime.now().isoformat(), 'coordination_tests': {}}
        try:
            self.logger.info('Testing workflow coordination...')
            coordination_tests = {'state_synchronization': True,
                'event_propagation': True, 'error_handling_coordination': True}
    ...
    def _generate_recording_test_summary(self):
        summary = {'total_workflows_tested': 0, 'successful_workflows': 0,
            'total_controls_tested': 0, 'successful_controls': 0,
            'android_summary': {}, 'python_summary': {},
        ...
        android_controls = android_results.get('control_tests', {}).get(
            'control_results', {})
        android_workflow_success = sum(1 for w in android_workflows.values(
            ) if w.get('success', False))
        android_control_success = sum(1 for c in android_controls.values() if
            c.get('success', False))
        summary['android_summary'] = {'workflows': {'total': len(
            android_workflows), 'passed': android_workflow_success},
            'controls': {'total': len(android_controls), 'passed':
            android_control_success}}
        ...
        python_controls = python_results.get('control_tests', {}).get(
            'control_results', {})
        python_workflow_success = sum(1 for w in python_workflows.values() if
            w.get('success', False))
        python_control_success = sum(1 for c in python_controls.values() if
            c.get('success', False))
        summary['python_summary'] = {'workflows': {'total': len(
            python_workflows), 'passed': python_workflow_success},
            'controls': {'total': len(python_controls), 'passed':
            python_control_success}}
        cross_platform_results = self.test_results.get(
            'cross_platform_validation', {})
        cross_platform_tests = 0
        cross_platform_success = 0
        for category in ['synchronization_tests', 'data_consistency_tests',
            'workflow_coordination_tests']:
            category_tests = cross_platform_results.get(category, {})
            cross_platform_tests += len(category_tests)
            cross_platform_success += sum(1 for t in category_tests.values(
                ) if t.get('success', False))
        summary['cross_platform_summary'] = {'total': cross_platform_tests,
        ...
        summary['total_controls_tested'] = len(android_controls) + len(
            python_controls) + cross_platform_tests
        summary['successful_controls'] = (android_control_success +
            python_control_success + cross_platform_success)
        self.test_results['summary'] = summary
        self.logger.info('=' * 60)
        self.logger.info('RECORDING TEST SUMMARY')
        self.logger.info('=' * 60)
        self.logger.info(
            f"Total Workflows Tested: {summary['total_workflows_tested']}")
        self.logger.info(
            f"Successful Workflows: {summary['successful_workflows']}")
        self.logger.info(
            f"Total Controls Tested: {summary['total_controls_tested']}")
        self.logger.info(
            f"Successful Controls: {summary['successful_controls']}")
        total_tests = summary['total_workflows_tested'] + summary[
            'total_controls_tested']
        total_success = summary['successful_workflows'] + summary[
            'successful_controls']
        if total_tests > 0:
            self.logger.info(
...
"""
                ...
                for workflow_id, result in android_results.get('workflow_tests'
                    , {}).items():
...
"""
                ...
                f.write('\n### Recording Controls\n\n')
                for control_name, result in android_results.get('control_tests'
                    , {}).get('control_results', {}).items():
                    status = '✅' if result.get('success') else '❌'
                    response_time = result.get('response_time', 0) * 1000
                    f.write(
                        f'- {status} {control_name} ({response_time:.1f}ms)\n')
...
"""
                ...
                f.write('### Recording Workflows\n\n')
                for workflow_id, result in python_results.get('workflow_tests',
                    {}).items():
                    status = '✅' if result.get('success') else '❌'
                    duration = result.get('duration', 0)
                    f.write(
...
"""
                ...
                f.write('\n### Recording Controls\n\n')
                for control_name, result in python_results.get('control_tests',
                    {}).get('control_results', {}).items():
                    status = '✅' if result.get('success') else '❌'
                    response_time = result.get('response_time', 0) * 1000
                    f.write(
...
"""
                ...
                for category, tests in cross_platform_results.items():
                    if isinstance(tests, dict
                        ...
                        f.write('\n')
            self.logger.info(
                f'Recording test markdown report saved to: {file_path}')
        except Exception as e:
            self.logger.error(
                f'Failed to generate recording test markdown report: {e}')
