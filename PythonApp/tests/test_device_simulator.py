...
class DeviceSimulator:
    ...
    def send_message(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"Device {self.device_id} sent message: {message.get('type', 'unknown')}")
            return True
    ...
    def send_preview_frame(self, frame_type: str = "rgb", 
    ...
    def send_ack(self, command: str, success: bool = True, 
                message: str = "Command executed successfully") -> bool:
        """
    ...
    def send_notification(self, event_type: str, event_data: Dict[str, Any] = None) -> bool:
        """
    ...
    def receive_message(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Device {self.device_id} received: {message.get('type', 'unknown')}")
            return message
    ...
    def run_simulation(self, duration: int = 30):
        ...
        logger.info(f"Device {self.device_id} starting simulation for {duration} seconds")
        ...
        try:
            while self.running and (time.time() - start_time) < duration:
                ...
                if current_time - last_status >= 5:
                    battery = max(10, 100 - int((current_time - start_time) * 2))  # Simulate battery drain
                ...
                try:
                    ...
                    if message and message.get('type') == 'command':
                        ...
                        logger.info(f"Device {self.device_id} received command: {command}")
                        
                        # Send acknowledgment
                        if command in ['start_recording', 'stop_recording', 'capture_calibration']:
                            self.send_ack(command, success=True)
                        else:
                            self.send_ack(command, success=False, message="Unknown command")
