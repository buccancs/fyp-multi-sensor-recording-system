...
logging.basicConfig(level=logging.INFO, format=
    ...
    'ide_integration_test.log'), logging.StreamHandler(sys.stdout)])
...
class NavigationGraph:
    ...
    def __init__(self):
        self.navigation_flow = {'nav_recording': {'name': 'Recording',
            'type': 'fragment', 'buttons': ['start_recording',
            'stop_recording', 'preview_toggle'], 'can_navigate_to': [
            'nav_devices', 'nav_calibration', 'nav_files'], 'required_for':
            'core_recording_functionality'}, 'nav_devices': {'name':
            'Devices', 'type': 'fragment', 'buttons': ['connect_devices',
            'scan_devices', 'device_settings'], 'can_navigate_to': [
            'nav_recording', 'nav_calibration', 'nav_files'],
            'required_for': 'device_management'}, 'nav_calibration': {
            'name': 'Calibration', 'type': 'fragment', 'buttons': [
            'start_calibration', 'calibration_settings', 'view_results'],
            'can_navigate_to': ['nav_recording', 'nav_devices', 'nav_files'
            ], 'required_for': 'camera_calibration'}, 'nav_files': {'name':
            'Files', 'type': 'fragment', 'buttons': ['browse_files',
            'export_data', 'delete_session'], 'can_navigate_to': [
            'nav_recording', 'nav_devices', 'nav_calibration'],
            'required_for': 'data_management'}, 'nav_settings': {'name':
            'Settings', 'type': 'activity', 'buttons': ['save_settings',
            'reset_settings', 'back'], 'can_navigate_to': ['main_activity'],
            'required_for': 'app_configuration'}, 'nav_network': {'name':
            'Network Config', 'type': 'activity', 'buttons': [
            'configure_network', 'test_connection', 'back'],
            'can_navigate_to': ['main_activity'], 'required_for':
            'network_setup'}, 'nav_shimmer': {'name': 'Shimmer Config',
            'type': 'activity', 'buttons': ['configure_shimmer',
            'test_sensors', 'back'], 'can_navigate_to': ['main_activity'],
            'required_for': 'sensor_configuration'}, 'bottom_nav_recording':
            {'name': 'Record (Bottom Nav)', 'type': 'bottom_nav', 'target':
            'nav_recording', 'buttons': ['quick_record'], 'required_for':
            'quick_access'}, 'bottom_nav_monitor': {'name':
            'Monitor (Bottom Nav)', 'type': 'bottom_nav', 'target':
            'nav_devices', 'buttons': ['monitor_devices'], 'required_for':
            'device_monitoring'}, 'bottom_nav_calibrate': {'name':
            'Calibrate (Bottom Nav)', 'type': 'bottom_nav', 'target':
            'nav_calibration', 'buttons': ['quick_calibrate'],
            'required_for': 'quick_calibration'}}
        self.test_sequence = ['nav_recording', 'nav_devices',
            'nav_calibration', 'nav_files', 'nav_settings', 'nav_network',
            'nav_shimmer', 'bottom_nav_recording', 'bottom_nav_monitor',
...
class PythonAppController:
    ...
    def __init__(self):
        ...
        self.ui_elements = {'tabs': ['Recording', 'Devices', 'Calibration',
            'Files'], 'recording_tab_buttons': ['start_recording',
            'stop_recording', 'preview'], 'devices_tab_buttons': [
            'connect_pc', 'connect_android', 'connect_shimmer',
            'scan_devices'], 'calibration_tab_buttons': [
            'start_calibration', 'load_calibration', 'save_calibration'],
            'files_tab_buttons': ['export_data', 'open_folder',
            'delete_session']}
    ...
    async def test_ui_elements(self) ->Dict[str, Any]:
        results = {'tab_navigation': {}, 'button_interactions': {},
        ...
        try:
            ...
            for tab, buttons in [(k, v) for k, v in self.ui_elements.items(
                ) if k.endswith('_buttons')]:
    ...
    async def _test_tab_navigation(self, tab_name: str) ->Dict[str, Any]:
        result = {'tab': tab_name, 'success': False, 'timestamp': datetime.
    ...
    async def _test_buttons(self, tab_name: str, buttons: List[str]) ->Dict[
        str, Any]:
        results = {}
        for button in buttons:
            button_result = {'button': button, 'success': False,
                'timestamp': datetime.now().isoformat(), 'response_time': 0}
            try:
...
class AndroidAppController:
    ...
    async def launch_through_intellij(self) ->bool:
        try:
            ...
            if self.adb_available and self.device_id:
                ...
                build_result = subprocess.run(build_cmd, cwd=Path(__file__)
                    .parent, capture_output=True, text=True)
                if build_result.returncode != 0:
                    self.logger.error(
                        f'Failed to build Android app: {build_result.stderr}')
                    return False
                install_cmd = ['./gradlew', ':AndroidApp:installDebug']
                install_result = subprocess.run(install_cmd, cwd=Path(
                ...
                if launch_result.returncode == 0:
                    self.logger.info(
                        'Android app launched successfully on device')
    ...
    async def test_navigation_graph(self) ->Dict[str, Any]:
        results = {'navigation_tests': {}, 'button_tests': {},
            'flow_validation': {}, 'overall_success': True}
        try:
            for destination in self.navigation_graph.test_sequence:
                self.logger.info(
                    f'Testing navigation destination: {destination}')
                nav_result = await self._test_navigation_destination(
                    destination)
                results['navigation_tests'][destination] = nav_result
                if not nav_result['success']:
                    results['overall_success'] = False
                button_result = await self._test_destination_buttons(
                    destination)
    ...
    async def _test_navigation_destination(self, destination: str) ->Dict[
        str, Any]:
        result = {'destination': destination, 'success': False, 'timestamp':
            datetime.now().isoformat(), 'navigation_method': '', 'load_time': 0
            }
        try:
            dest_info = self.navigation_graph.navigation_flow[destination]
            self.logger.info(
                f"Navigating to {dest_info['name']} ({dest_info['type']})")
            start_time = time.time()
    ...
    async def _test_destination_buttons(self, destination: str) ->Dict[str, Any
        ...
        results = {'destination': destination, 'button_results': {},
            'overall_success': True}
        for button in buttons:
            button_result = {'button': button, 'success': False,
                'timestamp': datetime.now().isoformat(), 'response_time': 0}
            try:
                self.logger.info(
                    f"Testing button: {button} in {dest_info['name']}")
                start_time = time.time()
    ...
    async def _test_navigation_flows(self, source_destination: str) ->Dict[
        str, Any]:
        ...
        results = {'source': source_destination, 'flow_tests': {},
            'overall_success': True}
        for target in possible_destinations:
            flow_result = {'target': target, 'success': False, 'timestamp':
                datetime.now().isoformat(), 'flow_time': 0}
            try:
                self.logger.info(
                    f'Testing flow: {source_destination} -> {target}')
                start_time = time.time()
                ...
                self.logger.info(
                    f'Flow {source_destination} -> {target} successful')
            except Exception as e:
                self.logger.error(
                    f'Flow {source_destination} -> {target} failed: {e}')
...
class IDEIntegrationTestSuite:
    ...
    async def run_complete_test_suite(self) ->Dict[str, Any]:
        ...
        self.logger.info('=' * 80)
        try:
            self.test_results = {'test_suite': 'IDE Integration Test Suite',
                'start_time': self.start_time.isoformat(), 'python_app': {},
                'android_app': {}, 'overall_success': True, 'summary': {}}
    ...
    def _generate_test_summary(self):
        summary = {'total_tests': 0, 'passed_tests': 0, 'failed_tests': 0,
            'python_app_summary': {}, 'android_app_summary': {},
            'navigation_coverage': {}}
        ...
        python_tab_passed = sum(1 for result in python_results.get(
            'tab_navigation', {}).values() if result.get('success', False))
        python_button_passed = sum(sum(1 for btn_result in buttons.values() if
            btn_result.get('success', False)) for buttons in python_results
            .get('button_interactions', {}).values())
        summary['python_app_summary'] = {'tab_tests': {'total':
            python_tab_tests, 'passed': python_tab_passed}, 'button_tests':
            {'total': python_button_tests, 'passed': python_button_passed}}
        android_results = self.test_results.get('android_app', {})
        android_nav_tests = len(android_results.get('navigation_tests', {}))
        android_button_tests = sum(len(result.get('button_results', {})) for
            result in android_results.get('button_tests', {}).values())
        android_flow_tests = sum(len(result.get('flow_tests', {})) for
            result in android_results.get('flow_validation', {}).values())
        android_nav_passed = sum(1 for result in android_results.get(
            'navigation_tests', {}).values() if result.get('success', False))
        android_button_passed = sum(sum(1 for btn_result in result.get(
            'button_results', {}).values() if btn_result.get('success', 
            False)) for result in android_results.get('button_tests', {}).
            values())
        android_flow_passed = sum(sum(1 for flow_result in result.get(
            'flow_tests', {}).values() if flow_result.get('success', False)
            ) for result in android_results.get('flow_validation', {}).values()
            )
        summary['android_app_summary'] = {'navigation_tests': {'total':
            android_nav_tests, 'passed': android_nav_passed},
            'button_tests': {'total': android_button_tests, 'passed':
            android_button_passed}, 'flow_tests': {'total':
            android_flow_tests, 'passed': android_flow_passed}}
        ...
        summary['failed_tests'] = summary['total_tests'] - summary[
            'passed_tests']
        total_destinations = len(self.navigation_graph.navigation_flow)
        tested_destinations = len(android_results.get('navigation_tests', {}))
        summary['navigation_coverage'] = {'total_destinations':
            total_destinations, 'tested_destinations': tested_destinations,
            'coverage_percentage': tested_destinations / total_destinations *
            100 if total_destinations > 0 else 0}
    ...
    async def _save_test_results(self):
        try:
            ...
            results_file = (results_dir /
                f'ide_integration_test_results_{timestamp}.json')
            with open(results_file, 'w') as f:
                json.dump(self.test_results, f, indent=2, default=str)
            self.logger.info(f'Test results saved to: {results_file}')
            summary_file = (results_dir /
                f'ide_integration_summary_{timestamp}.md')
...
"""
                ...
                for tab, result in python_results.get('tab_navigation', {}
                    ).items():
                    status = '✅' if result.get('success') else '❌'
                    f.write(f'- {status} {tab}\n')
                f.write('\n### Button Interactions\n\n')
                for tab, buttons in python_results.get('button_interactions',
                    {}).items():
...
"""
                ...
                for dest, result in android_results.get('navigation_tests', {}
                    ).items():
                    status = '✅' if result.get('success') else '❌'
                    method = result.get('navigation_method', 'unknown')
                    time_ms = result.get('load_time', 0) * 1000
                    f.write(
                        f'- {status} {dest} via {method} ({time_ms:.1f}ms)\n')
                f.write('\n### Button Tests\n\n')
                for dest, result in android_results.get('button_tests', {}
                    ).items():
                    f.write(f'#### {dest}\n\n')
                    for button, btn_result in result.get('button_results', {}
                        ).items():
                ...
                for source, result in android_results.get('flow_validation', {}
                    ).items():
                    f.write(f'#### From {source}\n\n')
                    for target, flow_result in result.get('flow_tests', {}
                        ).items():
