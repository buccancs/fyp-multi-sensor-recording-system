...
def main():
    parser = argparse.ArgumentParser(description=
        'Hand Segmentation CLI Tool for Post-Session Processing',
...
Examples:
  ...
  python hand_segmentation_cli.py process-session SESSION_ID \\
    ...
    parser.add_argument('--recordings-dir', default='recordings', help=
        'Base directory containing session recordings (default: recordings)')
    subparsers = parser.add_subparsers(dest='command', help=
        'Available commands')
    list_parser = subparsers.add_parser('list-sessions', help=
        'List available sessions that contain videos')
    session_parser = subparsers.add_parser('process-session', help=
        'Process all videos in a session')
    session_parser.add_argument('session_id', help='Session ID to process')
    add_processing_args(session_parser)
    video_parser = subparsers.add_parser('process-video', help=
        'Process a single video file')
    video_parser.add_argument('video_path', help='Path to video file')
    video_parser.add_argument('--output-dir', help=
        'Output directory (default: same directory as video)')
    ...
    cleanup_parser = subparsers.add_parser('cleanup', help=
        'Clean up segmentation outputs for a session')
...
def add_processing_args(parser):
    parser.add_argument('--method', choices=['mediapipe', 'color_based',
        'contour_based'], default='mediapipe', help=
        'Segmentation method to use (default: mediapipe)')
    parser.add_argument('--confidence', type=float, default=0.5, help=
    ...
    parser.add_argument('--max-hands', type=int, default=2, help=
        'Maximum number of hands to detect (default: 2)')
    parser.add_argument('--output-cropped', action='store_true', help=
        'Generate cropped hand region videos')
    parser.add_argument('--output-masks', action='store_true', help=
        'Generate hand mask videos')
    parser.add_argument('--crop-padding', type=int, default=20, help=
        'Padding around detected hand regions (default: 20)')
...
def cmd_list_sessions(processor: SessionPostProcessor) ->int:
    ...
    for session in sessions:
        ...
        processed_count = sum(1 for is_processed in status.values() if
            is_processed)
        if processed_count > 0:
            print(
                f'    - {processed_count}/{len(videos)} videos already processed'
                )
    return 0
...
def cmd_process_session(processor: SessionPostProcessor, args) ->int:
    ...
    if args.session_id not in sessions:
        ...
        print(
            f"Available sessions: {', '.join(sessions) if sessions else 'None'}"
            )
        return 1
    config_kwargs = {'min_detection_confidence': args.confidence,
        'min_tracking_confidence': args.tracking_confidence,
        'max_num_hands': args.max_hands, 'output_cropped': args.
        output_cropped, 'output_masks': args.output_masks, 'crop_padding':
        args.crop_padding}
    results = processor.process_session(args.session_id, args.method, **
        config_kwargs)
    if results:
...
def cmd_process_video(processor: SessionPostProcessor, args) ->int:
    ...
    config_kwargs = {'min_detection_confidence': args.confidence,
        ...
        output_cropped, 'output_masks': args.output_masks, 'crop_padding':
        args.crop_padding}
    result = processor.process_video_file(str(video_path), args.output_dir,
        args.method, **config_kwargs)
    if result.success:
