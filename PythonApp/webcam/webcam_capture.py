...
class WebcamCapture(QThread):
    ...
    def initialize_camera(self) ->bool:
        try:
            ...
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, self.
    ...
    def start_recording(self, session_id: str) ->bool:
        ...
        try:
            ...
            self.recording_filepath = os.path.join(self.output_directory,
                filename)
            self.video_writer = cv2.VideoWriter(self.recording_filepath,
                self.recording_codec, self.recording_fps, self.
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                if (self.is_previewing and current_time - last_frame_time >=
                    self.frame_interval):
    ...
    def frame_to_pixmap(self, frame: np.ndarray, max_width: int=640,
        max_height: int=480) ->Optional[QPixmap]:
        try:
    ...
    def set_recording_parameters(self, fps: int=30, resolution: tuple=(1280,
        720), codec: str='mp4v'):
    ...
    def cleanup(self):
        try:
            ...
            if hasattr(self, '_finished'
                ) and not self._finished and self.isRunning():
                self.quit()
                self.wait(1000)
            if self.cap:
                self.cap.release()
...
def test_webcam_access():
    ...
    if ret:
        ...
        print(
            f'[DEBUG_LOG] SUCCESS: Webcam accessible, frame size: {width}x{height}'
            )
    else:
