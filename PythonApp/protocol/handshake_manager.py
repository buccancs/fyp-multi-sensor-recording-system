...
class HandshakeManager:
    ...
    def process_handshake(self, handshake_message: Dict[str, Any]) ->Tuple[
        bool, str]:
        try:
            ...
            compatible = self._are_versions_compatible(client_protocol_version,
                self.protocol_version)
            if not compatible:
                message = (
                    f'Protocol version mismatch: client v{client_protocol_version}, server v{self.protocol_version}'
                    )
    ...
    def send_handshake_ack(self, sock: socket.socket, compatible: bool,
        message: str='') ->bool:
        try:
    ...
    def process_handshake_ack(self, ack_message: Dict[str, Any]) ->bool:
        try:
            ...
            logger.info(
                f'Received handshake ack from {server_name} v{server_version}')
    ...
    def _create_handshake_message(self) ->Dict[str, Any]:
        return self.schema_manager.create_message('handshake',
            protocol_version=self.protocol_version, device_name=self.
            _get_device_name(), app_version=self.app_version, device_type='pc')

    def _create_handshake_ack(self, compatible: bool, message: str='') ->Dict[
        str, Any]:
        ack_data = {'protocol_version': self.protocol_version,
            'server_name': 'Python PC Controller', 'server_version': self.
            app_version, 'compatible': compatible}
        if message:
    ...
    def _are_versions_compatible(self, client_version: int, server_version: int
        ) ->bool:
...
def send_handshake_ack(sock: socket.socket, compatible: bool, message: str=''
    ) ->bool:
