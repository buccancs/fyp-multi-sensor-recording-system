...
class SessionSynchronizer:
    ...
    def __init__(self, message_sender: Optional[Callable]=None):
        ...
        logger.info('[SessionSynchronizer] Initialized with sync interval: %ss'
            , self.sync_interval)
    ...
    def register_device(self, device_id: str):
        with self.sync_lock:
            if device_id not in self.sync_states:
                ...
                logger.info('[SessionSynchronizer] Device registered: %s',
                    device_id)
    ...
    def unregister_device(self, device_id: str):
        with self.sync_lock:
            if device_id in self.sync_states:
                ...
                logger.info('[SessionSynchronizer] Device unregistered: %s',
                    device_id)
    ...
    def sync_session_state(self, android_state: Dict[str, Any]) ->bool:
        try:
            ...
            if not device_id:
                logger.error(
                    '[SessionSynchronizer] No device_id in android_state')
                return False
            with self.sync_lock:
                ...
                if old_state and self._has_significant_changes(old_state,
                    session_state):
                    logger.info(
            ...
            logger.error(
                '[SessionSynchronizer] Failed to sync session state: %s',
                str(e))
            if device_id:
                with self.sync_lock:
    ...
    def handle_android_disconnect(self, device_id: str):
        with self.sync_lock:
            if device_id in self.sync_states:
                ...
                logger.warning('[SessionSynchronizer] Device disconnected: %s',
                    device_id)
                self._start_message_queuing(device_id)

    def recover_session_on_reconnect(self, device_id: str) ->Optional[
        SessionState]:
        with self.sync_lock:
            if device_id in self.offline_devices:
                offline_duration = (datetime.now() - self.offline_devices[
                    device_id]).total_seconds()
                logger.info(
                ...
                self.sync_states[device_id] = SessionSyncState.SYNCHRONIZED
                logger.info(
                    '[SessionSynchronizer] Session recovery completed for %s',
                    device_id)
                return last_state
            else:
                logger.info(
    ...
    def queue_message(self, device_id: str, message_type: str, payload:
        Dict[str, Any], priority: MessagePriority=MessagePriority.NORMAL):
        if device_id not in self.message_queues:
            self.register_device(device_id)
        message = QueuedMessage(device_id=device_id, message_type=
            message_type, payload=payload, priority=priority, timestamp=
            datetime.now())
    ...
    def get_sync_status(self) ->Dict[str, Any]:
        with self.sync_lock:
            ...
            for device_id in self.sync_states:
                ...
                queue_size = self.message_queues[device_id].qsize(
                    ) if device_id in self.message_queues else 0
            ...
            return {'running': self.is_running, 'sync_interval': self.
                ...
                sync_attempts, 'sync_successes': self.sync_successes,
                'sync_failures': self.sync_failures, 'success_rate_percent':
                success_rate, 'messages_queued': self.messages_queued,
                'messages_delivered': self.messages_delivered}, 'devices':
                device_status, 'total_devices': len(self.sync_states),
                'online_devices': len(self.sync_states) - len(self.
                offline_devices), 'offline_devices': len(self.offline_devices)}
    ...
    def _sync_loop(self):
        ...
        while self.is_running:
            try:
                ...
                logger.error('[SessionSynchronizer] Error in sync loop: %s',
                    str(e))
                time.sleep(1.0)
        logger.info('[SessionSynchronizer] Sync loop stopped')

    def _convert_android_state(self, android_state: Dict[str, Any]
        ) ->SessionState:
        return SessionState(session_id=android_state.get('session_id', ''),
            recording_active=android_state.get('recording_active', False),
            devices_connected=android_state.get('devices_connected', {}),
            recording_start_time=self._parse_datetime(android_state.get(
            'recording_start_time')), recording_duration=android_state.get(
            'recording_duration', 0.0), file_count=android_state.get(
            'file_count', 0), total_file_size=android_state.get(
            'total_file_size', 0), calibration_status=android_state.get(
            'calibration_status', {}), sync_timestamp=datetime.now(),
    ...
    def _has_significant_changes(self, old_state: SessionState, new_state:
        SessionState) ->bool:
        return (old_state.recording_active != new_state.recording_active or
            old_state.session_id != new_state.session_id or abs(old_state.
            file_count - new_state.file_count) > 0 or old_state.
            calibration_status != new_state.calibration_status)
    ...
    def _deliver_queued_messages(self, device_id: str):
        ...
        while not queue.empty():
            try:
                ...
                logger.error(
                    '[SessionSynchronizer] Error delivering message: %s',
                    str(e))
        logger.info('[SessionSynchronizer] Delivered %d queued messages to %s',
            delivered_count, device_id)

    def _send_message(self, message: QueuedMessage) ->bool:
        try:
            if self.message_sender:
                result = self.message_sender(message.device_id, message.
                    message_type, message.payload)
                return bool(result)
            return False
        except Exception as e:
            logger.error('[SessionSynchronizer] Failed to send message: %s',
                str(e))
            return False
    ...
    def _cleanup_stale_devices(self):
        ...
        with self.sync_lock:
            for device_id, offline_time in self.offline_devices.items():
                if (current_time - offline_time).total_seconds(
                    ) > self.max_offline_time:
                    stale_devices.append(device_id)
        for device_id in stale_devices:
            logger.warning(
                '[SessionSynchronizer] Removing stale offline device: %s',
                device_id)
...
if __name__ == '__main__':
    ...
    def mock_message_sender(device_id: str, message_type: str, payload:
        Dict[str, Any]) ->bool:
    ...
    try:
        ...
        android_state = {'device_id': device_id, 'session_id':
            'test_session_123', 'recording_active': True,
            'devices_connected': {'shimmer': True, 'thermal': False},
            'recording_start_time': datetime.now().isoformat(),
            'recording_duration': 120.5, 'file_count': 5, 'total_file_size':
            1024000, 'calibration_status': {'rgb': 'complete', 'thermal':
            'pending'}, 'metadata': {'test': True}}
        ...
        sync.queue_message(device_id, 'test_command', {'action': 'start'},
            MessagePriority.HIGH)
        sync.queue_message(device_id, 'test_status', {'status': 'ok'},
        ...
        print(
            f'[DEBUG_LOG] Sync status: {json.dumps(status, indent=2, default=str)}'
            )
    finally:
