...
class SessionManager:
    ...
    @staticmethod
    def generate_device_filename(device_id: str, file_type: str, extension:
        str, timestamp: Optional[datetime]=None) ->str:
    ...
    def create_session(self, session_name: Optional[str]=None) ->Dict:
        ...
        if session_name is None:
            ...
            safe_name = ''.join(c if c.isalnum() or c in ('-', '_') else
                '_' for c in session_name.replace(' ', '_')).strip('_')
    ...
    def end_session(self) ->Optional[Dict]:
        ...
        metadata_file = Path(self.current_session['folder_path']
    ...
    def add_device_to_session(self, device_id: str, device_type: str,
        ...
        if not self.current_session:
            ...
            return
        device_info = {'device_type': device_type, 'capabilities':
            capabilities, 'added_time': datetime.now().isoformat(),
            'status': 'connected'}
        self.current_session['devices'][device_id] = device_info
        self._update_session_metadata()
        print(
            f'[DEBUG_LOG] Device added to session: {device_id} ({device_type})'
            )

    def add_file_to_session(self, device_id: str, file_type: str, file_path:
        str, file_size: Optional[int]=None):
        if not self.current_session:
        ...
        file_info = {'file_type': file_type, 'file_path': file_path,
            'file_size': file_size, 'created_time': datetime.now().isoformat()}
    ...
    def get_session_folder(self, session_id: Optional[str]=None) ->Optional[
        Path]:
    ...
    def trigger_post_session_processing(self, session_id: Optional[str]=
        None, enable_hand_segmentation: bool=True, segmentation_method: str
        ='mediapipe') ->Dict[str, any]:
        results = {'session_id': session_id, 'hand_segmentation': {
            'enabled': enable_hand_segmentation, 'available': self.
            has_hand_segmentation_available(), 'success': False, 'results':
            None, 'error': None}}
        if not enable_hand_segmentation:
            results['hand_segmentation']['success'] = True
            return results
        if not self.has_hand_segmentation_available():
            results['hand_segmentation']['error'
        ...
        try:
            ...
            processor = create_session_post_processor(str(self.
                base_recordings_dir))
            ...
            if not target_session:
                results['hand_segmentation']['error'
                    ] = 'No session found to process'
                return results
            print(
                f'[INFO] Starting post-session hand segmentation for: {target_session}'
                )
            segmentation_results = processor.process_session(target_session,
                method=segmentation_method, output_cropped=True,
                output_masks=True)
            results['hand_segmentation']['success'] = True
            results['hand_segmentation']['results'] = segmentation_results
            self._update_session_post_processing_status(target_session, True)
            print(
                f'[INFO] Post-session processing completed for: {target_session}'
                )
    ...
    def _update_session_post_processing_status(self, session_id: str,
        completed: bool):
        try:
            ...
            if session_folder:
                ...
                if metadata_file.exists():
                    ...
                    metadata['post_processing'] = {
                        ...
                        'hand_segmentation_timestamp': datetime.now().
                        isoformat() if completed else None}
                    with open(metadata_file, 'w') as f:
