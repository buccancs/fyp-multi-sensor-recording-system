...
class DeviceClient(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self._supported_capabilities = {'recording', 'streaming',
            'calibration', 'thermal_imaging', 'gsr_monitoring', 'audio_capture'
            }
        self._message_stats = {'sent': 0, 'received': 0, 'errors': 0,
    ...
    def run(self):
        ...
        try:
            ...
            if self._ssl_enabled and self._ssl_context:
                self.server_socket = self._ssl_context.wrap_socket(self.
                    server_socket, server_side=True)
                print(f'[DEBUG_LOG] SSL/TLS enabled for secure communication')
            self.server_socket.bind(('0.0.0.0', self.server_port))
            self.server_socket.listen(5)
            print(
                f'[DEBUG_LOG] DeviceClient server started on port {self.server_port}'
                )
            while self.running:
                try:
                    ...
                    if not self._check_rate_limit(address[0]):
                        print(
                            f'[DEBUG_LOG] Rate limited connection from {address}'
                            )
                        client_socket.close()
                        continue
                    print(f'[DEBUG_LOG] New connection from {address}')
                    self._message_stats['connection_count'] += 1
                    connection_thread = threading.Thread(target=self.
                        handle_device_connection, args=(client_socket,
                        address), daemon=True)
    ...
    def configure_ssl(self, certfile: str, keyfile: str, ca_certs: Optional
        [str]=None) ->bool:
        try:
            self._ssl_context = ssl.create_default_context(ssl.Purpose.
                CLIENT_AUTH)
    ...
    def _check_rate_limit(self, device_ip: str) ->bool:
        ...
        requests[:] = [req_time for req_time in requests if current_time -
            req_time < 60]
    ...
    def connect_to_device(self, device_ip: str, device_port: int=8080) ->bool:
        ...
        try:
            ...
            handshake_data = {'type': 'handshake', 'client_type':
                'recording_controller', 'protocol_version': '1.0',
                'timestamp': time.time()}
            ...
            if response_data.get('status') == 'accepted':
                with self._device_lock:
                    ...
                    self.devices[device_id] = {'socket': device_socket,
                        'ip': device_ip, 'port': device_port, 'status':
                        'connected', 'last_heartbeat': time.time(),
            ...
            self.error_occurred.emit(
                f'Connection timeout to {device_ip}:{device_port}')
            return False
        except ConnectionRefusedError:
            self.error_occurred.emit(
                f'Connection refused by {device_ip}:{device_port}')
            return False
        except json.JSONDecodeError:
            self.error_occurred.emit(
                f'Invalid handshake response from {device_ip}')
            return False
        except Exception as e:
            self.error_occurred.emit(
                f'Failed to connect to {device_ip}: {str(e)}')
            return False
    ...
    def disconnect_device(self, device_index: int) ->None:
        ...
        with self._device_lock:
            if device_index in self.devices:
                ...
                try:
                    disconnect_message = {'type': 'disconnect', 'reason':
                        'client_initiated', 'timestamp': time.time()}
                    device_socket = device['socket']
                    device_socket.send(json.dumps(disconnect_message).
                        encode('utf-8'))
                ...
                print(
                    f'[DEBUG_LOG] Device {device_index} disconnected successfully'
                    )
            else:
                print(
                    f'[DEBUG_LOG] Device {device_index} not found for disconnection'
                    )

    def send_command(self, device_index: int, command: str, parameters:
        Optional[Dict[str, Any]]=None, require_ack: bool=True) ->bool:
        print(
            f"[DEBUG_LOG] Sending command '{command}' to device {device_index}"
            )
        with self._device_lock:
            ...
            try:
                ...
                message = {'type': 'command', 'command': command,
                    'parameters': parameters or {}, 'timestamp': timestamp,
                    'message_id': message_id, 'require_ack': require_ack}
                device_socket = device['socket']
                json_data = json.dumps(message).encode('utf-8')
                device_socket.send(json_data)
                if require_ack:
                    self._pending_acknowledgments[message_id] = {'device_index'
                        : device_index, 'command': command, 'timestamp':
                        timestamp, 'attempts': 1, 'max_attempts': self.
                        _retry_attempts}
                    QTimer.singleShot(self._ack_timeout * 1000, lambda :
                        self._handle_ack_timeout(message_id))
                ...
                error_msg = (
                    f"Failed to send command '{command}' to device {device_index}: {str(e)}"
                    )
                ...
                if device_index in self.devices:
                    try:
                        ...
                        message = {'type': 'command', 'command': command,
                            ...
                            'message_id': message_id, 'require_ack': True,
                        ...
                        print(
                            f'[DEBUG_LOG] Retry failed for message {message_id}: {e}'
                            )
                        del self._pending_acknowledgments[message_id]
                        self._remove_failed_device(device_index)
        else:
            print(
                f'[DEBUG_LOG] Max retry attempts reached for message {message_id}'
                )
            del self._pending_acknowledgments[message_id]
            self.error_occurred.emit(
    ...
    def negotiate_capabilities(self, device_index: int,
        requested_capabilities: List[str]) ->Dict[str, bool]:
        print(
            f'[DEBUG_LOG] Negotiating capabilities with device {device_index}')
        with self._device_lock:
            if device_index not in self.devices:
                return {}
            device = self.devices[device_index]
            ...
            negotiation_message = {'type': 'capability_negotiation',
    ...
    def get_performance_metrics(self) ->Dict[str, Any]:
        ...
        return {'connected_devices': connected_devices, 'total_connections':
            self._message_stats['connection_count'], 'messages_sent': self.
            _message_stats['sent'], 'messages_received': self.
            _message_stats['received'], 'error_count': self._message_stats[
            'errors'], 'average_latency_ms': self._message_stats[
            'avg_latency'] * 1000, 'pending_acknowledgments': len(self.
            _pending_acknowledgments), 'ssl_enabled': self._ssl_enabled,
        ...
        with self._device_lock:
            ...
            for device_id, device_data in self.devices.items():
                devices_info[device_id] = {'ip': device_data['ip'], 'port':
                    device_data['port'], 'status': device_data['status'],
                    'last_heartbeat': device_data['last_heartbeat'],
                    'connection_time': time.time() - device_data.get(
                    'connected_at', time.time()), 'device_info':
                    device_data.get('device_info', {}), 'capabilities':
                    device_data.get('capabilities', [])}
            return devices_info

    def handle_device_connection(self, client_socket: socket.socket,
        address: tuple) ->None:
        ...
        try:
            ...
            if handshake.get('type') == 'handshake':
                response = {'status': 'accepted', 'server_info': {'type':
                    'recording_controller', 'version': '1.0'}, 'timestamp':
                    time.time()}
                client_socket.send(json.dumps(response).encode('utf-8'))
                with self._device_lock:
                    device_id = self.device_counter
                    self.device_counter += 1
                    self.devices[device_id] = {'socket': client_socket,
                        'ip': address[0], 'port': address[1], 'status':
                        'connected', 'last_heartbeat': time.time(),
                        'connected_at': time.time(), 'device_info':
                        handshake.get('device_info', {}), 'capabilities':
                        handshake.get('capabilities', [])}
                device_info = f'{address[0]}:{address[1]}'
                self.device_connected.emit(device_id, device_info)
                monitor_thread = threading.Thread(target=self.
                    _monitor_device, args=(device_id,), daemon=True)
                ...
                response = {'status': 'rejected', 'error':
                    'Invalid handshake', 'timestamp': time.time()}
                client_socket.send(json.dumps(response).encode('utf-8'))
                client_socket.close()
        except Exception as e:
    ...
    def _process_device_message(self, device_id: int, message: Dict[str, Any]
        ) ->None:
    ...
    def _update_latency_stats(self, latency: float) ->None:
        ...
        if self._message_stats['avg_latency'] == 0:
            ...
            self._message_stats['avg_latency'] = alpha * latency + (1 - alpha
