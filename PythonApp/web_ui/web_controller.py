...
class WebController:
    ...
    def inject_dependencies(self, session_manager=None, shimmer_manager=
        None, android_device_manager=None, json_server=None, webcam_capture
        =None):
            ...
            try:
                if self.android_device_manager.initialize():
                    ...
                    self.android_device_manager.add_data_callback(self.
                        _on_android_sensor_data)
                    self.android_device_manager.add_session_callback(self.
                        _on_android_session_event)
                    logger.info(
                        'Connected to AndroidDeviceManager network protocols')
                else:
                    logger.error(
    ...
    def _on_android_device_status(self, device_id: str, android_device):
        try:
            status_data = {'type': 'android', 'status': 'connected' if
                android_device.connected else 'disconnected', 'battery':
                android_device.status.get('battery', 0), 'temperature':
                android_device.status.get('temperature', 0), 'storage':
                android_device.status.get('storage', 0), 'recording':
                android_device.is_recording, 'capabilities': android_device
                .capabilities, 'connection_time': android_device.
                connection_time, 'messages_received': android_device.
                messages_received, 'data_samples': android_device.
    ...
    def _on_android_sensor_data(self, shimmer_data_sample):
        try:
            sensor_data = {'timestamp': shimmer_data_sample.timestamp,
            ...
            self.sensor_data_received.emit(shimmer_data_sample.
                android_device_id, sensor_data)
            logger.debug(
    ...
    def start_recording_session(self, session_id: str) ->bool:
        try:
            if self.android_device_manager:
                success = self.android_device_manager.start_session(session_id
                    =session_id, record_shimmer=True, record_video=True,
                    record_thermal=True)
                if success:
                    ...
                    logger.error(
                        f'Failed to start recording session: {session_id}')
                    return False
            else:
                logger.warning(
    ...
    def stop_recording_session(self) ->bool:
        try:
            if self.android_device_manager and self._current_session_id:
                ...
                logger.warning(
                    'No active session or AndroidDeviceManager to stop')
                return False
        except Exception as e:
    ...
    def _check_session_status(self):
        ...
        try:
            if hasattr(self.session_manager, 'current_session'):
                ...
                if (current_session and current_session != self.
                    _current_session_id):
                    session_id = current_session.get('session_id'
                        ) if isinstance(current_session, dict) else str(
                        current_session)
    ...
    def _check_device_status(self):
        try:
            ...
            if self.shimmer_manager and hasattr(self.shimmer_manager,
                'get_all_device_status'):
                try:
                    real_shimmer_devices = (self.shimmer_manager.
                        get_all_device_status() or {})
                except:
                    pass
            if real_shimmer_devices:
                for device_id, status in real_shimmer_devices.items():
                    self.device_status_received.emit(device_id, {'type':
                        'shimmer', 'status': 'connected' if status.get(
                        'is_connected', False) else 'disconnected',
            ...
            if self.android_device_manager and hasattr(self.
                android_device_manager, 'get_connected_devices'):
                try:
                    real_android_devices = (self.android_device_manager.
                        get_connected_devices() or {})
                except:
                    pass
            ...
            try:
                ...
                if self.webcam_capture and hasattr(self.webcam_capture,
                    'get_available_cameras'):
                    try:
                        real_webcams = (self.webcam_capture.
                            get_available_cameras() or [])
                    except:
                        pass
            if real_webcams:
                for camera_info in real_webcams:
                    camera_id = f"webcam_{camera_info.get('index', 0)}"
                    self.device_status_received.emit(camera_id, {'type':
                        'webcam', 'status': camera_info.get('status',
                        'active'), 'name': camera_info.get('name',
                        f"Camera {camera_info.get('index', 0)}"),
                        'resolution': camera_info.get('resolution',
                        'Unknown'), 'fps': camera_info.get('fps', 30),
                        'recording': False})
        except Exception as e:
    ...
    def _check_sensor_data(self):
        try:
            ...
            if self.shimmer_manager and hasattr(self.shimmer_manager,
                'get_all_device_status'):
                try:
                    shimmer_devices = (self.shimmer_manager.
                        get_all_device_status() or {})
                    for device_id, status in shimmer_devices.items():
                        if status.get('is_connected', False):
                            if hasattr(self.shimmer_manager, 'get_sensor_data'
                                ...
                                sensor_data = (self.shimmer_manager.
            ...
            if self.android_device_manager and hasattr(self.
                ...
                try:
                    ...
                    for device_id, device_info in android_devices.items():
                        if hasattr(self.android_device_manager,
                            'get_sensor_data'):
                            sensor_data = (self.android_device_manager.
                                get_sensor_data(device_id))
                            if sensor_data:
    ...
    def start_recording(self, session_config: Dict[str, Any]=None) ->bool:
        try:
            ...
            if self.session_manager and hasattr(self.session_manager,
                'start_session'):
                try:
                    result = self.session_manager.start_session(session_id=
                        session_id, session_config=session_config or {})
                    if result:
                        self._current_session_id = session_id
                        self.recording_started.emit(session_id)
                        self.session_status_changed.emit(session_id, True)
    ...
    def stop_recording(self) ->bool:
        ...
        try:
            ...
            if self.session_manager and hasattr(self.session_manager,
                ...
                logger.error(
                    'No SessionManager available for stopping recording')
                return False
        except Exception as e:
    ...
    def get_device_status(self) ->Dict[str, Any]:
        device_status = {'shimmer_devices': {}, 'android_devices': {},
            'webcam_devices': {}}
        try:
            if self.shimmer_manager and hasattr(self.shimmer_manager,
                'get_all_device_status'):
            ...
            if self.webcam_capture and hasattr(self.webcam_capture,
                ...
                device_status['webcam_devices'] = {f'webcam_{i}': cam for i,
                    cam in enumerate(cameras)}
        except Exception as e:
            logger.error(f'Error getting device status: {e}')
        return device_status

    def get_session_info(self) ->Dict[str, Any]:
        session_info = {'active': self._current_session_id is not None,
            'session_id': self._current_session_id, 'start_time': None,
            'duration': 0}
        try:
            if self.session_manager and hasattr(self.session_manager,
                'current_session'):
...
def create_web_controller_with_real_components():
    ...
    try:
        if ANDROID_DEVICE_MANAGER_AVAILABLE:
            ...
            logger.info(
                'Created AndroidDeviceManager with network server on port 9000'
                )
    except Exception as e:
    ...
    controller.inject_dependencies(session_manager=session_manager,
        shimmer_manager=shimmer_manager, android_device_manager=
        android_device_manager, json_server=None, webcam_capture=webcam_capture
        )
