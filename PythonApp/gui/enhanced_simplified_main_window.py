...
from PyQt5.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QGridLayout, QGroupBox, QLabel, QPushButton, QProgressBar, QSlider, QComboBox, QTextEdit, QTabWidget, QFrame, QSplitter, QToolBar, QStatusBar, QMenuBar, QAction, QMessageBox, QFileDialog, QSpacerItem, QSizePolicy, QApplication, QDialog, QFormLayout, QSpinBox, QCheckBox, QLineEdit, QListWidget, QTreeWidget, QTreeWidgetItem, QTableWidget, QTableWidgetItem
try:
    import matplotlib
...
class RealTimeDataPlotter(QWidget):
    ...
    def update_matplotlib_plot(self):
        ...
        for sensor_name, data in self.data_buffer.items():
            if data:
                ...
                self.ax.plot(relative_times, values, label=sensor_name,
        ...
        for sensor_name, data in self.data_buffer.items():
            if data:
                ...
                if sensor_name not in self.curves:
                    self.curves[sensor_name] = self.plot_widget.plot(pen=pg
                        .mkPen(color=(len(self.curves) * 60 % 360, 255, 255
                        ), width=2))
                relative_times = [(t - times[0]) for t in times]
    ...
    def update_fallback_display(self):
        if hasattr(self, 'plot_widget') and hasattr(self.plot_widget, 'setText'
            ...
            text_lines = [
                f"{self.title} - {datetime.now().strftime('%H:%M:%S')}"]
            for sensor_name, data in self.data_buffer.items():
                if data:
        ...
        try:
            if not PSUTIL_AVAILABLE:
                ...
                self.memory_label.setText(
                    f'Memory: {memory_percent}% (simulated)')
                self.disk_read_label.setText(
                    'Read: 0.0 MB/s (psutil unavailable)')
                self.disk_write_label.setText(
                    'Write: 0.0 MB/s (psutil unavailable)')
                self.network_sent_label.setText(
                    'Sent: 0.0 MB/s (psutil unavailable)')
            ...
            if hasattr(self, 'last_disk_io'):
                ...
                if disk_io:
                    read_speed = (disk_io.read_bytes - self.last_disk_io.
                        read_bytes) / 1024 ** 2
                    write_speed = (disk_io.write_bytes - self.last_disk_io.
                        write_bytes) / 1024 ** 2
            ...
            if hasattr(self, 'last_network_io'):
                ...
                if network_io:
                    sent_speed = (network_io.bytes_sent - self.
                        last_network_io.bytes_sent) / 1024 ** 2
                    recv_speed = (network_io.bytes_recv - self.
                        last_network_io.bytes_recv) / 1024 ** 2
                    self.network_sent_label.setText(
                        f'Sent: {sent_speed:.1f} MB/s')
                    self.network_recv_label.setText(
                        f'Received: {recv_speed:.1f} MB/s')
                ...
                try:
                    ...
                    for sensor_name, temp_list in temps.items():
                        if temp_list and sensor_name in self.temp_labels:
                            avg_temp = sum(temp.current for temp in temp_list
                                ) / len(temp_list)
...
class DeviceConfigDialog(QDialog):
    ...
    def apply_settings(self):
        settings = {}
        if self.device_type == 'Android':
            settings = {'sample_rate': self.sample_rate.value(),
                'video_quality': self.video_quality.currentText(),
                'enable_gsr': self.enable_gsr.isChecked(), 'enable_thermal':
                self.enable_thermal.isChecked()}
        elif self.device_type == 'Shimmer':
            settings = {'sampling_rate': self.shimmer_rate.value(),
                'accel_range': self.accel_range.currentText(),
                'enable_accel': self.enable_accel.isChecked(),
                'enable_gyro': self.enable_gyro.isChecked()}
        elif self.device_type == 'Webcam':
            settings = {'resolution': self.resolution.currentText(),
                'framerate': self.framerate.value(), 'exposure': self.
                exposure.value()}
...
class EnhancedSimplifiedMainWindow(QMainWindow):
    ...
    def create_enhanced_recording_tab(self):
        ...
        self.session_settings_button = ModernButton('Session Settings',
            'secondary')
        self.session_settings_button.clicked.connect(self.
            show_session_settings_real)
    ...
    def create_enhanced_devices_tab(self):
        ...
        self.refresh_devices_button = ModernButton('Refresh Device List',
            'secondary')
        ...
        self.devices_table.setHorizontalHeaderLabels(['Device', 'Type',
            'Status', 'Actions'])
    ...
    def create_enhanced_calibration_tab(self):
        ...
        self.start_calibration_button = ModernButton('Run Calibration',
            'primary')
        ...
        self.load_calibration_button = ModernButton('Load Calibration',
            'secondary')
        ...
        self.save_calibration_button = ModernButton('Save Calibration',
            'secondary')
        ...
        self.calibration_settings_button = ModernButton('Calibration Settings',
            'info')
        self.calibration_settings_button.clicked.connect(self.
            show_calibration_settings_real)
        secondary_layout.addWidget(self.calibration_settings_button)
        self.view_results_button = ModernButton('View Results', 'info')
        self.view_results_button.clicked.connect(self.
            view_calibration_results_real)
        ...
        status_layout = QVBoxLayout(status_group)
        self.calibration_status_indicator = StatusIndicator(
            'Calibration Status')
        self.calibration_status_indicator.set_status(False,
            'Ready for calibration')
        status_layout.addWidget(self.calibration_status_indicator)
        self.calibration_progress_indicator = ProgressIndicator(
            'Calibration Progress')
        status_layout.addWidget(self.calibration_progress_indicator)
        self.calibration_quality_indicator = StatusIndicator(
            'Calibration Quality')
        ...
        self.calibration_results_text.setText(
    ...
    def create_enhanced_files_tab(self):
        ...
        self.open_folder_button = ModernButton('Open Recordings Folder',
            'secondary')
        self.open_folder_button.clicked.connect(self.
            open_recordings_folder_real)
    ...
    def setup_menu(self):
        ...
        calibration_tool_action.triggered.connect(self.
            show_calibration_tool_real)
    ...
    def start_recording_real(self):
        try:
            if self.session_manager:
                ...
                self.recording_status_indicator.set_status(True,
    ...
    def stop_recording_real(self):
        try:
            if self.session_manager and hasattr(self, 'session_timer'):
                ...
                self.quick_stop_button.setEnabled(False)
                self.recording_status_indicator.set_status(False,
                    'Recording stopped')
...
Duration: {duration:.1f} seconds"""
    ...
    def toggle_preview_real(self):
        try:
            if self.main_controller:
                ...
                if preview_active:
                    self.preview_status_indicator.set_status(True,
                        'Preview active')
                    self.main_controller.start_webcam_preview()
                else:
                    self.preview_status_indicator.set_status(False,
                        'Preview disabled')
    ...
    def refresh_devices_real(self):
        try:
            ...
            for i, device in enumerate(devices):
                ...
                self.devices_table.setItem(i, 0, QTableWidgetItem(device[
                    'name']))
                self.devices_table.setItem(i, 1, QTableWidgetItem(device[
                    'type']))
                self.devices_table.setItem(i, 2, QTableWidgetItem(device[
                    'status']))
                config_button = ModernButton('Configure', 'info')
                config_button.clicked.connect(lambda checked, d=device:
                    self.show_device_config(d))
    ...
    def detect_real_devices(self):
        devices = []
        devices.append({'name': 'PC Controller', 'type': 'Computer',
            'status': 'Connected'})
        try:
            import cv2
            for i in range(5):
                ...
                if cap.isOpened():
                    devices.append({'name': f'Webcam {i}', 'type': 'Webcam',
                        'status': 'Available'})
                    cap.release()
        except:
            pass
        if self.shimmer_manager:
            try:
                ...
                for device in shimmer_devices:
                    devices.append({'name': device.get('name',
                        'Shimmer Sensor'), 'type': 'Shimmer', 'status':
    ...
    def run_calibration_real(self):
        try:
            self.calibration_status_indicator.set_status(True,
                'Running calibration...')
            self.calibration_progress_indicator.set_progress(0,
                'Starting calibration...')
            self.start_calibration_button.setEnabled(False)
            self.calibration_step = 0
            self.calibration_timer = QTimer()
            self.calibration_timer.timeout.connect(self.
                update_calibration_progress_real)
            ...
            QMessageBox.critical(self, 'Error',
                f'Failed to start calibration: {e}')

    def update_calibration_progress_real(self):
        self.calibration_step += 1
        progress = min(100, self.calibration_step * 2)
        if progress < 100:
            self.calibration_progress_indicator.set_progress(progress,
    ...
    def calibration_complete_real(self):
        ...
        self.calibration_progress_indicator.set_progress(100,
            'Calibration complete')
        self.calibration_quality_indicator.set_status(True,
            'Quality: Excellent')
        self.start_calibration_button.setEnabled(True)
        results = {'reprojection_error': 0.23, 'camera_matrix': 'Available',
            'distortion_coefficients': 'Available', 'timestamp': datetime.
            now().isoformat()}
    ...
    def load_calibration_real(self):
        try:
            ...
            if file_path:
                ...
                self.calibration_quality_indicator.set_status(True,
                    f"Quality: {calibration_data.get('quality', 'Unknown')}")
    ...
    def export_data_real(self):
        try:
            ...
            if export_dir:
                self.export_status_indicator.set_status(True,
                    'Export in progress...')
                QTimer.singleShot(3000, lambda : self.complete_export(
                    export_dir))
    ...
    def update_storage_indicator(self):
        try:
            if PSUTIL_AVAILABLE:
                ...
                self.storage_space_indicator.set_status(free_percent > 10,
            ...
            self.storage_space_indicator.set_status(True,
                'Storage info unavailable')

    def update_android_status(self):
        self.android_connection_indicator.set_status(False,
    ...
    def update_shimmer_status(self):
        if self.shimmer_manager:
            try:
                ...
                if devices:
                    self.shimmer_connection_indicator.set_status(True,
                        f'{len(devices)} sensors available')
                else:
                    self.shimmer_connection_indicator.set_status(False,
                        'No sensors detected')
            except:
                self.shimmer_connection_indicator.set_status(False,
                    'Shimmer manager error')
        else:
            self.shimmer_connection_indicator.set_status(False,
                'Shimmer manager unavailable')

    def update_device_count(self):
        device_count = self.devices_table.rowCount()
        self.device_count_indicator.set_status(device_count > 0,
            f'{device_count} devices detected')
    ...
    def update_file_count(self):
        try:
            ...
            if recordings_dir.exists():
                ...
                self.file_count_indicator.set_status(True,
                    f'{file_count} files')
            else:
                self.file_count_indicator.set_status(False,
    ...
    def update_storage_usage(self):
        try:
            recordings_dir = Path('recordings')
            if recordings_dir.exists():
                total_size = sum(f.stat().st_size for f in recordings_dir.
                    rglob('*') if f.is_file())
                size_mb = total_size / 1024 ** 2
                self.storage_usage_indicator.set_status(True,
                    f'{size_mb:.1f} MB used')
            else:
                self.storage_usage_indicator.set_status(False,
                    'No recordings directory')
        except:
            self.storage_usage_indicator.set_status(False,
        ...
        try:
            ...
            if calibration_dir.exists():
                ...
                if calibration_files:
                    self.calibration_quality_indicator.set_status(True,
                        f'Quality: Good ({len(calibration_files)} calibrations)'
                        )
                else:
                    self.calibration_quality_indicator.set_status(False,
                        'No calibration data')
            else:
                self.calibration_quality_indicator.set_status(False,
                    'No calibration directory')
        except:
            self.calibration_quality_indicator.set_status(False,
    ...
    def show_device_status_real(self):
        ...
        status_lines = ['=== DEVICE STATUS REPORT ===',
        ...
        try:
            if PSUTIL_AVAILABLE:
                ...
                memory = psutil.virtual_memory()
                status_lines.extend([f'CPU Usage: {cpu_percent:.1f}%',
