...
class CodecInfo:
    ...
    def __init__(self):
        self.qt_supported = ['.mp4', '.avi', '.mov', '.mkv', '.wmv', '.m4v',
            '.3gp']
        self.vlc_supported = ['.mp4', '.avi', '.mov', '.mkv', '.wmv',
            '.m4v', '.3gp', '.flv', '.webm', '.ogv', '.mpg', '.mpeg', '.ts',
            '.mts', '.m2ts']
...
class EnhancedTimingLogger:
    ...
    def get_precise_timestamp(self) ->Dict[str, float]:
        return {'system_time': self.system_clock(), 'monotonic_time': self.
            monotonic_clock(), 'performance_time': self.perf_clock(),
            'corrected_time': self.system_clock() - self.clock_offset}

    def start_experiment_log(self, video_file: str, backend: str) ->str:
        timestamp = time.strftime('%Y%m%d_%H%M%S')
        self.current_log_file = os.path.join(self.log_directory,
            f'enhanced_experiment_log_{timestamp}.json')
        timing_info = self.get_precise_timestamp()
        self.experiment_start_time = timing_info['corrected_time']
        self.event_counter = 0
        log_data = {'experiment_info': {'start_timestamps': timing_info,
            'start_time_formatted': datetime.fromtimestamp(self.
    ...
    def log_stimulus_start(self, video_duration_ms: int, frame_rate: float=None
        ...
        if not self.current_log_file or not self.experiment_start_time:
            return
        timing_info = self.get_precise_timestamp()
        event = {'event_type': 'stimulus_start', 'timestamps': timing_info,
            'timestamp_formatted': datetime.fromtimestamp(timing_info[
            'corrected_time']).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],
            'experiment_time': timing_info['corrected_time'] - self.
            experiment_start_time, 'video_duration_ms': video_duration_ms,
            'video_duration_s': video_duration_ms / 1000.0, 'frame_rate':
            frame_rate}
    ...
    def log_event_marker(self, video_position_ms: int, marker_label: str='',
        ...
        event = {'event_type': 'event_marker', 'marker_number': self.
            event_counter, 'marker_label': marker_label or
            f'Marker {self.event_counter}', 'timestamps': timing_info,
            'timestamp_formatted': datetime.fromtimestamp(timing_info[
            'corrected_time']).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],
            'experiment_time': timing_info['corrected_time'] - self.
            experiment_start_time, 'video_position_ms': video_position_ms,
            'video_position_s': video_position_ms / 1000.0, 'frame_number':
            frame_number}
        self._append_event(event)
...
class VLCVideoWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if not VLC_AVAILABLE:
            ...
            return
        self.vlc_instance = vlc.Instance(['--no-xlib', '--quiet', '--intf',
            'dummy'])
...
class EnhancedStimulusController(QWidget):
    ...
    def connect_signals(self):
        ...
        self.qt_media_player.mediaStatusChanged.connect(self.
            on_qt_media_status_changed)
        self.qt_media_player.positionChanged.connect(self.
            on_qt_position_changed)
        self.qt_media_player.durationChanged.connect(self.
            on_qt_duration_changed)
        self.qt_media_player.error.connect(self.on_qt_media_error)
        if self.vlc_video_widget:
            self.vlc_video_widget.position_changed.connect(self.
                on_vlc_position_changed)
            self.vlc_video_widget.duration_changed.connect(self.
                on_vlc_duration_changed)
            self.vlc_video_widget.state_changed.connect(self.
                on_vlc_state_changed)
    ...
    def load_video(self, file_path: str) ->bool:
        ...
        if self._load_with_backend(file_path, best_backend):
            ...
            return True
        fallback_backend = (VideoBackend.VLC if best_backend ==
            VideoBackend.QT_MULTIMEDIA else VideoBackend.QT_MULTIMEDIA)
        if fallback_backend == VideoBackend.VLC and not VLC_AVAILABLE:
            self.error_occurred.emit(
                'Primary backend failed and VLC not available')
            return False
        if self._load_with_backend(file_path, fallback_backend):
            ...
            return True
        self.error_occurred.emit(
            'Failed to load video with any available backend')
        return False
    ...
    def switch_backend(self):
        if not self.current_video_file:
            return
        if (self.current_backend == VideoBackend.QT_MULTIMEDIA and
            VLC_AVAILABLE):
        ...
        if self._load_with_backend(self.current_video_file, new_backend):
            ...
            self.error_occurred.emit(
                f'Failed to switch to {new_backend.value} backend')
    ...
    def _enable_controls(self):
        ...
        self.switch_backend_btn.setEnabled(VLC_AVAILABLE and self.
            current_video_file is not None)
    ...
    def start_stimulus_playback(self, screen_index: int=0) ->bool:
        ...
        if self.is_experiment_active:
            ...
            return False
        try:
            log_file = self.timing_logger.start_experiment_log(self.
                current_video_file, self.current_backend.value)
            ...
            self.status_changed.emit(
                'Enhanced Experiment Started - Stimulus Playing')
            return True
        except Exception as e:
            self.error_occurred.emit(
    ...
    def on_qt_state_changed(self, state):
        if self.current_backend != VideoBackend.QT_MULTIMEDIA:
            return
        state_names = {QMediaPlayer.StoppedState: 'Stopped', QMediaPlayer.
            PlayingState: 'Playing', QMediaPlayer.PausedState: 'Paused'}
    ...
    def on_qt_media_error(self, error):
        error_messages = {QMediaPlayer.NoError: 'No error', QMediaPlayer.
            ResourceError: 'Resource error - try VLC backend or check file',
            QMediaPlayer.FormatError:
            'Format error - try VLC backend for better codec support',
            QMediaPlayer.NetworkError: 'Network error', QMediaPlayer.
            AccessDeniedError: 'Access denied - check file permissions',
            QMediaPlayer.ServiceMissingError: 'Codec missing - try VLC backend'
            }
        error_msg = error_messages.get(error, f'Unknown error ({error})')
        if error in [QMediaPlayer.FormatError, QMediaPlayer.ServiceMissingError
            ] and VLC_AVAILABLE:
    ...
    def stop_stimulus_playback(self, reason: str='stopped'):
        ...
        try:
            ...
            self.status_changed.emit(
                f'Enhanced Experiment Ended - {reason.title()}')
        except Exception as e:
            self.error_occurred.emit(
