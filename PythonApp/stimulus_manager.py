...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QVBoxLayout, QDesktopWidget, QMainWindow
...
class StimulusWindow(QMainWindow):
    ...
    def setup_video_content(self, layout):
        ...
        if os.path.exists(self.config.content_path):
            media_content = QMediaContent(QUrl.fromLocalFile(self.config.
                content_path))
    ...
    def setup_image_content(self, layout):
        ...
        if os.path.exists(self.config.content_path):
            ...
            scaled_pixmap = pixmap.scaled(self.monitor_info.geometry.size(),
...
class StimulusManager:
    ...
    def initialize(self) ->bool:
        try:
            ...
            for i, screen in enumerate(screens):
                monitor_info = MonitorInfo(monitor_id=i, name=screen.name(),
                    geometry=screen.geometry(), is_primary=i == desktop.
                    primaryScreen(), dpi=screen.logicalDotsPerInch())
    ...
    def present_stimulus(self, config: StimulusConfig) ->bool:
        try:
            ...
            event = StimulusEvent(event_type='stimulus_start', timestamp=
                start_time, monitor_id=config.monitor_id, stimulus_config=
                config)
            ...
            if config.duration_ms > 0:
                ...
                timer.timeout.connect(lambda : self.
                    _stop_stimulus_presentation(config.monitor_id, start_time))
    ...
    def stop_stimulus(self, monitor_id: int) ->bool:
        try:
            if monitor_id in self.stimulus_windows:
                ...
                self.logger.warning(
                    f'No active stimulus on monitor {monitor_id}')
                return False
        except Exception as e:
    ...
    def present_synchronized_stimuli(self, configs: List[StimulusConfig]
        ) ->bool:
        try:
            if not configs:
                return False
            ...
            for config in configs:
                if config.monitor_id >= len(self.available_monitors):
                    self.logger.error(
                        f'Invalid monitor ID: {config.monitor_id}')
                    continue
                monitor_info = self.available_monitors[config.monitor_id]
            ...
            for window, config in windows:
                ...
                event = StimulusEvent(event_type=
    ...
    def add_event_callback(self, callback: Callable[[StimulusEvent], None]
        ) ->None:
    ...
    def _stop_stimulus_presentation(self, monitor_id: int, start_time: float
        ) ->None:
        try:
            if monitor_id in self.stimulus_windows:
                ...
                event = StimulusEvent(event_type='stimulus_stop', timestamp
                    =stop_time, monitor_id=monitor_id, stimulus_config=None,
