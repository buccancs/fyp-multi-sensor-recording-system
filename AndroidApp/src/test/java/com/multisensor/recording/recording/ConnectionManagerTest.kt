...
class ConnectionManagerTest {
    ...
    @Test
    fun `test connect with valid host and port`() = runTest {
        val result = connectionManager.connect(testHost, testPort)
        
        assertTrue("Connection should be attempted", result)
        verify { mockNetworkController.isNetworkAvailable() }
    }

    @Test
    fun `test connect with invalid host`() = runTest {
        val invalidHost = "invalid.host.address"
        
        val result = connectionManager.connect(invalidHost, testPort)
        
        assertFalse("Connection should fail with invalid host", result)
    }

    @Test
    fun `test connect with invalid port`() = runTest {
        val invalidPort = -1
        
        val result = connectionManager.connect(testHost, invalidPort)
        
        assertFalse("Connection should fail with invalid port", result)
    }

    @Test
    fun `test connect when network unavailable`() = runTest {
        every { mockNetworkController.isNetworkAvailable() } returns false
        
        val result = connectionManager.connect(testHost, testPort)
        
        assertFalse("Connection should fail when network unavailable", result)
    }
