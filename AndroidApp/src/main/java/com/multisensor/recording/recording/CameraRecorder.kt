...
constructor(
    ...
    companion object {
        ...
        private const val CAMERA_LOCK_TIMEOUT_MS = 10_000L // Increased to 10 seconds for better reliability
    ...
    private fun configureSamsungDngMetadata(
        ...
        try {
            ...
            if (isSamsungDevice) {
                ...
                if (gpsLocation != null) {
                    try {
                        val setLocationMethod =
                            dngCreatorClass.getMethod("setLocation", android.location.Location::class.java)
    ...
    fun isRawStage3Available(): Boolean {
        return try {
            ...
            for (cameraId in cameraManager.cameraIdList) {
                ...
                logger.info(
                    "RAW sensor sizes: ${rawSizes?.size}, Max resolution: ${
                        rawSizes?.maxByOrNull { it.width * it.height }?.let { "${it.width}x${it.height}" }
                    }"
                )
