...
class ThermalRecorder @Inject constructor(
    ...
    fun initialize(previewSurface: SurfaceView? = null, previewStreamer: PreviewStreamer? = null): Boolean {
        return try {
            ...
            topdonUsbMonitor = USBMonitor(context, object : USBMonitor.OnDeviceConnectListener {
    ...
    fun startRecording(sessionId: String): Boolean {
        ...
        return try {
            ...
            val sessionDir = sessionFilePaths?.sessionFolder ?: run {
                logger.error("Could not get session directory for session: $sessionId")
                return false
            }
            
            thermalDataFile = File(sessionDir, "thermal_${sessionId}.dat")
            ...
            logger.info("Thermal recording started successfully")
            true
        } catch (e: Exception) {
     ...
     */
    private fun processFrameForRecording(temperatureData: ByteArray, timestamp: Long) {
     ...
     */
    private fun processFrameForPreview(imageData: ByteArray, timestamp: Long) {
     ...
     */
    private fun initializeCameraWithControlBlock(device: UsbDevice, controlBlock: USBMonitor.UsbControlBlock) {
        try {
    ...
    private fun startBackgroundThreads() {
        ...
        backgroundThread = HandlerThread("ThermalRecorder-Background").apply {
            start()
            backgroundHandler = Handler(looper)
        }
        
        // File writer thread for disk I/O
        fileWriterThread = HandlerThread("ThermalRecorder-FileWriter").apply {
            start()
