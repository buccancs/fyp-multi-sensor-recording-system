 ...
 */
@Singleton
class FileTransferHandler @Inject constructor(
     ...
     */
    private suspend fun sendFile(filepath: String, filetype: String?) {
        ...
        try {
            ...
            val fileInfoMessage = FileInfoMessage(
                name = file.name,
                size = file.length()
            )
            ...
            FileInputStream(file).use { inputStream ->
                ...
                while (true) {
                    ...
                    // Send chunk message
                    val chunkMessage = FileChunkMessage(
                        seq = sequenceNumber,
                        data = base64Data
                    )
     ...
     */
    fun getExpectedFilePaths(sessionId: String, deviceId: String, capabilities: List<String>): List<String> {
