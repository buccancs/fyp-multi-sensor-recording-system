...
class CameraRecorderManualTest {
        ...
        InstrumentationRegistry.getInstrumentation().runOnMainSync {
            ...
            textureView.surfaceTexture?.let { surface ->
                textureView.surfaceTextureListener?.onSurfaceTextureSizeChanged(
                    surface, surfaceHeight, surfaceWidth
                )
            }
        }
     ...
     */
    @Test
    fun test2_videoOnlyRecordingTest() = runBlocking {
        ...
        currentSession = withTimeout(15000) {
        ...
        val stoppedSession = withTimeout(10000) {
     ...
     */
    @Test
    fun test3_rawOnlyCaptureTest() = runBlocking {
        ...
        val firstCaptureResult = withTimeout(10000) {
        ...
        val secondCaptureResult = withTimeout(10000) {
            cameraRecorder.captureRawImage()
        }
        ...
        rawFilePaths.forEachIndexed { index, filePath ->
            ...
            val isValidDng = (fileBytes[0] == 0x49.toByte() && fileBytes[1] == 0x49.toByte()) ||
        ...
        println("[DEBUG_LOG] Captured ${rawFilePaths.size} DNG files with total size: ${rawFilePaths.sumOf { File(it).length() }} bytes")
     ...
     */
    @Test
    fun test4_concurrentVideoRawTest() = runBlocking {
        ...
        val firstRawCapture = withTimeout(10000) {
            cameraRecorder.captureRawImage()
        }
        ...
        val secondRawCapture = withTimeout(10000) {
            cameraRecorder.captureRawImage()
        }
