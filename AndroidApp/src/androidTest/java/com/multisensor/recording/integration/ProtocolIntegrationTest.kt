...
class ProtocolIntegrationTest {
    ...
    fun testSchemaManagerLoading() {
        ...
        val expectedTypes = setOf(
            "start_record", "stop_record", "preview_frame", "file_chunk",
            "device_status", "ack", "calibration_start", "calibration_result"
        )
    ...
    fun testMessageValidation() {
        ...
        val validStartMessage = JSONObject().apply {
            put("type", "start_record")
            put("timestamp", System.currentTimeMillis())
            put("session_id", "test_session_123")
        }
        
        assertTrue("Valid start message should pass validation", 
                  schemaManager.validateMessage(validStartMessage))
        
        // Test invalid message rejection
        val invalidMessage = JSONObject().apply {
            put("type", "start_record")
            put("timestamp", System.currentTimeMillis())
            // Missing required session_id
        }
        
        assertFalse("Invalid message should fail validation",
    ...
    fun testMockSocketConnection() {
        ...
        val testMessage = schemaManager.createMessage("device_status").apply {
    ...
    fun testRecordingStateTransitions() {
        ...
        val startMessage = schemaManager.createMessage("start_record").apply {
        ...
        val stopMessage = schemaManager.createMessage("stop_record").apply {
            put("session_id", "test_session")
        }
    ...
    fun testCalibrationFlow() {
        ...
        // Start calibration
        val calibrationStart = schemaManager.createMessage("calibration_start").apply {
            put("pattern_type", "chessboard")
            put("pattern_size", JSONObject().apply {
                put("rows", 7)
                put("cols", 6)
            })
        }
        ...
        val calibrationResults = messageCollector.filter { 
    ...
    fun testConfigSchemaConsistency() {
        ...
        val calibrationMessage = schemaManager.createMessage("calibration_start").apply {
        ...
        assertTrue("Config-based message should validate", 
...
class MockConnectionManager {
    ...
    fun getReceivedMessages(): List<JSONObject> {
...
enum class RecordingState {
    IDLE, RECORDING, CALIBRATING, ERROR
}
...
class MockRecordingStateManager {
    ...
    private fun startGeneratingMessages() {
        thread {
            ...
            while (currentState == RecordingState.RECORDING) {
                ...
                val previewFrame = JSONObject().apply {
    ...
    private fun startCalibrationProcess() {
        thread {
            ...
            val calibrationResult = JSONObject().apply {
