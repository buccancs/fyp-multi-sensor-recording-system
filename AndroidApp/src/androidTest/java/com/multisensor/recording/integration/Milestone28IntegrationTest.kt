...
class Milestone28IntegrationTest {
    ...
    @Test
    fun testEndToEndCalibrationCaptureWorkflow() = runTest {
        ...
        assertTrue("Synced timestamp should be greater than device timestamp", 
    ...
    @Test
    fun testMultiDeviceCalibrationCoordination() = runTest {
        ...
        assertTrue("Synced timestamp should be adjusted", 
            calibrationResult.syncedTimestamp != calibrationResult.timestamp)
    ...
    @Test
    fun testCalibrationCaptureWithSyncFailure() = runTest {
        println("[DEBUG_LOG] Testing calibration capture with sync failure scenarios")
        
        // Test calibration without clock synchronization
        val calibrationResult1 = calibrationCaptureManager.captureCalibrationImages(
        ...
        assertEquals("Timestamps should be equal without sync", 
            calibrationResult1.timestamp, calibrationResult1.syncedTimestamp)
        ...
        val calibrationResult2 = calibrationCaptureManager.captureCalibrationImages(
    ...
    @Test
    fun testCalibrationFileManagement() = runTest {
    ...
    @Test
    fun testSyncClockAccuracy() = runTest {
        ...
        assertTrue("Clock offset should remain stable", 
            kotlin.math.abs(actualOffset - initialOffset) < 50L) // 50ms tolerance
    ...
    @Test
    fun testCalibrationCaptureErrorHandling() = runTest {
        ...
        val result1 = calibrationCaptureManager.captureCalibrationImages(
        ...
        assertTrue("Error should mention RGB failure", 
        ...
        val result2 = calibrationCaptureManager.captureCalibrationImages(
        ...
        assertTrue("Error should mention thermal failure", 
            result2.errorMessage?.contains("thermal", ignoreCase = true) == true)
        
        // Test partial capture (RGB only)
        coEvery { mockCameraRecorder.captureCalibrationImage(any()) } returns true
        
        val result3 = calibrationCaptureManager.captureCalibrationImages(
    ...
    @Test
    fun testConcurrentCalibrationOperations() = runTest {
        ...
        results.forEachIndexed { index, result ->
            ...
            assertTrue("Each operation should have unique ID", 
    ...
    @Test
    fun testMilestone28FeatureIntegration() = runTest {
        ...
        assertTrue("Sync statistics should contain sync info", 
